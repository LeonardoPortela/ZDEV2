...
FORM criar_field_catalog.
  ...
  PERFORM estrutura_alv USING:
      ...
      0  'ZSDT0358'  'FKART'            'IT_SAIDA' 'FKART'                 'Tipo Fatura'      '10'  'X'    '' ' ' ' ' ' ' 'X' ' ' ' ',
      1  ''          'DESC_FKART'       'IT_SAIDA' 'DESC_FKART'            'Descrição'        '40'  ''     '' ' ' ' ' ' ' '' ' ' ' ',
      2  'ZSDT0358'  'DATA_CRIACAO'     'IT_SAIDA' 'DATA_CRIACAO'          'Data Criação'     '10'  ''     '' ' ' ' ' ' ' 'X' ' ' ' ',
      3  'ZSDT0358'  'HORA_CRIACAO'     'IT_SAIDA' 'HORA_CRIACAO'          'Hora Criação'     '10'  ''     '' ' ' ' ' ' ' 'X' ' ' ' ',
      4  'ZSDT0358'  'USUARIO_CRIACAO'  'IT_SAIDA' 'USUARIO_CRIACAO'       '  '               '30'  ''     '' ' ' ' ' ' ' 'X' ' ' ' ',
      5  'ZSDT0358'  'EXCLUIDO'         'IT_SAIDA' 'EXCLUIDO'              '  '               '2'   'X'    '' ' ' ' ' ' ' '' ' ' 'X',
      6  'ZSDT0358'  'DATA_EXCLUSAO'    'IT_SAIDA' 'DATA_EXCLUSAO'         '  '               '2'   'X'    '' ' ' ' ' ' ' '' ' ' 'X',
      7  'ZSDT0358'  'HORA_EXCLUSAO'    'IT_SAIDA' 'HORA_EXCLUSAO'         '  '               '2'   'X'    '' ' ' ' ' ' ' '' ' ' 'X',
      8  'ZSDT0358'  'USUARIO_EXCLUSAO' 'IT_SAIDA' 'USUARIO_EXCLUSAO'      '  '               '2'   'X'    '' ' ' ' ' ' ' '' ' ' 'X'.
...
FORM selecionar_dados .
  ...
  SELECT *
    INTO TABLE it_zsdt0358
    FROM zsdt0358
    WHERE excluido EQ space.
  IF sy-subrc IS INITIAL.
    DATA(lt_zsdt0358) = it_zsdt0358.
    SORT lt_zsdt0358 BY fkart.
    DELETE ADJACENT DUPLICATES FROM lt_zsdt0358 COMPARING fkart.

    SELECT *
      FROM TVFKT
      INTO TABLE @DATA(lt_TVFKT)
      FOR ALL ENTRIES IN @lt_zsdt0358
      WHERE fkart = @lt_zsdt0358-fkart
        AND spras = @sy-langu.
    IF sy-subrc IS INITIAL.
  ...
  LOOP AT it_zsdt0358 INTO wa_zsdt0358.
    CLEAR: wa_saida.
    MOVE-CORRESPONDING wa_zsdt0358 TO wa_saida.

    READ TABLE lt_TVFKT ASSIGNING FIELD-SYMBOL(<fs_TVFKT>)
    WITH KEY fkart = wa_saida-fkart
    BINARY SEARCH.
    IF sy-subrc IS INITIAL.
      wa_saida-desc_fkart = <fs_TVFKT>-vtext.
    ENDIF.

    ls_style-fieldname = 'FKART'.
...
FORM deletar_reg.
  ...
  MOVE-CORRESPONDING wa_saida TO wa_zsdt0358.
  UPDATE zsdt0358 SET excluido          = abap_true
                      data_exclusao     = sy-datum
                      hora_exclusao     = sy-uzeit
                      usuario_exclusao  = sy-uname
                  WHERE fkart            = wa_zsdt0358-fkart
                    AND data_criacao    = wa_zsdt0358-data_criacao
                    AND hora_criacao    = wa_zsdt0358-hora_criacao
                    AND usuario_criacao = wa_zsdt0358-usuario_criacao.

  DELETE it_saida WHERE fkart            = wa_zsdt0358-fkart
                         AND data_criacao    = wa_zsdt0358-data_criacao
                         AND hora_criacao    = wa_zsdt0358-hora_criacao
                         AND usuario_criacao = wa_zsdt0358-usuario_criacao
                         AND excluido        = wa_zsdt0358-excluido.
...
FORM salvar_reg .
  ...
  IF gv_modif IS NOT INITIAL AND gv_erro IS INITIAL.
    ...
    DELETE it_saida WHERE desc_fkart IS INITIAL.
    ...
    IF it_saida IS NOT INITIAL.
      ...
      LOOP AT it_saida ASSIGNING <l_saida>.
        ...
        MOVE-CORRESPONDING <l_saida> TO wa_zsdt0358.

        IF wa_zsdt0358-fkart IS INITIAL.
          ROLLBACK WORK.
        ...
        MODIFY zsdt0358 FROM wa_zsdt0358.
      ENDLOOP.
