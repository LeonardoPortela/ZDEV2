...
TYPES: BEGIN OF ty_saida,
         user_form_lote   TYPE zsdt0360-user_form_lote,
         desc_USER        TYPE char100,
         data_criacao     TYPE zsdt0360-data_criacao,
         hora_CRIACAO     TYPE zsdt0360-hora_criacao,
         usuario_criacao  TYPE zsdt0360-usuario_criacao,
         excluido         TYPE zsdt0360-excluido,
         data_exclusao    TYPE zsdt0360-data_exclusao,
         hora_EXCLUSAO    TYPE zsdt0360-hora_exclusao,
         usuario_EXCLUSAO TYPE zsdt0360-usuario_exclusao,
...
DATA: it_saida        TYPE TABLE OF ty_saida,
      ...
      it_zsdt0360     TYPE TABLE OF zsdt0360,
      wa_zsdt0360     TYPE zsdt0360,
      wa_zsdt0360_aux TYPE zsdt0360.
  ...
  METHOD handle_data_changed.
    ...
    IF et_good_cells IS NOT INITIAL.
      ...
      DELETE lt_good WHERE value IS INITIAL OR
                           fieldname <> 'USER_FORM_LOTE'.

      DATA(lt_saida) = it_saida.
      DELETE lt_saida WHERE desc_user IS INITIAL.
      SORT lt_saida BY user_form_lote.

      DATA(lt_saida2) = it_saida.
      DELETE lt_saida2 WHERE desc_user IS NOT INITIAL.
      SORT lt_saida2 BY user_form_lote.
      ...
      IF lt_good IS NOT INITIAL.
        ...
        SELECT a~bname, b~name_first, b~name_last
          ...
          WHERE a~bname = @lt_good-value(12).
        ...
        LOOP AT lt_good ASSIGNING FIELD-SYMBOL(<fs_changed>).
          ...
          WITH KEY bname = <fs_changed>-value
          BINARY SEARCH.
          IF sy-subrc IS INITIAL.
            READ TABLE lt_saida TRANSPORTING NO FIELDS
            WITH KEY user_form_lote = <fs_changed>-value
            BINARY SEARCH.
            IF sy-subrc IS INITIAL.
            ...
            ELSE.
              ...
              IF sy-subrc IS INITIAL.
                ...
                CONCATENATE <fs_usr21>-name_first <fs_usr21>-name_last INTO <fs_saida>-desc_user SEPARATED BY space.
        ...
        IF lt_cell IS NOT INITIAL.
          ...
          MESSAGE 'Já existe um cadastro com este Usuário' TYPE 'S' DISPLAY LIKE 'E'.
