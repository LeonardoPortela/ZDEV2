...
FORM criar_field_catalog.
  ...
  PERFORM estrutura_alv USING:
      ...
      0  'ZSDT0356'  'MATKL'            'IT_SAIDA' 'MATKL'                 'Grp. Mercadorias' '10'  'X'    '' ' ' ' ' ' ' 'X' ' ' ' ',
      1  ''          'DESC_MATKL'       'IT_SAIDA' 'DESC_MATKL'            'Descrição'        '40'  ''     '' ' ' ' ' ' ' '' ' ' ' ',
      2  'ZSDT0356'  'DATA_CRIACAO'     'IT_SAIDA' 'DATA_CRIACAO'          'Data Criação'     '10'  ''     '' ' ' ' ' ' ' 'X' ' ' ' ',
      3  'ZSDT0356'  'HORA_CRIACAO'     'IT_SAIDA' 'HORA_CRIACAO'          'Hora Criação'     '10'  ''     '' ' ' ' ' ' ' 'X' ' ' ' ',
      4  'ZSDT0356'  'USUARIO_CRIACAO'  'IT_SAIDA' 'USUARIO_CRIACAO'       '  '               '30'  ''     '' ' ' ' ' ' ' 'X' ' ' ' ',
      5  'ZSDT0356'  'EXCLUIDO'         'IT_SAIDA' 'EXCLUIDO'              '  '               '2'   'X'    '' ' ' ' ' ' ' '' ' ' 'X',
      6  'ZSDT0356'  'DATA_EXCLUSAO'    'IT_SAIDA' 'DATA_EXCLUSAO'         '  '               '2'   'X'    '' ' ' ' ' ' ' '' ' ' 'X',
      7  'ZSDT0356'  'HORA_EXCLUSAO'    'IT_SAIDA' 'HORA_EXCLUSAO'         '  '               '2'   'X'    '' ' ' ' ' ' ' '' ' ' 'X',
      8  'ZSDT0356'  'USUARIO_EXCLUSAO' 'IT_SAIDA' 'USUARIO_EXCLUSAO'      '  '               '2'   'X'    '' ' ' ' ' ' ' '' ' ' 'X'.
...
FORM selecionar_dados .
  ...
  SELECT *
    INTO TABLE it_ZSDT0356
    FROM zsdt0356
    WHERE excluido EQ space.
  IF sy-subrc IS INITIAL.
    DATA(lt_ZSDT0356) = it_ZSDT0356.
    SORT lt_ZSDT0356 BY matkl.
    DELETE ADJACENT DUPLICATES FROM lt_ZSDT0356 COMPARING matkl.

    SELECT *
      FROM t023t
      INTO TABLE @DATA(lt_T023T)
      FOR ALL ENTRIES IN @lt_ZSDT0356
      WHERE matkl = @lt_ZSDT0356-matkl
        AND spras = @sy-langu.
    IF sy-subrc IS INITIAL.
  ...
  LOOP AT it_ZSDT0356 INTO wa_ZSDT0356.
    CLEAR: wa_saida.
    MOVE-CORRESPONDING wa_ZSDT0356 TO wa_saida.

    READ TABLE lt_T023T ASSIGNING FIELD-SYMBOL(<fs_T023T>)
    WITH KEY matkl = wa_saida-matkl
    BINARY SEARCH.
    IF sy-subrc IS INITIAL.
      wa_saida-desc_matkl = <fs_T023T>-wgbez.
    ENDIF.

    ls_style-fieldname = 'MATKL'.
...
FORM deletar_reg.
  ...
  MOVE-CORRESPONDING wa_saida TO wa_ZSDT0356.
  UPDATE zsdt0356 SET excluido          = abap_true
                      data_exclusao     = sy-datum
                      hora_exclusao     = sy-uzeit
                      usuario_exclusao  = sy-uname
                  WHERE matkl            = wa_ZSDT0356-matkl
                    AND data_criacao    = wa_ZSDT0356-data_criacao
                    AND hora_criacao    = wa_ZSDT0356-hora_criacao
                    AND usuario_criacao = wa_ZSDT0356-usuario_criacao.

  DELETE it_saida WHERE matkl            = wa_ZSDT0356-matkl
                         AND data_criacao    = wa_ZSDT0356-data_criacao
                         AND hora_criacao    = wa_ZSDT0356-hora_criacao
                         AND usuario_criacao = wa_ZSDT0356-usuario_criacao
                         AND excluido        = wa_ZSDT0356-excluido.
...
FORM salvar_reg .
  ...
  IF gv_modif IS NOT INITIAL AND gv_erro IS INITIAL.
    ...
    DELETE it_saida WHERE desc_matkl IS INITIAL.
    ...
    IF it_saida IS NOT INITIAL.
      ...
      LOOP AT it_saida ASSIGNING <l_saida>.
        ...
        MOVE-CORRESPONDING <l_saida> TO wa_ZSDT0356.

        IF wa_ZSDT0356-matkl IS INITIAL.
          ROLLBACK WORK.
          MESSAGE 'Grp. Mercadorias é um campo obrigatório!' TYPE 'S'.
          RETURN.
          EXIT.
        ENDIF.

        MODIFY zsdt0356 FROM wa_ZSDT0356.
      ENDLOOP.
