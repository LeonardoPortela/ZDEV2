...
FORM criar_field_catalog.
  ...
  PERFORM estrutura_alv USING:
      ...
      0  'ZSDT0360'  'USER_FORM_LOTE'   'IT_SAIDA' 'USER_FORM_LOTE'        'Usuário Form Lote' '20'  'X'    '' ' ' ' ' ' ' 'X' ' ' ' ',
      1  ''          'DESC_USER'        'IT_SAIDA' 'DESC_USER'             'Descrição'         '40'  ''     '' ' ' ' ' ' ' '' ' ' ' ',
      2  'ZSDT0360'  'DATA_CRIACAO'     'IT_SAIDA' 'DATA_CRIACAO'          'Data Criação'      '10'  ''     '' ' ' ' ' ' ' 'X' ' ' ' ',
      3  'ZSDT0360'  'HORA_CRIACAO'     'IT_SAIDA' 'HORA_CRIACAO'          'Hora Criação'      '10'  ''     '' ' ' ' ' ' ' 'X' ' ' ' ',
      4  'ZSDT0360'  'USUARIO_CRIACAO'  'IT_SAIDA' 'USUARIO_CRIACAO'       '  '                '30'  ''     '' ' ' ' ' ' ' 'X' ' ' ' ',
      5  'ZSDT0360'  'EXCLUIDO'         'IT_SAIDA' 'EXCLUIDO'              '  '                '2'   'X'    '' ' ' ' ' ' ' '' ' ' 'X',
      6  'ZSDT0360'  'DATA_EXCLUSAO'    'IT_SAIDA' 'DATA_EXCLUSAO'         '  '                '2'   'X'    '' ' ' ' ' ' ' '' ' ' 'X',
      7  'ZSDT0360'  'HORA_EXCLUSAO'    'IT_SAIDA' 'HORA_EXCLUSAO'         '  '                '2'   'X'    '' ' ' ' ' ' ' '' ' ' 'X',
      8  'ZSDT0360'  'USUARIO_EXCLUSAO' 'IT_SAIDA' 'USUARIO_EXCLUSAO'      '  '                '2'   'X'    '' ' ' ' ' ' ' '' ' ' 'X'.
...
FORM selecionar_dados .
  ...
  SELECT *
    INTO TABLE it_zsdt0360
    FROM zsdt0360
    WHERE excluido EQ space.
  IF sy-subrc IS INITIAL.
    DATA(lt_zsdt0360) = it_zsdt0360.
    SORT lt_zsdt0360 BY user_form_lote.
    DELETE ADJACENT DUPLICATES FROM lt_zsdt0360 COMPARING user_form_lote.
    ...
    SELECT a~bname, b~name_first, b~name_last
      ...
      FOR ALL ENTRIES IN @lt_zsdt0360
      WHERE a~bname = @lt_zsdt0360-user_form_lote.
    IF sy-subrc IS INITIAL.
  ...
  LOOP AT it_zsdt0360 INTO wa_zsdt0360.
    CLEAR: wa_saida.
    MOVE-CORRESPONDING wa_zsdt0360 TO wa_saida.

    READ TABLE lt_usr21 ASSIGNING FIELD-SYMBOL(<fs_usr21>)
    WITH KEY bname = wa_saida-user_form_lote
    BINARY SEARCH.
    IF sy-subrc IS INITIAL.
      CONCATENATE <fs_usr21>-name_first <fs_usr21>-name_last INTO wa_saida-desc_user SEPARATED BY space.
    ENDIF.

    ls_style-fieldname = 'USER_FORM_LOTE'.
  ...
  SORT it_saida BY user_form_lote.
...
FORM deletar_reg.
  ...
  MOVE-CORRESPONDING wa_saida TO wa_zsdt0360.
  UPDATE zsdt0360 SET excluido          = abap_true
                      data_exclusao     = sy-datum
                      hora_exclusao     = sy-uzeit
                      usuario_exclusao  = sy-uname
                  WHERE user_form_lote      = wa_zsdt0360-user_form_lote
                    AND data_criacao    = wa_zsdt0360-data_criacao
                    AND hora_criacao    = wa_zsdt0360-hora_criacao
                    AND usuario_criacao = wa_zsdt0360-usuario_criacao.

  DELETE it_saida WHERE user_form_lote      = wa_zsdt0360-user_form_lote
                    AND data_criacao    = wa_zsdt0360-data_criacao
                    AND hora_criacao    = wa_zsdt0360-hora_criacao
                    AND usuario_criacao = wa_zsdt0360-usuario_criacao
                    AND excluido        = wa_zsdt0360-excluido.
...
FORM salvar_reg .
  ...
  IF gv_modif IS NOT INITIAL AND gv_erro IS INITIAL.
    ...
    DELETE it_saida WHERE desc_user IS INITIAL.
    ...
    IF it_saida IS NOT INITIAL.
      ...
      LOOP AT it_saida ASSIGNING <l_saida>.
        ...
        MOVE-CORRESPONDING <l_saida> TO wa_zsdt0360.

        IF wa_zsdt0360-user_form_lote IS INITIAL.
          ROLLBACK WORK.
          MESSAGE 'user_form_lote é um campo obrigatório!' TYPE 'S'.
          RETURN.
          EXIT.
        ENDIF.

        MODIFY zsdt0360 FROM wa_zsdt0360.
      ENDLOOP.
