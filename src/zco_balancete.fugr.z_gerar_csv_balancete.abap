FUNCTION z_gerar_csv_balancete.
*"----------------------------------------------------------------------
*"*"Interface local:
*"  IMPORTING
*"     REFERENCE(IT_SALDO_CONTAS) TYPE  ZSCO_SALDO_CONTAS_TT
*"     REFERENCE(IV_AUTOTP_MOEDA) TYPE  FLAG
*"  EXPORTING
...
*    CONDENSE <interna>-moeda_interna NO-GAPS.
    ...
    IF <interna>-moeda_interna EQ '0'.
      DELETE gt_saldo_contas_interna INDEX sy-tabix.
      CONTINUE.
    ENDIF.
...
*    CONDENSE <indice>-moeda_indice NO-GAPS.
  ...
  "DELETE gt_saldo_contas_interna WHERE moeda_interna = '0,00'.
  "DELETE gt_saldo_contas_forte   WHERE moeda_forte   = '0,00'.
  "DELETE gt_saldo_contas_indice  WHERE moeda_indice  = '0,00'.
  DELETE gt_tp_moeda             WHERE vlr_brl = 0 AND vlr_usd = 0.
...
* RJF - Ini - CS2022001091 - Accountfy - API envio Balancetes
  ELSE. "TP Moeda Balancete automático
    MOVE 'C:\TEMP\ARQ_BALANC.CSV' TO diretorio.
    gv_automoeda = abap_true.
    PERFORM f_gerar_csv TABLES gt_tp_moeda
                          USING 'ZSCO_TP_MOEDA'.
