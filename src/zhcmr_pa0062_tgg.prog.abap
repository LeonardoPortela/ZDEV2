...
DATA: zcl_rsdata        TYPE REF TO zcl_hcm_rsdata_tgg,
...
* InsertEmpregados:
      IF ( lt_func[] IS NOT INITIAL ).
        ...
        "  zcl_rsdata->insert_empresa( ). "PBI 6326 --  Retirar metodo de dentro de busca empregados
        ...
        IF ( request-rsdata-empregados-empregado[] IS NOT INITIAL ).
          ...
          WHILE request-rsdata-empregados-empregado[] IS NOT INITIAL.

            DATA(request_500) = request.
            CLEAR: request_500-rsdata-empregados-empregado[].

            LOOP AT request-rsdata-empregados-empregado[] INTO DATA(lwa_empregado).

              APPEND INITIAL LINE TO request_500-rsdata-empregados-empregado[]
                  ASSIGNING FIELD-SYMBOL(<lfs_request>).
              <lfs_request> = lwa_empregado.
              DELETE request-rsdata-empregados-empregado[] WHERE nr_matricula = <lfs_request>-nr_matricula.
              "  *"        Se a quantidade de linhas à enviar for igual ou maior ao limite, enviar.
              IF ( lines( request_500-rsdata-empregados-empregado[] ) >= v_limite_envio ).
                EXIT.
              ENDIF.

            ENDLOOP.
                                                            "PBI 6326
            DELETE ADJACENT DUPLICATES FROM request_500-rsdata-empregados-empregado[] COMPARING nr_matricula.
            "-> Chama método para enviar os empregados:
            zcl_rsdata->insert_empregados( EXPORTING i_request = request_500 ).
