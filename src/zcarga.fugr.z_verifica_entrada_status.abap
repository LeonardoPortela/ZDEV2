FUNCTION Z_VERIFICA_ENTRADA_STATUS.
*"----------------------------------------------------------------------
*"*"Interface local:
*"  IMPORTING
...
*"----------------------------------------------------------------------
  ...
  WHILE R_STATUS EQ 'W'.
    ...
    LOOP AT IT_ENTRADAS INTO DATA(WA_ENTRADAS).

      IF WA_ENTRADAS-ZRG_ATLZ EQ 1.

        JOB_EXECUTANDO = ABAP_TRUE.

        WHILE JOB_EXECUTANDO EQ ABAP_TRUE.
          TRY.
              OBJ_CARGA->ZIF_CARGA~GET_CHECK_JOB_EXECUCAO( ).
        ...
        SELECT SINGLE * INTO @DATA(WA_DOCS)
         ...
         WHERE OBJ_KEY EQ @WA_ENTRADAS-OBJ_KEY.

        IF SY-SUBRC IS INITIAL AND WA_DOCS-DOCNUM IS NOT INITIAL.
