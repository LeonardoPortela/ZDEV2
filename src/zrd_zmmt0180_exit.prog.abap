...
*&
*&---------------------------------------------------------------------*
REPORT zrd_zmmt0180_exit.

DATA: t_return  TYPE STANDARD TABLE OF ddshretval.

FORM f_exit_zmmt0180_0001 CHANGING p_registro_manter TYPE any.


ENDFORM.

FORM f_exit_zmmt0180_0002 USING p_registro_manter TYPE any
                       CHANGING p_error.

  DATA: wl_zmmt0180 TYPE zmmt0180.

  CLEAR: wl_zmmt0180.

  MOVE-CORRESPONDING p_registro_manter TO wl_zmmt0180.

  CLEAR: p_error.

  IF  wl_zmmt0180-chave_nfe       IS INITIAL OR wl_zmmt0180-qtde_exc IS INITIAL.
...
*---- valida
*------------------------------
  IF wl_zmmt0180-chave_nfe IS NOT INITIAL.

    SELECT chave_nfe UP TO 1 ROWS
      INTO @DATA(l_chave_nfe)
      FROM zib_nfe_dist_itm
      WHERE chave_nfe = @wl_zmmt0180-chave_nfe.
    ENDSELECT.

    IF sy-subrc <> 0.
      MESSAGE 'Chave Informada está Incorreta.' TYPE 'S' DISPLAY LIKE 'E'.
      p_error = abap_true.
      EXIT.
    ENDIF.
...
FORM f_exit_zmmt0180_0003 CHANGING p_registro_manter TYPE any.

  DATA: wl_zmmt0180 TYPE zmmt0180.
  DATA: wl_zmmt0168_out TYPE zmmt0168_out.
...
wl_zmmt0180-usuario = sy-uname.
wl_zmmt0180-data = sy-datum.

  MOVE-CORRESPONDING wl_zmmt0180 TO p_registro_manter.

ENDFORM.

FORM f_exit_zmmt0180_0004 CHANGING p_saida TYPE any.
...
*  DATA gv_domvalue_l TYPE dd07v-domvalue_l.
  ...
  CLEAR: wl_zmmt0168_out.

  MOVE-CORRESPONDING p_saida TO wl_zmmt0168_out.
...
*  ENDIF.
  ...
  MOVE-CORRESPONDING wl_zmmt0168_out TO p_saida.

********************************************** RJF

ENDFORM.

FORM f_exit_zmmt0180_0009 TABLES it_excl_toolbar
                           USING p_db_tab.
...
FORM f_exit_zmmt0180_0008_v2 CHANGING  p_fcat_out TYPE lvc_s_fcat.
...
FORM f_exit_zmmt0180_0012 CHANGING p_registro_manter TYPE any
                          p_saida           TYPE any.
*
...
*  PERFORM (form_exit_0012) IN PROGRAM (program_exit) IF FOUND CHANGING p_registro_manter p_saida.
  ...
  MOVE-CORRESPONDING wl_zmmt0180 TO p_saida.
  MOVE-CORRESPONDING wl_zmmt0180 TO p_registro_manter.

ENDFORM.

FORM f_exit_zmmt0180_0016 USING p_ucomm TYPE sy-ucomm
...
ENDFORM.
FORM f_exit_zmmt0180_0017 USING p_tipo.
