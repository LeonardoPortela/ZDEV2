...
*"*"Interface local:
*"  IMPORTING
*"     REFERENCE(I_CD_CHAVE_CTE) TYPE  ZDE_CHAVE_DOC_E
*"  EXPORTING
*"     REFERENCE(E_DOMNAME) TYPE  CHAR30
*"     REFERENCE(E_DDTEXT) TYPE  VAL_TEXT
*"  CHANGING
*"     VALUE(E_TP_APROVACAO) TYPE  ZDE_TIP_APROVACAO
*"  EXCEPTIONS
*"      CTE_NAO_LOC
*"      SEM_AUTORIZACAO
*"      SEM_ESCOLHA
*"----------------------------------------------------------------------
  ...
  IF E_TP_APROVACAO IS INITIAL OR E_TP_APROVACAO EQ '01'.
    ...
    IF SY-SUBRC IS INITIAL.
      "01	Chegada de Documentos
      IT_LIBERADO-DOMNAME	   = 'ZDM_TIP_APROVACAO'.
...
* local de destino, com isso n├úo ter├úo uma entrada de peso e data de chegada.
  ...
  IF E_TP_APROVACAO IS INITIAL.
    ...
    IF SY-SUBRC IS INITIAL.
      ...
      IT_LIBERADO-DOMNAME	   = 'ZDM_AUT_FRETE_TER'.
