...
AT SELECTION-SCREEN OUTPUT.
  ...
  LOOP AT SCREEN.
    IF screen-name EQ 'P_EXTR' OR screen-name EQ '%_P_EXTR_%_APP_%-TEXT'.
      screen-active = 0.
      MODIFY SCREEN.
    ENDIF.
  ENDLOOP.
...
** 152099 - Ajustes fluxo doc - PANF - 11.09.24 - INICIO
    ...
    ENDIF.
                                                            ...
                                                            "US 152483
    SORT it_vbrk_mi BY vbeln.
    DELETE ADJACENT DUPLICATES FROM it_vbrk_mi COMPARING vbeln.
                                                            "US 152483
...
*&---------------------------------------------------------------------*
START-OF-SELECTION.
***<<<------"152483 - NMS - INI------>>>      "Comentado código 04/03/2025 linha 3620 até 3667 / IR226476 /BUG #169582 / AOENNING
* Veridica se o processamento é background.
  IF NOT sy-batch IS INITIAL.
    IF p_extr IS INITIAL.
      p_extr = abap_true.
      SELECT SINGLE low
       FROM tvarvc
      ...
      IF gv_job EQ 1.
        ...
        CALL FUNCTION 'RP_CALC_DATE_IN_INTERVAL'
          EXPORTING
            ...
            years     = 5
          IMPORTING
            calc_date = vl_dt_ini_venc.

        CONCATENATE vl_dt_ini_venc+0(4) '0101' INTO vl_dt_ini_venc.
        ...
        CALL FUNCTION 'RP_CALC_DATE_IN_INTERVAL'
          EXPORTING
            ...
            years     = 5
          IMPORTING
            calc_date = vl_dt_fim_venc.

        CONCATENATE vl_dt_fim_venc+0(4) '1231' INTO vl_dt_fim_venc.
        REFRESH p_venc.

        p_venc-sign   = 'I'.
        p_venc-option = 'BT'.
        p_venc-low    = vl_dt_ini_venc.
        p_venc-high   = vl_dt_fim_venc.
        APPEND  p_venc.
...
**********************************************************************
** Ajustes Performance - PANF - 28.08.24 - INICIO
        SELECT zid_lanc, vbeln, seq, docnum, ajuste, mont_moeda
              FROM zfit0026
        ...
        IF sy-subrc = 0 .
          ...
          DELETE lt_zfit0026 WHERE docnum EQ 0 AND ajuste EQ ' '.
          SORT lt_zfit0026 BY vbeln.

        ENDIF.

        SELECT zid_lanc, vbeln, seq, vlr_juros_calc, vlr_juros_rbdo, vlr_desc_jros,
               vlr_desc_mult, vlr_multa_calc, vlr_multa_rbdo
              FROM zfit0026
              INTO TABLE @DATA(lt_zfit0026_tot)
              FOR ALL ENTRIES IN @t_saida
              WHERE vbeln EQ @t_saida-vbeln.
        IF sy-subrc = 0 .
          SORT lt_zfit0026_tot BY vbeln.
        ENDIF.

        SELECT vbeln , netwr, mwsbp
           FROM vbap
           INTO TABLE @DATA(lt_vbap_vlr)
           FOR ALL ENTRIES IN @t_saida
           WHERE vbeln EQ @t_saida-vbeln
        ...
        DATA:
          vlr_total_final   TYPE netwr,
          vlr_parcial_final TYPE netwr,
          vlr_mont_calc     TYPE zfit0026-vlr_juros_calc,
          total_ov          TYPE vbap-netwr,
          vlr_sald_fin      TYPE zfit0026-vlr_juros_calc,
          vlr_total_saldo   TYPE netwr.
...
** Ajustes Performance - PANF - 28.08.24 - INICIO
**=>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
**   Codigo abaixo foi retirado do metodo zcl_dados_ov=>i_vlr_ov e refatorado
**   para retirar os selects dentro do loop
**=<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
***          ).
            ...
            CLEAR: vlr_total_final, vlr_parcial_final, vlr_mont_calc, total_ov.
...
*** Totalizar o total de saida de juros e multa.
            READ TABLE lt_zfit0026 TRANSPORTING NO FIELDS
                      WITH KEY vbeln = <l_saida>-vbeln
                      BINARY SEARCH.
            IF sy-subrc = 0.
              LOOP AT lt_zfit0026 ASSIGNING FIELD-SYMBOL(<fs_zfit0026>) FROM sy-tabix.
                IF <fs_zfit0026>-vbeln <> <l_saida>-vbeln .
                  EXIT.
                ENDIF.
                ADD <fs_zfit0026>-mont_moeda TO vlr_mont_calc.
              ENDLOOP.
            ENDIF.

****Selecionar informações da OV.
            READ TABLE lt_vbap_vlr TRANSPORTING NO FIELDS
                WITH KEY vbeln = <l_saida>-vbeln
                BINARY SEARCH.
            IF sy-subrc = 0.

              LOOP AT lt_vbap_vlr ASSIGNING FIELD-SYMBOL(<fs_vbap_vlr>) FROM sy-tabix.

                IF <fs_vbap_vlr>-vbeln <> <l_saida>-vbeln.
                  EXIT.
                ENDIF.

                ADD <fs_vbap_vlr>-netwr TO total_ov.
                ADD <fs_vbap_vlr>-mwsbp TO total_ov.
...
** 151268 Verificar OV`s devolução - PANF - 28.08.24 - INICIO
              READ TABLE gt_ov_devolucao ASSIGNING FIELD-SYMBOL(<fs_ov_devolucao>)
               WITH KEY low = <fs_vbak>-auart
               BINARY SEARCH.
*              IF <fs_vbak>-auart EQ 'ZRPF'  OR <fs_vbak>-auart EQ 'ZROB'  OR <fs_vbak>-auart EQ 'ZREB'.
              IF sy-subrc = 0.
** 151268 Verificar OV`s devolução - PANF - 28.08.24 - Fim
********************************************************************
                total_ov  = total_ov  * -1.
              ELSEIF  <fs_vbak>-auart EQ 'ZREM' OR  <fs_vbak>-auart EQ 'ZRFU'.
                total_ov = 0.
              ENDIF.

            ENDIF.

            vlr_parcial_final = ( total_ov - vlr_mont_calc ).

** Ajustes Performance - PANF - 28.08.24 - Fim
**********************************************************************

**********************************************************************
** Erro busca saldos #151268 - PANF - 05.09.24 - INICIO
*            ADD vlr_saldo_parcial TO tot_saldo.
            ADD vlr_parcial_final TO tot_saldo.
** Erro busca saldos #151268 - PANF - 05.09.24 - Fim
**********************************************************************
            CLEAR: vlr_saldo_parcial.


            CLEAR: vlr_total_saldo, vlr_sald_fin.
**********************************************************************
** Ajustes Performance - PANF - 28.08.24 - INICIO
...
***  Buscar movimentações em relação recebimento.
**=>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
**   Codigo abaixo foi retirado do metodo zcl_dados_ov=>i_vlr_saldo_financ e refatorado
**   para retirar os selects dentro do loop
**=<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
***            ).
            READ TABLE lt_zfit0026_tot TRANSPORTING NO FIELDS
**********************************************************************
** 152099 Erro sintaxe - PANF - 10.09.24 - INICIO
*                  WITH KEY <l_saida>-vbeln
                  WITH KEY vbeln = <l_saida>-vbeln
** 152099 Erro sintaxe- PANF - 10.09.24 - INICIO
**********************************************************************
            ...
            IF sy-subrc = 0.
              ...
              LOOP AT lt_zfit0026_tot ASSIGNING FIELD-SYMBOL(<fs_zfit0026_tot>) FROM sy-tabix.

                IF <fs_zfit0026_tot>-vbeln <> <l_saida>-vbeln.
                  EXIT.
                ENDIF.

                vlr_sald_fin = ( <fs_zfit0026_tot>-vlr_juros_calc - <fs_zfit0026_tot>-vlr_juros_rbdo - <fs_zfit0026_tot>-vlr_desc_jros )
                             + ( <fs_zfit0026_tot>-vlr_multa_calc - <fs_zfit0026_tot>-vlr_multa_rbdo - <fs_zfit0026_tot>-vlr_desc_mult ).

                vlr_total_saldo = ( vlr_total_saldo + vlr_sald_fin ).
...
**********************************************************************
            ...
            ADD vlr_total_saldo TO tot_saldo_fin.
...
****************"Selecionando recebimento com base na OV*************************************
        ...
        ENDLOOP.
      ENDIF.
***<<<------"152483 - NMS - INI------>>> { Comentado 04/03/2025 / AOENNING.
*      if gv_extract is initial.         { Comentado 04/03/2025 / AOENNING.
***<<<------"152483 - NMS - FIM------>>> { Comentado 04/03/2025 / AOENNING.
      IF p_extr IS INITIAL.        "IR226476 /BUG #169582 / AOENNING
...
* Prepara dados para exportar para a tabela.
        PERFORM zf_export_data_to_table USING gc_mi. "MI = Mercado Interno
...
***                                             IMPORTING e_zdart = DATA(z_zdart) ).
          ...
          READ TABLE lt_zdart ASSIGNING <fs_zdart>
                      WITH KEY vbeln = w_saida-vbeln
                      BINARY SEARCH.
** Ajustes Performance - PANF - 28.08.24 - Fim
**********************************************************************
...
****************"Selecionando recebimento com base na OV*************************************
        ...
        ENDLOOP.
      ENDIF.
**<<<------"152483 - NMS - INI------>>>   { Comentado 04/03/2025 / IR226476 /BUG #169582 / AOENNING
*      if gv_extract is initial.          { Comentado 04/03/2025 / IR226476 /BUG #169582 / AOENNING
**<<<------"152483 - NMS - FIM------>>>   { Comentado 04/03/2025 / IR226476 /BUG #169582 / AOENNING
      IF p_extr IS INITIAL.         "IR226476 /BUG #169582 / AOENNING
        PERFORM:  f_classificacao,
                  f_build_layout,
                  f_imprime_dados.
...
** 152099 - Ajustes DUMP - PANF - 10.09.24 - INICIO
  ...
  SELECT vbeln vbelv
    ...
    FOR ALL ENTRIES IN r_vbeln
          WHERE ( vbeln EQ r_vbeln-low OR
                  vbelv EQ r_vbeln-low )
            AND doc_simulacao IN p_docsi
...
*              AND ESTORNO NE ABAP_TRUE.
    ...
    IF g_vbeln IS NOT INITIAL.
      ...
      FROM zsdt0090
        APPENDING TABLE it_vbel
      FOR ALL ENTRIES IN g_vbeln
        WHERE vbelv EQ g_vbeln-low
          AND estorno NE abap_true.
...
**********************************************************************
  ...
  IF NOT r_belnr[] IS INITIAL.            "<<RIM-SKM-IR107590-12.08.22
    SELECT bukrs kunnr belnr augbl budat augdt dmbe2 dmbtr
      ...
      FOR ALL ENTRIES IN r_belnr "// wbarbosa BUG-165866 110225
    WHERE bukrs IN r_bukrs
      AND belnr EQ r_belnr-low "// wbarbosa BUG-165866 110225
...
*   FIM MODIFICAÇÃO WELGEM
  ...
  IF r_belnr IS NOT INITIAL.
    ...
    SELECT bukrs kunnr belnr augbl budat augdt dmbe2 dmbtr
      FROM bsad
      INTO CORRESPONDING FIELDS OF TABLE it_bsad
      FOR ALL ENTRIES IN r_belnr
    WHERE bukrs IN r_bukrs
      AND belnr EQ r_belnr-low
      AND kunnr IN r_kunnr
*    AND augdt IN p_pgto
      AND bschl IN ( '09', '01' ).
...
FORM user_command1 USING ucomm TYPE sy-ucomm
  ...
  TYPES:
    BEGIN OF ty_log,
      ...
      valor_apr  TYPE zfit186-vl_moeda_doc,
...
**********************************************************************
    ...
    WHEN '&ZFIS26'. "Programa -> ZFIR0022
      ...
      ELSE.
        MESSAGE e208(00) WITH 'Você não possui autorização a Trazação ZFIS26' .
      ENDIF.
...
*      ENDIF.
      ...
      SELECT SINGLE vl_moeda_doc, status_solicit "146630 - RGA
              FROM zfit186
...
FORM get_text_editor .
  ...
  IF ( lt_text[] IS INITIAL ).
    MESSAGE 'A Justificativa é obrigatória!' TYPE 'S' DISPLAY LIKE 'E'.
    EXIT.
  ENDIF.
  "US 146630- SMC <<<<
...
*** Rubenilson - 18.04.2024 - 131863 - Envio email
  ...
  "146630 - RGA - CS2024000604 Isenção de Juros Insumos - Parte 1
...
* Objetos para enviar email
  ...
  CONSTANTS c_cockpit TYPE sy-sysid VALUE 'ZFIR0125'.
...
* Texto
  ...
  CONCATENATE  'Valor Doc.: ' lv_valor INTO objtxt-line SEPARATED BY space.
  APPEND objtxt.
  CLEAR objtxt.
