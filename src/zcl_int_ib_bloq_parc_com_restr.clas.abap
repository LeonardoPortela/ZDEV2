CLASS zcl_int_ib_bloq_parc_com_restr DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    DATA:
      ...
      BEGIN OF zde_data_request ,
        bloqueios TYPE zsdt_zfit0144,
      END OF zde_data_request .

    CONSTANTS at_id_interface TYPE zde_id_interface VALUE '141' ##NO_TEXT.
...
CLASS ZCL_INT_IB_BLOQ_PARC_COM_RESTR IMPLEMENTATION.
...
*-----------------------------------------------------------------------------------------------------------------------*
    ...
    DATA(lt_bloq) = lwa_data_request-bloqueios.
    DELETE lt_bloq WHERE cnpj_cpf IS INITIAL.
    IF lt_bloq IS INITIAL.
      r_msg_erro = 'Não há dados para processar!'.
      RETURN.
    ENDIF.
  ...
  METHOD zif_integracao_inject~set_integrar_inbound.
    ...
    DATA(lt_zfit0144) = lwa_data_request-bloqueios.
    ...
    LOOP AT lt_zfit0144 ASSIGNING FIELD-SYMBOL(<fs_zfit0144>).

      ADD 1 TO lv_id.
      ADD 1 TO lv_sequencia.

      <fs_zfit0144>-id         = lv_id.
      <fs_zfit0144>-sequencia  = lv_sequencia.

    ENDLOOP.

    MODIFY zfit0144 FROM TABLE lt_zfit0144.
