...
parameter p_codep like regut-codepage no-display.   " note 970892
parameter p_unico like boole-boole    no-display.

PARAMETER p_rcvprt LIKE edidc-rcvprt NO-display.    " note 01441799
PARAMETER p_rcvprn LIKE edidc-rcvprn NO-display.
...
begin of tab_grn occurs 0,             "garnishment remittance advice
  zbukr           like reguh-zbukr,
  vblnr           like reguh-vblnr,
  dest            like tbdestination-rfcdest,
  sgtxt           like regup-sgtxt,
...
*Table for paying company codes of a DME file for a
...
* tabs and column widths for information output list
constants:
  tab1              type i value 40,
  tab2              type i value 53,
  tab3              type i value 68,
  tab_right         type i value 81,
  tab11             type i value 41,
  tab21             type i value 54,
  tab31             type i value 69,
  len_col1          type i value 39,
  len_col2          type i value 12,
  len_col3          type i value 14,
  len_col4          type i value 12,
  len_col_dme       type i value 40,
  len_uline         type i value 79.

* payment accompanying list (use same list layouts as PMW)
constants:
  gc_list_report(8) type c value 'SAPFPAYM'.
...
data:
  ...
  text_080          like rfpdo2-fordtext,
  text_081          like rfpdo2-fordtext,
  text_082          like rfpdo2-fordtext,
  text_083          like rfpdo2-fordtext,
  text_084          like rfpdo2-fordtext,
  text_085          like rfpdo2-fordtext,
  ...
  text_097          like rfpdo2-fordtext,
  text_098          like rfpdo2-fordtext,
...
* Flags, pointer (FLG) -------------------------------------------------
data:
  flg_acc_tech(1)   type c,            "X - bank account number technical
...
data:
  ...
  hlp_apdfaf        like fpayhx-pdfaf, "alternative PDF advice form
  hlp_alvb          type xfeld,        "flag: accompanying list as ALV
  hlp_layb          type slis_vari,    "layout for ALV list
  hlp_disb          type xscreen_fpm,  "flag: display ALV list online
  hlp_auth          like itcpo-tdautority, " print authority
  hlp_tdprintmod    like itctg-tdprintmod, "Druckmodus SIMPLEX,DUPLEX...
  hlp_pages         like itctg occurs 0 with header line,
  hlp_betrag(18)    type c,            "amount CURRENCY ...
  hlp_checf_restart like payr-checf,   "first check (restart)
  hlp_cr(1)         type c,            "constant CR (ASCII Hex'0D')
  hlp_crlf(2)       type c,            "constant CR/LF (ASCII Hex'0D0A')
  ...
  hlp_eof(1)        type c,            "constant EOF (ASCII Hex'1A')
  hlp_fbtch(2)      type n,            "action type (authority check)
  hlp_filename(47)  type c,            "file name (DME)
  gd_filename_new(47) type c,          " alternative filename, note 1460101
  gd_renum_already_filled(1) type c,   " renum filled in exit, note 1460101
  ...
  hlp_pdfformular   like fpayhx-pdfaf, "PDF form for advice
  hlp_pdfspoolid    like itcpp-tdspoolid,"current spool for PDF advice
...
* Globale Hilfsvariablen für PDF-Formulardruck
data:
  gt_paym_note_text type table of tline,
  gs_fpayh          like fpayh,
  gs_fpayhx         like fpayhx,
  gs_fpparams       type sfpoutputparams,
  gt_fpayp          type table of fpayp.

* ALV Zahlungsbegleitliste ---------------------------------------------
data:
  gt_accompanying_list like fpm_list   occurs 0 with header line.
