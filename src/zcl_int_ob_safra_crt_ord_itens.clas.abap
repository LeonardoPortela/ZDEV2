class ZCL_INT_OB_SAFRA_CRT_ORD_ITENS definition
  public
  final
  create public .
...
public section.
  ...
  constants AT_ID_INTERFACE type ZDE_ID_INTERFACE value '297' ##NO_TEXT.
  constants:
    lc_servico TYPE c LENGTH 19 value 'ORDEM_SAFRA_CONTROL_ITENS' ##NO_TEXT.
  constants:
  ...
  data AT_BODY type ZDE_SAFRA_CONTROL_ORDEM_ITENS .
...
CLASS ZCL_INT_OB_SAFRA_CRT_ORD_ITENS IMPLEMENTATION.
  ...
  METHOD zif_integracao_outbound~execute_request.
    ...
    ls_order_itens       = i_info_request.
    me->at_id_referencia = ls_order_itens-externalid.
  ...
  METHOD zif_integracao_outbound~get_data.
    ...
    CHECK at_metodo_http ne zif_integracao_inject=>co_request_method_get.
    ...
    REPLACE ALL OCCURRENCES OF 'itemcode'                    IN e_data WITH 'itemCode'.
    REPLACE ALL OCCURRENCES OF 'itemcode'                    IN e_data WITH 'itemCode'.
    REPLACE ALL OCCURRENCES OF 'unitvalue'                   IN e_data WITH 'unitValue'.
    REPLACE ALL OCCURRENCES OF 'totalvalue'                  IN e_data WITH 'totalValue'.
    REPLACE ALL OCCURRENCES OF 'unitvaluereal'               IN e_data WITH 'unitValueReal'.
    REPLACE ALL OCCURRENCES OF 'purchaseordercode'           IN e_data WITH 'purchaseOrderCode'.
    REPLACE ALL OCCURRENCES OF 'supplierordercode'           IN e_data WITH 'supplierOrderCode'.
    REPLACE ALL OCCURRENCES OF 'schedulingdate'              IN e_data WITH 'schedulingDate'.
    REPLACE ALL OCCURRENCES OF 'shippingaddress'             IN e_data WITH 'shippingAddress'.
    REPLACE ALL OCCURRENCES OF 'extrainfo'                   IN e_data WITH 'extraInfo'.
    REPLACE ALL OCCURRENCES OF 'conversionfactor'            IN e_data WITH 'conversionFactor'.
  ...
  METHOD zif_integracao_outbound~get_id_referencia.
    ...
    r_if_integracao_outbound   = me.
    e_referencia-tp_referencia = 'OB_SAFRA_ORDER_ITENS'.
  ...
  METHOD ZIF_INTEGRACAO_OUTBOUND~GET_INSTANCE.
    ...
    IF zif_integracao_outbound~at_if_integracao_outbound IS NOT BOUND.
      CREATE OBJECT zif_integracao_outbound~at_if_integracao_outbound TYPE zcl_int_ob_safra_crt_ord_itens.
    ENDIF.
  ...
  METHOD zif_integracao_outbound~set_url.
    ...
    SELECT SINGLE *
     ...
     WHERE service EQ 'ORDEM_SAFRA_CONTROL_ITENS'.
