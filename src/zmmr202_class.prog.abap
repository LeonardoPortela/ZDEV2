...
*
*      SELECT SINGLE * FROM ZMMT0186 WHERE investidora = @<wa_pop001>-investidora AND investida = @<wa_pop001>-investida AND moeda_funcional = @<wa_pop001>-moeda_funcional INTO @DATA(lr_ZMMT0186).
*      "IF sy-subrc <> 0.
*      CLEAR: wa_saida.
...
*    DATA: it_changed TYPE STANDARD TABLE OF ZMMT0186 INITIAL SIZE 0.
...
*...Process individual columns
    ...
    TRY.
        ...
        lo_column->set_edit_mask( '__________' ).
        ...
        lo_column ?= lo_cols->get_column( 'DTREC' ).
        lo_column->set_short_text( 'Data' ).
        lo_column->set_medium_text( 'Data' ).
        lo_column->set_long_text( 'Data' ).
        lo_column->set_optimized( abap_false ).
...
*  ENDMETHOD.
  ...
  METHOD get_data.
    ...
    FREE: it_saida.
    SELECT * FROM zmmt0186 INTO TABLE @it_saida.
  ...
  METHOD generate_output.
    ...
    l_title = |Bloqueio de Lançamentos de Miro|.
...
*...Process individual columns
    ...
    lo_cols->set_column_position( columnname = 'CFOP'                 position   = 01 ).
    lo_cols->set_column_position( columnname = 'USUARIO'              position   = 02 ).
    lo_cols->set_column_position( columnname = 'DTREC'                 position   = 03 ).
    ...
    CASE e_salv_function.
      WHEN 'MODIFICAR'.
        IF qtd_rows = 1.
          ...
          CALL SCREEN 0200 STARTING AT 1 1 ENDING AT 50 5.
        ENDIF.
      WHEN 'NOVO'.
        CLEAR:wa_saida.
        wa_saida-usuario = sy-uname.
        wa_saida-dtrec = SY-DATUM.
        CALL SCREEN 0200 STARTING AT 1 1 ENDING AT 50 5.
      ...
      WHEN 'GRAVAR'.
        ...
        IF it_saida IS NOT INITIAL.
          LOOP AT it_saida ASSIGNING FIELD-SYMBOL(<_read>).
            ...
            SELECT SINGLE * FROM ZMMT0186 WHERE CFOP = @<_read>-CFOP INTO @DATA(lr_ZMMT0186).

            IF sy-subrc <> 0.
              MODIFY ZMMT0186 FROM <_read>.
              IF sy-subrc = 0.
                ls_tab-msgty  = 'S'.
        ...
        ELSE.
          MESSAGE 'Não Existe CFOP a SER Gravado!' TYPE 'I'.
        ENDIF.
