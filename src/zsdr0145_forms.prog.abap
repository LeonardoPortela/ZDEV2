...
FORM f_selecao_dados  USING p_lote
  ...
  FREE: t_saida, t_ekpo, t_zsdt0306, t_zsdt0225, p_erro.

* CHECK p_svinc = abap_true OR p_nvinc = abap_true.
  ...
  IF     p_lote IS NOT INITIAL.
    ...
    SELECT ekpo~ebeln ekpo~ebelp eket~etenr eket~charg
     ...
     WHERE ekko~ebeln  = t_zsdt0306-ebeln
       AND ekpo~ebelp  = t_zsdt0306-ebelp.

  ELSEIF s_lote[] IS NOT INITIAL.
    SELECT *
      FROM zsdt0306
      INTO TABLE t_zsdt0306
...
FORM f_processa_dados USING p_sort.
  ...
  LOOP AT t_ekpo INTO w_ekpo.
    ...
    w_saida-status      = COND #( WHEN w_zsdt0306 IS INITIAL THEN icon_failure
                                                             ELSE icon_relation ).

    IF     p_svinc = abap_true  "AND p_nvinc = abap_false
       AND w_saida-status = icon_failure.
      CONTINUE.
*   ELSEIF p_svinc = abap_false "AND p_nvinc = abap_true
*      AND w_saida-status = icon_relation.
*     CONTINUE.
    ENDIF.
    ...
    "w_saida-operacao      = w_zsdt0306-operacao.
    APPEND w_saida     TO t_saida.
  ENDLOOP.

* IF p_sort = abap_true.
*   SORT t_saida BY id_seq.
* ENDIF.

  IF t_saida[] IS INITIAL.
    MESSAGE s024(sd) WITH TEXT-141 DISPLAY LIKE 'W'.
  ELSE.
    SORT t_saida BY id_seq.
  ENDIF.
...
FORM f_vincular.
  ...
  DATA: t_lista  TYPE TABLE OF spopli,
...
*<--- 01/06/2023 - Migração S4 - JS
  ...
  IF t_rows[] IS INITIAL.
    ...
    EXIT.
  ELSE.
    DESCRIBE TABLE t_rows[] LINES DATA(linhas).
    IF linhas > 1.
...
*--------------------------
  ...
  IF l_id_seq IS INITIAL.
    PERFORM f_get_next_number USING 'ZSEQ_SD108'
                                    '01'
                           CHANGING l_id_seq
                                    l_erro.
  ENDIF.
...
*-CS2022000686-09.02.2023-#102944-JT-inicio
  SELECT *
    FROM zsdt0307
    INTO TABLE t_zsdt0307
  ...
  IF lines( t_zsdt0307 ) > 1.
    FREE: t_lista.
    LOOP AT t_zsdt0307 INTO w_zsdt0307.
      DATA(l_linha) = |{ w_zsdt0307-emp_pedido } / { w_zsdt0307-emp_fat_serv } / { w_zsdt0307-centro_fat_serv }|.
...
*    call function 'ISH_POPUP_TO_DECIDE_LIST' "MIGNOW - Diego Tersariolli - 19.07.2023 - MG5648
    CALL FUNCTION 'POPUP_TO_DECIDE_LIST'      "MIGNOW - Diego Tersariolli - 19.07.2023 - MG5648
      EXPORTING
        cursorline = 1
        start_col  = 70
        start_row  = 05
*       SHOW_ROW   = 12                    "MIGNOW - Diego Tersariolli - 19.07.2023 - MG5648
        textline1  = l_texto1
        textline2  = l_texto2
        titel      = 'Fatura Serviço - Parametros Pedido'
*       COLTITLE   = 'Empr.Pedido / Empr.Fat.Serv / Centro Fat.Serv' "MIGNOW - Diego Tersariolli - 19.07.2023 - MG5648
      IMPORTING
        answer     = l_resp
      TABLES
*       IM_EX_LISTE = T_LISTA
        t_spopli   = t_lista                "MIGNOW - Diego Tersariolli - 19.07.2023 - MG5648
      EXCEPTIONS
        no_entries = 1
        OTHERS     = 2.
    ...
    ELSE.
      ...
      DELETE t_zsdt0307 WHERE emp_fat_serv <> w_lista-varoption+7(4).
    ENDIF.
  ELSEIF lines( t_zsdt0307 ) = 1.
  ENDIF.

  IF t_zsdt0307[] IS INITIAL.
    l_erro        = abap_true.
  ...
  CLEAR w_zsdt0307.
  READ TABLE t_zsdt0307 INTO w_zsdt0307 INDEX 1.
*-CS2022000686-09.02.2023-#102944-JT-fim
...
*--------------------------
  ...
  LOOP AT t_rows INTO w_rows.
    READ TABLE t_saida INTO w_saida INDEX w_rows-index.
    CHECK sy-subrc = 0.
    "140791 CS2023000951 Melhoria ZSDT0158 - Permitir ajuste no peso- PARTE 3 - PSA
***********************************************************************
*
...
*
*    DATA(msg_popup) = |O lote não foi gerqado, refaça a operação!|.
*
*    SELECT ddtext,domvalue_l  FROM dd07t
*      INTO TABLE @DATA(it_dd07t)
*      WHERE domname = 'ZDM_OPER_AQUAV'
*      AND domvalue_l IN ('IM','VI').
*
*    CLEAR: lt_list,lt_list[].
*    LOOP AT  it_dd07t ASSIGNING FIELD-SYMBOL(<_mode_value>).
*      CLEAR: ls_list.
*      ls_list-key = <_mode_value>-ddtext.
*      ls_list-value = <_mode_value>-domvalue_l.
...
*      IF sy-subrc = 0.
*        w_saida-operacao  = ls_return-fieldval.
*        w_saida-bukrs_fat  = w_zsdt0307-emp_fat_serv.
*        w_saida-werks_fat  = w_zsdt0307-centro_fat_serv.
*        MODIFY t_saida  FROM w_saida INDEX sy-tabix.
*
*        PERFORM f_gravar_tabelas USING l_id_seq.
*
*      ELSE.
*        MESSAGE msg_popup TYPE 'S' DISPLAY LIKE 'E'.
*        EXIT.
*      ENDIF.
*    ELSE.
*      MESSAGE msg_popup TYPE 'S' DISPLAY LIKE 'E'.
...
**********************************************************************
  ...
  LOOP AT t_0225 INTO w_0225.
    CLEAR w_0225_aux.
    READ TABLE t_0225_aux  INTO w_0225_aux WITH KEY id_seq       = w_0225-id_seq
                                                    bukrs        = w_0225-bukrs
                                                    werks        = w_0225-werks
                                                    ano_viagem   = w_0225-ano_viagem
                                                    cl_codigo    = w_0225-cl_codigo
                                                    safra        = w_0225-safra
                                                    cod_material = w_0225-cod_material.
    IF sy-subrc = 0.
      w_0225_aux-peso_vinculado = w_0225_aux-peso_vinculado + w_0225-peso_vinculado.
      MODIFY t_0225_aux      FROM w_0225_aux INDEX sy-tabix.
    ELSE.
      APPEND w_0225            TO t_0225_aux.
    ENDIF.
  ENDLOOP.

  "140791 CS2023000951 Melhoria ZSDT0158 - Permitir ajuste no peso- PARTE 3 - PSA
  "MODIFY zsdt0225 FROM TABLE t_0225_aux.

  COMMIT WORK AND WAIT.

  MESSAGE s024(sd) WITH TEXT-250.
...
* gravar tabelas
**********************************************************************
FORM f_gravar_tabelas USING p_id_seq.

  CLEAR: w_zsdt0306, w_zsdt0225.

  w_zsdt0306-mandt          = sy-mandt.
  w_zsdt0306-id_seq         = p_id_seq.
  w_zsdt0306-ebeln          = w_saida-ebeln.
  w_zsdt0306-ebelp          = w_saida-ebelp.
  w_zsdt0306-lifnr          = w_saida-lifnr.
  w_zsdt0306-bukrs          = w_saida-bukrs.
  w_zsdt0306-bukrs_fat      = w_saida-bukrs_fat.    "*-CS2022000686-09.02.2023-#102944-JT
  w_zsdt0306-werks_fat      = w_saida-werks_fat.    "*-CS2022000686-09.02.2023-#102944-JT
  w_zsdt0306-werks          = w_saida-werks.
  w_zsdt0306-lgort          = w_saida-lgort.
  w_zsdt0306-charg          = w_saida-charg.
  w_zsdt0306-unsez          = w_saida-unsez.
  w_zsdt0306-matnr          = w_saida-matnr.
  w_zsdt0306-quantidade     = w_saida-menge.
  ...
  " w_zsdt0306-operacao       = w_saida-operacao.
  MODIFY zsdt0306        FROM w_zsdt0306.

  w_zsdt0225-mandt          = sy-mandt.
  w_zsdt0225-id_seq         = p_id_seq.
  w_zsdt0225-bukrs          = w_saida-bukrs_fat.          "*-CS2022000686-09.02.2023-#102944-JT
  w_zsdt0225-werks          = w_saida-werks_fat.          "*-CS2022000686-09.02.2023-#102944-JT
  w_zsdt0225-ano_viagem     = '0000'.
  w_zsdt0225-cl_codigo      = |{ w_saida-werks ALPHA = IN }|.
  w_zsdt0225-safra          = w_saida-unsez. "w_saida-lgort.  "*-IR194132-29.10.2024-#156085-JT-inicio
  w_zsdt0225-cod_material   = w_saida-matnr.
  w_zsdt0225-origem         = 'PI'.
  "w_zsdt0225-operacao       = 'IM'.  "*-CS2022000686-09.02.2023-#102944-JT
  w_zsdt0225-peso_vinculado = w_saida-menge.
...
* desvincular pedidos
**********************************************************************
FORM f_desvincular USING p_ucomm.
...
*--------------------------
* antes de desvincular, checa se ja OV ou NF emitida
*--------------------------
  FREE: l_erro.
  ...
  IF l_erro = abap_true.
    MESSAGE s024(sd) WITH TEXT-120 TEXT-121 DISPLAY LIKE 'E'.
    EXIT.
  ENDIF.
...
*--------------------------
  ...
  LOOP AT t_rows INTO w_rows.
    ...
    DELETE FROM zsdt0306 WHERE id_seq = w_saida-id_seq
                           AND ebeln  = w_saida-ebeln
                           AND ebelp  = w_saida-ebelp.

    w_lote-id_seq   = w_saida-id_seq.
    w_lote-ebeln    = w_saida-ebeln.
    w_lote-ebelp    = w_saida-ebelp.
    APPEND w_lote  TO t_lote.
  ENDLOOP.

*--------------------------
* ajustar tabela zsdt0225
*--------------------------
  SELECT *
    FROM zsdt0225
    INTO TABLE t_0225
     FOR ALL ENTRIES IN t_lote
   WHERE id_seq = t_lote-id_seq.

  LOOP AT t_rows INTO w_rows.
    READ TABLE t_saida INTO w_saida INDEX w_rows-index.
    CHECK sy-subrc = 0.

    l_werks = |{ w_saida-werks ALPHA = IN }|.

    READ TABLE t_0225  INTO w_0225 WITH KEY id_seq       = w_saida-id_seq
                                            bukrs        = w_saida-bukrs_fat
                                            werks        = w_saida-werks_fat
                                            ano_viagem   = 0
                                            cl_codigo    = l_werks
                                            safra        = w_saida-lgort
                                            cod_material = w_saida-matnr.
    CHECK sy-subrc = 0.

    w_0225-peso_vinculado = w_0225-peso_vinculado - w_saida-menge.
    MODIFY t_0225      FROM w_0225 INDEX sy-tabix.

    IF w_0225-peso_vinculado IS INITIAL.
      DELETE FROM zsdt0225 WHERE id_seq       = w_saida-id_seq
                             AND bukrs        = w_saida-bukrs_fat
                             AND werks        = w_saida-werks_fat
                             AND ano_viagem   = 0
                             AND cl_codigo    = l_werks
                             AND safra        = w_saida-lgort
                             AND cod_material = w_saida-matnr
                             AND origem       = 'PI'.
    ELSE.
      "140791 CS2023000951 Melhoria ZSDT0158 - Permitir ajuste no peso- PARTE 3 - PSA
...
FORM f_editar_lote.
  ...
  IF l_lines <> 1.
    MESSAGE s024(sd) WITH TEXT-220 DISPLAY LIKE 'E'.
    EXIT.
  ENDIF.
  ...
  IF w_saida-id_seq IS INITIAL.
    MESSAGE s024(sd) WITH TEXT-222 DISPLAY LIKE 'E'.
    EXIT.
  ENDIF.

  IF w_saida-nr_ov  IS NOT INITIAL OR
     w_saida-docnum IS NOT INITIAL.
    MESSAGE s024(sd) WITH TEXT-221 DISPLAY LIKE 'E'.
    EXIT.
  ENDIF.

  l_editar       = abap_true.
  l_lote_editado = w_saida-id_seq.
...
*--------------------------
* seleciona lote para edicao
*--------------------------
  PERFORM f_selecao_dados  USING l_lote_editado
                        CHANGING l_erro.
  PERFORM f_processa_dados USING abap_off.
...
* adicionar lote
**********************************************************************
FORM f_adicionar_lote CHANGING p_erro.

  p_erro   = abap_false.
  l_editar = abap_true.

*--------------------------
* escolhe novos pedidos
*--------------------------
  CALL FUNCTION 'ZSD_LISTA_TRANSBORDO'
    EXPORTING
      i_id_seq    = l_lote_editado
    TABLES
      t_resultado = t_saida_aux.
...
*--------------------------
  ...
  LOOP AT t_saida_aux INTO w_saida.
    READ TABLE t_0225 INTO DATA(w_0225) WITH KEY id_seq = w_saida-id_seq.
    IF sy-subrc = 0.
      IF w_0225-nr_ov  IS NOT INITIAL OR
         w_0225-docnum IS NOT INITIAL.
        p_erro = abap_true.
        EXIT.
      ENDIF.
    ENDIF.
...
*--------------------------
  LOOP AT t_saida_aux  INTO w_saida.
    PERFORM f_gravar_tabelas USING l_lote_editado.
  ENDLOOP.
...
* gera sequenciador
**********************************************************************
FORM f_get_next_number USING p_object
                             p_nr_range
                    CHANGING p_number
                             p_erro.

  CLEAR: p_number, p_erro.
...
FORM f_fieldcatalog.
 ...
 "   03  ''      ''       'T_ALV' 'OPERACAO'            'Oper.'                    '05'  ' ' ' ' ' ' ' '  ' ' ' ' ' ' ' ' ' ' ' ',
    04  'LFA1'  'LIFNR'  'T_ALV' 'LIFNR'               'Cód.Fornec.'              '12'  ' ' ' ' ' ' ' '  ' ' ' ' ' ' ' ' ' ' ' ',
    05  ''      ''       'T_ALV' 'LIFNR_NAME'          'Descr.Fornec.'            '20'  ' ' ' ' ' ' ' '  ' ' ' ' ' ' ' ' ' ' ' ',
    06  ''      ''       'T_ALV' 'EBELN'               'Nr.Pedido'                '12'  ' ' ' ' ' ' ' '  ' ' ' ' ' ' ' ' ' ' ' ',
    07  ''      ''       'T_ALV' 'EBELP'               'Item Ped.'                '09'  ' ' ' ' ' ' ' '  ' ' ' ' ' ' ' ' ' ' ' ',
    08  ''      ''       'T_ALV' 'LGORT'               'Lote/Safra'               '10'  ' ' ' ' ' ' ' '  ' ' ' ' ' ' ' ' ' ' ' ',
    09  ''      ''       'T_ALV' 'BUKRS'               'Empresa'                  '07'  ' ' ' ' ' ' ' '  ' ' ' ' ' ' ' ' ' ' ' ',
    10  ''      ''       'T_ALV' 'WERKS'               'Filial'                   '07'  ' ' ' ' ' ' ' '  ' ' ' ' ' ' ' ' ' ' ' ',
    11  'MARA'  'MATNR'  'T_ALV' 'MATNR'               'Cód.Material'             '15'  ' ' ' ' ' ' ' '  ' ' ' ' ' ' ' ' ' ' ' ',
    12  ''      ''       'T_ALV' 'TXZ01'               'Descrição Material'       '20'  ' ' ' ' ' ' ' '  ' ' ' ' ' ' ' ' ' ' ' ',
    13  ''      ''       'T_ALV' 'MATKL'               'Grp.Merc'                 '08'  ' ' ' ' ' ' ' '  ' ' ' ' ' ' ' ' ' ' ' ',
    14  ''      ''       'T_ALV' 'MENGE'               'Quantidade'               '13'  ' ' 'X' ' ' ' '  ' ' ' ' ' ' ' ' ' ' ' ',
    15  ''      ''       'T_ALV' 'MEINS'               'Und.Med'                  '07'  ' ' ' ' ' ' ' '  ' ' ' ' ' ' ' ' ' ' ' '.
*    16  ''      ''       'T_ALV' 'DOCNUM'              'NFPS'                     '10'  ' ' ' ' ' ' ' '  ' ' ' ' ' ' ' ' ' ' ' '.
