...
REPORT ZSDI0009 NO STANDARD PAGE HEADING MESSAGE-ID SD.
...
*-#133089-21.02.2024-JT-fim
  ...
  IF V_NR_ROMANEIO IS NOT INITIAL.
    ...
    IF ( WA_TL_ZSDT0001-MATNR IS INITIAL ). "CS2017000598 22.05.2017
      ...
      LOOP AT TG_VBAP ASSIGNING FIELD-SYMBOL(<FS_VBAP>) WHERE LGORT IS INITIAL.
        PERFORM F_GET_DEPOSITO_ROMANEIO USING WA_TL_ZSDT0001 CHANGING <FS_VBAP>-LGORT.
      ENDLOOP.
      "SD - Geração Ordem Venda sem Deposito US #153341 - WPP <<<---
...
*     SORT tg_0023 BY  werks ASCENDING cwerks DESCENDING. "CS2016001304
      SORT TG_0023 BY  WERKS ASCENDING MATNR ASCENDING MATKL ASCENDING CWERKS DESCENDING."141033 CS2023000120 - ZMM0029 Controle por Grupo de Mercadoria PSA
        ...
        TRY .
            ...
            ZCL_DEPOSITO=>ZIF_DEPOSITO~GET_INSTANCE(
                    ...
                    EXPORTING
                      ...
                      I_EUDR       = WA_TL_ZSDT0001-EUDR "Parâmetros Centro Fixo x Centro Afixar EUDR - BG #152940
                    IMPORTING
            ...
            "SD - Geração Ordem Venda sem Deposito US #153341 - WPP --->>>
            IF S_VBAP-LGORT IS INITIAL AND E_LGORT IS NOT INITIAL.
              S_VBAP-LGORT = E_LGORT.
            ENDIF.
            "SD - Geração Ordem Venda sem Deposito US #153341 - WPP <<<---
...
*-#133089-21.02.2024-JT-inicio
            CASE VG_FATURAMENTO_AUTOM.
              WHEN ABAP_OFF.
                "SD - Geração Ordem Venda sem Deposito US #153341 - WPP --->>>
                IF S_VBAP-LGORT IS INITIAL.
                  EX_DEPOSITO->ZIF_ERROR~PUBLISHED_ERRO( EXPORTING I_MSGTY = 'E' I_MSGTY_DISPLAY = 'E' ).
                  RETURN.
                ELSE.
                  "SD - Geração Ordem Venda sem Deposito US #153341 - WPP <<<---
              ...
              WHEN ABAP_TRUE.
                "SD - Geração Ordem Venda sem Deposito US #153341 - WPP --->>>
                IF S_VBAP-LGORT IS INITIAL.
                  MESSAGE ID EX_DEPOSITO->MSGID TYPE 'E' NUMBER EX_DEPOSITO->MSGNO WITH EX_DEPOSITO->MSGV1 EX_DEPOSITO->MSGV2 EX_DEPOSITO->MSGV3 EX_DEPOSITO->MSGV4 INTO DATA(L_MESG2).
                  LC_FATURAMENTO_AUTOMATICO->SET_GRAVAR_MENSAGEM( I_CH_REFERENCIA = V_NR_ROMANEIO I_TYPE = 'E' I_MSG = CONV #( L_MESG2 ) I_STATUS = 'REME' ).
                  LC_FATURAMENTO_AUTOMATICO->SET_MENSAGEM( I_COD = '999' I_MESG = CONV #( L_MESG2 ) ).
                  RETURN.
                ELSE.
                  "SD - Geração Ordem Venda sem Deposito US #153341 - WPP <<<---
...
*-#133089-21.02.2024-JT-inicio
      CASE VG_FATURAMENTO_AUTOM.
        ...
        WHEN ABAP_TRUE.
          ...
          LC_FATURAMENTO_AUTOMATICO->SET_GRAVAR_MENSAGEM( I_CH_REFERENCIA = V_NR_ROMANEIO I_TYPE = 'E' I_MSG = CONV #( L_MESG ) I_STATUS = 'REME' ).
          LC_FATURAMENTO_AUTOMATICO->SET_MENSAGEM( I_COD = '999' I_MESG = CONV #( L_MESG ) ).
          EXIT. "SD - Geração Ordem Venda sem Deposito US #153341 - WPP --->>>
      ENDCASE.
*-#133089-21.02.2024-JT-fim
    ENDIF.
    ...
    "SD - Geração Ordem Venda sem Deposito US #153341 - WPP --->>>
    IF S_VBAP-LGORT IS INITIAL.
      PERFORM F_GET_DEPOSITO_ROMANEIO USING SL_ROMA CHANGING S_VBAP-LGORT.
    ENDIF.
    "SD - Geração Ordem Venda sem Deposito US #153341 - WPP <<<---
...
FORM Z_CRIA_REMESSA  RAISING ZCX_ERROR. "*-#133089-21.02.2024-JT-inicio..
  ...
  IF ( S_VBAK-AUART EQ 'ZRDC' ) OR ( S_VBAK-AUART EQ 'ZRFL' ) OR ( S_VBAK-AUART EQ 'ZIND' ).
    ...
    DATA(T_VBPAVB_LR) = T_VBPAVB[].
    DELETE T_VBPAVB_LR WHERE PARVW NE 'LR'.
    ...
    IF NOT T_VBPAVB[] IS INITIAL.
      ...
      READ TABLE T_VBPAVB_LR INTO DATA(WA_VBPAVB_LR) INDEX 1. "Parâmetros de Centro Real x Centro Virtual EUDR - BG #153255 - Fim
...
*          zsdt_depara_depo = wa_depara.
      ...
      DATA(_EUDR_CHECK) = SL_ZSDT0001-EUDR.

      IF WA_VBPAVB-LIFNR IS NOT INITIAL.
        SELECT SINGLE *
          FROM ZSDT0351 INTO @DATA(LWA_ZSDT0351)
          WHERE LIFNR EQ @WA_VBPAVB-LIFNR.

        IF SY-SUBRC NE 0. "Se terminal porto nao tiver parametrização EUDR
          CLEAR: _EUDR_CHECK. "Não buscar por parametro EUDR
        ENDIF.
      ENDIF.

      IF WA_VBPAVB_LR-KUNNR IS NOT INITIAL.
        SELECT SINGLE *
          FROM ZSDT0351 INTO @LWA_ZSDT0351
          WHERE KUNNR EQ @WA_VBPAVB_LR-KUNNR.

        IF ( SY-SUBRC NE 0 ) OR "Se transbordo nao tiver parametrização EUDR
           ( SY-SUBRC EQ 0 AND LWA_ZSDT0351-RECEBIMENTO_EUDR = '3' ). "transbordo parametrização como "Recebimento Não EUDR"
          CLEAR: _EUDR_CHECK. "Não buscar por parametro EUDR
        ENDIF.
      ENDIF.
...
*Parâmetros de Centro Real x Centro Virtual EUDR - BG #153255  - FIM
      ...
      ELSE.
        ...
        "SD - Geração Ordem Venda sem Deposito US #153341 - WPP --->>>
        MESSAGE |Centro: { S_VBAP-VSTEL } Terminal: { WA_VBPAVB-LIFNR } Operação:{ _OPERA } Class.EUDR: { SL_ZSDT0001-EUDR } sem parâmetros na transação ZSDT0020! |  TYPE 'S'.
        RETURN.
        "SD - Geração Ordem Venda sem Deposito US #153341 - WPP <<<---
      ENDIF.
    ENDIF.
...
*-#133089-21.02.2024-JT-fim
        WL_ERRO = 'X'.
        RETURN. "SD - Geração Ordem Venda sem Deposito US #153341 - WPP
      ENDIF.
    ENDLOOP.
  ENDIF.
    ...
    TRY .
        ...
        ZCL_DEPOSITO=>ZIF_DEPOSITO~GET_INSTANCE(
                ...
                EXPORTING
                  ...
                  I_EUDR       = SL_ZSDT0001-EUDR "Parâmetros Centro Fixo x Centro Afixar EUDR - BG #152940
                IMPORTING
                  E_LGORT      = DATA(E_LGORT)    " Depósito
            ).
...
*-#133089-21.02.2024-JT-fim
    ...
    IF WA_VBAP-LGORT NE E_LGORT AND WA_VBAP-LGORT IS NOT INITIAL. "Parâmetros Centro Fixo x Centro Afixar EUDR - BG #152940.
*-#133089-21.02.2024-JT-inicio
      CASE VG_FATURAMENTO_AUTOM.
        WHEN ABAP_OFF.
...
*"// WBARBOSA US-169528 ZLES0136 INICIO
        IF SL_ZSDT0001-ID_INTERFACE = '48'. "Sementes
          DO 100 TIMES.

            SELECT SINGLE COUNT(*)
              FROM LIKP
              WHERE VBELN EQ VL_DELIVERY.

            IF SY-SUBRC IS INITIAL.

              CALL METHOD ZCL_CARGA_SAIDA_INSUMOS=>GET_AGENTE_FRETE
                EXPORTING
                  I_BUKRS  = SL_ZSDT0001-BUKRS
                  I_BRANCH = SL_ZSDT0001-BRANCH
                  I_NRO_CG = SL_ZSDT0001-NRO_CG
                IMPORTING
                  E_INCO1  = DATA(E_INCO1).

              IF E_INCO1 IS NOT INITIAL.
                CALL METHOD ZCL_CARGA_SAIDA_INSUMOS=>CHANGE_DELIVERY
                  EXPORTING
                    I_VBELN  = VL_DELIVERY
                    I_INCO1  = E_INCO1
                  IMPORTING
                    E_RETURN = DATA(E_RETURN).

                IF LINE_EXISTS( E_RETURN[ TYPE = 'E' ] ).
                  PERFORM F_ESTORNO_RES  CHANGING SL_ZSDT0001.
                  PERFORM Z_MONTA_ERRO TABLES E_RETURN
                                        USING SL_ZSDT0001.
                ENDIF.
              ENDIF.
...
*"// WBARBOSA US-169528 ZLES0136 FIM
...
*   Atualiza ZSDT0001
      ...
      IF SY-TCODE NE 'ZLES0106'
         AND SY-TCODE NE 'ZSDT0112' "SD - Faturamento Saida Insumos - Sementes US 169508 - WPP --->>>
         AND SY-TCODE NE 'ZLES0115' AND SY-TCODE NE 'ZLES0136' AND  SY-TCODE NE 'ZMM0127' AND SY-BATCH NE ABAP_TRUE.
        UPDATE ZSDT0001
                 SET ST_PROC = '99'
               WHERE CH_REFERENCIA EQ P_ZSDT0001-CH_REFERENCIA.
      ENDIF.
...
*   Retorna Erro
  ...
  IF ( C_R EQ 'X' ).
    ...
    IF SY-TCODE NE 'ZLES0106'
      AND SY-TCODE NE 'ZSDT0112' "SD - Faturamento Saida Insumos - Sementes US 169508 - WPP --->>>
      AND SY-TCODE NE 'ZLES0115' AND SY-TCODE NE 'ZMM0127' AND SY-BATCH NE ABAP_TRUE.
      UPDATE ZSDT0001
               SET ST_PROC = '99'
             WHERE CH_REFERENCIA EQ P_ZSDT0001-CH_REFERENCIA.
    ENDIF.
...
*      <-- 19.06.2023 - Migration S4 – End
  ...
  "SD - Geração Ordem Venda sem Deposito US #153341 - WPP --->>>
  SELECT SINGLE VBELN, POSNR, LGORT, VGBEL
    FROM LIPS INTO @DATA(LWA_LIPS_TMP)
   WHERE VBELN EQ @P_DELIVERY
     AND POSNR EQ @TG_LIPS-POSNR.

  IF SY-SUBRC EQ 0 AND LWA_LIPS_TMP-LGORT IS INITIAL.
    APPEND INITIAL LINE TO ITEM_DATA_SPL ASSIGNING FIELD-SYMBOL(<FS_DATA_SPL>).
    <FS_DATA_SPL>-DELIV_NUMB = P_DELIVERY.
    <FS_DATA_SPL>-DELIV_ITEM = TG_LIPS-POSNR.
    PERFORM F_GET_DEPOSITO_ROMANEIO USING P_ZSDT0001 CHANGING <FS_DATA_SPL>-STGE_LOC.
  ENDIF.
  "SD - Geração Ordem Venda sem Deposito US #153341 - WPP <<<---
...
FORM F_GET_DEPOSITO_ROMANEIO USING P_ZSDT0001 TYPE ZSDT0001
                          CHANGING C_LGORT TYPE LGORT_D.
  ...
  TRY .
      ...
      ZCL_DEPOSITO=>ZIF_DEPOSITO~GET_INSTANCE(
              ...
              EXPORTING
                I_MATNR      = P_ZSDT0001-MATNR
                I_TP_PRODUTO = CONV #( COND STRING( WHEN P_ZSDT0001-TP_TRANSGENIA(1) EQ 'C' THEN ZIF_CARGA=>ST_TP_TRANSGENIASE_CO ELSE 'RR' ) )    " Tipo de Produto
                I_BUKRS      = P_ZSDT0001-BUKRS
                I_BRANCH     = P_ZSDT0001-BRANCH
                I_EUDR       = P_ZSDT0001-EUDR
              IMPORTING
                E_LGORT      = C_LGORT
          ).
    CATCH ZCX_DEPOSITO INTO DATA(EX_DEPOSITO).    " .
