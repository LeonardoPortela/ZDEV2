CLASS zcl_int_ob_get_protocolo_eudr DEFINITION
  ...
  PUBLIC SECTION.
    ...
    CONSTANTS: at_id_interface TYPE zde_id_interface VALUE '237' ##NO_TEXT.
    CONSTANTS:
      lc_servico       TYPE c LENGTH 24 VALUE 'EUDR_GET_DADOS_PROTOCOLO' ##NO_TEXT.
    CONSTANTS:
    ...
    DATA at_params TYPE TABLE OF zstruct_get_protocolo_eudr.
    DATA at_body TYPE zstruct_get_protocolo_eudr. "// Estrutura para JSON
...
CLASS ZCL_INT_OB_GET_PROTOCOLO_EUDR IMPLEMENTATION.
...
*        r_json = e_data.
  ...
  METHOD zif_integracao_outbound~get_instance.
    ...
    IF zif_integracao_outbound~at_if_integracao_outbound IS NOT BOUND.
      CREATE OBJECT zif_integracao_outbound~at_if_integracao_outbound TYPE zcl_int_ob_get_protocolo_eudr.
    ENDIF.
  ...
  METHOD zif_integracao_outbound~set_url.
    ...
    lv_contador = 0.
    LOOP AT at_params INTO DATA(ls_params).
      IF lv_url IS INITIAL.
        lv_url = |{ lwa_webservice-url }?codes[{ lv_contador }]={ ls_params-protocolo }|.
      ELSE.
        lv_url = |{ lv_url }&codes[{ lv_contador }]={ ls_params-protocolo }|.
      ENDIF.
