...
*& Request..: DEVK945561                                              &*
*&--------------------------------------------------------------------&*
REPORT  zmmr0030_job  MESSAGE-ID ztracecotton.
TYPE-POOLS vrm.

TABLES: zppt0002, zppt0006.

SELECTION-SCREEN BEGIN OF BLOCK b1.
  PARAMETERS    : p_id_ref TYPE zde_id_referencia.
  PARAMETERS    : p_chqueb TYPE char50.
  PARAMETERS    : p_chlock TYPE char50.
...
DATA: it_zppt0002       TYPE TABLE OF zppt0002,
      ...
      it_zppt0002_all   TYPE TABLE OF zppt0002,
      t_trans           TYPE TABLE OF type_mchb,
      it_saida          TYPE TABLE OF ty_saida,
      it_zppt0006       TYPE TABLE OF zppt0006,
      it_zppt0006_trace TYPE TABLE OF zppt0006, "*-US 125503 - 01.11.2023 - JT
      wa_zppt0006_trace TYPE zppt0006.          "*-US 125503 - 01.11.2023 - JT

** WORK AREAS
**----------------------------------------------------------------------
DATA: wa_zppt0002      TYPE zppt0002,
      wa_zppt0002_tot  TYPE zppt0002,
      wa_zppt0002_all  TYPE zppt0002,
      wa_saida         TYPE ty_saida,
      wa_zppt0006      TYPE zppt0006,
      w_zppt0030_trace TYPE zppt0030_trace,
      wa_mara          TYPE mara,
      sl_trans         TYPE type_mchb,
      v_objek          TYPE inob-objek,
      v_objek2         TYPE objnum. "inob-objek.
...
*-lock processamento
*-------------------------------------
  MESSAGE 'Lock processamento' TYPE 'S'.

*  DO 3 TIMES.
*    vl_index = sy-index.
*
*    l_erro = zcl_integracao_cotton_sap=>lock_objeto( p_chlock ).
*    IF l_erro = abap_false.
*      EXIT.
*    ELSEIF vl_index NE 3.
*      WAIT UP TO 2 SECONDS.
*    ENDIF.
*  ENDDO.
*
*  IF l_erro = abap_true.
*    MESSAGE s024(sd) WITH 'Lock inicial processo: ' p_chlock.
*    PERFORM f_trata_reenvio.
*    EXIT.
*  ENDIF.
...
*-------------------------------------
  ...
  UPDATE zppt0030 SET jobname       = p_jobnam
                      data_ini      = sy-datum
                      hora_ini      = sy-uzeit
                      time_ini      = l_time
                WHERE id_referencia = p_id_ref
                  AND chave_quebra  = p_chqueb.
  COMMIT WORK.
...
*-------------------------------------
  GET TIME STAMP FIELD l_time.
  UPDATE zppt0030 SET data_fim       = sy-datum
                      hora_fim       = sy-uzeit
                      time_fim       = l_time
                      emproc_normal  = abap_true
                      proces_normal  = abap_true
                WHERE id_referencia  = p_id_ref
                  AND chave_quebra   = p_chqueb.
  COMMIT WORK.

*-------------------------------------
*-unlock processamento
*-------------------------------------
* zcl_integracao_cotton_sap=>unlock_objeto( p_chlock ).

  PERFORM f_envia_trace.

*&---------------------------------------------------------------------*
*&      Form  F_TRATA_REENVIO
*&---------------------------------------------------------------------*
FORM f_trata_reenvio.

*  LOOP AT it_zppt0002_all INTO wa_zppt0002_all.
*    UPDATE zppt0002 SET status          = wa_zppt0002_all-status
*                        status_registro = wa_zppt0002_all-status_registro
*                  WHERE id_referencia   = p_id_ref
*                    AND chave_quebra    = p_chqueb.
*  ENDLOOP.

  UPDATE zppt0030 SET emproc_normal   = abap_false
                      proces_normal   = abap_false
                      retorno_lock    = abap_true
                WHERE id_referencia   = p_id_ref
                  AND chave_quebra    = p_chqueb.
...
FORM f_seleciona_dados.
  ...
  REFRESH: it_zppt0006_trace, it_zppt0002, it_saida, it_zppt0002_far, it_zppt0002_tra.

  MESSAGE '287 - Selecionando dados' TYPE 'S'.
  SELECT *
    FROM zppt0002
    INTO TABLE it_zppt0002_all
   WHERE id_referencia EQ p_id_ref
     AND chave_quebra  EQ p_chqueb.
...
* Verifica Duplicata e estorna
  ...
  WHERE id_referencia   = p_id_ref
    AND chave_quebra    = p_chqueb
    AND ( status_registro  EQ '07' "TRANSFERE DEPOSITO
...
*----------------------------------------------------------------------*
FORM f_fardao.
  MESSAGE '416 - form _Fardao ' TYPE 'S'.
  ...
  SELECT *
    ...
    WHERE a~id_cotton  = @it_zppt0002_far-id_cotton
...
*-#125503 = 26.10.2023 - JT - inicio
  SELECT *
    ...
    INTO TABLE @DATA(it_mseg)
     FOR ALL ENTRIES IN @it_zppt0002_far
   WHERE xblnr_mkpf EQ @it_zppt0002_far-id_cotton
...
*-#125503 = 26.10.2023 - JT - inicio
  ...
  LOOP AT it_zppt0002_far INTO wa_zppt0002.
    ...
    IF wa_zppt0002-id_cotton IS NOT  INITIAL.
      ...
      IF sy-subrc = 0 AND wa_zpps_ximfbf_log-charg IS NOT INITIAL.
      ...
      ELSE.
        ...
        IF sy-subrc = 0 AND wa_zmmt0006-batch_d IS NOT INITIAL.
          wa_zppt0002-charg = wa_zmmt0006-batch_d.
          MODIFY it_zppt0002_far FROM wa_zppt0002 INDEX tabix TRANSPORTING charg.
        ELSE.
*-#125503 = 26.10.2023 - JT - inicio
          READ TABLE it_mseg INTO DATA(w_mseg) WITH KEY xblnr_mkpf = wa_zppt0002-id_cotton.
          IF sy-subrc EQ 0 AND w_mseg-charg IS NOT INITIAL.
            wa_zppt0002-charg = w_mseg-charg.
            MODIFY it_zppt0002_far FROM wa_zppt0002 INDEX tabix TRANSPORTING charg.
          ENDIF.
*-#125503 = 26.10.2023 - JT - fim
  ...
  LOOP AT it_mseg_tot ASSIGNING FIELD-SYMBOL(<f_mseg_tot>).
    CHECK <f_mseg_tot>-smbln IS INITIAL.
      ...
      IF mdpmx-shkzg = 'H'. "somente saida
        ...
        IF sy-subrc IS INITIAL.
          UPDATE zppt0002 SET status_registro = '02'
                        ...
                        WHERE acharg          = wa_zppt0002_tot-acharg
                          AND werks           = wa_zppt0002_tot-werks
                          AND id_referencia   = p_id_ref
                          AND chave_quebra    = p_chqueb.
...
*         CONCATENATE 'Material Bloqueado ' mdpmx-matnr 'Dep.' mdpmx-lgpro 'Lote' wa_zppt0002_tot-charg INTO wa_return-message SEPARATED BY space.
          CONCATENATE 'Material sem Saldo ' mdpmx-matnr 'Dep.' mdpmx-lgpro 'Lote' wa_zppt0002_tot-charg INTO wa_return-message SEPARATED BY space.
          APPEND wa_return TO it_return.
        ENDIF.
      ENDIF.
...
* <--- S4 Migration - 21/06/2023 - MA
    LOOP AT it_goodsmovements INTO wa_goodsmovements.
      ...
      "bloqueio
      DO 3 TIMES.
        MESSAGE s024(sd) WITH '417 - form _Fardao - Lock:' wa_goodsmovements-material wa_goodsmovements-plant.
* ---> S4 Migration - 21/06/2023 - MA
...
*        ENDIF.
      ...
      IF sperr_user IS NOT INITIAL.
        ...
        APPEND wa_return TO it_return.
        MESSAGE s024(sd) WITH 'Material ' wa_goodsmovements-material 'bloqueado por ' sperr_user.
      ENDIF.

      "bloqueio lote
      IF wa_goodsmovements-batch IS NOT INITIAL.
        DO 3 TIMES.
          MESSAGE s024(sd) WITH '418 - form _Fardao - Lock:' wa_goodsmovements-material wa_goodsmovements-plant.
* ---> S4 Migration - 21/06/2023 - MA
          lv_material = wa_goodsmovements-material.
...
*          ENDIF.
        ...
        IF sperr_user IS NOT INITIAL.
          ...
          APPEND wa_return TO it_return.
          MESSAGE s024(sd) WITH 'Lote ' wa_goodsmovements-batch 'bloqueado por ' sperr_user.
        ENDIF.
      ENDIF.
...
*----------------------------------------------------------------------*
FORM f_fardinho.
  MESSAGE '1007 - Form f_fardinho' TYPE 'S'.
    ...
    IF wa_zppt0002-status_registro EQ '03'.
      ...
      IF sy-subrc IS INITIAL.
        ...
        IF sy-subrc IS INITIAL.
          ...
          UPDATE zppt0002 SET status_registro = '04'
...
*----------------------------------------------------------------------*
FORM f_gravar_dados.
  MESSAGE '1171 - form f_gravar_dados ' TYPE 'S'.
...
* ---> S4 Migration - 21/06/2023 - MA - *-Equalização RISE x PRD - 28.08.2023 - JT - inicio
...
*          return             = it_return2.
      ...
      IF strlen( v_objek ) > 50.  "-Equalização RISE x PRD - 28.08.2023 - JT - inicio
        CALL FUNCTION 'BAPI_OBJCL_CHANGE' "#EC CI_USAGE_OK[2438131]
          ...
          TABLES
            ...
            return             = it_return2.
      ELSE.
        v_objek2 = v_objek.  "-Equalização RISE x PRD - 28.08.2023 - JT - inicio
...
*           objectkey_long     = v_objek
          ...
          TABLES
            ...
            return             = it_return2.
      ENDIF.
* <--- S4 Migration - 21/06/2023 - MA - *-Equalização RISE x PRD - 28.08.2023 - JT - fim
      CALL FUNCTION 'BAPI_TRANSACTION_COMMIT'
        EXPORTING
          wait = c_x.
    ENDIF.
...
** MONTAR MSG DE SUCESSO E ERRO
  MESSAGE '1347 - FORM f_monta_msg_fd' TYPE 'S'.
  LOOP AT it_return INTO wa_return.
    ...
    wa_zppt0006-id_referencia2 = p_id_ref.
    wa_zppt0006-chave_quebra   = p_chqueb.
  ...
  APPEND LINES OF it_zppt0006[] TO it_zppt0006_trace[].  "*-US 125503 - 01.11.2023 - JT
...
*  UPDATE zppt0002 SET status = 'P'
*                      status_registro = wa_return-message_v1+0(3)
*   WHERE charg  EQ wa_saida-charg_con
*     AND acharg EQ wa_saida-acharg
*     AND status EQ 'R'
*     AND id_referencia   = p_id_ref
*     AND chave_quebra    = p_chqueb.
...
FORM f_transferencia .
  ...
  MESSAGE '1553 - Transferencia' TYPE 'S'.
  LOOP AT it_zppt0002_tot INTO wa_zppt0002_tot.
    vg_qtd_bloco      = 0.
    tabix = sy-tabix.
    ...
    IF sy-subrc = 0.
      ...
      ELSE. "troca de material
        ...
        IF sy-subrc = 0.
          ...
          ENDIF.
        ELSE.
*-BUG SOLTO-18.05.2023-#112272-JT-inicio
          sl_return-message_v1     = 'E10'. "Erro  transf. material
          sl_return-message        = | { 'Classificação' } { wa_zppt0002-cd_classificacao } { 'não existe' } |.
          APPEND sl_return        TO tl_return.
...
FORM z_cria_trans.
  ...
  CLEAR sl_header.
  MESSAGE '1661 - FORM z_cria_trans' TYPE 'S'.
...
** MONTAR MSG DE SUCESSO E ERRO fardinho
  LOOP AT it_return INTO wa_return.
    ...
    READ TABLE it_zppt0002_tra INTO wa_zppt0002 WITH KEY acharg = sl_trans-charg
    ...
    SELECT MAX( id ) INTO vid
      FROM zppt0006
     WHERE werks  = wa_zppt0002-werks
       AND charg  = wa_zppt0002-charg
...
FORM f_envia_trace.
  ...
  DATA: t_zppt0006        TYPE TABLE OF zppt0006.
  DATA: l_id_ref  TYPE char11.
  ...
  RANGES: in_ctt FOR zppt0002-id_cotton.

*-US 125503 - 01.11.2023 - JT - inicio
*  SELECT *
...
*  CHECK sy-subrc <> 0.
  ...
  MESSAGE  '1926 - FORM f_envia_trace' TYPE 'S'.
...
*     AND proces_normal = @abap_true.

* CHECK t_0030[] IS NOT INITIAL. "*-US 125503 - 01.11.2023 - JT

*  FREE: in_ctt.
*  LOOP AT t_0030 INTO DATA(w_0030).
*    in_ctt-sign   = 'I'.
*    in_ctt-option = 'EQ'.
*    in_ctt-low    = w_0030-id_cotton.
*    APPEND in_ctt.
*  ENDLOOP.
...
*   WHERE id_referencia2 = @p_id_ref
*     AND chave_quebra   = @p_chqueb.
*-US 125503 - 01.11.2023 - JT - fim

*  SELECT *
*    FROM zppt0006 AS a
*    INTO TABLE @DATA(t_zppt0006)
*    WHERE a~id_referencia2 = @p_id_ref.
*      AND a~flag_envio EQ 'R'
*      AND a~id_cotton  IN @in_ctt
...
*-US 125503 - 01.11.2023 - JT - inicio
  t_zppt0006[] = it_zppt0006_trace[].

  IF t_zppt0006[] IS INITIAL.
    READ TABLE it_zppt0002_all     INTO DATA(w_0002) INDEX 1.
    wa_zppt0006_trace-werks           = w_0002-werks.
    wa_zppt0006_trace-charg           = w_0002-charg.
    wa_zppt0006_trace-acharg          = w_0002-acharg.
    wa_zppt0006_trace-status_registro = w_0002-status_registro.
    APPEND wa_zppt0006_trace         TO t_zppt0006.
  ENDIF.
*-US 125503 - 01.11.2023 - JT - fim

  SORT t_zppt0006      BY werks charg acharg id DESCENDING.
  DELETE ADJACENT DUPLICATES FROM t_zppt0006
...
*   WHERE id_referencia   = p_id_ref
*     AND chave_quebra    = p_chqueb.
**    AND werks           = it_zppt0006_tot-werks
**    AND charg           = it_zppt0006_tot-charg.

  it_zppt0002_far[] = it_zppt0002_all[].
*-US 125503 - 01.11.2023 - JT - fim
  ...
  IF t_fardo[] IS NOT INITIAL.
    ...
    CALL METHOD obj_trace->atualiza_trace
      EXPORTING
        ...
        id_referencia = l_id_ref   "in_ref     " Id Referencia do LOG da Integração
      RECEIVING
        ret_code      = l_ret_code.      " Status Code

    IF l_ret_code EQ 200.

      LOOP AT it_zppt0006_tot INTO wa_zppt0006_tot.
        UPDATE zppt0006 SET flag_envio     = 'P'
                            id_referencia  = p_id_ref  "in_ref
                      WHERE id_referencia2 = p_id_ref
                        AND chave_quebra   = p_chqueb.
...
*-US 125503 - 01.11.2023 - JT - inicio
*----------------------------------------
*-Gravar tabela envio Trace
*----------------------------------------
  CALL METHOD /ui2/cl_json=>serialize( EXPORTING data = t_fardo RECEIVING r_json = l_json ).

  LOOP AT it_zppt0002_all         INTO DATA(w_0002_all).
    w_zppt0030_trace-mandt           = sy-mandt.
    w_zppt0030_trace-id_referencia   = p_id_ref.
    w_zppt0030_trace-id_cotton       = w_0002_all-id_cotton.
    w_zppt0030_trace-acharg          = w_0002_all-acharg.
    w_zppt0030_trace-werks           = w_0002_all-werks.
    w_zppt0030_trace-status_registro = w_0002_all-status_registro.
    w_zppt0030_trace-status_envio    = l_ret_code.
    w_zppt0030_trace-user_reg        = sy-uname.
    w_zppt0030_trace-data_reg        = sy-datum.
    w_zppt0030_trace-hora_reg        = sy-uzeit.
    w_zppt0030_trace-json_retorno    = l_json.
    MODIFY zppt0030_trace         FROM w_zppt0030_trace.
  ENDLOOP.
