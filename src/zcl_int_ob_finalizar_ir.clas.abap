class ZCL_INT_OB_FINALIZAR_IR definition
  public
  final
  create public .
...
public section.
  ...
  constants AT_ID_INTERFACE type ZDE_ID_INTERFACE value '282' ##NO_TEXT.
  data AT_DADOS_FINALIZAR_IR type ZFIE_OB_FINALIZAR_IR .
...
CLASS ZCL_INT_OB_FINALIZAR_IR IMPLEMENTATION.
  ...
  METHOD ZIF_INTEGRACAO_OUTBOUND~BUILD_INFO_REQUEST.
    ...
    DATA: lt_finalizar_ir TYPE ZFIE_OB_FINALIZAR_IR.

    r_if_integracao_outbound = me.

    MOVE-CORRESPONDING i_info_request TO lt_finalizar_ir.

    me->at_dados_finalizar_ir = lt_finalizar_ir.
  ...
  METHOD ZIF_INTEGRACAO_OUTBOUND~GET_DATA.
    ...
    CALL METHOD /UI2/CL_JSON=>SERIALIZE
      EXPORTING
        DATA   = ME->AT_DADOS_FINALIZAR_IR
      RECEIVING
        R_JSON = E_DATA.

    REPLACE ALL OCCURRENCES OF 'INSTANCIA'    IN E_DATA WITH 'instancia'.
    REPLACE ALL OCCURRENCES OF 'ID_ATIVIDADE' IN E_DATA WITH 'id_atividade'.
    REPLACE ALL OCCURRENCES OF 'ID_SEQUENCIA' IN E_DATA WITH 'id_sequencia'.

*    loop at me->at_dados_finalizar_ir into data(wa_dados).
*
*
*      e_data = '{"instancia":[ "' && wa_dados-instancia && '"], "form": { "n3sistdesc": "' && me->at_dados_set_coment_ir-form-n3sistdesc && '","recorrencia": "' && me->at_dados_set_coment_ir-form-recorrencia
*      && '", "itatendn1n2": "' && me->at_dados_set_coment_ir-form-itatendn1n2 && '"}, "entity_id": "' && me->at_dados_set_coment_ir-entity_id && '"}'.
*
*    endloop.
*{
*    "instancia": [
...
*    ],
*    "form": {
*        "n3sistdesc": "Falha no job falta de parametros cadastrados",
*        "recorrencia": "0",
*        "itatendn1n2": "1"
*    },
*    "entity_id": "IR"
...
*    e_referencia-id_referencia = me->at_dados_due-id_due.
  ...
  method ZIF_INTEGRACAO_OUTBOUND~GET_INSTANCE.
    ...
    IF zif_integracao_outbound~at_if_integracao_outbound IS NOT BOUND.
      CREATE OBJECT zif_integracao_outbound~at_if_integracao_outbound TYPE zcl_int_ob_finalizar_ir.
    ENDIF.
