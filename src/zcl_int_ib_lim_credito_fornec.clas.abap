CLASS zcl_int_ib_lim_credito_fornec DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      BEGIN OF ty_limites,
        zfit0146 TYPE zfit0146,
      END OF ty_limites .

    DATA:
*
      BEGIN OF zde_data_request ,
        limites TYPE zsdt_zfit0146,
...
CLASS ZCL_INT_IB_LIM_CREDITO_FORNEC IMPLEMENTATION.
...
*-----------------------------------------------------------------------------------------------------------------------*
    ...
    DATA(lt_limites) = lwa_data_request-limites.
    DELETE lt_limites WHERE lifnr IS INITIAL.
    IF lt_limites IS INITIAL.
  ...
  METHOD zif_integracao_inject~set_integrar_inbound.
    ...
    DATA(lt_zfit0146) = lwa_data_request-limites.

    MODIFY zfit0146 FROM TABLE lt_zfit0146.
