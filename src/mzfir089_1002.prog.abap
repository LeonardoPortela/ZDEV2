...
*----------------------------------------------------------------------*
  ...
  DATA: it_zfit184_C TYPE TABLE OF TP_CUSTO WITH HEADER LINE,

        IT_zfit182_  TYPE TABLE OF zfit182 WITH HEADER LINE,
        IT_zfit183_  TYPE TABLE OF zfit183 WITH HEADER LINE,
        it_zfit184_  TYPE TABLE OF TP_CUSTO WITH HEADER LINE,
...
**
**    SELECT * INTO TABLE IT_zfit182 FROM zfit182 WHERE VERSN EQ WA_DRE_DADOS_ALV-VERSN.
**    SELECT * INTO TABLE IT_zfit183 FROM zfit183 WHERE VERSN EQ WA_DRE_DADOS_ALV-VERSN.
**    SELECT * INTO TABLE it_zfit184 FROM zfit184 WHERE VERSN EQ WA_DRE_DADOS_ALV-VERSN.
...
**
**    MOVE: IT_zfit182[] TO IT_zfit182_[],
**          IT_zfit183[] TO IT_zfit183_[],
...
**
**    LOOP AT it_zfit184.
**      MOVE-CORRESPONDING it_zfit184 TO it_zfit184_.
**      it_zfit184_-KOKRS = TG_TKA02-KOKRS.
**      APPEND it_zfit184_.
**    ENDLOOP.
**
**    SORT IT_zfit182_ BY NIVEL.
**    DELETE ADJACENT DUPLICATES FROM IT_zfit182_ COMPARING NIVEL.
**    SORT IT_zfit183_ BY KTOPL SAKNR.
**    DELETE ADJACENT DUPLICATES FROM IT_zfit183_ COMPARING KTOPL SAKNR.
**    SORT it_zfit184_ BY KOSAR.
**    DELETE ADJACENT DUPLICATES FROM it_zfit184_ COMPARING KOSAR.
...
**
**    "Conta Razão **************************************************************************
**    IF NOT IT_zfit183_[] IS INITIAL.
**      IF SY-UCOMM NE OK_CEXCEL.
**        SELECT * INTO TABLE IT_SKAT
**          FROM SKAT
**           FOR ALL ENTRIES IN IT_zfit183_
**         WHERE SPRAS EQ SY-LANGU
**           AND KTOPL EQ IT_zfit183_-KTOPL
**           AND SAKNR EQ IT_zfit183_-SAKNR.
**      ELSE.
...
**
**    "Conta Razão / Centro de Custo ********************************************************
**    IF NOT it_zfit184_[] IS INITIAL.
**      SELECT * INTO TABLE IT_CSKS
**        FROM CSKS
**         FOR ALL ENTRIES IN it_zfit184_
**       WHERE KOKRS EQ it_zfit184_-KOKRS
**         AND KOSAR EQ it_zfit184_-KOSAR
...
**
**      IF SY-SUBRC IS INITIAL.
**        READ TABLE it_zfit184 TRANSPORTING NO FIELDS
...
**
**        IF SY-SUBRC IS NOT INITIAL.
**          it_zfit184-BUKRS = IT_ZGL022_DRE_DADOS-BUKRS.
**          it_zfit184-VERSN = IT_ZGL022_DRE_DADOS-VERSN.
**          it_zfit184-NIVEL = IT_ZGL022_DRE_DADOS-NIVEL.
**          it_zfit184-KTOPL = IT_SKAT-KTOPL.
**          it_zfit184-SAKNR = IT_ZGL022_DRE_DADOS-SAKNR.
**          it_zfit184-KOSAR = IT_CSKS_AUX-KOSAR.
**
**          MOVE-CORRESPONDING: it_zfit184 TO it_zfit184_.
**          APPEND it_zfit184.
**          APPEND it_zfit184_.
...
**
**      ENDIF.
**      CLEAR: it_zfit184_, IT_ZGL022_DRE_DADOS, IT_CSKS_AUX.
**    ENDLOOP.
**
...
**
**        IF SY-SUBRC IS NOT INITIAL.
**          it_zfit184-BUKRS = IT_ZGL026_DRE_ACM-BUKRS.
**          it_zfit184-VERSN = IT_ZGL026_DRE_ACM-VERSN.
**          it_zfit184-NIVEL = IT_ZGL026_DRE_ACM-NIVEL.
**          it_zfit184-KTOPL = IT_SKAT-KTOPL.
**          it_zfit184-SAKNR = IT_ZGL026_DRE_ACM-SAKNR.
**          it_zfit184-KOSAR = IT_CSKS_AUX-KOSAR.
**
**          MOVE-CORRESPONDING: it_zfit184 TO it_zfit184_.
**          APPEND it_zfit184.
...
**    LOOP AT it_zfit184.
**      CLEAR: it_zfit184_C.
**      MOVE-CORRESPONDING it_zfit184 TO it_zfit184_C.
**      LOOP AT IT_CSKS WHERE KOSAR EQ it_zfit184_C-KOSAR.
**        READ TABLE IT_ZGL022_DRE_DADOS WITH KEY NIVEL = it_zfit184_C-NIVEL
**                                                SAKNR = it_zfit184_C-SAKNR
**                                                KOSTL = IT_CSKS-KOSTL
**                                                BINARY SEARCH.
**        IF NOT SY-SUBRC IS INITIAL.
**          READ TABLE IT_ZGL026_DRE_ACM WITH KEY NIVEL = it_zfit184_C-NIVEL
**                                                SAKNR = it_zfit184_C-SAKNR
**                                                KOSTL = IT_CSKS-KOSTL
**                                                BINARY SEARCH.
**          IF NOT SY-SUBRC IS INITIAL.
...
**          ENDIF.
**        ENDIF.
**        it_zfit184_C-KOKRS = IT_CSKS-KOKRS.
**        it_zfit184_C-KOSTL = IT_CSKS-KOSTL.
**        READ TABLE IT_CSKT WITH KEY KOKRS = IT_CSKS-KOKRS KOSTL = IT_CSKS-KOSTL.
**        IF SY-SUBRC IS INITIAL.
**          it_zfit184_C-LTEXT = IT_CSKT-LTEXT.
**        ENDIF.
**        APPEND it_zfit184_C.
**      ENDLOOP.
**    ENDLOOP.
...
**    "**************************************************************************************
**    "**************************************************************************************
**    SORT: it_zfit184_C BY KOKRS KOSTL BUKRS VERSN NIVEL KTOPL SAKNR KOSAR.
**    DELETE ADJACENT DUPLICATES FROM it_zfit184_C COMPARING KOKRS KOSTL BUKRS VERSN NIVEL KTOPL SAKNR KOSAR.
**
**    SORT: it_zfit184 BY NIVEL KTOPL SAKNR.
**    SORT: it_zfit184_C BY NIVEL KTOPL SAKNR KOSAR.
**    SORT: IT_ZGL022_DRE_DADOS BY NIVEL SAKNR KOSTL.
**    SORT: IT_ZGL022_DRE_DADOS BY NIVEL SAKNR KOSTL.
**    "Níveis
**    "Contas.***********************************************************************************************************************************************
**    LOOP AT IT_zfit182 WHERE NIVEL_TOTAL EQ SPACE.
**
**
**      CLEAR: IT_zfit183  ,
**             it_zfit184_C,
...
**             IT_ZGL015_DRE_EST07  .
**
**      PERFORM INSERT_EST USING IT_zfit182 IT_zfit183 it_zfit184_C IT_ZGL015_DRE_EST05 IT_ZGL015_DRE_EST06 IT_ZGL015_DRE_EST07 SPACE.
**
**      LOOP AT IT_zfit183 WHERE NIVEL EQ IT_zfit182-NIVEL.
**
**        CLEAR: it_zfit184_C,
**               IT_ZGL015_DRE_EST05  ,
**               IT_ZGL015_DRE_EST06  .
**
**        READ TABLE IT_ZGL021_DRE_DADOS WITH KEY NIVEL = IT_zfit182-NIVEL
**                                                SAKNR = IT_zfit183-SAKNR.
**
**        IF SY-SUBRC IS NOT INITIAL.
**          READ TABLE IT_ZGL025_DRE_ACM WITH KEY NIVEL = IT_zfit182-NIVEL
**                                                SAKNR = IT_zfit183-SAKNR.
**        ENDIF.
**
**        IF SY-SUBRC IS INITIAL.
**          PERFORM INSERT_EST USING IT_zfit182 IT_zfit183 it_zfit184_C IT_ZGL015_DRE_EST05 IT_ZGL015_DRE_EST06 IT_ZGL015_DRE_EST07 C_X.
**          CONTINUE.
**        ELSE.
**          PERFORM INSERT_EST USING IT_zfit182 IT_zfit183 it_zfit184_C IT_ZGL015_DRE_EST05 IT_ZGL015_DRE_EST06 IT_ZGL015_DRE_EST07 SPACE.
**        ENDIF.
**
**        READ TABLE it_zfit184
**          WITH KEY NIVEL = IT_zfit182-NIVEL
**                   KTOPL = IT_zfit183-KTOPL
**                   SAKNR = IT_zfit183-SAKNR
**                   BINARY SEARCH.
**        IF SY-SUBRC IS INITIAL.
**          WL_TABIX = SY-TABIX.
...
**
**          "Centro de custo
**          LOOP AT it_zfit184 FROM WL_TABIX.
***        LOOP AT it_zfit184 WHERE nivel EQ it_zfit182-nivel
***                                      AND ktopl EQ it_zfit183-ktopl
***                                      AND saknr EQ it_zfit183-saknr.
**            IF it_zfit184-NIVEL NE IT_zfit182-NIVEL
**            OR it_zfit184-KTOPL NE IT_zfit183-KTOPL
**            OR it_zfit184-SAKNR NE IT_zfit183-SAKNR.
**              EXIT.
**            ENDIF.
**
**            READ TABLE it_zfit184_C
**           WITH KEY NIVEL = IT_zfit182-NIVEL
**                    KTOPL = IT_zfit183-KTOPL
**                    SAKNR = IT_zfit183-SAKNR
**                    KOSAR = it_zfit184-KOSAR
**                    BINARY SEARCH.
**            IF SY-SUBRC IS INITIAL.
**              WL_TABIX2 = SY-TABIX.
**
**
**              LOOP AT it_zfit184_C FROM WL_TABIX2.
***          LOOP AT it_zfit184_c WHERE nivel EQ it_zfit182-nivel
***                                          AND ktopl EQ it_zfit183-ktopl
***                                          AND saknr EQ it_zfit183-saknr
***                                          AND kosar EQ it_zfit184-kosar.
**
**                IF it_zfit184_C-NIVEL NE IT_zfit182-NIVEL
**                OR it_zfit184_C-KTOPL NE IT_zfit183-KTOPL
**                OR it_zfit184_C-SAKNR NE IT_zfit183-SAKNR
**                OR it_zfit184_C-KOSAR NE it_zfit184-KOSAR.
**                  EXIT.
**                ENDIF.
**
...
**        ENDIF.
**        "Centro de Lucro
**        LOOP AT IT_ZGL015_DRE_EST05 WHERE NIVEL EQ IT_zfit182-NIVEL AND KTOPL EQ IT_zfit183-KTOPL AND SAKNR EQ IT_zfit183-SAKNR.
**
**          CLEAR: it_zfit184_C,
**                 IT_ZGL015_DRE_EST06  .
**
...
**
**        "Grupo de Material
**        LOOP AT IT_ZGL015_DRE_EST06 WHERE NIVEL EQ IT_zfit182-NIVEL AND KTOPL EQ IT_zfit183-KTOPL AND SAKNR EQ IT_zfit183-SAKNR.
**
**          CLEAR: it_zfit184_C,
**                 IT_ZGL015_DRE_EST05  .
**          PERFORM INSERT_EST USING IT_zfit182 IT_zfit183 it_zfit184_C IT_ZGL015_DRE_EST05 IT_ZGL015_DRE_EST06 IT_ZGL015_DRE_EST07 SPACE.
...
**
**    LOOP AT IT_zfit182 WHERE NIVEL_TOTAL NE SPACE.
**
**      READ TABLE IT_ZGL015_DRE_EST07 WITH KEY NIVEL = IT_zfit182-NIVEL(2).
***      LOOP AT IT_ZGL015_DRE_EST07
***        WHERE NIVEL = IT_zfit182-NIVEL(2).
**
**
**      IF ( SY-SUBRC EQ 0 ).
...
*       text
*----------------------------------------------------------------------*
**  Begin of CS2022000708  #83806 FF   12.12.2022 14:59:40
**
**  FORM INSERT_EST USING P_zfit182   TYPE zfit182
**                        P_zfit183   TYPE zfit183
**                        P_zfit184_C TYPE TP_CUSTO
...
**          TEXT_SOMA     TYPE zfit182-NITXT,
**          WL_ZGL029_EST LIKE LINE OF IT_ZGL029_EST,
**          WL_TABIX      TYPE SY-TABIX.
**
**
**    CLEAR: IT_ZGL029_EST, IT_EXCEL.
**    IT_ZGL029_EST-NIVEL    = P_zfit182-NIVEL.
**    IT_ZGL029_EST-NITXT    = P_zfit182-NITXT.
**
**    IF NOT P_zfit183 IS INITIAL.
**      IT_ZGL029_EST-SAKNR    = P_zfit183-SAKNR.
**      READ TABLE IT_SKAT WITH KEY KTOPL = P_zfit183-KTOPL SAKNR = P_zfit183-SAKNR.
**      IF SY-SUBRC IS INITIAL.
...
**        LOOP AT IT_ZGL021_DRE_DADOS WHERE NIVEL EQ P_zfit183-NIVEL
**                                      AND SAKNR EQ P_zfit183-SAKNR.
**
...
**        ENDLOOP.
**        LOOP AT IT_ZGL025_DRE_ACM WHERE NIVEL EQ P_zfit183-NIVEL
**                                    AND SAKNR EQ P_zfit183-SAKNR.
**          IF TG_T005-WAERS EQ WA_DRE_DADOS_ALV-WAERS.
...
**    ENDIF.
**
**    IF NOT P_zfit184_C IS INITIAL.
**      IT_ZGL029_EST-KOSTL    = P_zfit184_C-KOSTL.
**      IT_ZGL029_EST-LTEXT_CC = P_zfit184_C-LTEXT.
**      IT_ZGL029_EST-VLR_REA  = 0.
**      IT_ZGL029_EST-VLR_ACM  = 0.
**      IT_ZGL029_EST-QTD_TON  = 0.
**      IT_ZGL029_EST-QTD_ACM  = 0.
...
**        WITH KEY NIVEL = P_zfit184_C-NIVEL
**                 SAKNR = P_zfit184_C-SAKNR
**                 KOSTL = P_zfit184_C-KOSTL
**                 BINARY SEARCH.
**
**      IF SY-SUBRC IS INITIAL.
**        WL_TABIX = SY-TABIX.
...
***                                    AND saknr EQ p_zfit184_c-saknr
***                                    AND kostl EQ p_zfit184_c-kostl.
**
**          IF IT_ZGL022_DRE_DADOS-NIVEL NE  P_zfit184_C-NIVEL
**          OR IT_ZGL022_DRE_DADOS-SAKNR NE  P_zfit184_C-SAKNR
**          OR IT_ZGL022_DRE_DADOS-KOSTL NE  P_zfit184_C-KOSTL.
**            EXIT.
**          ENDIF.
...
***        LOOP AT it_zgl026_dre_acm WHERE nivel EQ p_zfit184_c-nivel
***                                    AND saknr EQ p_zfit184_c-saknr
***                                    AND kostl EQ p_zfit184_c-kostl.
***        it_zgl029_est-vlr_acm = it_zgl029_est-vlr_acm + it_zgl026_dre_acm-vlr_rea.
**          IF IT_ZGL026_DRE_ACM-NIVEL NE  P_zfit184_C-NIVEL
**          OR IT_ZGL026_DRE_ACM-SAKNR NE  P_zfit184_C-SAKNR
**          OR IT_ZGL026_DRE_ACM-KOSTL NE  P_zfit184_C-KOSTL.
**            EXIT.
**          ENDIF.
**          IF TG_T005-WAERS EQ WA_DRE_DADOS_ALV-WAERS.
**            IT_ZGL029_EST-VLR_ACM = IT_ZGL029_EST-VLR_ACM + IT_ZGL026_DRE_ACM-VLR_REA.
...
*
    CLEAR: IT_zfit182.
    READ TABLE IT_zfit182 WITH KEY NIVEL = P_IT_ZGL029_EST-NIVEL.
*    read table it_zfit183 with key nivel = it_zfit182-nivel.
*    if sy-subrc is initial.
*      vg_achou = c_x.
*    else.
*      clear: it_zfit182.
*      read table it_zfit182 with key nivel_ant = it_zfit182-nivel.
*      if sy-subrc is initial.
*        read table it_zfit183 with key nivel = it_zfit182-nivel.
*        if sy-subrc is initial.
*          vg_achou = c_x.
...
*    if not vg_achou is initial.
    ...
    ELSEIF ( SY-SUBRC IS INITIAL ) ."AND ( IT_zfit182-NIVEL_TOTAL IS INITIAL ).
      LS_NODE-N_IMAGE   = ICON_NEXT_NODE.
      LS_NODE-EXP_IMAGE = ICON_PREVIOUS_NODE.
    ELSE.
