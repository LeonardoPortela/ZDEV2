class ZCL_INT_IB_PAR_TIPO_ENT_FILIAL definition
  public
  final
  create public .
...
public section.
  ...
  constants AT_ID_INTERFACE type ZDE_ID_INTERFACE value '102' ##NO_TEXT.
...
CLASS ZCL_INT_IB_PAR_TIPO_ENT_FILIAL IMPLEMENTATION.
  ...
  method ZIF_INTEGRACAO_INBOUND~VALIDAR_DADOS_INBOUND.
    ...
    DATA: lit_data_inbound  TYPE zsdt0001teln_t.
...
*-----------------------------------------------------------------------------------------------------------------------*
      ...
      CASE me->zif_integracao_inject~at_info_request_http-ds_metodo.
        WHEN C_POST OR C_DELETE.
          ...
          IF lwa_data-nr_safra IS INITIAL.
            r_msg_erro = 'Safra não foi informada!' && ' Index Objeto: ' && _tabix.
            RETURN.
          ENDIF.
          ...
          IF lwa_data-id_branch IS INITIAL.
            r_msg_erro = 'Filial não foi informada!' && ' Index Objeto: ' && _tabix.
            RETURN.
          ENDIF.
    ...
    LOOP AT lit_data_inbound INTO DATA(lwa_data_inbound).
      ...
      CASE me->zif_integracao_inject~at_info_request_http-ds_metodo.
        WHEN 'POST'. "Inclusão/Modificação
          ...
          SELECT SINGLE *
            FROM zsdt0001TEln INTO @DATA(lwa_zsdt0001TEln)
           WHERE id_entrada  EQ @lwa_data_inbound-id_entrada
             AND nr_safra    EQ @lwa_data_inbound-nr_safra
             AND id_bukrs    EQ @lwa_data_inbound-id_bukrs
             AND id_branch   EQ @lwa_data_inbound-id_branch.

          IF SY-SUBRC EQ 0.
            lwa_data_inbound-dt_registro = lwa_zsdt0001teln-dt_registro.
            lwa_data_inbound-hr_registro = lwa_zsdt0001teln-hr_registro.
          ELSE.
          ...
          MODIFY ZSDT0001TEln FROM lwa_data_inbound.
        ...
        WHEN 'DELETE'.
          ...
          SELECT SINGLE *
            FROM ZSDT0001TEln INTO lwa_zsdt0001teln
           WHERE id_entrada  EQ lwa_data_inbound-id_entrada
             AND nr_safra    EQ lwa_data_inbound-nr_safra
             AND id_bukrs    EQ lwa_data_inbound-id_bukrs
             AND id_branch   EQ lwa_data_inbound-id_branch.
