class ZCL_INT_IB_PARAM_LOC_EST_PERM definition
  public
  final
  create public .
...
public section.
  ...
  constants AT_ID_INTERFACE type ZDE_ID_INTERFACE value '106' ##NO_TEXT.
...
CLASS ZCL_INT_IB_PARAM_LOC_EST_PERM IMPLEMENTATION.
  ...
  method ZIF_INTEGRACAO_INBOUND~VALIDAR_DADOS_INBOUND.
    ...
    DATA: lit_data_inbound  TYPE zsdt0001lp_t.
...
*-----------------------------------------------------------------------------------------------------------------------*
      ...
      CASE me->zif_integracao_inject~at_info_request_http-ds_metodo.
        WHEN C_POST OR C_DELETE.
          ...
          IF lwa_data-id_material IS INITIAL.
            r_msg_erro = 'Id. Material não foi informado!' && ' Index Objeto: ' && _tabix.
            RETURN.
          ENDIF.

          IF lwa_data-id_bukrs IS INITIAL.
            r_msg_erro = 'Id. Empresa não foi informado!' && ' Index Objeto: ' && _tabix.
            RETURN.
          ENDIF.

          IF lwa_data-id_branch IS INITIAL.
            r_msg_erro = 'Id. Filial Entrega não foi informado!' && ' Index Objeto: ' && _tabix.
            RETURN.
          ENDIF.
    ...
    LOOP AT lit_data_inbound INTO DATA(lwa_data_inbound).
      ...
      CASE me->zif_integracao_inject~at_info_request_http-ds_metodo.
        WHEN 'POST'. "Inclusão/Modificação
          ...
          SELECT SINGLE *
            FROM zsdt0001lp INTO @DATA(lwa_zsdt0001lp)
           WHERE id_local_entrega EQ @lwa_data_inbound-id_local_entrega
             and id_material      EQ @lwa_data_inbound-id_material
             and id_bukrs         EQ @lwa_data_inbound-id_bukrs
             and id_branch        EQ @lwa_data_inbound-id_branch.

          IF SY-SUBRC EQ 0.
            lwa_data_inbound-dt_registro = lwa_zsdt0001lp-dt_registro.
            lwa_data_inbound-hr_registro = lwa_zsdt0001lp-hr_registro.
          ELSE.
          ...
          MODIFY zsdt0001lp FROM lwa_data_inbound.
        ...
        WHEN 'DELETE'.
          ...
          SELECT SINGLE *
            FROM zsdt0001lp INTO lwa_zsdt0001lp
           WHERE id_local_entrega EQ lwa_data_inbound-id_local_entrega
             and id_material      EQ lwa_data_inbound-id_material
             and id_bukrs         EQ lwa_data_inbound-id_bukrs
             and id_branch        EQ lwa_data_inbound-id_branch.
