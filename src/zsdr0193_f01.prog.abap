...
FORM criar_field_catalog.
  ...
  PERFORM estrutura_alv USING:
      ...
      0  'ZSDT0363'  'REGIO'            'IT_SAIDA' 'REGIO'                 'UF'               '10'  'X'    '' ' ' ' ' ' ' 'X' ' ' ' ',
      1  ''          'DESC_REGIO'       'IT_SAIDA' 'DESC_REGIO'            'Descrição'        '40'  ''     '' ' ' ' ' ' ' '' ' ' ' ',
      2  'ZSDT0363'  'DATA_CRIACAO'     'IT_SAIDA' 'DATA_CRIACAO'          'Data Criação'     '10'  ''     '' ' ' ' ' ' ' 'X' ' ' ' ',
      3  'ZSDT0363'  'HORA_CRIACAO'     'IT_SAIDA' 'HORA_CRIACAO'          'Hora Criação'     '10'  ''     '' ' ' ' ' ' ' 'X' ' ' ' ',
      4  'ZSDT0363'  'USUARIO_CRIACAO'  'IT_SAIDA' 'USUARIO_CRIACAO'       '  '               '30'  ''     '' ' ' ' ' ' ' 'X' ' ' ' ',
      5  'ZSDT0363'  'EXCLUIDO'         'IT_SAIDA' 'EXCLUIDO'              '  '               '2'   'X'    '' ' ' ' ' ' ' '' ' ' 'X',
      6  'ZSDT0363'  'DATA_EXCLUSAO'    'IT_SAIDA' 'DATA_EXCLUSAO'         '  '               '2'   'X'    '' ' ' ' ' ' ' '' ' ' 'X',
      7  'ZSDT0363'  'HORA_EXCLUSAO'    'IT_SAIDA' 'HORA_EXCLUSAO'         '  '               '2'   'X'    '' ' ' ' ' ' ' '' ' ' 'X',
      8  'ZSDT0363'  'USUARIO_EXCLUSAO' 'IT_SAIDA' 'USUARIO_EXCLUSAO'      '  '               '2'   'X'    '' ' ' ' ' ' ' '' ' ' 'X'.
...
FORM selecionar_dados .
  ...
  SELECT *
    INTO TABLE it_ZSDT0363
    FROM zsdt0363
    WHERE excluido EQ space.
  IF sy-subrc IS INITIAL.
    DATA(lt_ZSDT0363) = it_ZSDT0363.
    SORT lt_ZSDT0363 BY regio.
    DELETE ADJACENT DUPLICATES FROM lt_ZSDT0363 COMPARING regio.

    SELECT *
      FROM t005u
      INTO TABLE @DATA(lt_t005u)
      FOR ALL ENTRIES IN @lt_ZSDT0363
      WHERE bland = @lt_ZSDT0363-regio
        AND land1 = 'BR'
        AND spras = @sy-langu.
    IF sy-subrc IS INITIAL.
  ...
  LOOP AT it_ZSDT0363 INTO wa_ZSDT0363.
    CLEAR: wa_saida.
    MOVE-CORRESPONDING wa_ZSDT0363 TO wa_saida.

    READ TABLE lt_t005u ASSIGNING FIELD-SYMBOL(<fs_t005u>)
    WITH KEY bland = wa_saida-regio
    BINARY SEARCH.
    IF sy-subrc IS INITIAL.
      wa_saida-desc_REGIO = <fs_t005u>-bezei.
    ENDIF.

    ls_style-fieldname = 'REGIO'.
...
FORM deletar_reg.
  ...
  MOVE-CORRESPONDING wa_saida TO wa_ZSDT0363.
  UPDATE zsdt0363 SET excluido          = abap_true
                      data_exclusao     = sy-datum
                      hora_exclusao     = sy-uzeit
                      usuario_exclusao  = sy-uname
                  WHERE regio           = wa_ZSDT0363-regio
                    AND data_criacao    = wa_ZSDT0363-data_criacao
                    AND hora_criacao    = wa_ZSDT0363-hora_criacao
                    AND usuario_criacao = wa_ZSDT0363-usuario_criacao.

  DELETE it_saida WHERE regio            = wa_ZSDT0363-regio
                         AND data_criacao    = wa_ZSDT0363-data_criacao
                         AND hora_criacao    = wa_ZSDT0363-hora_criacao
                         AND usuario_criacao = wa_ZSDT0363-usuario_criacao
                         AND excluido        = wa_ZSDT0363-excluido.
...
FORM salvar_reg .
  ...
  IF gv_modif IS NOT INITIAL AND gv_erro IS INITIAL.
    ...
    DELETE it_saida WHERE desc_REGIO IS INITIAL.
    ...
    IF it_saida IS NOT INITIAL.
      ...
      LOOP AT it_saida ASSIGNING <l_saida>.
        ...
        MOVE-CORRESPONDING <l_saida> TO wa_ZSDT0363.

        IF wa_ZSDT0363-regio IS INITIAL.
          ROLLBACK WORK.
        ...
        MODIFY zsdt0363 FROM wa_ZSDT0363.
      ENDLOOP.
