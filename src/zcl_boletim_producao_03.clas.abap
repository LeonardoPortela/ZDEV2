class ZCL_BOLETIM_PRODUCAO_03 definition
  public
...
public section.
  ...
  methods ZIF_BOLETIM_PRODUCAO~GERAR_RATEIO_PRODUTOS
    redefinition .
  methods ZIF_BOLETIM_PRODUCAO~GET_DOC_BOLETIM_VALIDO
    redefinition .
...
CLASS ZCL_BOLETIM_PRODUCAO_03 IMPLEMENTATION.
...
*------------------------Inicio verifica produto terceiro / CS20230000708 / AOENNING
    ...
    DATA(wl_zsdt0250) = me->zif_boletim_producao~get_material_boletim( i_tp_produto =  'OI'). "Oleo degomado industrializado próprio.
    ELSE.
    wl_zsdt0250       = me->zif_boletim_producao~get_material_boletim( i_tp_produto =  'OT'). "Oleo degomado industrializado de terceiro.
    ENDIF.
...
*    es_bflushdatagen-materialnr    = wl_zsdt0250-matnr.                "Material
    ...
    IF v_len > 18.
      CLEAR: es_bflushdatagen-materialnr.                                  "Material
      es_bflushdatagen-materialnr_long = wl_zsdt0250-matnr.                "Material
    ELSE.
      es_bflushdatagen-materialnr      = wl_zsdt0250-matnr.                "Material
      CLEAR: es_bflushdatagen-materialnr_long.                             "Material
    ENDIF.
*<--- 29/06/2023 - Migração S4 - JP - fim

    es_bflushdatagen-backflquant    = wl_zsdt0252-qtde_oi_doc_01.                          "Quantidade
...
*------------------------Inicio verifica produto terceiro / CS20230000708 / AOENNING
    IF me->zif_boletim_producao~AT_OI_MAT_TERCEIRO EQ ABAP_FALSE.
    wl_zsdt0250 = me->zif_boletim_producao~get_material_boletim( i_tp_produto =  'OI'). "Oleo degomado industrializado próprio.
    ELSE.
    wl_zsdt0250       = me->zif_boletim_producao~get_material_boletim( i_tp_produto =  'OT'). "Oleo degomado industrializado de terceiro.
    ENDIF.
*------------------------Fim verifica produto terceiro / CS20230000708 / AOENNING
...
*   wa_goodsmovements-material        = wl_zsdt0250-matnr.             "Material Consumo
    v_len = strlen(  wl_zsdt0250-matnr ).
    IF v_len > 18.
      CLEAR: wa_goodsmovements-material.
      wa_goodsmovements-material_long = wl_zsdt0250-matnr.             "Material Consumo
    ELSE.
      wa_goodsmovements-material      = wl_zsdt0250-matnr.             "Material Consumo
      CLEAR: wa_goodsmovements-material_long.
    ENDIF.
*<--- 29/06/2023 - Migração S4 - JP - fim
...
*    wa_goodsmovements-batch         = wl_zsdt0252-charg.          "Lote Consumo
    ...
    wa_goodsmovements-entry_qnt     = wl_zsdt0252-qtde_oi_doc_01. "Quantidade Consumo
*    wa_goodsmovements-entry_uom     = 'KG'.                       "UM Consumo
    ...
    wl_zsdt0250 = me->zif_boletim_producao~get_material_boletim( i_tp_produto =  'OD').
...
*<--- 29/06/2023 - Migração S4 - JP - fim
    ...
    wa_goodsmovements-entry_qnt     = wl_zsdt0252-qtde_od_doc_01. "Quantidade Consumo
*    wa_goodsmovements-entry_uom     = 'KG'.                       "UM Consumo
    me->zif_boletim_producao~get_und_material( EXPORTING i_matnr =  wl_zsdt0250-matnr IMPORTING e_meins = wa_goodsmovements-entry_uom ).
    APPEND wa_goodsmovements TO it_goodsmovements.
...
*<--- 29/06/2023 - Migração S4 - JP - fim
  ...
  METHOD zif_boletim_producao~gerar_doc_producao_02.
    ...
    DATA: v_qtde TYPE zsdt0252-qtde_af_doc_02,
          saldo  TYPE zsdt0252-qtde_af_doc_02.
...
*    IF zcl_boletim_producao=>zif_boletim_producao~at_date_doc IS INITIAL.
*      zcl_boletim_producao=>zif_boletim_producao~at_date_doc = sy-datum .
*    ENDIF.
*
*    IF zcl_boletim_producao=>zif_boletim_producao~at_date_lanc IS INITIAL.
*      zcl_boletim_producao=>zif_boletim_producao~at_date_lanc = sy-datum.
*    ENDIF.


    ls_header = VALUE #( pstng_date = sy-datum "Data lançamento
                           doc_date = sy-datum "Data documento
                         ref_doc_no = ''
...
*alterado por Guilherme Rabelo fim 26.04.2023
    "================================================================================================================================================
    "Soda Caustica
    DATA(wl_zsdt0250) = zcl_boletim_producao=>zif_boletim_producao~get_material_boletim( i_tp_produto =  'SC').
...
*---> 28/06/2023 - Migração S4 - DG
    DATA(v_len) = strlen(  wl_zsdt0250-matnr ).
    DATA(lv_MATERIAL)      = wl_zsdt0250-matnr.
    DATA(lv_MATERIAL_long) = wl_zsdt0250-matnr.
    IF v_len > 18.
      CLEAR: lv_MATERIAL.
    ELSE.
      CLEAR: lv_MATERIAL_long.
    ENDIF.
*<--- 28/06/2023 - Migração S4 - JP
...
*<--- 28/06/2023 - Migração S4 - JP
                          ...
                          entry_qnt   = wl_zsdt0252-qtde_sc_doc_02 "Soda Caustica
                          entry_uom   = meins
                          batch       = wa_mchb-charg
                          move_type   = '201'
                          costcenter  = '0010550462'        "10550462
                          gl_account  = '0000412007'        "0000412007
                        ) TO lt_item.
...
*
            ...
            CLEAR: saldo.
*            IF wa_mchb-clabs >= wl_zsdt0252-qtde_sc_doc_02.
*              wl_zsdt0252-qtde_sc_doc_02 = 0.
*            ELSE.
*              wl_zsdt0252-qtde_sc_doc_02 = wl_zsdt0252-qtde_sc_doc_02 - wa_mchb-clabs.
*            ENDIF.
          ENDLOOP.
        ELSE.
...
*<--- 28/06/2023 - Migração S4 - JP
    ...
    "================================================================================================================================================
    "Acido Fosforico
    CLEAR: wl_zsdt0250, saldo, wa_mara, v_qtde, saldo.
    wl_zsdt0250 = zcl_boletim_producao=>zif_boletim_producao~get_material_boletim( i_tp_produto =  'AF').
    me->zif_boletim_producao~get_und_material( EXPORTING i_matnr = wl_zsdt0250-matnr IMPORTING e_meins = meins ).
...
*---> 28/06/2023 - Migração S4 - JP
    CLEAR: v_len, lv_MATERIAL_long, lv_MATERIAL.
    v_len = strlen(  wl_zsdt0250-matnr ).

    IF v_len > 18.
      lv_MATERIAL_long = wl_zsdt0250-matnr.
    ELSE.
      lv_MATERIAL      = wl_zsdt0250-matnr.
    ENDIF.
*<--- 28/06/2023 - Migração S4 - JP
    ...
    IF sy-subrc EQ 0.
      ...
      IF sy-subrc EQ 0.
        ...
        ENDLOOP.
*
        IF v_qtde >= wl_zsdt0252-qtde_af_doc_02.
*
          CLEAR: wa_mchb.
          LOOP AT t_mchb INTO wa_mchb.
*
            IF wl_zsdt0252-qtde_af_doc_02 EQ 0.
              EXIT.
            ENDIF.

            "Verifica se a quantidade é suficiente para o consumuo do material.
            IF wa_mchb-clabs >= wl_zsdt0252-qtde_af_doc_02.
              saldo = 0.
            ELSE.
              saldo = wl_zsdt0252-qtde_af_doc_02 - wa_mchb-clabs.
              wl_zsdt0252-qtde_af_doc_02 = wa_mchb-clabs.
            ENDIF.
...
*<--- 28/06/2023 - Migração S4 - JP
                        plant       = wl_zsdt0252-werks_v
                        stge_loc    = 'IN02'"wl_zsdt0252-lgort_v
                        entry_qnt   = wl_zsdt0252-qtde_af_doc_02 "Acido Fosforico
                        entry_uom   = meins
                        batch       = wa_mchb-charg
                        move_type   = '201'
                        costcenter  = '0010550462'          "10550462
...
*<--- 28/06/2023 - Migração S4 - JP
                           plant     = wl_zsdt0252-werks_v
                           stge_loc  = 'IN02'"wl_zsdt0252-lgort_v
                           entry_qnt = wl_zsdt0252-qtde_af_doc_02 ) TO it_goodsmovements_aux.
*

            "Verifica se o lote tem saldo suficiente para quantidade solicitada.
            wl_zsdt0252-qtde_af_doc_02 = saldo.
            CLEAR: saldo.
*            IF wa_mchb-clabs >= wl_zsdt0252-qtde_af_doc_02.
*              wl_zsdt0252-qtde_af_doc_02 = 0.
*            ELSE.
*              wl_zsdt0252-qtde_af_doc_02 = wl_zsdt0252-qtde_af_doc_02 - wa_mchb-clabs.
*            ENDIF.
          ENDLOOP.
...
*--------------------------------------------------------------------------------------------*
    ...
    "Óleo Neutro
    DATA(wl_zsdt0250) = me->zif_boletim_producao~get_material_boletim( i_tp_produto =  'ON').
...
*<--- 30/06/2023 - Migração S4 - JP
    ...
    es_bflushdatagen-backflquant    = wl_zsdt0252-qtde_on_doc_03.                          "Quantidade
*    es_bflushdatagen-unitofmeasure  = 'KG'.                                                "UM
    ...
    es_bflushdatagen-prodversion    = me->zif_boletim_producao~get_versao( i_matnr = CONV #( wl_zsdt0250-matnr  ) i_werks = CONV #( wl_zsdt0252-werks_v ) ). "Versão
*    es_bflushdatagen-prodversion = |{ es_bflushdatagen-prodversion ALPHA = IN }|.
*    es_bflushdatagen-batch          = wl_zsdt0252-charg.      "Lote
    es_bflushdatagen-storageloc     = wl_zsdt0252-lgort_v.    "Deposito
...
*    wa_goodsmovements-batch         = wl_zsdt0252-charg.                                  "Lote Consumo
    wa_goodsmovements-move_type     = '131'.                                              "Tipo Movimento
    wa_goodsmovements-entry_qnt     = wl_zsdt0252-qtde_on_doc_03.                         "Quantidade Consumo
*    wa_goodsmovements-entry_uom     = 'KG'.                                               "UM Consumo
    me->zif_boletim_producao~get_und_material(
      EXPORTING
        i_matnr =     wl_zsdt0250-matnr
    ...
    wl_zsdt0250 = me->zif_boletim_producao~get_material_boletim( i_tp_produto =  'BO').
...
*    wa_goodsmovements-batch         = wl_zsdt0252-charg.                                  "Lote Consumo
    ...
    wa_goodsmovements-entry_qnt     = wl_zsdt0252-qtde_bo_doc_03.                         "Quantidade Consumo
*    wa_goodsmovements-entry_uom     = 'KG'.                                               "UM Consumo
    me->zif_boletim_producao~get_und_material(
      EXPORTING
        i_matnr =     wl_zsdt0250-matnr
    ...
    LOOP AT t_zsdt0247 ASSIGNING FIELD-SYMBOL(<ws_zsdt0247>).
      LOOP AT t_zsdt0252 ASSIGNING FIELD-SYMBOL(<ws_zsdt0252>) WHERE id_boletim EQ <ws_zsdt0247>-id_boletim.
        CASE <ws_zsdt0247>-tp_produto_producao.
          WHEN 'OI'.
            <ws_zsdt0252>-qtde_od_doc_01 = <ws_zsdt0247>-qtde_consumo.
            <ws_zsdt0252>-qtde_oi_doc_01 = <ws_zsdt0247>-qtde_consumo.
          WHEN 'SC'.
    ...
    LOOP AT t_zsdt0248 ASSIGNING FIELD-SYMBOL(<ws_zsdt0248>).
      LOOP AT t_zsdt0252 ASSIGNING <ws_zsdt0252> WHERE id_boletim EQ <ws_zsdt0247>-id_boletim.
        CASE <ws_zsdt0248>-tp_produto_producao.
          WHEN 'ON'.
            <ws_zsdt0252>-qtde_on_doc_03 = <ws_zsdt0248>-qtde.
          WHEN 'BO'.
            <ws_zsdt0252>-qtde_bo_doc_03 = <ws_zsdt0248>-qtde.
          WHEN OTHERS.
        ENDCASE.
      ENDLOOP.
    ENDLOOP.
      ...
      TRY.
          ...
          CASE <fs_zsdt0248>-tp_produto_producao.
            WHEN 'ON'. "Oleo neutro
              ...
              READ TABLE i_zsdt0247 INTO wl_zsdt0247 WITH KEY tp_produto_producao = 'OI'.
...
*    APPEND wl_zsdt0250 TO r_tp_produtos.
    ...
    wl_zsdt0250 = zcl_boletim_producao=>zif_boletim_producao~get_material_boletim( i_tp_produto =  'AF'). "Acido Fosforico
    APPEND wl_zsdt0250 TO r_tp_produtos.
  ...
  METHOD zif_boletim_producao~get_tp_produtos_rendimento.
    ...
    DATA(wl_zsdt0250) = zcl_boletim_producao=>zif_boletim_producao~get_material_boletim( i_tp_produto =  'ON'). "Oleo neutro
    APPEND wl_zsdt0250 TO r_tp_produtos.

*    wl_zsdt0250 = zcl_boletim_producao=>zif_boletim_producao~get_material_boletim( i_tp_produto =  'OI'). "Oleo degomado industrializado
*    APPEND wl_zsdt0250 TO r_tp_produtos.

    wl_zsdt0250 = zcl_boletim_producao=>zif_boletim_producao~get_material_boletim( i_tp_produto =  'BO'). "Borra de oleo
    APPEND wl_zsdt0250 TO r_tp_produtos.
...
*       AND charg      EQ @wl_zsdt0252_ck-charg
    ...
    DATA(wl_zsdt0250) = zcl_boletim_producao=>zif_boletim_producao~get_material_boletim( i_tp_produto =  'OI'). " "Oleo degomado industrializado próprio.
      ...
      WHEN '03'.
        ...
        "Oleo degomado industrializado
        wl_zsdt0250 = zcl_boletim_producao=>zif_boletim_producao~get_material_boletim( i_tp_produto =  'OI'). " "Oleo degomado industrializado próprio.
...
*-----------------------------------------------------------------------------------------------------*
    ...
    DATA(_gerar_oi)       = abap_false.
    DATA(_gerar_ac)       = abap_false.
    DATA(_gerar_af)       = abap_false.
    ...
    LOOP AT it_produtos_prod INTO DATA(wl_produto_prod).
      ...
      CONCATENATE 'da Produção de oleo neutro( Consumo ' v_msg_aux ')' INTO v_msg_aux SEPARATED BY space.
    ...
    IF ( _dados_prod_inf EQ abap_false ).
      v_msg_aux = 'Dados Produção de Oleo Neutro!'.
      RAISE EXCEPTION TYPE zcx_boletim_producao
        EXPORTING
          textid = VALUE #( msgid = zcx_boletim_producao=>zcx_dados_incompletos-msgid
                            msgno = zcx_boletim_producao=>zcx_dados_incompletos-msgno
    ...
    IF  me->zif_boletim_producao~at_cabecalho-qtde_si IS INITIAL.
      RAISE EXCEPTION TYPE zcx_boletim_producao
        EXPORTING
          textid = VALUE #( msgid = zcx_boletim_producao=>zcx_data_not_informed-msgid
                            msgno = zcx_boletim_producao=>zcx_data_not_informed-msgno
                            attr1 = CONV #( 'Quantidade Oleo Neutro Industrialização' )
                           )
          msgty  = 'E'
          msgno  = zcx_boletim_producao=>zcx_data_not_informed-msgno
          msgid  = zcx_boletim_producao=>zcx_data_not_informed-msgid
          msgv1  = CONV #( 'Quantidade Oleo Neutro Industrialização' ).
    ENDIF.
...
*-----------------------------------------------------------------------------------------------------*
    ...
    LOOP AT me->zif_boletim_producao~at_dados_rendimento INTO wl_dados_rend.
      ...
      CASE wl_dados_rend-tp_produto_producao.
        WHEN 'OI'. "Oleo degomado industrializado
          CHECK _gerar_oi EQ abap_true.

        WHEN 'ON'. "Soda caustica
          CHECK _gerar_ac EQ abap_true.
        WHEN 'BO'. "Acido Fosforico
          CHECK _gerar_af EQ abap_true.
        WHEN OTHERS.
