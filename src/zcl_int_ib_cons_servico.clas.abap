CLASS zcl_int_ib_cons_servico DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      BEGIN OF ty_amsd,
        ...
        lvorm TYPE   asmd-lvorm,
        matkl TYPE   asmd-matkl,
        mstae TYPE   asmd-mstae,
      END OF ty_amsd.

    DATA:
      tb_asnum TYPE TABLE OF asnum .
    DATA:
      BEGIN OF z_table_asmdt,
       asnum      TYPE asmdt-asnum,
       asktx      TYPE asmdt-asktx,
       meins      TYPE asmd-meins,
       lvorm      TYPE asmd-lvorm,
       matkl      TYPE asmd-matkl,
       mstae      TYPE asmd-mstae,
      END OF z_table_asmdt .

      DATA: it_asmd TYPE STANDARD TABLE OF ty_amsd.
    ...
    DATA:
      BEGIN OF zde_data_response,
        servicos LIKE TABLE OF z_table_asmdt,
      END OF zde_data_response .
    CONSTANTS at_id_interface TYPE zde_id_interface VALUE '161' ##NO_TEXT.
...
CLASS ZCL_INT_IB_CONS_SERVICO IMPLEMENTATION.
...
*-----------------------------------------------------------------------------------------------------------------------*
  ...
  METHOD zif_integracao_inject~set_integrar_inbound.
    ...
    DATA: lra_asnum  TYPE RANGE OF asnum.
    ...
    LOOP AT lwa_data_request-asnum INTO DATA(wa_asnum).
      CALL FUNCTION 'CONVERSION_EXIT_ALPHA_INPUT'
        EXPORTING
          input  = wa_asnum
        IMPORTING
          output = wa_asnum.
      APPEND VALUE #( sign = 'I' option = 'EQ' low = wa_asnum ) TO lra_asnum.
    ENDLOOP.
    SELECT t~asnum,
           t~asktx,
           s~meins,
      ...
      INTO TABLE @it_asmd
    FROM asmd AS s
     INNER JOIN asmdt AS t
      ON s~asnum = t~asnum
      AND t~spras = 'P'
     WHERE s~asnum IN @lra_asnum.
    IF sy-subrc IS INITIAL.
      SORT it_asmd BY asnum.

      LOOP AT it_asmd INTO DATA(ls_asmd).
        APPEND INITIAL LINE TO lwa_data_response-servicos ASSIGNING FIELD-SYMBOL(<fs_response>).
        <fs_response>-asnum = ls_asmd-asnum.
        <fs_response>-asktx = ls_asmd-asktx.
        <fs_response>-lvorm = ls_asmd-lvorm.
        <fs_response>-matkl = ls_asmd-matkl.
        <fs_response>-meins = ls_asmd-meins.
        <fs_response>-mstae = ls_asmd-mstae.
      ENDLOOP.
    ENDIF.
