...
*&                                          |Chamado: 174132.                                            *
*--------------------------------------------------------------------------------------------------------*
class ZCL_INT_OB_GET_FRETE_CARGUERO definition
  public
  final
  create public .
...
public section.
  ...
  constants AT_ID_INTERFACE type ZDE_ID_INTERFACE value '310' ##NO_TEXT.
  constants:
    at_servico      TYPE c LENGTH 100 value 'CARGUERO_HOST_TERCEIRO' ##NO_TEXT.
  data AT_DADOS_VIAGEM type ZLEST0181 .
...
CLASS ZCL_INT_OB_GET_FRETE_CARGUERO IMPLEMENTATION.
    ...
    TRY .
        CAST zcl_integracao_token(
               zcl_integracao_token=>zif_integracao_token~get_instance(
                 )->set_empresa_token( EXPORTING i_bukrs = me->at_dados_viagem-bukrs
                 )
  ...
  METHOD zif_integracao_outbound~build_info_request.
    ...
    DATA: lwa_viagem TYPE zlest0181.

    r_if_integracao_outbound = me.
    MOVE-CORRESPONDING i_info_request TO lwa_viagem.

    SELECT SINGLE *
      FROM zlest0181 INTO me->at_dados_viagem
     WHERE nro_cg_sai_in = lwa_viagem-nro_cg_sai_in.

    me->at_id_referencia =  me->at_dados_viagem-id_carguero.

    IF sy-subrc NE 0 OR lwa_viagem-nro_cg_sai_in IS INITIAL.
      RAISE EXCEPTION TYPE zcx_integracao
        EXPORTING
          textid = VALUE #( msgid = zcx_integracao=>zcx_erro_geral-msgid
                            msgno = zcx_integracao=>zcx_erro_geral-msgno
                            attr1 = 'Viagem não encontrada! Id:'
                            attr2 = CONV #( lwa_viagem-nro_cg_sai_in ) )
          msgid  = zcx_integracao=>zcx_erro_geral-msgid
          msgno  = zcx_integracao=>zcx_erro_geral-msgno
          msgty  = 'E'
          msgv1  = 'Viagem não encontrada! Id:'
          msgv2  = CONV #( lwa_viagem-nro_cg_sai_in ).
    ENDIF.
  ...
  METHOD zif_integracao_outbound~get_id_referencia.
    ...
    r_if_integracao_outbound = me.
    e_referencia-tp_referencia = 'FRETE_CARGUERO'.
  ...
  METHOD zif_integracao_outbound~get_instance.
    ...
    IF zif_integracao_outbound~at_if_integracao_outbound IS NOT BOUND.
      CREATE OBJECT zif_integracao_outbound~at_if_integracao_outbound TYPE zcl_int_ob_get_frete_carguero.
    ENDIF.
  ...
  METHOD zif_integracao_outbound~set_url.
    ...
    me->zif_integracao_inject~at_info_request_http-ds_url                = lwa_webservice-url && 'lotes-embarcadores/' && me->at_dados_viagem-id_carguero.
