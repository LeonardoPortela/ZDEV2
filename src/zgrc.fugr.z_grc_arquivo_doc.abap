...
*"     REFERENCE(I_CNPJ) TYPE  ZXNFE_CNPJ OPTIONAL
  ...
  ELSEIF i_chave IS NOT INITIAL.
    ...
    SELECT SINGLE * INTO @wa_j_1bnfe_active
     ...
     WHERE regio    EQ @wa_active-regio
       ...
       AND NOT EXISTS ( SELECT * FROM zib_nfe_dist_ter AS c WHERE c~chave_nfe EQ @i_chave ).

    IF sy-subrc IS NOT INITIAL.

      TRY .
          zcl_doc_eletronico=>zif_doc_eletronico~get_xml_terceiro(
            EXPORTING
  ...
  CASE tp_doc.
    WHEN 'OUT'.
      ...
      IF wa_j_1bnfe_active-cancel EQ abap_true.
        RAISE EXCEPTION TYPE zcx_doc_eletronico
          EXPORTING
            textid = VALUE #( msgid = zcx_doc_eletronico=>zcx_doc_cancel-msgid
                              msgno = zcx_doc_eletronico=>zcx_doc_cancel-msgno
                              attr1 = wa_j_1bnfe_active-nfnum9
                              attr2 = wa_j_1bnfe_active-serie
                              attr3 = wa_j_1bnfe_active-model
                              attr4 = wa_j_1bnfe_active-stcd1
                               )
            msgid  = zcx_doc_eletronico=>zcx_doc_cancel-msgid
            msgno  = zcx_doc_eletronico=>zcx_doc_cancel-msgno
