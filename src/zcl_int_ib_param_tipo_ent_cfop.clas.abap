class ZCL_INT_IB_PARAM_TIPO_ENT_CFOP definition
  public
  final
  create public .
...
public section.
  ...
  constants AT_ID_INTERFACE type ZDE_ID_INTERFACE value '101' ##NO_TEXT.
...
CLASS ZCL_INT_IB_PARAM_TIPO_ENT_CFOP IMPLEMENTATION.
  ...
  method ZIF_INTEGRACAO_INBOUND~VALIDAR_DADOS_INBOUND.
    ...
    DATA: lit_data_inbound  TYPE zsdt0001tecfop_t.
...
*-----------------------------------------------------------------------------------------------------------------------*
      ...
      CASE me->zif_integracao_inject~at_info_request_http-ds_metodo.
        WHEN C_POST OR C_DELETE.
          ...
          IF lwa_data-cfop IS INITIAL.
            r_msg_erro = 'CFOP não foi informado!' && ' Index Objeto: ' && _tabix.
            RETURN.
          ENDIF.

          IF lwa_data-regio IS INITIAL.
            r_msg_erro = 'Estado não foi informado!' && ' Index Objeto: ' && _tabix.
            RETURN.
          ENDIF.

          IF lwa_data-tp_pessoa IS INITIAL.
            r_msg_erro = 'Tipo Pessoa não foi informado!' && ' Index Objeto: ' && _tabix.
            RETURN.
          ENDIF.
    ...
    LOOP AT lit_data_inbound INTO DATA(lwa_data_inbound).
      ...
      CASE me->zif_integracao_inject~at_info_request_http-ds_metodo.
        WHEN 'POST'. "Inclusão/Modificação
          ...
          SELECT SINGLE *
            FROM zsdt0001TECFOP INTO @DATA(lwa_zsdt0001TECFOP)
           WHERE id_entrada EQ @lwa_data_inbound-id_entrada
             AND cfop       EQ @lwa_data_inbound-cfop
             AND regio      EQ @lwa_data_inbound-regio
             AND tp_pessoa  EQ @lwa_data_inbound-tp_pessoa.

          IF SY-SUBRC EQ 0.
            lwa_data_inbound-dt_registro = lwa_zsdt0001tecfop-dt_registro.
            lwa_data_inbound-hr_registro = lwa_zsdt0001tecfop-hr_registro.
          ELSE.
          ...
          MODIFY ZSDT0001TECFOP FROM lwa_data_inbound.
        ...
        WHEN 'DELETE'.
          ...
          SELECT SINGLE *
            FROM ZSDT0001TECFOP INTO lwa_zsdt0001tecfop
           WHERE id_entrada EQ lwa_data_inbound-id_entrada
             AND cfop       EQ lwa_data_inbound-cfop
             AND regio      EQ lwa_data_inbound-regio
             AND tp_pessoa  EQ lwa_data_inbound-tp_pessoa.
