...
* ATUALIZA saldo da OV.
    ...
    LOOP AT it_saida ASSIGNING <f_saida> WHERE vbeln EQ i_saida-vbeln.
        ...
        IF sy-subrc EQ 0.
          IF <w_ov>-status EQ 'Y'.
            ...
            IF w_vbfa IS NOT INITIAL.
              ...
              <w_saida>-vbeln_g = w_vbfa-vbeln.
              <w_saida>-vbeln_p = w_vbfa-vbelv.
            ELSE.
              <w_saida>-vbeln_s = i_saida-vbeln_s.
...
*       AND b~zterm NE 'I006'
    ...
    IF t_vbak IS NOT INITIAL.
      ...
      SELECT rk~vbeln rk~fkart rk~fktyp rk~zterm fa~vbelv
      FROM   vbrk  AS rk
      INNER JOIN vbfa AS fa  ON fa~vbeln = rk~vbeln
      INTO TABLE ti_vbrk
...
*    WA_SAIDA-TX_JUROS = WA_ZSDT0051-TX_JUROS. "Comentado Aoenning
    ...
    IF i_saida-auart EQ 'ZRPF'  OR  i_saida-auart EQ 'ZROB'  OR  i_saida-auart EQ 'ZREB'.
      xtotalq_ov  = xtotalq_ov  * -1.
      xtotalvl_ov = xtotalvl_ov * -1.
...
FORM get_text_editor .
  ...
  IF sy-subrc IS INITIAL.
    MESSAGE 'Já existe solicitação de isenção de juros para a OV selecionada' TYPE 'S' DISPLAY LIKE 'S'.
    SET SCREEN 0.
    LEAVE SCREEN.
  ENDIF.
...
FORM f_exibe_estrategia .
  ...
  SELECT *
    ...
    WHERE valor_de <= @wa_cabec-saldo
      AND valor_ate >= @wa_cabec-saldo.
  IF sy-subrc IS INITIAL.
    LOOP AT lt_0336 ASSIGNING FIELD-SYMBOL(<fs_0336>).
