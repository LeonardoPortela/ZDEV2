...
FORM criar_field_catalog.
  ...
  PERFORM estrutura_alv USING:
      ...
      0  'ZSDT0362'  'CFOP'             'IT_SAIDA' 'CFOP'                  'CFOP'             '10'  'X'    '' ' ' ' ' ' ' 'X' ' ' ' ',
      1  ''          'DESC_CFOP'        'IT_SAIDA' 'DESC_CFOP'             'Descrição'        '40'  ''     '' ' ' ' ' ' ' '' ' ' ' ',
      2  'ZSDT0362'  'DATA_CRIACAO'     'IT_SAIDA' 'DATA_CRIACAO'          'Data Criação'     '10'  ''     '' ' ' ' ' ' ' 'X' ' ' ' ',
      3  'ZSDT0362'  'HORA_CRIACAO'     'IT_SAIDA' 'HORA_CRIACAO'          'Hora Criação'     '10'  ''     '' ' ' ' ' ' ' 'X' ' ' ' ',
      4  'ZSDT0362'  'USUARIO_CRIACAO'  'IT_SAIDA' 'USUARIO_CRIACAO'       '  '               '30'  ''     '' ' ' ' ' ' ' 'X' ' ' ' ',
      5  'ZSDT0362'  'EXCLUIDO'         'IT_SAIDA' 'EXCLUIDO'              '  '               '2'   'X'    '' ' ' ' ' ' ' '' ' ' 'X',
      6  'ZSDT0362'  'DATA_EXCLUSAO'    'IT_SAIDA' 'DATA_EXCLUSAO'         '  '               '2'   'X'    '' ' ' ' ' ' ' '' ' ' 'X',
      7  'ZSDT0362'  'HORA_EXCLUSAO'    'IT_SAIDA' 'HORA_EXCLUSAO'         '  '               '2'   'X'    '' ' ' ' ' ' ' '' ' ' 'X',
      8  'ZSDT0362'  'USUARIO_EXCLUSAO' 'IT_SAIDA' 'USUARIO_EXCLUSAO'      '  '               '2'   'X'    '' ' ' ' ' ' ' '' ' ' 'X'.
...
FORM selecionar_dados .
  ...
  SELECT *
    INTO TABLE it_ZSDT0362
    FROM ZSDT0362
    WHERE excluido EQ space.
  IF sy-subrc IS INITIAL.
    DATA(lt_ZSDT0362) = it_ZSDT0362.
    SORT lt_ZSDT0362 BY cfop.
    DELETE ADJACENT DUPLICATES FROM lt_ZSDT0362 COMPARING cfop.

    SELECT *
      FROM j_1bagnt
      INTO TABLE @DATA(lt_j_1bagnt)
      FOR ALL ENTRIES IN @lt_ZSDT0362
      WHERE cfop = @lt_ZSDT0362-cfop.
    IF sy-subrc IS INITIAL.
      SORT lt_j_1bagnt BY cfop.
    ENDIF.
  ENDIF.

  LOOP AT it_ZSDT0362 INTO wa_ZSDT0362.
    CLEAR: wa_saida.
    MOVE-CORRESPONDING wa_ZSDT0362 TO wa_saida.

    READ TABLE lt_j_1bagnt ASSIGNING FIELD-SYMBOL(<fs_j_1bagnt>)
    WITH KEY cfop = wa_saida-cfop
    BINARY SEARCH.
    IF sy-subrc IS INITIAL.
      wa_saida-desc_cfop = <fs_j_1bagnt>-cfotxt.
    ENDIF.

    ls_style-fieldname = 'CFOP'.
...
FORM deletar_reg.
  ...
  MOVE-CORRESPONDING wa_saida TO wa_ZSDT0362.
  UPDATE ZSDT0362 SET excluido          = abap_true
                      data_exclusao     = sy-datum
                      hora_exclusao     = sy-uzeit
                      usuario_exclusao  = sy-uname
                  WHERE cfop            = wa_ZSDT0362-cfop
                    AND data_criacao    = wa_ZSDT0362-data_criacao
                    AND hora_criacao    = wa_ZSDT0362-hora_criacao
                    AND usuario_criacao = wa_ZSDT0362-usuario_criacao.

  DELETE it_saida WHERE cfop            = wa_ZSDT0362-cfop
                         AND data_criacao    = wa_ZSDT0362-data_criacao
                         AND hora_criacao    = wa_ZSDT0362-hora_criacao
                         AND usuario_criacao = wa_ZSDT0362-usuario_criacao
                         AND excluido        = wa_ZSDT0362-excluido.
...
FORM salvar_reg .
  ...
  IF gv_modif IS NOT INITIAL AND gv_erro IS INITIAL.
    ...
    DELETE it_saida WHERE desc_cfop IS INITIAL.
    ...
    IF it_saida IS NOT INITIAL.
      ...
      LOOP AT it_saida ASSIGNING <l_saida>.
        ...
        MOVE-CORRESPONDING <l_saida> TO wa_ZSDT0362.

        IF wa_ZSDT0362-cfop IS INITIAL.
          ROLLBACK WORK.
          MESSAGE 'CFOP é um campo obrigatório!' TYPE 'S'.
          RETURN.
          EXIT.
        ENDIF.

        MODIFY ZSDT0362 FROM wa_ZSDT0362.
      ENDLOOP.
