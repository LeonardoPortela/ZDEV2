REPORT zmmr189_clcust.

TABLES: zfit0207.
DATA : oid TYPE numc4.
DATA: it_dados TYPE STANDARD TABLE OF zfit0207 WITH HEADER LINE.
DATA: wa_dados TYPE zfit0207.
DATA: aux_dados TYPE zfit0207.
...
SELECTION-SCREEN BEGIN OF BLOCK a WITH FRAME TITLE TEXT-001.
  ...
  SELECT-OPTIONS p_cdind FOR zfit0207-cd_indic NO INTERVALS.
...
*      TRY.
*          MODIFY zfit0207 FROM TABLE it_dados.
*          COMMIT WORK.
*        CATCH cx_salv_not_found.
*      ENDTRY.
...
*...Colunas ALV - Alterar Nome Coluna (Pequeno,Médio Grande)P23_lr_column
  TRY.
      ...
      P23_lr_column = P23_ir_columns->get_column( 'KSTAR' ).
      P23_lr_column->set_optimized( abap_false ).
      P23_lr_column->set_short_text( 'CL.CUSTO' ).
      P23_lr_column->set_medium_text( 'CLASSE CUSTO' ).
      P23_lr_column->set_long_text( 'CLASSE CUSTO' ).
...
*... set the columns technical
  ...
  P23_lr_columns->set_column_position(  columnname = 'KSTAR' position = 2 ).
...
*  TRY.
*      P23_lr_column ?= P23_lr_columns->get_column( columnname = 'KSTAR' ).
*      P23_lv_ddic = VALUE #( table = 'CSKB' field = 'KSTAR').
...
*... set list title
  ...
  P23_l_title = |Cadastro de KSTAR por Indicador - { p_cdind-low } |.
...
FORM seleciona_dados_P23.
  SELECT * FROM zfit0207 INTO TABLE @it_dados WHERE cd_indic = @p_cdind-low .
ENDFORM.
...
*&---------------------------------------------------------------------*
*&      Module  zmmr189_KSTAR_pbo  OUTPUT
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
MODULE zmmr189_KSTAR_pbo OUTPUT.
  PERFORM zmmr189_KSTAR_pbo.
ENDMODULE.                 " zmmr189_KSTAR_pbo  OUTPUT

*&---------------------------------------------------------------------*
*&      Module  zmmr189_KSTAR_pai  INPUT
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
MODULE zmmr189_KSTAR_pai INPUT.
  PERFORM zmmr189_KSTAR_pai.
ENDMODULE.                 " zmmr189_KSTAR_pai  INPUT

*&---------------------------------------------------------------------*
*&      Form  zmmr189_KSTAR_pbo
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
FORM zmmr189_KSTAR_pbo .
...
ENDFORM.                                                    " zmmr189_KSTAR_pbo

*&---------------------------------------------------------------------*
*&      Form  zmmr189_KSTAR_pai
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
FORM zmmr189_KSTAR_pai .
...
FORM check_values.
  ...
  LOOP AT it_dados ASSIGNING FIELD-SYMBOL(<it_dados>).
    ...
    DATA : l_dupl TYPE i.
    CLEAR: l_dupl.
    LOOP AT it_dados ASSIGNING FIELD-SYMBOL(<duplicados>) WHERE KSTAR = <it_dados>-KSTAR.
      l_dupl = l_dupl + 1.
    ENDLOOP.

    IF l_dupl > 1.
      DELETE it_dados WHERE KSTAR = <it_dados>-KSTAR.
    ELSE.
