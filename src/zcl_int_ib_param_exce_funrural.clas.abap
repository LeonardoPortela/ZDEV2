class ZCL_INT_IB_PARAM_EXCE_FUNRURAL definition
  public
  final
  create public .
...
public section.
  ...
  constants AT_ID_INTERFACE type ZDE_ID_INTERFACE value '104' ##NO_TEXT.
...
CLASS ZCL_INT_IB_PARAM_EXCE_FUNRURAL IMPLEMENTATION.
  ...
  method ZIF_INTEGRACAO_INBOUND~VALIDAR_DADOS_INBOUND.
    ...
    DATA: lit_data_inbound  TYPE zsdt0001funex_t.
...
*-----------------------------------------------------------------------------------------------------------------------*
      ...
      CASE me->zif_integracao_inject~at_info_request_http-ds_metodo.
        WHEN C_POST OR C_DELETE.
          ...
          IF lwa_data-dt_inicio IS INITIAL.
            r_msg_erro = 'Data Inicio não foi informado!' && ' Index Objeto: ' && _tabix.
            RETURN.
          ENDIF.

          IF lwa_data-dt_final IS INITIAL.
            r_msg_erro = 'Data Final não foi informado!' && ' Index Objeto: ' && _tabix.
            RETURN.
          ENDIF.
    ...
    LOOP AT lit_data_inbound INTO DATA(lwa_data_inbound).
      ...
      CASE me->zif_integracao_inject~at_info_request_http-ds_metodo.
        WHEN 'POST'. "Inclusão/Modificação
          ...
          SELECT SINGLE *
            FROM zsdt0001funex INTO @DATA(lwa_zsdt0001funex)
           WHERE lifnr       EQ @lwa_data_inbound-lifnr
             AND dt_inicio   eq @lwa_data_inbound-dt_inicio
             AND dt_final    eq @lwa_data_inbound-dt_final.

          IF SY-SUBRC EQ 0.
            lwa_data_inbound-dt_registro = lwa_zsdt0001funex-dt_registro.
            lwa_data_inbound-hr_registro = lwa_zsdt0001funex-hr_registro.
          ELSE.
          ...
          MODIFY ZSDT0001funex FROM lwa_data_inbound.
        ...
        WHEN 'DELETE'.
          ...
          DELETE FROM ZSDT0001funex
           WHERE lifnr       EQ lwa_data_inbound-lifnr
             AND dt_inicio   eq lwa_data_inbound-dt_inicio
             AND dt_final    eq lwa_data_inbound-dt_final.

          SELECT SINGLE *
            FROM ZSDT0001funex INTO lwa_ZSDT0001funex
           WHERE lifnr       EQ lwa_data_inbound-lifnr
             AND dt_inicio   eq lwa_data_inbound-dt_inicio
             AND dt_final    eq lwa_data_inbound-dt_final.
