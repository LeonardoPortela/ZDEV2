...
TYPES: BEGIN OF ty_caminhao_5620,
         ...
         cor(4)            TYPE c,
         niforn            TYPE zsdt0062-nr_forn,
*        User Story 153510 // MMSILVA
         desc_status_icone TYPE icon-name.
*        User Story 153510 // MMSILVA
         INCLUDE    STRUCTURE zsdt0138.
TYPES:   dados_adicionais  TYPE char5. "Rubenilson - 05.09.24 - #140377
...
DATA: tb_textos2 TYPE TABLE OF tdline.
TYPES:
  BEGIN OF ty_textos_5620,
    chave  TYPE tdobname,
    linhas LIKE tb_textos2,
  END OF ty_textos_5620.
...
DATA: it_sol_5620          TYPE STANDARD TABLE OF ty_sol_5620,
      ...
      gt_textos_5620       TYPE TABLE OF ty_textos_5620. " Rubenilson - 05.09.24 - #140377
...
*-----CS2019001891 - 08.03.2021 - JT - fim
        ...
        IF vg_sol_5620 IS NOT INITIAL AND ( vg_sol_5620-inco1 NE 'CPT' AND vg_sol_5620-inco1 NE 'CFR').
...
*-CS2019001896 - 12.08.2021 - JT - inicio
    IF e_column_id-fieldname EQ 'EDITAR'.
      ...
      READ TABLE it_sol_5620 INTO wa_sol_5620 INDEX e_row_id.
      IF wa_sol_5620-editar = '@0Z@'. "change
        g_editar = abap_false.
      ELSE.
        g_editar = abap_true.
      ENDIF.

      PERFORM click_em_solic_5620 USING e_row_id.
      PERFORM bloqueia_linhas_caminhao_5620.
    ...
    IF e_column_id-fieldname EQ 'ICONE'.
      ...
      IF sy-subrc IS INITIAL.
        IF wa_sol_5620_det-editar = '@0Z@'. "change
...
*** Inicio - Rubenilson - 05.09.24 - #140377
  METHOD on_but_clk.
    ...
    DATA: tl_texto  TYPE catsxt_longtext_itab,
          ...
          title     TYPE sytitle,
          v_cont    TYPE i,
          lt_texto2 TYPE catsxt_longtext_itab,
          lt_texto  TYPE TABLE OF tline,
    ...
    READ TABLE it_caminhao_5620 ASSIGNING FIELD-SYMBOL(<fs_caminhao>) INDEX es_row_no-row_id.
    IF sy-subrc IS INITIAL.

      lv_name = <fs_caminhao>-seq_cam && <fs_caminhao>-nro_sol && <fs_caminhao>-seq && <fs_caminhao>-filial_resp.

      READ TABLE gt_textos_5620 ASSIGNING FIELD-SYMBOL(<fs_textos_5620>)
      WITH KEY chave = lv_name
      BINARY SEARCH.
      IF sy-subrc IS NOT INITIAL.
        ...
        LOOP AT lt_texto ASSIGNING FIELD-SYMBOL(<fs_texto>).
          APPEND INITIAL LINE TO lt_texto2 ASSIGNING FIELD-SYMBOL(<fs_texto2>).
          MOVE: <fs_texto>-tdline TO <fs_texto2>.
        ENDLOOP.
*
      ELSE.

        LOOP AT <fs_textos_5620>-linhas ASSIGNING FIELD-SYMBOL(<fs_linhas>).

          APPEND INITIAL LINE TO lt_texto2 ASSIGNING <fs_texto2>.
          MOVE: <fs_linhas> TO <fs_texto2>.
          SHIFT <fs_texto2> LEFT DELETING LEADING space.
        ENDLOOP.

      ENDIF.

      IF <fs_caminhao>-edit EQ abap_true.
        lv_mode = abap_false.
      ELSE.
        lv_mode = abap_true.
      ENDIF.
      ...
      CALL FUNCTION 'CATSXT_SIMPLE_TEXT_EDITOR'
        EXPORTING
          im_title        = 'Observação'
          im_display_mode = lv_mode
        CHANGING
          ch_text         = lt_texto2.

      IF lt_texto2 IS NOT INITIAL.

        READ TABLE gt_textos_5620 ASSIGNING <fs_textos_5620>
        WITH KEY chave = lv_name
        BINARY SEARCH.
        IF sy-subrc IS NOT INITIAL.

          APPEND INITIAL LINE TO gt_textos_5620 ASSIGNING <fs_textos_5620>.
          <fs_textos_5620>-chave = lv_name.

        ELSE.

          CLEAR <fs_textos_5620>-linhas.

        ENDIF.

        FREE lt_texto.
        LOOP AT lt_texto2 ASSIGNING <fs_texto2>.
          APPEND INITIAL LINE TO lt_lines ASSIGNING FIELD-SYMBOL(<lt_lines>).
          <lt_lines>-tdline = <fs_texto2>.

        ENDLOOP.

        <fs_textos_5620>-linhas = lt_lines.
...
*-CS2019001896 - 12.08.2021 - JT - fim
    ...
    wa_tool-function = 'NrIntForn'.
    wa_tool-icon     = '@VV@'.
    wa_tool-disabled  = l_disabled.
    wa_tool-quickinfo = 'Nr. Int. Forn'.
    APPEND wa_tool TO e_object->mt_toolbar.
    CLEAR wa_tool.
  ...
  METHOD user_command_5620.
    ...
    DATA: it_selected_rows TYPE lvc_t_row,
          ...
          vl_lines         TYPE i,
          lw_header        TYPE thead, "Rubenilson - 05.09.24 - #140377
          lt_lines         TYPE TABLE OF tline, "Rubenilson - 05.09.24 - #140377
          lv_chave         TYPE ty_textos_5420-chave. "Rubenilson - 05.09.24 - #140377
    ...
    IF e_ucomm = 'ADD_ROW'.
      ...
      IF vg_sol_5620 IS NOT INITIAL.
        ...
        MOVE icon_display_more TO wa_caminhao_5620-dados_adicionais." Rubenilson - 05.09.24 - #140377
...
*** Inicio - Rubenilson - 05.09.24 - #140377
                lv_chave = wa_caminhao_5620-seq_cam && wa_caminhao_5620-nro_sol && wa_caminhao_5620-seq && wa_caminhao_5620-filial_resp.

                READ TABLE gt_textos_5620 ASSIGNING FIELD-SYMBOL(<fs_textos>)
                WITH KEY chave = lv_chave
                BINARY SEARCH.
                IF sy-subrc IS INITIAL.
                  ...
                  lw_header-tdname = lv_chave.

                  lt_lines = <fs_textos>-linhas.
                  ...
                  IF sy-subrc IS INITIAL.
                    ...
                    DELETE gt_textos_5620 WHERE chave = lv_chave.
                    FREE: lt_lines.
                    CLEAR: lw_header,
                           lv_chave.
...
*    "Selecionar apenas uma linha
      ...
      LOOP AT t_row_no INTO w_row_no.
        ...
        IF wa_caminhao-edit EQ 'X'.
          MESSAGE  TEXT-171 TYPE 'S' DISPLAY LIKE 'E'.
        ELSE.
          popup-nr_forn = wa_caminhao-niforn.
        ENDIF.
      ENDLOOP.

      CALL SCREEN '8000' STARTING AT 35 10 ENDING AT 90 10.
      CLEAR l_linhas.
    ENDIF.
...
*-----CS2019001891 - 08.03.2021 - JT - fim
      ...
      IF lv_cont = 0.
        ...
        l_matnr = vg_sol_5620-matnr.
        WHILE strlen( l_matnr ) LT 40.
        ...
        <matnr>-option = 'EQ'.
        <matnr>-low = l_matnr.
        APPEND INITIAL LINE TO lr_matnr ASSIGNING <matnr>.
        <matnr>-sign = 'I'.
        <matnr>-option = 'EQ'.
        <matnr>-low = vg_sol_5620-matnr.
...
*-CS2019001896 - 12.08.2021 - JT - inicio
  LOOP AT it_sol_5620 INTO wa_sol_5620.
    ...
    wa_sol_5620-editar = '@10@'.

    l_gname = 'IT_ZSDT0082'.
    CONCATENATE sy-mandt          wa_sol_5620-nro_sol wa_sol_5620-seq
                wa_sol_5620-vbeln wa_sol_5620-posnr
           INTO l_garg.
    ...
    IF it_enq[] IS NOT INITIAL.
      wa_sol_5620-editar = '@06@'.
    ENDIF.
    MODIFY it_sol_5620 FROM wa_sol_5620 INDEX l_tabix.
  ENDLOOP.
*-CS2019001896 - 12.08.2021 - JT - fim
...
*-----CS2019001891 - 08.03.2021 - JT - inicio
      08 'ICONE'                      ' '             ' '  ' '  ' '  'X'   'X'   ' '   ' '   ' '   'P.Imp.',
      09 'DESC_STATUS_ICONE'          ' '             ' '  ' '  ' '  ''    ''    ' '   ' '   ' '   'Desc. Status Icone'. " User Story 153510 // MMSILVA
*-----CS2019001891 - 08.03.2021 - JT - fim
    ENDIF.
    PERFORM fill_it_fieldcatalog TABLES it_fieldcatalog_sol_5620 USING:
    ...
    PERFORM fill_it_fieldcatalog TABLES it_fieldcatalog_caminhao_5620 USING:
          ...
          19 'SERIES'               ' '         ' '     'X'  ' '  ' '   'X'   ' '   ' '   ' '   'Série',
          20 'NIFORN'               ' '         ' '     'X'  ' '  ' '   'X'   ' '   ' '   ' '   'Nr. Intern. fornecedor',
          21 'NETWR'                'ZSDT0001'  ' '     'X'  ' '  ' '   'X'   ' '   ' '   ' '   'Valor NF',
          22 'DOCDAT_NF'            'ZSDT0138'  ' '     'X'  ' '  ' '   'X'   ' '   ' '   ' '   'Data Emissão NF',
          23 'NR_ROMANEIO'          ' '         ' '     ' '  ' '  ' '   'X'   ' '   ' '   ' '   'Nr. Romaneio',
          24 'AUT_EMBARQUE'         ' '         ' '     ' '  ' '  ' '   'X'   ' '   ' '   ' '   'Autorização Embarque',
          25 'DADOS_ADICIONAIS'     ' '         ' '     ' '  ' '  ' '   'X'   ' '   ' '   ' '   'Observação',"Inicio - Rubenilson - 05.09.24 - #140377
          26 'DESC_STATUS_ICONE'    ' '         ' '     ' '  ' '  ' '   ''    ' '   ' '   ' '   'Desc. Status Icone'." User Story 153510 // MMSILVA
...
*-----CS2019001891 - 08.03.2021 - JT - fim
    ...
    SET HANDLER:
      ...
      lcl_event_handler_5620=>on_but_clk FOR ctl_alv2_5620." Rubenilson - 05.09.24 - #140377
...
*** Inicio - Rubenilson - 05.09.24 - #140377
        LOOP AT it_caminhao_5620 ASSIGNING FIELD-SYMBOL(<fs_5620>).
          <fs_5620>-dados_adicionais = icon_display_more.
        ENDLOOP.
*** Fim - Rubenilson - 05.09.24 - #140377
        ...
        SELECT * FROM zsdt0062
           INTO TABLE @DATA(t_zsdt0062)
          WHERE vbeln EQ @vg_sol_5620-vbeln AND
          posnr EQ @vg_sol_5620-posnr AND
          nro_sol EQ @vg_sol_5620-nro_sol AND
      ...
      LOOP AT it_caminhao_5620 INTO wa_caminhao_5620.
        ...
        IF wa_caminhao_5620-status EQ 1.
          ...
          wa_caminhao_5620-desc_status_icone = 'Lote aprovado'. " User Story 153510 // MMSILVA
        ELSEIF wa_caminhao_5620-status EQ 2.
          wa_caminhao_5620-icone = '@0Q@'.
          wa_caminhao_5620-desc_status_icone = 'Romaneio Gerado'. " User Story 153510 // MMSILVA
        ENDIF.
        ...
        IF sy-subrc IS INITIAL.
          ...
          IF wa_zsdt0001-st_proc EQ '99'.
            ...
            wa_caminhao_5620-desc_status_icone = 'Romaneio Finalizado'. " User Story 153510 // MMSILVA
          ENDIF.
          "US #154513 - MMSILVA - 28.02.2025 - Inicio
          SELECT SINGLE docsta FROM j_1bnfe_active WHERE docnum EQ @wa_zsdt0001-nro_nf_prod INTO @DATA(wa_j_1bnfe_active).
          IF sy-subrc IS INITIAL.
            IF ( wa_j_1bnfe_active EQ '1').

              wa_caminhao_5620-icone = '@5Y@'.
              wa_caminhao_5620-desc_status_icone = 'Nota Fiscal Autorizada'.
            ENDIF.
          ENDIF.
        ...
        READ TABLE t_zsdt0062 INTO DATA(w_zsdt0062) WITH KEY ebeln = wa_caminhao_5620-ebeln
                                                             ebelp = wa_caminhao_5620-ebelp.
        IF sy-subrc IS INITIAL.
          wa_caminhao_5620-niforn = w_zsdt0062-nr_forn.
        ENDIF.
...
FORM salva_romaneios_5620 CHANGING vl_check_rom.
  ...
  IF vl_check_rom IS INITIAL.
    ...
    LOOP AT it_caminhao_aux_5620 INTO wa_caminhao_5620.
      ...
      wa_zsdt0001-peso_subtotal = wa_caminhao_5620-qtd_embarq. "IR096917 - 01.06.2012 - RMNI
...
*-CS2019001891 - 28.06.2021 - JT - inicio
                            AND filial_resp EQ wa_caminhao_5620-filial_resp.
*                           AND filial_resp EQ g_filial_resp.
*                           AND filial_resp EQ 'TROO'.
*-CS2019001891 - 28.06.2021 - JT - fim
...
FORM salva_frete_5620 .
  ...
  DATA: vl_check_initial  TYPE char1,
        ...
        lt_lines          TYPE TABLE OF tline, "Rubenilson - 05.09.24 - #140377
        lv_chave          TYPE ty_textos_5420-chave, "Rubenilson - 05.09.24 - #140377
        lv_chave2         TYPE ty_textos_5420-chave. "Rubenilson - 05.09.24 - #140377
...
*       WHERE filial_resp EQ 'TROO'
        WHERE filial_resp EQ g_filial_resp
*-CS2019001891 - 28.06.2021 - JT - fim
          AND seq         EQ vg_sol_5620-seq
          AND nro_sol     EQ vg_sol_5620-nro_sol.
...
*-CS2019001891 - 28.06.2021 - JT - inicio
        wa_zsdt0138-filial_resp        = g_filial_resp.
...
*** Inicio - Rubenilson - 05.09.24 - #140377
        lv_chave = wa_zsdt0138-seq_cam && wa_zsdt0138-nro_sol && wa_zsdt0138-seq && wa_zsdt0138-filial_resp.

        READ TABLE gt_textos_5620 ASSIGNING FIELD-SYMBOL(<fs_textos>)
        WITH KEY chave = lv_chave
        BINARY SEARCH.
        IF sy-subrc IS NOT INITIAL.
          lv_chave2 = '000' && wa_zsdt0138-nro_sol && wa_zsdt0138-seq.
          READ TABLE gt_textos_5620 ASSIGNING <fs_textos>
          WITH KEY chave = lv_chave2
          BINARY SEARCH.
          IF sy-subrc IS INITIAL.
            <fs_textos>-chave = lv_chave.
          ENDIF.
        ENDIF.
        IF sy-subrc IS INITIAL.
