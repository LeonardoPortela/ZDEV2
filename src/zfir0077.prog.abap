...
"#115227 SMC Fim <<<<
  ...
  IF ( v_jobnm = 'INFORME_PGTO_FORNECEDOR_EXC' ).

    SELECT SINGLE jobname
        INTO @DATA(_count)
        FROM tbtco
        WHERE jobname = 'INFORME_PGTO_FORNECEDOR_EXC'
          AND status = 'R'.
...
* Início - RJF
    ...
    WHERE bs~bukrs BETWEEN '0001' AND '0050'
     ...
     " AND bs~augbl EQ bs~belnr
      AND bs~gjahr EQ @vl_ano
      AND bs~zfbdt BETWEEN @vl_augdt AND @vl_trin "sy-datum
      AND bs~zlspr EQ ''
      AND bk~blart IN ('FT').
* Fim - RJF

  ELSEIF ( v_jobnm = 'INFORME_PGTO_FORNECEDOR' ).
  ...
  IF tg_bsik IS NOT INITIAL.
    ...
    WHERE bukrs EQ @tg_bsik-bukrs
     ...
     " AND augbl EQ @tg_bsik-augbl.
      AND blart IN ('FT')          "LP
      AND zfbdt BETWEEN @vl_augdt AND @vl_trin" LP sy-datum
      AND zlspr EQ ''.
* Fim - RJF
...
* Início - RJF
  ...
  " FGM 09/01/2025 incluido
    SELECT lf~lifnr,
           lf~name1,
           lf~adrnr,
           ad~smtp_addr
      FROM lfa1 AS lf
      INNER JOIN but020 AS bu ON lf~lifnr = bu~partner
      INNER JOIN adr6 AS ad ON bu~addrnumber = ad~addrnumber
      APPENDING TABLE @tg_lfa1_aux
      FOR ALL ENTRIES IN @tg_bsik
      WHERE lifnr = @tg_bsik-lifnr.

    DELETE ADJACENT DUPLICATES FROM tg_lfa1_aux[].
...
" FGM 09/01/2025 incluido
  ...
  DELETE ADJACENT DUPLICATES FROM tg_lfa1[].
