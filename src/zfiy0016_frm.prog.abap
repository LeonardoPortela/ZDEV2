...
* taxed
    IF sum_tab_taxes-not_taxed EQ 0.
...
FORM fill_tab_beleg.
  DATA: flg_gl       TYPE c,         " Check docs with special G/L item
        gl_amount(8) TYPE p.         " Total: Docs with special G/L item
...
*  check bkpf-xblnr+5(8) is initial.
  ...
  ELSE.                                " Anzahlungsverrechnung
    ...
    IF *bkpf-xblnr EQ space.
      DATA etl1227c6r7064 TYPE TABLE OF bseg.
      DATA rldnr_l1227c6r2646 TYPE rldnr.
      CALL FUNCTION 'FAGL_GET_LEADING_LEDGER'
        IMPORTING
          e_rldnr       = rldnr_l1227c6r2646
        EXCEPTIONS
          not_found     = 1
          more_than_one = 2.
      IF sy-subrc = 0.
        CALL FUNCTION 'FAGL_GET_GL_DOCUMENT'
          EXPORTING
            i_rldnr   = rldnr_l1227c6r2646
            i_bukrs   = *bkpf-bukrs
            i_belnr   = *bkpf-belnr
            i_gjahr   = *bkpf-gjahr
            i_buzei   = tab_bseg-rebzz
          IMPORTING
            et_bseg   = etl1227c6r7064
          EXCEPTIONS
            not_found = 1.
      ENDIF.
      IF sy-subrc = 0 AND lines( etl1227c6r7064 ) = 1.
        *bseg = etl1227c6r7064[ 1 ].
        sy-dbcnt = 1.
      ELSE.
        sy-subrc = 4.
        sy-dbcnt = 0.
...
*----------------------------------------------------------------------*
FORM check_sd_foreign_currency.
  DATA: max_amount(8)  TYPE p, " Biggest amount of all line entries
        comp_total(8)  TYPE p, " Sum of all line totals for document
        beleg_total(8) TYPE p, " Document total: Compare without sign
        diff_curr(8)   TYPE p, " Difference: beleg_total - comp_total
...
*         AND koart = 'K'.
      ...
      lv_belnr = CONV #( txt_zeile7+22(10) ).
...
*           and BUZID = 'T'.
        ...
        DATA: lt_bseg_S4  TYPE fagl_t_bseg,
              lv_belnr_S4 TYPE belnr_d.
        ...
        DELETE lt_bseg_S4 WHERE mwskz EQ '' AND buzid NE 'T'.

        IF sy-subrc = 0 AND lines( lt_bseg ) > 0.
          MOVE-CORRESPONDING lt_bseg_S4 TO t_bseg_aux[].
          sy-dbcnt = lines( lt_bseg_S4 ).
        ELSE.
          sy-subrc = 4.
          sy-dbcnt = 0.
        ENDIF.
...
* <--- S4 Migration - 22/06/2023 - MA
        CALL FUNCTION 'CONVERSION_EXIT_ALPHA_INPUT'
          EXPORTING
            input  = txt_zeile7+22(10)
          IMPORTING
            output = t_alv-belnr.

        MOVE tab_ep-ktnra TO t_alv-lifnr.
        READ TABLE tab_adrs WITH KEY stcd1 = tab_ep-ktnra.
        MOVE: tab_adrs-stcd1 TO t_alv-stcd1,
              tab_adrs-stcdt TO t_alv-stcdt.
        t_alv-belnr = txt_zeile7+22(10).
        SHIFT t_alv-belnr RIGHT DELETING TRAILING space.
        OVERLAY t_alv-belnr WITH '0000000000'.
        t_alv-name1 = txt_zeile7+40(23).
        t_alv-tpcbio = tab_ep-kursf.
        t_alv-blart  = tab_ep-blart.
*         concatenate TAB_EP-xblnr(4) TAB_EP-xblnr+5(11)
*         into t_alv-xblnr  .
        t_alv-flag = space.
...
*                                        koart = 'K'.
        ...
        WRITE: tab_ep-bldat TO t_alv-bldat,
               tab_ep-budat TO t_alv-budat.

*        t_alv-xblnr = tab_ep-xblnr.
        t_alv-xblnr = hlp_epos1s-xblnr.
*        t_alv-xblnr+4(1) = '-'.
*        t_alv-xblnr+4(1) = '-'.
* Guardo el tipo de cambio
        t_alv-tpcbio = tab_ep-kursf.
*        WRITE tab_ep-tpcbio TO t_alv-tpcbio.
...
* Guardo en la cuenta el numero de cliente
        t_alv-hkont = tab_ep-hkont."v_lifnr.
...
*        t_alv-stcdt = '80'.
*
*        SELECT mwskz FROM bseg INTO TABLE t_mwskz
...
*          SELECT dmbtr shkzg FROM bseg INTO TABLE t_gravado
*            WHERE bukrs = t_bkpf_aux-bukrs AND
*                  gjahr = t_bkpf_aux-gjahr AND
*                  belnr = t_bkpf_aux-belnr AND
*                  mwskz NE space           AND
...
*        ENDLOOP.
        ...
        IF t_alv-vat = 0.
          ...
          t_alv-rate = vl_dmbtr.
        ENDIF.
        SHIFT t_alv-rate RIGHT DELETING TRAILING ' '.
        APPEND t_alv.
        CLEAR t_alv.
      ENDIF. "v_saldo NE 0.
    ELSE.
...
**------------------------------*
    ...
    MOVE: tab_ep-taxed        TO hlp_epos02-taxed       .
    MOVE: tab_ep-not_taxed    TO hlp_epos02-not_taxed   .
    MOVE: tab_ep-vat          TO hlp_epos02-vat         .
    MOVE: tab_ep-rnr_vat      TO hlp_epos02-rnr_vat     .
    MOVE: tab_ep-vat_percep   TO hlp_epos02-vat_percep  .
    MOVE: tab_ep-other_percep TO hlp_epos02-other_percep.
    MOVE: tab_ep-exemption    TO hlp_epos02-exemption   .
...
*        MOVE tab_ep-j_1arfz TO hlp_epos02-exempt.
      ELSE.
        CLEAR hlp_epos2-exempt.
        CLEAR hlp_epos02-exempt.
      ENDIF.
      CLEAR find_exempted .
* End of note 988302
      WRITE tab_ep-earn_per TO hlp_epos2-earn_per NO-ZERO
                                               CURRENCY tab_001-waers.
      MOVE tab_ep-earn_per TO hlp_epos02-earn_per.
    ENDIF.
...
*    ENDDO.
    ...
    DO 5 TIMES.
      ...
      REPLACE ALL OCCURRENCES OF '|' IN hlp_epos2-mwskz        WITH space.
      REPLACE ALL OCCURRENCES OF '|' IN hlp_epos2-rate         WITH space.
      REPLACE ALL OCCURRENCES OF '|' IN hlp_epos2-taxed        WITH space.
      REPLACE ALL OCCURRENCES OF '|' IN hlp_epos2-not_taxed    WITH space.
      REPLACE ALL OCCURRENCES OF '|' IN hlp_epos2-vat          WITH space.
      REPLACE ALL OCCURRENCES OF '|' IN hlp_epos2-rnr_vat      WITH space.
      REPLACE ALL OCCURRENCES OF '|' IN hlp_epos2-vat_percep   WITH space.
      REPLACE ALL OCCURRENCES OF '|' IN hlp_epos2-other_percep WITH space.
      REPLACE ALL OCCURRENCES OF '|' IN hlp_epos2-exemption    WITH space.
      REPLACE ALL OCCURRENCES OF '|' IN hlp_epos2-exports      WITH space.
      REPLACE ALL OCCURRENCES OF '|' IN hlp_epos2-percepnoc    WITH space.
      REPLACE ALL OCCURRENCES OF '|' IN hlp_epos2-line_total   WITH space.
      REPLACE ALL OCCURRENCES OF '|' IN hlp_epos2-exempt       WITH space.
      REPLACE ALL OCCURRENCES OF '|' IN hlp_epos2-earn_per     WITH space.
    ...
    MOVE-CORRESPONDING hlp_epos1 TO t_alv." tab_ep TO t_alv.
    ...
    CALL FUNCTION 'CONVERSION_EXIT_ALPHA_INPUT'
      EXPORTING
        input  = hlp_epos1-belnr
      IMPORTING
        output = t_alv-belnr.
* exportacion

    IF tab_ep-kschl = '!EXP'.
      hlp_epos02-exemption         = t_alv-total.
    ENDIF.

*    MOVE-CORRESPONDING hlp_epos02 TO t_alv.
    MOVE:       hlp_epos02-vat           TO t_alv-vat          ,
                hlp_epos02-vat           TO t_alv-ivat         ,
                hlp_epos02-taxed         TO t_alv-taxed        ,
                hlp_epos02-taxed         TO t_alv-itaxed       ,
                hlp_epos02-not_taxed     TO t_alv-not_taxed    ,
                hlp_epos02-rnr_vat       TO t_alv-rnr_vat      ,
                hlp_epos02-vat_percep    TO t_alv-vat_percep   ,
                hlp_epos02-other_percep  TO t_alv-other_percep ,
                hlp_epos02-exemption     TO t_alv-exemption    ,
                hlp_epos02-taxed         TO t_alv-itaxed       ,
                hlp_epos02-not_taxed     TO t_alv-inot_taxed   ,
                hlp_epos02-rnr_vat       TO t_alv-irnr_vat     ,
                hlp_epos02-vat_percep    TO t_alv-ivat_percep  ,
                hlp_epos02-other_percep  TO t_alv-iother_percep,
                hlp_epos02-exemption     TO t_alv-iexemption   .
    ...
    CLEAR vl_mwskz.
    vl_mwskz = txt_zeile8+1(6).
    CONDENSE vl_mwskz NO-GAPS.

    t_alv-mwskz = vl_mwskz.

    t_alv-total = hlp_epos02-line_total.
*    t_alv-mwskz = txt_zeile8+1(6).
    t_alv-rate  = txt_zeile8+8(8).
    t_alv-blart = tab_ep-blart.
*--------------- exportacion
    IF tab_ep-kschl  = '!EXP'.
      t_alv-exemption  = hlp_epos02-line_total.
      t_alv-iexemption  = hlp_epos02-line_total.
    ENDIF.
*--------------------------------*
    CONDENSE t_alv-mwskz NO-GAPS.
    ...
    ENDIF.
    t_alv-tpcbio = tab_ep-kursf.
*read TABLE tab_adrs WITH KEY stcd1 = t_alv-stcd1.
    MOVE tab_ep-ktnra TO t_alv-lifnr.
...
*  ENDIF.
    ...
    WHEN OTHERS.
      ...
      x_fieldcat-tabname = 'T_ALV'.
      x_fieldcat-seltext_m = 'Nro Documento Sap'.
      ...
      x_fieldcat-tabname = 'T_ALV'.
      x_fieldcat-seltext_m = 'Nro Cliente'.
      ...
      x_fieldcat-tabname = 'T_ALV'.
      x_fieldcat-seltext_m = 'Fecha documento'.
      x_fieldcat-outputlen = 12.
      x_fieldcat-just = 'R'. "s
      x_fieldcat-ddictxt = 'M'.
      APPEND x_fieldcat TO p_gt_fieldcat.
      ...
      x_fieldcat-tabname = 'T_ALV'.
      x_fieldcat-seltext_m = 'Nro de documento'.
      x_fieldcat-outputlen = 15.
      x_fieldcat-just = 'R'. "s
      x_fieldcat-ddictxt = 'M'.
      APPEND x_fieldcat TO p_gt_fieldcat.
      ...
      x_fieldcat-tabname = 'T_ALV'.
      x_fieldcat-seltext_m = 'Tipo de cambio'.
      x_fieldcat-outputlen = 15.
      x_fieldcat-just = 'R'. "s
      x_fieldcat-ddictxt = 'M'.
      APPEND x_fieldcat TO p_gt_fieldcat.
...
*  IF par_cust IS INITIAL. "Compras
      ...
      x_fieldcat-tabname = 'T_ALV'.
      x_fieldcat-seltext_m = 'Percepción de IIBB'.
      x_fieldcat-outputlen = 15.
      x_fieldcat-just = 'R'. "s
      x_fieldcat-ddictxt = 'M'.
      x_fieldcat-do_sum = 'X'.
...
*0000462: Columna en el ALV ini
      x_fieldcat-fieldname = 'HWSTE'.
      x_fieldcat-tabname = 'T_ALV'.
      x_fieldcat-seltext_m = 'Impuestos Internos'.
      x_fieldcat-outputlen = 15.
      x_fieldcat-just = 'R'. "s
      x_fieldcat-ddictxt = 'M'.
      x_fieldcat-no_out = 'X'.
      ...
      APPEND x_fieldcat TO p_gt_fieldcat.
      CLEAR x_fieldcat.
*0000462: Columna en el ALV fin

      x_fieldcat-fieldname = 'STCDT'.
      x_fieldcat-tabname = 'T_ALV'.
      x_fieldcat-seltext_m = 'Nro de Identificación'.
      x_fieldcat-outputlen = 19.
      x_fieldcat-just = 'R'. "s
      x_fieldcat-ddictxt = 'M'.
      APPEND x_fieldcat TO p_gt_fieldcat.
...
FORM f_initialization .
  ...
  s_drver = '04'.                      " version of the daily reports
...
* par_cexi = ' '.                      " don't run with customer exit
  par_dspt = 'X'.                      " display tax base as zero for manual postings
...
FORM f_armo_totales .
  ...
  LOOP AT t_alv.
    ...
    WHERE belnr EQ t_alv-belnr
    AND   buzid EQ  ' '
    AND   koart EQ 'S'
    AND ( mwskz NE '**' AND
          mwskz NE 'SD'  ).

* Eliminado en 03.06.2011 - Diego
*      IF stl_bseg-mwskz = 'DB' OR  stl_bseg-mwskz IS INITIAL.
*        IF t_alv-itaxed > 0.
**          t_alv-itaxed     =  t_alv-itaxed      - stl_bseg-dmbtr .
**          t_alv-taxed      =  t_alv-taxed       - stl_bseg-dmbtr .
*        ENDIF.
*        t_alv-inot_taxed =  t_alv-inot_taxed  + stl_bseg-dmbtr .
*        t_alv-not_taxed  =  t_alv-not_taxed   + stl_bseg-dmbtr .
*        MODIFY t_alv INDEX vl_tabix.
*      ENDIF.
* Fin 03.06.2011 ------------------------------------------------------

      CLEAR t_alv-total.
*Armo el total
      t_alv-total =  t_alv-itaxed
                  +  t_alv-inot_taxed
                  +  t_alv-ivat
                  +  t_alv-irnr_vat
                  +  t_alv-ivat_percep
...
*  SORT t_iva BY rate.
  ...
  LOOP AT t_alv.
* Modificado en 08.06.2011 - Diego
*    WHERE rate NE '   0.00%'
*    AND   rate NE '   0,00%'
*    AND   rate NE '        ' .
* Fin 08.06.2011
...
* Iva No Gravado
  ...
  IF t_kna1[] IS NOT INITIAL.
    ...
    LOOP AT  t_alv.
      ...
      READ TABLE t_kna1 INTO stl_kna1
      WITH KEY kunnr = t_alv-lifnr.

      READ TABLE tl_desc INTO stl_desc
      WITH KEY j_1afitp = stl_kna1-fityp.
      ...
      IF t_alv-rate = '    0.00'.
        PERFORM f_agregar_exen_nograv USING t_alv-iexemption
                                            t_alv-inot_taxed CHANGING t_operaciones-rate.
      ENDIF.
...
*      READ TABLE t_tipo_doc WITH KEY blart = t_alv-blart.
      MOVE:
            t_alv-budat         TO stl_posicion-budat,
      ...
      IF t_alv-blart = 'GB' OR t_alv-blart = 'DX'.
...
*  po_libro_iva-direccion = vl_street.
...
*  Tablas de percepciones iibb
  PERFORM f_percepcion_iibb.
...
* Modificado en 08.06.2011 - Diego
*        IF   stl_bset-ktosl EQ 'J1J'
*        OR   stl_bset-ktosl EQ 'J1M'
*        OR   stl_bset-ktosl EQ 'J1K'
*        OR   stl_bset-ktosl EQ 'J1L'.
        IF   stl_bset-ktosl(2) EQ 'J1'.
* Fin 08.06.2011

          IF stl_bset-shkzg EQ 'S'.
...
*      t_periibb-imp = stl_bset-hwste.   " + t_periibb-imp.
          t_periibb-imp = CONV #( stl_bset-hwste ).
*<--- 09/06/2023 - Migração S4 - JS
...
*<--- 09/06/2023 - Migração S4 - JS
      ...
      WRITE vl_kbetr TO vl_kbetrl LEFT-JUSTIFIED.
...
*&---------------------------------------------------------------------*
*&      Form  F_PERCEPCION_IVA
*&---------------------------------------------------------------------*
FORM f_percepcion_iibb .
...
ENDFORM. " F_PERCEPCION_IVA

*&---------------------------------------------------------------------*
*&      Form  F_AGREGAR_EXEN_NOGRAV
*&---------------------------------------------------------------------*
FORM f_agregar_exen_nograv USING p_iexe p_ntaxed CHANGING p_rate.

  IF p_ntaxed IS NOT INITIAL.
    p_rate = 'No Grav.'.
*  ELSEIF p_iexe IS NOT INITIAL.
  ELSE.
    p_rate = 'Exento'.
  ENDIF.
...
FORM f_agregar_campos_adic .
  ...
  LOOP AT t_alv.
    ...
    IF t_alv-lifnr IS NOT INITIAL.
      READ TABLE t_kna1 INTO el_kna1
      WITH KEY kunnr = t_alv-lifnr.
      IF sy-subrc = 0.
        t_alv-fityp = el_kna1-fityp.
      ENDIF.
    ENDIF.

    READ TABLE it_bkpf_aux INTO wa_bkpf_aux WITH KEY belnr =  t_alv-belnr.

    READ TABLE t_tipo_doc
      WITH KEY blart = t_alv-blart  j_1aprtchr = wa_bkpf_aux-xblnr+4(1).
    IF sy-subrc = 0.
