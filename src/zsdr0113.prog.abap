...
*-US 155653-04-11-2024-#155653-RJF-Inicio
  ...
  read table it_rsparams into data(wa_rsp) with key selname = 'P_BUKRS'.
  ...
  zcl_calendario=>get_calendario( exporting i_bukrs            = lv_bukrs
...
*     factory_calendar = 'ZF'         " -US 155653-04-11-2024-#155653-RJF
      date_from        = dt_atual
      date_to          = vg_last_day
    tables
  ...
  read table it_data into wa_data with key dia = 7."3. PSA 72hrs
...
form trata_dados.
  ...
  loop at it_entrada into data(wa_entrada)
    ...
    clear: wa_saida_brl, wa_entrada, wa_vkbur.
  endloop.
  ...
  loop at it_entrada into wa_entrada
    ...
    clear: wa_saida_usd, wa_entrada, wa_vkbur.
  endloop.
...
*  lr_vkbur[] = VALUE #( FOR wa_vkbur_brl IN it_saida_brl ( option = 'EQ' sign = 'I' low = wa_vkbur_brl-vkbur ) ).
*  lr_vkbur[] = VALUE #( FOR wa_vkbur_usd IN it_saida_usd ( option = 'EQ' sign = 'I' low = wa_vkbur_usd-vkbur ) ).
*  SORT lr_vkbur     BY low.
*  DELETE ADJACENT DUPLICATES FROM lr_vkbur.
*
***********************************************************************
**SE as tabelas BRL e USD forem vazias - exit "110301 CS2023000290 Melhoria de Job de email Fluxo de Caixa Insumos
***********************************************************************
*  IF it_saida_brl IS INITIAL AND it_saida_usd IS INITIAL.
*    EXIT.
*  ENDIF.
***********************************************************************
*
...
*  AND email    NE ' '.
*
*  SORT it_zsdt0060 BY usnam vkorg.
*
*  LOOP AT it_vkbur INTO wa_vkbur.
*
*    REFRESH: lt_mailrecipientes[],  lt_mailtxt[].
*
*    IF wa_vkbur-vkbur IS NOT INITIAL.
*
*      IF sy-sysid = 'QAS'.
*        APPEND VALUE #( receiver =   'samuel.cabana@amaggi.com.br' rec_type =   'U'  rec_date =   '00000000' ) TO lt_mailrecipientes.
*      ELSE.
*        APPEND VALUE #( receiver =   'csc.fluxodecaixa@amaggi.com.br' rec_type =   'U'  rec_date =   '00000000' ) TO lt_mailrecipientes.
*      ENDIF.
*
*      LOOP AT it_zsdt0060 INTO DATA(wa_zsdt0060)
...
*      "CONCATENATE 'CR Insumos p/ prox. 72h - Filial ' WA_VKBUR-TEXT INTO  LT_MAILSUBJECT-OBJ_DESCR SEPARATED BY SPACE. PSA
*      CONCATENATE 'CR INS. FLUXO DE CAIXA - ' wa_vkbur-vkbur ' - Venc. ' dt_vencimento_txt INTO  lt_mailsubject-obj_descr SEPARATED BY space.
*
*      lt_mailtxt = '<html><head><body><br>'.
*      APPEND lt_mailtxt.
*
*      "LT_MAILTXT = 'Senhores(as) segue relação de contas a receber insumos para as próximas 72h.'.
*      CONCATENATE 'Senhores(as) segue relação de contas a receber insumos para os próximos 7 dias úteis.'
*      'Precisamos do retorno se haverá depósito o quanto antes sobre a confirmação da OV abaixo, devido ao impacto em fluxo de caixa e hedge.' INTO lt_mailtxt.
*
*      APPEND lt_mailtxt.
*
*      lt_mailtxt = '<br><br>'.
*      APPEND lt_mailtxt.
*
*      lt_mailtxt = '<i>Favor informar na coluna Fluxo de Caixa da planilha em anexo se é Deposito ou Acerto.</i>'.
*      APPEND lt_mailtxt.
*
*      lt_mailtxt = '<br><br><br>'.
*      APPEND lt_mailtxt.
*
*
*      READ TABLE it_saida_brl INTO DATA(wa_sd_brl) WITH KEY  vkbur = wa_vkbur-vkbur.
*
*      IF wa_sd_brl-vkbur IS NOT INITIAL.
*
*        PERFORM tabela_html. "PSA
*
...
*          APPEND lt_mailtxt.
*
*          CONCATENATE '<td><p style="font-size:80%">' wa_saida_brl-auart '</p></td>'  '<td><p style="font-size:80%">' wa_saida_brl-safra '</p></td>' INTO lt_mailtxt.
*          APPEND lt_mailtxt.
*
*          CONCATENATE '<td><p style="font-size:80%">' wa_saida_brl-meio_pago '</p></td>'  '<td><p style="font-size:80%">' wa_saida_brl-kunnr '</p></td>' INTO lt_mailtxt.
*          APPEND lt_mailtxt.
*
...
*          APPEND lt_mailtxt.
*
*          CONCATENATE '<td><p style="font-size:80%">' wa_saida_brl-data_venc '</p></td>' INTO lt_mailtxt.
*          APPEND lt_mailtxt.
*
*          lt_mailtxt = '</tr>'.
*          APPEND lt_mailtxt.
*
*          CLEAR:  wa_saida_brl, saldo_ov.
*        ENDLOOP.
*
*        lt_mailtxt = '</table>'.
*        APPEND lt_mailtxt.
...
*
*      IF wa_sd_usd-vkbur IS NOT INITIAL.
*
*        PERFORM tabela_html. "PSA
*
...
*          APPEND lt_mailtxt.
*
*          CONCATENATE '<td><p style="font-size:80%">' wa_saida_usd-auart '</p></td>'  '<td><p style="font-size:80%">' wa_saida_usd-safra '</p></td>' INTO lt_mailtxt.
*          APPEND lt_mailtxt.
*
*          CONCATENATE '<td><p style="font-size:80%">' wa_saida_usd-meio_pago '</p></td>'  '<td><p style="font-size:80%">' wa_saida_usd-kunnr '</p></td>' INTO lt_mailtxt.
*          APPEND lt_mailtxt.
*
...
*          APPEND lt_mailtxt.
*
*          CONCATENATE '<td><p style="font-size:80%">' wa_saida_usd-data_venc '</p></td>' INTO lt_mailtxt.
*          APPEND lt_mailtxt.
*
*          lt_mailtxt = '</tr>'.
*          APPEND lt_mailtxt.
*
*          CLEAR:  wa_saida_usd, saldo_ov.
*        ENDLOOP.
*
*        lt_mailtxt = '</table>'.
*        APPEND lt_mailtxt.
...
*      APPEND lt_mailtxt.
*
*      IF wa_saida_usd-vkbur IS NOT INITIAL.
*        PERFORM enviar.
*      ENDIF.
*
*      CLEAR: wa_zsdt0060, lt_mailtxt.
*
*    ENDIF.
*
*  ENDLOOP.
...
*              '<td><b><p style="font-size:90%">Nro.O.V</p></b></td>' INTO lt_mailtxt.
*  APPEND lt_mailtxt.
*
*  CONCATENATE '<td><b><p style="font-size:90%">Tipo O.V.</p></b></td>'
*              '<td><b><p style="font-size:90%">Safra</p></b></td>' INTO lt_mailtxt.
*  APPEND lt_mailtxt.
*
*  CONCATENATE '<td><b><p style="font-size:90%">Meio Pagamento</p></b></td>' "110301 CS2023000290 Melhoria de Job de email Fluxo de Caixa Insumos
...
*  APPEND lt_mailtxt.
*
*  CONCATENATE '<td><b><p style="font-size:90%">Dt.Vcto</p></b></td>'
*               '<td><b><p style="font-size:90%">Fluxo de Caixa</p></b></td>' INTO lt_mailtxt. "110301 CS2023000290 Melhoria de Job de email Fluxo de Caixa Insumos
*  APPEND lt_mailtxt.
*
*  lt_mailtxt = '</tr>'.
*  APPEND lt_mailtxt.
...
* Inicio IR229413 - RBRIBEIRO - 16/05/2025 - Stefanini
    SORT aux_empresa.

    DELETE ADJACENT DUPLICATES FROM aux_empresa.
* Fim IR229413 - RBRIBEIRO - 16/05/2025 - Stefanini
...
*Texto do Corpo E-mail
    ...
    append '<body>' to html.
    append '<br>' to html.
    append 'Senhores(as) segue relação de contas a receber insumos para os próximos 7 dias úteis.<br>' to html.
