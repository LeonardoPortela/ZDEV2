class ZCL_INT_IB_SF_CARGA_ROT_CANC definition
  public
  final
  create public .
...
*      END OF ZDE_DATA_RESPONSE .
  constants AT_ID_INTERFACE type ZDE_ID_INTERFACE value '295' ##NO_TEXT.
...
CLASS ZCL_INT_IB_SF_CARGA_ROT_CANC IMPLEMENTATION.
  ...
  METHOD zif_integracao_inject~set_integrar_inbound.
    ...
    IF e_msg_erro IS INITIAL.
      ...
      SELECT SINGLE *
       ...
       WHERE id_carga_safra_control eq @lwa_data_request-carga-identifiercargo
         AND status ne 'X'.

      IF sy-subrc NE 0.
        e_msg_erro = |Carga com identificador { lwa_data_request-carga-identifiercargo } não encontrada!|.
      ELSE.
        zcl_carga_saida_insumos=>cancela_carga( EXPORTING
            i_nro_carga = lwa_zsdt0133-nro_cg
            i_origem_cancelamento = 'S'
            i_motivo    = 'Origem Cancelamento Sistema Safra Control'
            IMPORTING
