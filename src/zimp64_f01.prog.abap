...
*&---------------------------------------------------------------------*
*&---------------------------------------------------------------------*
*& Form  f_processa_comprovantes_tributos
*&---------------------------------------------------------------------*
*& text
*&---------------------------------------------------------------------*
*& -->  p1        text
...
FORM f_processa_comprovantes .
  ...
  SELECT *
    FROM zimt004
  INTO TABLE @DATA(lt_zimt004)
    WHERE consulta_pgto <> '0200'.

  IF lt_zimt004[] IS NOT INITIAL.
    LOOP AT lt_zimt004 INTO DATA(lw_zimt004).

      CONCATENATE lw_zimt004-idtransacao ';' lw_zimt004-agencia ';' lw_zimt004-conta ';' lw_zimt004-tipoconta INTO lv_info_request.

      TRY .
          zcl_int_ob_cons_pagcont_bbd=>zif_integracao_outbound~get_instance( )->execute_request( EXPORTING i_info_request = lv_info_request IMPORTING e_integracao = DATA(r_response) ).
