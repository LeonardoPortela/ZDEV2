...
FORM documentos_processar .
  ...
  CALL FUNCTION 'GET_DATA_ZFIR0103'
    EXPORTING
      i_bukrs    = p_emp
      i_gjahr    = p_ano
      i_dtini    = dtini
      i_dtfim    = dtfim
    IMPORTING
      e_zfir0103 = it_selecao.


*  IF it_selecao IS NOT INITIAL.
*
*    DATA: it_belnr_exclusao TYPE STANDARD TABLE OF zib_contabil_chv INITIAL SIZE 0.
*
*    SELECT DISTINCT *
*    FROM zib_contabil_chv
*    WHERE substring( obj_key,1,3 ) = 'led' INTO TABLE @it_belnr_exclusao.
*
*    SORT it_belnr_exclusao BY belnr ASCENDING.
*    SORT it_selecao BY belnr ASCENDING.
*
*    LOOP AT it_selecao ASSIGNING FIELD-SYMBOL(<_del>).
*      DELETE it_selecao WHERE belnr = <_del>-belnr.
*    ENDLOOP.
  ...
  IF it_selecao IS NOT INITIAL.
    ...
    SORT it_acdoca BY rbukrs gjahr belnr belnr ASCENDING.

    DELETE it_acdoca WHERE hsl = 0 AND ksl = 0.
    DELETE it_acdoca WHERE racct = '0000212200'.
    DELETE it_acdoca WHERE blart = 'VA'.
    ...
    IF it_acdoca IS NOT INITIAL.
      ...
      LOOP AT it_acdoca ASSIGNING FIELD-SYMBOL(<acdoca>).
        ...
        wa_saida-obj_key        = |LED2{ <acdoca>-rbukrs }{ <acdoca>-belnr }|.
...
FORM gera_contabilizacao.
  ...
  IF it_save IS NOT INITIAL.
    ...
    LOOP AT it_zib_contabil ASSIGNING FIELD-SYMBOL(<_prepara_zib_contabil>).
      CASE <_prepara_zib_contabil>-bschl.
        WHEN '40'.
          <_prepara_zib_contabil>-bschl = '50'.
        WHEN '50'.
          <_prepara_zib_contabil>-bschl = '40'.
        WHEN OTHERS.
      ENDCASE.
    ENDLOOP.
