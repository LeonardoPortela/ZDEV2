CLASS zcl_int_ob_imobilizado DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    CONSTANTS at_id_interface TYPE zde_id_interface VALUE '165' ##NO_TEXT.
    DATA at_dados_imobi TYPE zsdst0043 .
...
CLASS ZCL_INT_OB_IMOBILIZADO IMPLEMENTATION.
  ...
  METHOD zif_integracao_outbound~build_info_request.
    ...
    DATA: lwa_imobi TYPE zsdst0043.

    r_if_integracao_outbound = me.

*    MOVE-CORRESPONDING i_info_request TO lwa_imobi.
    MOVE-CORRESPONDING i_info_request TO me->at_dados_imobi.
  ...
  METHOD zif_integracao_outbound~get_instance.
    ...
    IF zif_integracao_outbound~at_if_integracao_outbound IS NOT BOUND.
      CREATE OBJECT zif_integracao_outbound~at_if_integracao_outbound TYPE zcl_int_ob_imobilizado.
    ENDIF.
  ...
  METHOD zif_integracao_outbound~set_url.
    ...
    SELECT SINGLE *
     ...
     WHERE service = 'SISTEMAS_LEGADOS_ENV_IMOBILIZADO'.
      ...
      RAISE EXCEPTION TYPE zcx_integracao
        EXPORTING
          textid = VALUE #( msgid = zcx_integracao=>zcx_erro_geral-msgid
                            ...
                            attr2 = 'SISTEMAS_LEGADOS_ENV_IMOBILIZADO' )
          msgid  = zcx_integracao=>zcx_erro_geral-msgid
          msgno  = zcx_integracao=>zcx_erro_geral-msgno
          msgty  = 'E'
          msgv1  = 'Serviço não configurado: '
          msgv2  = 'SISTEMAS_LEGADOS_ENV_IMOBILIZADO'.
    ENDIF.
