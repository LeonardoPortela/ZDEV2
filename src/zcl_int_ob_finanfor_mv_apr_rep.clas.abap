*&-------------------------------------------------------------------------------------------------------*
*& Report         : ZCL_INT_OB_FINANFOR_MV_APR_REP                                                        *
...
*&-------------------------------------------------------------------------------------------------------*
*&-------------------------------------------------------------------------------------------------------*
*& 14/07/2025  |DEVK9A2OOH  |NSEGATIN       |Integração Finanfor - Aprova/Reprova Movimento. Dev. inicial*
*&                                          |Chamado: 184694.                                            *
*--------------------------------------------------------------------------------------------------------*
CLASS zcl_int_ob_finanfor_mv_apr_rep DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    DATA at_param_mov_apr_rep TYPE zfie_finanfor_mov_apr_rep.
...
CLASS ZCL_INT_OB_FINANFOR_MV_APR_REP IMPLEMENTATION.
  ...
  METHOD zif_integracao_outbound~build_info_request.
    ...
    MOVE-CORRESPONDING i_info_request TO at_param_mov_apr_rep.
  ...
  METHOD zif_integracao_outbound~get_instance.
    ...
    IF zif_integracao_outbound~at_if_integracao_outbound IS NOT BOUND.
      CREATE OBJECT zif_integracao_outbound~at_if_integracao_outbound TYPE zcl_int_ob_finanfor_mv_apr_rep.
    ENDIF.
  ...
  METHOD zif_integracao_outbound~set_url.
    ...
    CONSTANTS: cl_content_type TYPE string VALUE 'Content-Type',
               ...
               cl_assets       TYPE string VALUE 'assets',       "Corresponde ao parâmetro "category" da URL
               cl_patch        TYPE string VALUE 'PATCH'.
    ...
    me->zif_integracao_inject~at_info_request_http-ds_metodo             = cl_patch.
    lwa_webservice-url = lwa_webservice-url && cl_assets.
