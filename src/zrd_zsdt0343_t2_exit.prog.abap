*&--------------------------------------------------------------------&*
*&                     Relatório Módulo - SD                          &*
*&--------------------------------------------------------------------&*
*& Projeto..: AMAGGI                                                  &*
*& Autor....: Ronaldo Freitas                                         &*
*& Data.....: 04/07/2024                                              &*
*& Descrição: Cadastro de valores UPF e Alíquotas Retenção nas Vendas &*
*& Transação:                                                         &*
*&--------------------------------------------------------------------&*
*& Projeto  :                                                         &*
*& Código Espec.Funcional/Técnica:                                    &*
*&--------------------------------------------------------------------&*
*&                    Histórico de Modificações                       &*
*& Autor           Request      Data         Descrição                &*
*& ABAP                                                               &*
*&--------------------------------------------------------------------&*
*&---------------------------------------------------------------------*
*& Report  ZRD_ZSDT0343_EXIT
*&
*&---------------------------------------------------------------------*
*&
*&
*&---------------------------------------------------------------------*
REPORT zrd_zsdt0343_t2_exit.

FORM f_exit_zsdt0343_t2_0002 USING p_registro_manter TYPE any
                             CHANGING p_error.

  DATA: wl_zsdt0343_t2 TYPE zsdt0343_t2_out.
  DATA: lv_num(10).

  CLEAR: wl_zsdt0343_t2.

  MOVE-CORRESPONDING p_registro_manter TO wl_zsdt0343_t2.

  IF wl_zsdt0343_t2-bukrs IS INITIAL.
    p_error = abap_true.
    MESSAGE 'Preencha a Empresa!' TYPE 'I'.
    EXIT.
  ENDIF.

  IF wl_zsdt0343_t2-kschl IS INITIAL.
    p_error = abap_true.
    MESSAGE 'Preencha a condição!' TYPE 'I'.
    EXIT.
  ENDIF.

  IF wl_zsdt0343_t2-branch IS INITIAL.
    p_error = abap_true.
    MESSAGE 'Preencha o Local de Negócio!' TYPE 'I'.
    EXIT.
  ENDIF.

  IF wl_zsdt0343_t2-uf IS INITIAL.
    p_error = abap_true.
    MESSAGE 'Preencha o Estado!' TYPE 'I'.
    EXIT.
  ENDIF.

  IF wl_zsdt0343_t2-docdatd GT wl_zsdt0343_t2-docdata.
    p_error = abap_true.
    MESSAGE 'Data (De) não pode ser maior que (Até)!' TYPE 'I'.
    EXIT.
  ENDIF.

  IF wl_zsdt0343_t2-vtext IS INITIAL.
    SELECT vtext UP TO 1 ROWS
      FROM t685t
      INTO @DATA(lv_desc)
      WHERE spras EQ @sy-langu
        AND kschl EQ @wl_zsdt0343_t2-kschl.
    ENDSELECT.
  ENDIF.

  SELECT * FROM zsdt0343_t2
    INTO TABLE @DATA(it_found)
    WHERE bukrs    EQ @wl_zsdt0343_t2-bukrs
      AND branch   EQ @wl_zsdt0343_t2-branch
      AND kschl    EQ @wl_zsdt0343_t2-kschl
      AND uf       EQ @wl_zsdt0343_t2-uf
*      AND ( docdatd BETWEEN @wl_zsdt0343_t2-docdatd AND @wl_zsdt0343_t2-docdata
*      AND docdata BETWEEN @wl_zsdt0343_t2-docdatd AND @wl_zsdt0343_t2-docdata )
*      AND ( ( docdata  GE @wl_zsdt0343_t2-docdatd
*      AND   docdatd  LE @wl_ZSDT0343_t2-docdatd )
*      AND ( docdata  GE @wl_zsdt0343_t2-docdata
*      AND   docdatd  LE @wl_ZSDT0343_t2-docdata ) )
      AND cancel   EQ @abap_off.
*  ENDSELECT.

  IF sy-subrc IS INITIAL AND sy-ucomm EQ 'NOVO'.

    LOOP AT it_found ASSIGNING FIELD-SYMBOL(<fs_found>).
      IF ( wl_zsdt0343_t2-docdatd GE <fs_found>-docdatd AND wl_zsdt0343_t2-docdatd LE <fs_found>-docdata )
      OR ( wl_zsdt0343_t2-docdata GE <fs_found>-docdatd AND wl_zsdt0343_t2-docdata LE <fs_found>-docdata )
      OR ( wl_zsdt0343_t2-docdatd GE <fs_found>-docdatd AND wl_zsdt0343_t2-docdata LE <fs_found>-docdata )
*      OR ( wl_zsdt0343_t2-docdatd LE <fs_found>-docdatd AND wl_zsdt0343_t2-docdata LE <fs_found>-docdata )
      OR ( wl_zsdt0343_t2-docdatd LE <fs_found>-docdatd AND wl_zsdt0343_t2-docdata GE <fs_found>-docdata ).
        p_error = abap_true.
        MESSAGE 'Já existe cadastro ativo para o período total/parcial informado!' TYPE 'I'.
        EXIT.
*      ELSEIF wl_zsdt0343_t2-docdata GE <fs_found>-docdatd AND wl_zsdt0343_t2-docdata LE <fs_found>-docdata.
*        p_error = abap_true.
*        MESSAGE 'Cadastro já existe!' TYPE 'I'.
*        EXIT.
*      ELSEIF wl_zsdt0343_t2-docdatd GE <fs_found>-docdatd AND wl_zsdt0343_t2-docdata LE <fs_found>-docdata.
*        p_error = abap_true.
*        MESSAGE 'Cadastro já existe!' TYPE 'I'.
*        EXIT.
      ENDIF.
    ENDLOOP.
  ENDIF.

*  SELECT * FROM ZSDT0343_t2
*    UP TO 1 ROWS
*    INTO @lv_found
*    WHERE bukrs    EQ @wl_ZSDT0343_t2-bukrs
*      AND branch   EQ @wl_ZSDT0343_t2-branch
*      AND kschl    EQ @wl_ZSDT0343_t2-kschl
*      AND uf       EQ @wl_ZSDT0343_t2-uf
*      AND docdatd  LE @wl_ZSDT0343_t2-docdatd
*      AND docdata  GE @wl_ZSDT0343_t2-docdata
*      AND cancel   EQ @abap_on.
*  ENDSELECT.
*
*  IF sy-subrc IS INITIAL AND sy-ucomm EQ 'CHANGE' AND lv_found-cancel IS NOT INITIAL.
*    p_error = abap_true.
*    MESSAGE 'Registro cancelado, valor não pode ser modificado!' TYPE 'I'.
*    EXIT.
*  ENDIF.

  SELECT * FROM zsdt0343_t1
    UP TO 1 ROWS
    INTO @DATA(wa_zsdt0343_t1)
    WHERE bukrs    EQ @wl_zsdt0343_t2-bukrs
      AND uf       EQ @wl_zsdt0343_t2-uf
      AND meins    EQ @wl_zsdt0343_t2-meins
      AND cancel   EQ @abap_off.
  ENDSELECT.
  IF sy-subrc IS NOT INITIAL OR wa_zsdt0343_t1-vl_upf IS INITIAL.
    p_error = abap_true.
    MESSAGE 'Não existe UPF cadastrada para a empresa e Estado informado!' TYPE 'I'.
    EXIT.
  ENDIF.

*  IF sy-subrc IS INITIAL AND sy-ucomm EQ 'CHANGE' AND wl_ZSDT0343_t2-vl_upf NE lv_found-vl_upf.
*    p_error = abap_true.
*    MESSAGE 'Valor não pode ser modificado!' TYPE 'I'.
*    EXIT.
*  ENDIF.
  IF sy-ucomm EQ 'NOVO'.
    CALL FUNCTION 'NUMBER_GET_NEXT'
      EXPORTING
        nr_range_nr             = '01'
        object                  = 'ZSDT0343T2'
      IMPORTING
        number                  = lv_num
      EXCEPTIONS
        interval_not_found      = 1
        number_range_not_intern = 2
        object_not_found        = 3
        quantity_is_0           = 4
        quantity_is_not_1       = 5
        interval_overflow       = 6
        buffer_overflow         = 7
        OTHERS                  = 8.
    IF sy-subrc NE 0.
      p_error = abap_true.
      MESSAGE i836(sd) WITH 'O intervalo de numeração,'
                            'não foi encontrado!'.
      EXIT.
    ENDIF.
    wl_zsdt0343_t2-id = lv_num.
  ENDIF.

  wl_zsdt0343_t2-c_user = sy-uname.
  wl_zsdt0343_t2-hora = sy-uzeit.
  wl_zsdt0343_t2-data = sy-datum.
  IF wl_zsdt0343_t2-vtext IS NOT INITIAL.
    wl_zsdt0343_t2-vtext = wl_zsdt0343_t2-vtext.
  ELSE.
    wl_zsdt0343_t2-vtext = lv_desc.
  ENDIF.

  wl_zsdt0343_t2-fator = ( wa_zsdt0343_t1-vl_upf * wl_zsdt0343_t2-rate ) / 100.

  MOVE-CORRESPONDING wl_zsdt0343_t2 TO p_registro_manter.

ENDFORM.

FORM f_exit_zsdt0343_t2_0005 CHANGING p_registro_manter TYPE any.

  DATA: wl_zsdt0343_t2 TYPE zsdt0343_t2_out.

  CLEAR: wl_zsdt0343_t2.

  MOVE-CORRESPONDING p_registro_manter TO wl_zsdt0343_t2.


*  SELECT * FROM zsdt0343_t2
*    UP TO 1 ROWS
*    INTO @DATA(lv_found)
*    WHERE bukrs    EQ @wl_zsdt0343_t2-bukrs
*      AND branch   EQ @wl_zsdt0343_t2-branch
*      AND kschl    EQ @wl_zsdt0343_t2-kschl
*      AND uf       EQ @wl_zsdt0343_t2-uf
*      AND docdatd  LE @wl_zsdt0343_t2-docdatd
*      AND docdata  GE @wl_zsdt0343_t2-docdata.
**      AND cancel   EQ @abap_on.
*  ENDSELECT.
*
*  IF sy-subrc IS INITIAL AND lv_found-cancel IS NOT INITIAL. "AND sy-ucomm EQ 'CHANGE'
*    MESSAGE 'Registro cancelado, valor não pode ser modificado!' TYPE 'I'.
*
*    LOOP AT SCREEN.
*      IF screen-name(23) EQ '<FS_WA_REGISTRO_MANTER>'.
*        screen-input = 0.
*        MODIFY SCREEN.
*      ENDIF.
*    ENDLOOP.
*
*    EXIT.
*  ENDIF.

  LOOP AT SCREEN.
    IF screen-name = '<FS_WA_REGISTRO_MANTER>-FATOR'.
      screen-input = 0.
      MODIFY SCREEN.
    ENDIF.
    IF sy-ucomm EQ 'CHANGE'.
      IF screen-name = '<FS_WA_REGISTRO_MANTER>-VTEXT'.
        screen-input = 0.
        MODIFY SCREEN.
      ENDIF.
    ENDIF.

  ENDLOOP.

  MOVE-CORRESPONDING wl_zsdt0343_t2 TO p_registro_manter.

ENDFORM.

FORM f_exit_zsdt0343_t2_0010 TABLES it_zsdt0343.

  DATA: it_zsdt0343_t2 TYPE STANDARD TABLE OF zsdt0343_t2_out.

  it_zsdt0343_t2[] = CORRESPONDING #( it_zsdt0343[] ).

  IF it_zsdt0343_t2[] IS NOT INITIAL.

    SELECT *
      FROM t685t
      INTO TABLE @DATA(it_desc)
      FOR ALL ENTRIES IN @it_zsdt0343_t2
      WHERE spras EQ @sy-langu
        AND kschl EQ @it_zsdt0343_t2-kschl.

    LOOP AT it_zsdt0343_t2 ASSIGNING FIELD-SYMBOL(<fs_zsdt0343>).

      READ TABLE it_desc INTO DATA(wa_desc) WITH KEY kschl = <fs_zsdt0343>-kschl.
      IF sy-subrc IS INITIAL.
        <fs_zsdt0343>-vtext = wa_desc-vtext.
      ENDIF.
    ENDLOOP.

    it_zsdt0343[] = CORRESPONDING #( it_zsdt0343_t2[] ).

  ENDIF.
*  DATA: lwa_0293 TYPE ty_zsdt0293.
*
*  REFRESH it_zsdt0293.
*
*  SELECT *
*    FROM zsdt0293
*    INTO TABLE @DATA(t_zsdt0293)
*    WHERE status = @abap_false.
*
*  LOOP AT t_zsdt0293 INTO DATA(lwa_zsdt0293).
*    MOVE-CORRESPONDING lwa_zsdt0293 TO lwa_0293.
*    APPEND lwa_0293 TO it_zsdt0293.
*  ENDLOOP.

ENDFORM.

FORM f_exit_zsdt0343_t2_0019 USING p_registro_search TYPE any
                       CHANGING p_error
                                p_cond TYPE rsds_where.

  DATA: lwa_cond_line  TYPE rsdswhere.

  DATA: wl_zsdt0343_t2 TYPE zsdt0343_t2_out.

  CLEAR: wl_zsdt0343_t2, p_cond.

  MOVE-CORRESPONDING p_registro_search TO wl_zsdt0343_t2.

*------------------------------------------------------------
  SELECT * FROM zsdt0343_range
    INTO TABLE @DATA(it_zsdt0343_range)
    WHERE campo EQ 'BUKRS'
      AND c_option EQ 'EQ'.

  IF sy-subrc IS INITIAL.
    SORT it_zsdt0343_range BY c_option low.

    DELETE ADJACENT DUPLICATES FROM it_zsdt0343_range COMPARING c_option low.

    DATA(lv_line) = lines( it_zsdt0343_range ).

    IF lv_line GE '3'.
      LOOP AT it_zsdt0343_range ASSIGNING FIELD-SYMBOL(<fs_range>).

        AT FIRST.
          DATA(lv_str) = '( ' && | '{ <fs_range>-low }'| && ','.
          CONTINUE.
        ENDAT.
        AT NEW low.
          IF sy-tabix EQ lv_line.
            lv_str = lv_str && | '{ <fs_range>-low }' | && ')'.
            CONTINUE.
          ENDIF.
          lv_str = lv_str && | '{ <fs_range>-low }'| && ','.
        ENDAT.
*      AT LAST.
*        lv_str = lv_str && | '{ <fs_range>-low }' | && ')'.
*      ENDAT.
      ENDLOOP.
    ELSEIF lv_line EQ '2'.
      LOOP AT it_zsdt0343_range ASSIGNING <fs_range>.
        AT FIRST.
          lv_str = '( ' && | '{ <fs_range>-low }'| && ','.
        ENDAT.
        AT LAST.
          lv_str = lv_str && | '{ <fs_range>-low }' | && ')'.
        ENDAT.
      ENDLOOP.
    ELSEIF lv_line EQ '1'.
      LOOP AT it_zsdt0343_range ASSIGNING <fs_range>.
        AT LAST.
          lv_str = '( ' && | '{ <fs_range>-low }' | && ')'.
        ENDAT.
      ENDLOOP.
    ENDIF.

    IF lv_str IS NOT INITIAL.
      APPEND VALUE #( line = |( BUKRS IN { lv_str } ) | ) TO  p_cond-where_tab.
    ENDIF.

    IF lv_str IS NOT INITIAL.
      APPEND VALUE #( line = |AND ( CANCEL NE '{ abap_true }' ) | ) TO  p_cond-where_tab.
    ENDIF.

*    DELETE FROM ZSDT0343_range WHERE campo EQ 'BUKRS'.
*    COMMIT WORK.
  ENDIF.

*------------------------------------------------------------
  SELECT * FROM zsdt0343_range
    INTO TABLE @it_zsdt0343_range
    WHERE campo    EQ 'BUKRS'
      AND c_option NE 'EQ'.

  IF sy-subrc IS INITIAL.

    LOOP AT it_zsdt0343_range ASSIGNING <fs_range>.
      APPEND VALUE #( line = |AND ( BUKRS | && <fs_range>-c_option && | '{ <fs_range>-low }' ) | ) TO  p_cond-where_tab.
    ENDLOOP.

*    DELETE FROM ZSDT0343_range WHERE campo EQ 'BUKRS' AND c_option NE 'EQ'.
*    COMMIT WORK.

  ENDIF.
*--------------------------------------------------------------------
  SELECT * FROM zsdt0343_range
    INTO TABLE @it_zsdt0343_range
    WHERE campo EQ 'WERKS'
      AND c_option EQ 'EQ'.

  IF sy-subrc IS INITIAL.
    SORT it_zsdt0343_range BY c_option low.

    DELETE ADJACENT DUPLICATES FROM it_zsdt0343_range COMPARING c_option low.

    lv_line = lines( it_zsdt0343_range ).

    IF lv_line GE '3'.
      LOOP AT it_zsdt0343_range ASSIGNING <fs_range>.

        AT FIRST.
          lv_str = '( ' && | '{ <fs_range>-low }'| && ','.
          CONTINUE.
        ENDAT.

        AT NEW low.
          IF sy-tabix EQ lv_line.
            lv_str = lv_str && | '{ <fs_range>-low }' | && ')'.
            CONTINUE.
          ENDIF.
          lv_str = lv_str && | '{ <fs_range>-low }'| && ','.
        ENDAT.
*      AT LAST.
*        lv_str = lv_str && | '{ <fs_range>-low }' | && ')'.
*      ENDAT.
      ENDLOOP.
    ELSEIF lv_line EQ '2'.
      LOOP AT it_zsdt0343_range ASSIGNING <fs_range>.
        AT FIRST.
          lv_str = '( ' && | '{ <fs_range>-low }'| && ','.
        ENDAT.
        AT LAST.
          lv_str = lv_str && | '{ <fs_range>-low }' | && ')'.
        ENDAT.
      ENDLOOP.
    ELSEIF lv_line EQ '1'.
      LOOP AT it_zsdt0343_range ASSIGNING <fs_range>.
        AT LAST.
          lv_str = '( ' && |' { <fs_range>-low }' | && ')'.
        ENDAT.
      ENDLOOP.
    ENDIF.

    CLEAR lv_str.

    IF lv_str IS NOT INITIAL.
      APPEND VALUE #( line = |AND ( BRANCH IN { lv_str } ) | ) TO  p_cond-where_tab.
    ENDIF.

*    DELETE FROM ZSDT0343_range WHERE campo EQ 'WERKS' AND c_option EQ 'EQ'.
*    COMMIT WORK.
  ENDIF.
*-------------------------------------------------------------------------------------
  SELECT * FROM zsdt0343_range
    INTO TABLE @it_zsdt0343_range
    WHERE campo EQ 'WERKS'
      AND c_option EQ 'BT'.

  IF sy-subrc IS INITIAL.

    LOOP AT it_zsdt0343_range ASSIGNING <fs_range>.
      APPEND VALUE #( line = |AND ( BRANCH BETWEEN '{ <fs_range>-low  }' AND '{ <fs_range>-high  }' ) | ) TO  p_cond-where_tab.
    ENDLOOP.

*    DELETE FROM ZSDT0343_range WHERE campo EQ 'WERKS' AND c_option EQ 'BT'.
*    COMMIT WORK.
  ENDIF.

ENDFORM.

FORM f_exit_zsdt0343_t2_0013  TABLES p_tables.

  DATA: wl_zsdt0343_t2 TYPE zsdt0343_t2,
        it_zsdt0343_t2 TYPE TABLE OF zsdt0343_t2.

  LOOP AT p_tables ASSIGNING FIELD-SYMBOL(<fs_p_table>).
    wl_zsdt0343_t2 = CORRESPONDING #( <fs_p_table> ).

    IF wl_zsdt0343_t2-cancel IS INITIAL.
      wl_zsdt0343_t2-cancel = abap_on.
      wl_zsdt0343_t2-user_cancel = sy-uname.
      wl_zsdt0343_t2-data_cancel = sy-datum.
      wl_zsdt0343_t2-hora_cancel = sy-uzeit.

      MODIFY zsdt0343_t2 FROM wl_zsdt0343_t2.
      IF sy-subrc IS INITIAL.
        COMMIT WORK.
      ENDIF.
    ENDIF.
  ENDLOOP.
ENDFORM.
FORM f_exit_zsdt0343_t2_0008 CHANGING p_col_pos
                                p_ref_tabname
                                p_ref_fieldname
                                p_tabname
                                p_field
                                p_scrtext_l
                                p_outputlen
                                p_edit
                                p_sum
                                p_emphasize
                                p_just
                                p_hotspot
                                p_f4
                                p_check.

  IF p_ref_tabname = 'ZSDT0343_T2_OUT'. "= 'ZSDT0343 T2'.
*     p_field       = 'VTEXT'.
    p_outputlen = 12.
  ENDIF.

ENDFORM.
FORM  f_exit_zsdt0343_t2_0009 TABLES it_excl_toolbar
                              USING p_db_tab.

  IF p_db_tab = 'ZSDT0343_T2'.
    APPEND 'Deletar'      TO it_excl_toolbar.
    APPEND 'Modificar'    TO it_excl_toolbar.
  ENDIF.
ENDFORM.
FORM f_exit_zsdt0343_t2_0016 USING p_ucomm TYPE sy-ucomm
                             CHANGING p_registro_manter TYPE any
                                      p_saida TYPE any.

  DATA: wl_zsdt0343_t2 TYPE zsdt0343_t2_out.

  MOVE-CORRESPONDING p_registro_manter TO wl_zsdt0343_t2.

  IF wl_zsdt0343_t2-kschl IS NOT INITIAL.

    SELECT kschl, vtext UP TO 1 ROWS
    FROM t685t
    INTO @DATA(wa_desc)
    WHERE spras EQ @sy-langu
      AND kschl EQ @wl_zsdt0343_t2-kschl.
    ENDSELECT.
    IF sy-subrc IS INITIAL.
      wl_zsdt0343_t2-vtext = wa_desc-vtext.
    ENDIF.

    MOVE-CORRESPONDING wl_zsdt0343_t2 TO p_registro_manter.
    MOVE-CORRESPONDING wl_zsdt0343_t2 TO p_saida.

  ENDIF.

  CASE p_ucomm.
    WHEN 'BCALC'.
      SELECT * FROM zsdt0343_t1
        UP TO 1 ROWS
        INTO @DATA(wa_zsdt0343_t1)
        WHERE bukrs    EQ @wl_zsdt0343_t2-bukrs
          AND uf       EQ @wl_zsdt0343_t2-uf
          AND meins    EQ @wl_zsdt0343_t2-meins
          AND cancel   EQ @abap_off.
      ENDSELECT.
      IF sy-subrc IS NOT INITIAL OR wa_zsdt0343_t1-vl_upf IS INITIAL.
        MESSAGE 'Não existe UPF cadastrada para a empresa e Estado informado!' TYPE 'I'.
        EXIT.
      ELSE.
        wl_zsdt0343_t2-fator = ( wa_zsdt0343_t1-vl_upf * wl_zsdt0343_t2-rate ) / 100.
      ENDIF.

      MOVE-CORRESPONDING wl_zsdt0343_t2 TO p_registro_manter.
      MOVE-CORRESPONDING wl_zsdt0343_t2 TO p_saida.
*    WHEN OTHERS.

  ENDCASE.

ENDFORM.
FORM f_exit_zsdt0343_t2_0017 USING p_tipo.

  IF p_tipo = '0001'.
    PERFORM f4_val_kschl USING '<FS_WA_REGISTRO_MANTER>-KSCHL'
                               '<FS_WA_REGISTRO_MANTER>-VTEXT'.
  ENDIF.

ENDFORM.

*FORM f_exit_zsdt0343_t2_0020 CHANGING p_refresh_selecao.
*
*  p_refresh_selecao = abap_true.
*
*ENDFORM.
FORM f4_val_kschl USING p_cod TYPE help_info-dynprofld
                        p_desc TYPE help_info-dynprofld.


  FIELD-SYMBOLS: <fs_campo> TYPE any.
  FIELD-SYMBOLS: <fs_campo2> TYPE any.

  ASSIGN ('(ZREGISTER_DATA)<FS_WA_REGISTRO_MANTER>-KSCHL') TO <fs_campo>.
  ASSIGN ('(ZREGISTER_DATA)<FS_WA_REGISTRO_MANTER>-VTEXT') TO <fs_campo2>.

  IF <fs_campo> IS ASSIGNED.

    SELECT kschl, vtext UP TO 1 ROWS
      FROM t685t
      INTO @DATA(t_desc)
      WHERE spras EQ @sy-langu
        AND kschl EQ @<fs_campo>.
    ENDSELECT.

    IF sy-subrc IS INITIAL AND t_desc-vtext IS NOT INITIAL.
      IF <fs_campo2> IS ASSIGNED.
        <fs_campo2>  = t_desc-vtext.
      ENDIF.
    ENDIF.
  ENDIF.

**====>  Tabelas internas
*  DATA: BEGIN OF t_skat OCCURS 0,
*          hkont  TYPE hkont,
*          txt50  TYPE txt50,
*        END OF t_skat.
*
*  DATA: t_return  TYPE STANDARD TABLE OF ddshretval.
*  DATA: t_mapping TYPE STANDARD TABLE OF dselc.
*
**====>  Work Area
*  DATA: s_return  TYPE ddshretval.
*  DATA: s_mapping TYPE dselc.
**  DATA: c_pt    TYPE langu VALUE 'PT',
**        c_ktopl TYPE ktopl VALUE '0050'.
*
**  READ TABLE t_return ASSIGNING FIELD-SYMBOL(<fs>) WITH KEY fieldname = 'F0001'.
**  IF sy-subrc = 0.
*
**  SELECT saknr txt50
**    FROM  skat INTO TABLE t_skat
**    WHERE spras = c_pt
**      AND ktopl = c_ktopl.
*    SELECT kschl, vtext "UP TO 1 ROWS
*      FROM t685t
*      INTO table @DATA(t_desc)
*      WHERE spras EQ @sy-langu
*        AND kschl EQ @p_cod.
**    ENDSELECT.
*
**  ENDIF.
*
*  IF sy-subrc = 0.
*
*    s_mapping-fldname     = 'F0001'.
*    s_mapping-dyfldname   = p_cod.
*    APPEND s_mapping TO t_mapping.
*    CLEAR s_mapping.
*
*    s_mapping-fldname     = 'F0002'.
*    s_mapping-dyfldname   = p_desc.
*    APPEND s_mapping TO t_mapping.
*    CLEAR s_mapping.
*
*    CALL FUNCTION 'F4IF_INT_TABLE_VALUE_REQUEST'
*      EXPORTING
*        retfield        = 'KSCHL'
*        dynpprog        = sy-cprog
*        dynpnr          = sy-dynnr
*        dynprofield     = p_cod
*        window_title    = 'Tipos de Condição'
*        value_org       = 'S'
*      TABLES
*        value_tab       = t_desc
*        return_tab      = t_return
*        dynpfld_mapping = t_mapping
*      EXCEPTIONS
*        parameter_error = 1
*        no_values_found = 2
*        OTHERS          = 3.
*
*    IF sy-subrc <> 0.
*      MESSAGE ID sy-msgid TYPE sy-msgty NUMBER sy-msgno
*      WITH sy-msgv1 sy-msgv2 sy-msgv3 sy-msgv4.
*    ENDIF.
*  ENDIF.

ENDFORM.
