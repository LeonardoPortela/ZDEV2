class ZCL_INT_OB_SET_COMENT_IR definition
  public
  final
  create public .
...
public section.
  ...
  constants AT_ID_INTERFACE type ZDE_ID_INTERFACE value '281' ##NO_TEXT.
  data AT_DADOS_SET_COMENT_IR type ZFIE_OB_SET_COMENT_IR .
...
CLASS ZCL_INT_OB_SET_COMENT_IR IMPLEMENTATION.
  ...
  METHOD ZIF_INTEGRACAO_OUTBOUND~BUILD_INFO_REQUEST.
    ...
    DATA: lt_set_coment_ir TYPE ZFIE_OB_SET_COMENT_IR.

    r_if_integracao_outbound = me.

    MOVE-CORRESPONDING i_info_request TO lt_set_coment_ir.

    me->at_dados_set_coment_ir = lt_set_coment_ir.
  ...
  method ZIF_INTEGRACAO_OUTBOUND~GET_DATA.
    ...
    CALL METHOD /ui2/cl_json=>serialize
       EXPORTING
         data   = me->at_dados_set_coment_ir
       RECEIVING
         r_json = e_data.

    loop at me->at_dados_set_coment_ir-instancia into data(wa_dados).
...
*    e_referencia-id_referencia = me->at_dados_due-id_due.
  ...
  method ZIF_INTEGRACAO_OUTBOUND~GET_INSTANCE.
    ...
    IF zif_integracao_outbound~at_if_integracao_outbound IS NOT BOUND.
      CREATE OBJECT zif_integracao_outbound~at_if_integracao_outbound TYPE zcl_int_ob_set_coment_ir.
    ENDIF.
  ...
  METHOD ZIF_INTEGRACAO_OUTBOUND~SET_URL.
    ...
    SELECT SINGLE *
     ...
     WHERE service = 'OB_SET_COMENT_IR'.
