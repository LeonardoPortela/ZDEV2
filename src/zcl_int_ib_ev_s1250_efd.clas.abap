CLASS zcl_int_ib_ev_s1250_efd DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    DATA:
      BEGIN OF zde_data_request ,
        eventos TYPE zsdt_zhrst_efd_e1250m,
      END OF zde_data_request .
    CONSTANTS at_id_interface TYPE zde_id_interface VALUE '146' ##NO_TEXT.
...
CLASS ZCL_INT_IB_EV_S1250_EFD IMPLEMENTATION.
...
*-----------------------------------------------------------------------------------------------------------------------*
    ...
    DATA(lt_eventos) = lwa_data_request-eventos.
    DELETE lt_eventos WHERE bukrs IS INITIAL or
                            branch is INITIAL or
                            cpf    is INITIAL or
                            belnr  is INITIAL or
                            gjahr  is INITIAL.
    IF lt_eventos IS INITIAL.
      r_msg_erro = 'Favor preencher todos os campos chave!'.
      RETURN.
    ENDIF.
  ...
  METHOD zif_integracao_inject~set_integrar_inbound.
    ...
    DATA: lt_e1250m TYPE TABLE OF zhrst_efd_e1250m.
    ...
    lt_e1250m = lwa_data_request-eventos.

    MODIFY zhrst_efd_e1250m FROM TABLE lt_e1250m.
