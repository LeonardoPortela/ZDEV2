          ...
          IF ( wa_sol_5420-werks = '0175' ).
            TRY.
                PERFORM validar_material
                  USING
                    wa_sol_5420-matnr
                    '02'    "//St. Atividade: Fertilizante
                    'ZHAW'. "//Tipo Material: Importação
...
*FIM - AS -  02/10/2020 - CS2020001072
    ...
    IF vl_check_initial NE abap_true.
      ...
      LOOP AT it_frete_5420 INTO wa_frete_5420 WHERE edit EQ abap_true.
        ...
        READ TABLE it_zsdt0137 INTO wa_zsdt0137 WITH KEY nro_sol = vg_sol_5420-nro_sol "Incluindo seleção zsdt0137 "BUG 60282 - IR064717 - AOENNING.
                                                             seq = vg_sol_5420-seq.
...
INNER JOIN ekko AS b ON b~ebeln EQ a~ebeln
  ...
  ELSEIF
     ( popup_vinculacao-pedido   IS NOT INITIAL ).

    SELECT COUNT( * )
      FROM ekpo AS a
INNER JOIN ekko AS b ON b~ebeln EQ a~ebeln
     WHERE b~ebeln = popup_vinculacao-pedido
       AND b~bstyp = categoria.
