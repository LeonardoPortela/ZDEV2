...
*  id_mapa = |F05_| && |{ werks }| && |_CD_MAPA|.
CONCATENATE 'F05_'
            werks
            '_RG_MAPA'
            into id_mapa.

  READ TABLE _teor_real INTO DATA(wl_cd_mapah)
                        WITH KEY atnam = id_mapa.
  cd_mapah = wl_cd_mapah-ausp1.
...
FORM read_text_sd TABLES text TYPE STANDARD TABLE USING
  ...
  lv_matnr2 = lw_mvke-matnr.
  SHIFT lv_matnr2 LEFT DELETING LEADING ''.
  lv_name_obj = lv_matnr2 && lw_mvke-vkorg && lw_mvke-vtweg.
...
*                       cd_mapah  TYPE atwrt
  ...
  DATA: vl_classh type klah-class.
...
*  id_mapa = |F05_{ _shipment-werks }_CD_MAPA|.
*
*  READ TABLE _cd_mapa INTO wl_cd_mapa WITH KEY atnam = id_mapa.
*  cd_mapah = wl_cd_mapa-ausp1.

*if cd_mapah is initial.
*  cd_mapah = cd_mapa(9).
*endif.
