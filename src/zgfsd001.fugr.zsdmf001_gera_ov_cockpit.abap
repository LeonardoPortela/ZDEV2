FUNCTION ZSDMF001_GERA_OV_COCKPIT.
*"----------------------------------------------------------------------
*"*"Interface local:
*"  IMPORTING
...
*"     REFERENCE(I_EXIBE_RET) TYPE  FLAG DEFAULT 'X'
*"     REFERENCE(I_EXTCAL) TYPE  FLAG OPTIONAL
*"  EXPORTING
...
*        wl_bapiparex  TYPE bapiparex         ,
  ...
  REFRESH: TL_ITEMS_IN, TL_ITEMS_INX,TL_PARTNERS, TL_SCHEDULES_IN, TL_SCHEDULES_INX,TL_OVS,TL_VBUV,
           ...
           TL_TEXT_IN, TL_SAIDA_EXEC, ESTRUTURA, EVENTS, TL_MARA, TL_BAPIPAREX,IT_ZSDT0090.
  ...
  TRY.
      ...
      CL_PRC_RESULT_FACTORY=>GET_INSTANCE( )->GET_PRC_RESULT( )->GET_PRICE_ELEMENT_DB(
       ...
       ( FIELDNAME = 'KSCHL' VALUE = 'PR00' )
        ...
        IMPORTING ET_PRC_ELEMENT_CLASSIC_FORMAT = DATA(ETL178C2R7216) ).
      CLEAR TL_KONV.
      TYPES: BEGIN OF TYL178C2R8522,
               KPOSN TYPE KONV-KPOSN,
               KBETR TYPE KONV-KBETR,
               KMEIN TYPE KONV-KMEIN,
             END OF TYL178C2R8522.
      DATA: LML178C2R2352 TYPE TYL178C2R8522,
            LWL178C2R3886 LIKE LINE OF TL_KONV.
      LOOP AT ETL178C2R7216 REFERENCE INTO DATA(LDRL178C2R5345).
        LML178C2R2352-KPOSN = LDRL178C2R5345->KPOSN.
        LML178C2R2352-KBETR = LDRL178C2R5345->KBETR.
        LML178C2R2352-KMEIN = LDRL178C2R5345->KMEIN.
        LWL178C2R3886 = LML178C2R2352.
        APPEND LWL178C2R3886 TO TL_KONV.
      ENDLOOP.
    CATCH CX_PRC_RESULT .
      SY-SUBRC = 4.
  ENDTRY.
...
*---Monta dados de Parceiro
  ...
  DATA(LT_ITENS) = TL_ITENS[].
  SORT: LT_ITENS BY MATNR.
  DELETE ADJACENT DUPLICATES FROM LT_ITENS COMPARING MATNR.
  ...
  LOOP AT TL_ITENS.
    ...
    TL_ITEMS_IN-MATERIAL     = TL_ITENS-MATNR.

    DATA(V_LEN) = STRLEN( TL_ITENS-MATNR ).

    IF V_LEN > 18.
      TL_ITEMS_IN-MATERIAL_LONG     = TL_ITENS-MATNR.
    ELSE.
      TL_ITEMS_IN-MATERIAL     = TL_ITENS-MATNR.
    ENDIF.
    "*<--- 12/07/2023 - Migração S4 - LO
    TL_ITEMS_IN-TARGET_QTY   = IT_OV-ZMENG.
    TL_ITEMS_IN-TARGET_QU    = TL_ITENS-VRKME.
    TL_ITEMS_IN-SALES_UNIT   = TL_ITENS-VRKME.
    ...
    TL_ITEMS_IN-BATCH        = TL_ITENS-CHARG.
...
*                       WITH KEY matnr = tl_items_in-material
*                                BINARY SEARCH.
...
** deixou de ser necessário, pois será impresso direto na NF #83980
**Inicio USER STORY 69259 - ABAP terceiro - Gustavo Nobel
*    IF tl_items_in-plant    EQ '0175' AND
*       tl_itens-spart       EQ '02'   .
*       lwa_mara-mtart       EQ 'ZFER'.
*
*      CLEAR: tl_text_in.
...
*      APPEND tl_text_in.
*    ENDIF.
**Fim USER STORY 69259 - ABAP terceiro - Gustavo Nobel
** deixou de ser necessário, pois será impresso direto na NF #83980
    ...
    DATA(COEFICIENTE_D) = ZCL_SOLICITACAO_OV=>GET_IMPOSTO(
      ...
      _MATERIAL   = TL_ITENS-MATNR
      _TIPO_ORDEM = I_AUART
      _DIRECAO    = 'D'
      _WERKS      = TL_ITENS-WERKS
    ).

    IF WL_HEADER_IN-DOC_TYPE NE 'ZFUT'.
...
*      ORIGEM SEM IMPOSTO
  ...
  SORT TL_TEXT_IN.
  DELETE ADJACENT DUPLICATES FROM TL_TEXT_IN COMPARING ALL FIELDS.
...
* Verirfica se a ordem foi criada.
  ...
  IF NOT WL_VBELN IS INITIAL.
    ...
    "FF #173761 - inicio
    EXPORT I_VBELN  FROM WL_VBELN TO MEMORY ID 'memory_i_vbeln'. "Import será feito na transação ZSDT0079/ZSDR0037 e no ZSDR0038
    "FF #173761 - fim
         ...
         USING ''
          ...
          USING ''
                ...
                IT_OV-MATNR
                            CHANGING P_0090.
        ENDLOOP.
*"// US-169490 WBARBOSA 31/07/25 INICIO
        IF I_AUART NE 'ZFUT'.
          CALL METHOD ZCL_MANUTENCAO_INSUMOS=>REPROCESSA_DESCONTO_ABS
            EXPORTING
              I_SIMULADOR = W_DOC_SIMULACAO
              I_VBELN     = RT_VBELN
              I_MATNR     = P_0090-MATNR
            IMPORTING
...
*"// US-169490 WBARBOSA 31/07/25 INICIO
  ...
  ENDIF.
  IF I_EXTCAL IS INITIAL.                                   "FF #145609
    IF I_EXIBE_RET IS NOT INITIAL AND ( NOT LINE_EXISTS( TE_RETURN[ TYPE = 'E'] ) ).
...
*FF #171337 - inicio
  ELSE.
    ...
    EXPORT TE_RETURN FROM TE_RETURN TO MEMORY ID 'memory_te_return'. "Import será feito no ZSDR0038
    EXPORT TL_SAIDA_EXEC FROM TL_SAIDA_EXEC TO MEMORY ID 'memory_tl_saida_exec'. "Import será feito no ZSDR0038
