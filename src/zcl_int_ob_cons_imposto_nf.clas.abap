class ZCL_INT_OB_CONS_IMPOSTO_NF definition
  public
  final
  create public .
...
public section.
  ...
  constants AT_ID_INTERFACE type ZDE_ID_INTERFACE value '227' ##NO_TEXT.
  data AT_BODY type ZDE_ZMME0003 .
...
CLASS ZCL_INT_OB_CONS_IMPOSTO_NF IMPLEMENTATION.
...
*    e_referencia-id_referencia = me->at_dados_due-id_due.
  ...
  method ZIF_INTEGRACAO_OUTBOUND~GET_INSTANCE.
    ...
    if zif_integracao_outbound~at_if_integracao_outbound is not bound.
      create object zif_integracao_outbound~at_if_integracao_outbound type zcl_int_ob_cons_imposto_nf.
    endif.
  ...
  method ZIF_INTEGRACAO_OUTBOUND~SET_URL.
    ...
    select single *
     ...
     where service = 'OB_CONS_TIPO_IMPOSTO_NF_SIGAM'.
      ...
      raise exception type zcx_integracao
        exporting
          textid = value #( msgid = zcx_integracao=>zcx_erro_geral-msgid
                            ...
                            attr2 = 'OB_CONS_TIPO_IMPOSTO_NF_SIGAM' )
          msgid  = zcx_integracao=>zcx_erro_geral-msgid
          msgno  = zcx_integracao=>zcx_erro_geral-msgno
          msgty  = 'E'
          msgv1  = 'Serviço não configurado: '
          msgv2  = 'OB_CONS_TIPO_IMPOSTO_NF_SIGAM'.
    endif.
