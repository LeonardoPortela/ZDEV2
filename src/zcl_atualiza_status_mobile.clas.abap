CLASS zcl_atualiza_status_mobile DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    INTERFACES zif_atualiza_status_mobile .
...
CLASS ZCL_ATUALIZA_STATUS_MOBILE IMPLEMENTATION.


  METHOD constructor.

    me->zif_integracao_inject~at_id_interface      = zif_integracao=>at_id_interface_status_mobile.
  ...
  METHOD zif_atualiza_status_mobile~get_instance.

    IF zif_atualiza_status_mobile~at_if_atualiza_status_mobile IS NOT BOUND.
      CREATE OBJECT zif_atualiza_status_mobile~at_if_atualiza_status_mobile
        TYPE zcl_atualiza_status_mobile.
    ENDIF.
    r_if_atualiza_status_mobile = zif_atualiza_status_mobile~at_if_atualiza_status_mobile.

  ENDMETHOD.


  METHOD zif_atualiza_status_mobile~get_json.

    CLEAR: e_json.

    r_if_atualiza_status_mobile = me.

    CHECK me->zif_atualiza_status_mobile~at_status_nota IS NOT INITIAL.

** Construindo JSON.

    e_json = '{ ' && ' "usuario": ' && me->zif_atualiza_status_mobile~at_status_nota-usuario && '", '
                  && ' "status": ' && me->zif_atualiza_status_mobile~at_status_nota-status && '" } '.


  ENDMETHOD.


  METHOD zif_atualiza_status_mobile~post_atualiza_status_mobile.
...
*    "Inclui Json na Mesagem a Ser Enviada
    me->zif_atualiza_status_mobile~set_ds_url(
  ...
  METHOD zif_atualiza_status_mobile~set_ds_data.
  ...
  METHOD zif_atualiza_status_mobile~set_ds_url.
    ...
    SELECT SINGLE * INTO @DATA(wa_webservice)
     ...
     WHERE tipo    EQ '9'
       AND servico EQ '12'. "@me->zif_integracao_comb~at_servico.
      ...
      RAISE EXCEPTION TYPE zcx_integracao
        EXPORTING
          textid = VALUE #( msgid = zcx_integracao=>zcx_servico_http_config-msgid
                            ...
                            attr1 = 'UN'
                            attr2 = me->zif_atualiza_status_mobile~at_servico )
          msgid  = zcx_integracao=>zcx_servico_http_config-msgid
          msgno  = zcx_integracao=>zcx_servico_http_config-msgno
          msgty  = 'E'
          msgv1  = 'K'
          msgv2  = CONV #( me->zif_atualiza_status_mobile~at_servico ).
    ENDIF.
  ...
  METHOD zif_atualiza_status_mobile~set_send_msg.
