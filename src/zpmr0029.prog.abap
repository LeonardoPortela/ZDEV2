    ...
    LOOP AT items ASSIGNING FIELD-SYMBOL(<fs_data>).
      ...
      CALL METHOD me->get_charg_available_balance
        EXPORTING
          ...
          lote              = <fs_data>-ebeln
        RECEIVING
          available_balance = <fs_data>-clabs.
    ...
    CASE ucomm.
      ...
      WHEN 'BACK'.
        IF ( display_subscreen = screen-display_explosions ).
          display_reference->set_main_subscreen( screen-display ).
        ELSE.
          LEAVE PROGRAM.
        ENDIF.
...
CLASS cl_display_explosions IMPLEMENTATION.
  ...
  METHOD get_fieldcatalog.
    fieldscatalog = VALUE #(
        ...
        ( fieldname = 'CHILDREN_CHARG'   coltext = 'Lote'            outputlen = 10                                      )
...
*    DISPLAY_REFERENCE->ALV_GRID->REFRESH_TABLE_DISPLAY( IS_STABLE = VALUE #( COL = ABAP_TRUE ROW = ABAP_TRUE ) ).
  ...
  METHOD storno.
    ...
    TRY.
      ...
      CATCH cx_local_exception INTO DATA(_cx).
        RAISE EXCEPTION TYPE cx_local_exception EXPORTING texts = _cx->get_texts( ).
    ENDTRY.
    ...
    "OUTTAB_MAIN_LOTES[ EBELN = _MAIN_LOTE-EBELN EBELP = _MAIN_LOTE-EBELP ]-CHILDREN_LOTES  = OUTTAB_CHILDREN_LOTES.
    "OUTTAB_MAIN_LOTES[ EBELN = _MAIN_LOTE-EBELN EBELP = _MAIN_LOTE-EBELP ]-COUNT_CHILDRENS = |{ ICON_BATCH } ({ LINES( OUTTAB_CHILDREN_LOTES ) })|.
...
*    DATA(_DATA_VALIDADE)   = CONV SY-DATUM( DATA_VENCIMENTO + _DIAS_VENCIMENTO ).
    ...
    DATA(_header) = VALUE mcha( matnr = lv_matnr werks = centro charg = lote hsdat = dt_vencimento ).

    DATA(_characteristics) = VALUE clbatch_t( ( atnam = 'LOBM_LWEDT' atwtb = sy-datum )  ).

*      VALUE CLBATCH_T( ( ATNAM = 'FERTILIZANTES_MAPA'           ATWTB = _MARA-GROES     )
*                       ( ATNAM = 'FERTILIZANTES_DATAFABRICACAO' ATWTB = DATA_VENCIMENTO )
*                       ( ATNAM = 'FERTILIZANTES_DATAVALIDADE'   ATWTB = _DATA_VALIDADE  )
...
*
*      MESSAGE ID SY-MSGID TYPE SY-MSGTY NUMBER SY-MSGNO WITH SY-MSGV1 SY-MSGV2 SY-MSGV3 SY-MSGV4 INTO DATA(_MESSAGE).
*
...
*          TEXT = CONV #( _MESSAGE ).
  ...
  METHOD set_batch_deletion_flag.
    ...
    IF sy-subrc <> 0.
*      MESSAGE ID SY-MSGID TYPE SY-MSGTY NUMBER SY-MSGNO WITH SY-MSGV1 SY-MSGV2 SY-MSGV3 SY-MSGV4 INTO DATA(_MESSAGE).
*      RAISE EXCEPTION TYPE CX_LOCAL_EXCEPTION EXPORTING TEXT = CONV #( _MESSAGE ).
