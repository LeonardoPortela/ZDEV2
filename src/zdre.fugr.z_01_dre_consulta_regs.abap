FUNCTION z_01_dre_consulta_regs.
*"----------------------------------------------------------------------
*"*"Interface local:
*"  IMPORTING
...
*"----------------------------------------------------------------------
  ...
  IF p_bukrs IS NOT INITIAL.
    ...
    CALL FUNCTION 'Z_01_DRE_CONSULTA_REGS_BUKRS'
      EXPORTING
        ...
        EXEC SQL.
            ...
            SELECT DISTINCT
                   ...
                   CASE WHEN FI.AWTYP = 'VBRK' THEN
                          CASE WHEN CC.KOKRS = 'MAGI' THEN
                                 coalesce((SELECT VR.GEWEI FROM SAPHANADB.VBRP VR WHERE VR.MANDT = '300' AND VR.VBELN = FI.AWREF AND VR.POSNR = MGB.KDPOS AND VR.SPART <> '08'),FI.RUNIT)
                               WHEN CC.KOKRS = 'MGLD' THEN
                                 coalesce((SELECT VR.GEWEI FROM SAPHANADB.VBRP VR WHERE VR.MANDT = '300' AND VR.VBELN = FI.AWREF AND VR.POSNR = MGH.KDPOS AND VR.SPART <> '08'),FI.RUNIT)
                               WHEN CC.KOKRS = 'MGAR' THEN
                                 coalesce((SELECT VR.GEWEI FROM SAPHANADB.VBRP VR WHERE VR.MANDT = '300' AND VR.VBELN = FI.AWREF AND VR.POSNR = MGA.KDPOS AND VR.SPART <> '08'),FI.RUNIT)
                               WHEN CC.KOKRS = 'MGBG' THEN
                                 coalesce((SELECT VR.GEWEI FROM SAPHANADB.VBRP VR WHERE VR.MANDT = '300' AND VR.VBELN = FI.AWREF AND VR.POSNR = MGG.KDPOS AND VR.SPART <> '08'),FI.RUNIT)
                               WHEN CC.KOKRS = 'MGTF' THEN
                                 coalesce((SELECT VR.GEWEI FROM SAPHANADB.VBRP VR WHERE VR.MANDT = '300' AND VR.VBELN = FI.AWREF AND VR.POSNR = MGT.KDPOS AND VR.SPART <> '08'),FI.RUNIT)
                               WHEN CC.KOKRS = 'GAL5' THEN
                                 coalesce((SELECT VR.GEWEI FROM SAPHANADB.VBRP VR WHERE VR.MANDT = '300' AND VR.VBELN = FI.AWREF AND VR.POSNR = MGT.KDPOS AND VR.SPART <> '08'),FI.RUNIT)
                               WHEN CC.KOKRS = 'MTGG' THEN
                                 coalesce((SELECT VR.GEWEI FROM SAPHANADB.VBRP VR WHERE VR.MANDT = '300' AND VR.VBELN = FI.AWREF AND VR.POSNR = TGG.KDPOS AND VR.SPART <> '08'),FI.RUNIT)
                               ELSE FI.RUNIT
                          END
                        ELSE FI.RUNIT
                   END END AS RUNIT,
                   ...
                   CASE WHEN FI.AWTYP = 'VBRK' THEN
                          CASE WHEN CC.KOKRS = 'MAGI' THEN
                                 coalesce((SELECT VR.NTGEW FROM SAPHANADB.VBRP VR WHERE VR.MANDT = '300' AND VR.VBELN = FI.AWREF AND VR.POSNR = MGB.KDPOS AND VR.SPART <> '08') * CASE WHEN FI.DRCRK = 'S' THEN 1 ELSE -1 END ,FI.MSL)
                               WHEN CC.KOKRS = 'MGLD' THEN
                                 coalesce((SELECT VR.NTGEW FROM SAPHANADB.VBRP VR WHERE VR.MANDT = '300' AND VR.VBELN = FI.AWREF AND VR.POSNR = MGH.KDPOS AND VR.SPART <> '08') * CASE WHEN FI.DRCRK = 'S' THEN 1 ELSE -1 END ,FI.MSL)
                               WHEN CC.KOKRS = 'MGAR' THEN
                                 coalesce((SELECT VR.NTGEW FROM SAPHANADB.VBRP VR WHERE VR.MANDT = '300' AND VR.VBELN = FI.AWREF AND VR.POSNR = MGA.KDPOS AND VR.SPART <> '08') * CASE WHEN FI.DRCRK = 'S' THEN 1 ELSE -1 END ,FI.MSL)
                               WHEN CC.KOKRS = 'MGBG' THEN
                                 coalesce((SELECT VR.NTGEW FROM SAPHANADB.VBRP VR WHERE VR.MANDT = '300' AND VR.VBELN = FI.AWREF AND VR.POSNR = MGG.KDPOS AND VR.SPART <> '08') * CASE WHEN FI.DRCRK = 'S' THEN 1 ELSE -1 END ,FI.MSL)
                               WHEN CC.KOKRS = 'MGTF' THEN
                                 coalesce((SELECT VR.NTGEW FROM SAPHANADB.VBRP VR WHERE VR.MANDT = '300' AND VR.VBELN = FI.AWREF AND VR.POSNR = MGT.KDPOS AND VR.SPART <> '08') * CASE WHEN FI.DRCRK = 'S' THEN 1 ELSE -1 END ,FI.MSL)
                               WHEN CC.KOKRS = 'GAL5' THEN
                                 coalesce((SELECT VR.NTGEW FROM SAPHANADB.VBRP VR WHERE VR.MANDT = '300' AND VR.VBELN = FI.AWREF AND VR.POSNR = MGT.KDPOS AND VR.SPART <> '08') * CASE WHEN FI.DRCRK = 'S' THEN 1 ELSE -1 END ,FI.MSL)
                               WHEN CC.KOKRS = 'MTGG' THEN
                                 coalesce((SELECT VR.NTGEW FROM SAPHANADB.VBRP VR WHERE VR.MANDT = '300' AND VR.VBELN = FI.AWREF AND VR.POSNR = TGG.KDPOS AND VR.SPART <> '08') * CASE WHEN FI.DRCRK = 'S' THEN 1 ELSE -1 END ,FI.MSL)
                               ...
                               WHEN TRIM(FI.RUNIT) = '' AND SUBSTR(FI.AWREF,1,2) = 'ZG' AND SUBSTR(FI.AWREF,1,3) = 'ZGF' THEN
                                 (SELECT max( TRIM(J1.MATKL) )
                                   ...
                                   WHERE CO2.MANDT = '300'
                                     ...
                                     AND CO2.GJAHR = SUBSTR(FI.AWORG,4,4)
              ...
              LEFT JOIN
                (SELECT MANDT, AKTBO, PAOBJNR, PASUBNR, KNDNR, ARTNR, AUFNR, BUKRS, KOKRS, PRCTR, PPRCTR, MEINH, KMMAKL, AUART, KDPOS FROM SAPHANADB.CE4MGLD_ACCT T1 WHERE MANDT = '300' AND AKTBO   = 'X' AND PASUBNR = '0001'
                 UNION ALL
              ...
              LEFT JOIN
                (SELECT MANDT, AKTBO, PAOBJNR, PASUBNR, KNDNR, ARTNR, AUFNR, BUKRS, KOKRS, PRCTR, PPRCTR, MEINH, KMMAKL, AUART, KDPOS FROM SAPHANADB.CE4MGAR_ACCT T1 WHERE MANDT = '300' AND AKTBO   = 'X' AND PASUBNR = '0001'
                 UNION ALL
              ...
              LEFT JOIN
                (SELECT MANDT, AKTBO, PAOBJNR, PASUBNR, KNDNR, ARTNR, AUFNR, BUKRS, KOKRS, PRCTR, PPRCTR, MEINH, KMMAKL, AUART, KDPOS FROM SAPHANADB.CE4MGBG_ACCT T1 WHERE MANDT = '300' AND AKTBO   = 'X' AND PASUBNR = '0001'
                 UNION ALL
              ...
              LEFT JOIN
              (SELECT MANDT, AKTBO, PAOBJNR, PASUBNR, KNDNR, ARTNR, AUFNR, BUKRS, KOKRS, PRCTR, PPRCTR, MEINH, KMMAKL, AUART, KDPOS FROM SAPHANADB.CE4MGTF_ACCT T1 WHERE MANDT = '300' AND AKTBO   = 'X' AND PASUBNR = '0001'
                 UNION ALL
              ...
              LEFT JOIN
              (SELECT MANDT, AKTBO, PAOBJNR, PASUBNR, KNDNR, ARTNR, AUFNR, BUKRS, KOKRS, PRCTR, PPRCTR, MEINH, KMMAKL, AUART, KDPOS FROM SAPHANADB.CE4GAL5_ACCT T1 WHERE MANDT = '300' AND AKTBO   = 'X' AND PASUBNR = '0001'
                 UNION ALL
                SELECT MANDT, AKTBO, PAOBJNR, PASUBNR, KNDNR, ARTNR, AUFNR, BUKRS, KOKRS, PRCTR, PPRCTR, MEINH, KMMAKL, AUART, KDPOS FROM SAPHANADB.CE4GAL5 T2 WHERE MANDT = '300' AND AKTBO   = 'X' AND PASUBNR = '0001' ) AL5

              ON CC.PAOBJNR  = AL5.PAOBJNR
               AND CC.KOKRS    = AL5.KOKRS


              LEFT JOIN
              (SELECT MANDT, AKTBO, PAOBJNR, PASUBNR, KNDNR, ARTNR, AUFNR, BUKRS, KOKRS, PRCTR, PPRCTR, MEINH, KMMAKL, AUART, KDPOS FROM SAPHANADB.CE4MTGG_ACCT T1 WHERE MANDT = '300' AND AKTBO   = 'X' AND PASUBNR = '0001'
                 UNION ALL
                SELECT MANDT, AKTBO, PAOBJNR, PASUBNR, KNDNR, ARTNR, AUFNR, BUKRS, KOKRS, PRCTR, PPRCTR, MEINH, KMMAKL, AUART, KDPOS FROM SAPHANADB.CE4MTGG T2 WHERE MANDT = '300' AND AKTBO   = 'X' AND PASUBNR = '0001' ) TGG

              ON CC.PAOBJNR  = TGG.PAOBJNR
                           AND CC.KOKRS    = TGG.KOKRS
             ...
             WHERE FI.RCLNT   = '300'
               ...
               AND FI.RBUKRS  IN ('0004','0022','0002','0027','0001','0030','0034','0025','0028','0029','0024','0036','0018','0014','0015','0010','0026','0017','0023','0032','0037','0035','0038','0100',
                                  '0005','0020','0040','0042','0200','0201','0202','0044', '0045' , '0046', '0047', '0203', '0050','0051','0052','0053','0054','0055','0056','0048','0021','0013','0072' )
               AND FI.RLDNR   = '0L'
               AND FI.RYEAR   >= '2015'
         ...
         AND SUBSTR(CO.PAOBJNR,1,10) = '0000000000'
            ...
            SELECT DISTINCT
             ...
             WHERE FI.RCLNT    = '300'
               AND FI.TIMESTAMP BETWEEN :LC_DATA_INIC AND  :LC_DATA_FIMC
               AND FI.RBUKRS IN ('0004','0022','0002','0027','0001','0030','0034','0025','0028','0029','0024','0036','0018','0014','0015','0010','0026','0017','0023','0032','0005','0020','0040','0044', '0045' ,
                                 '0046', '0047', '0203', '0050','0051','0052','0053','0054','0055','0056','0048','0021','0013','0072' )
               AND FI.RLDNR    = '0L'
               AND FI.RYEAR    >= '2015'
               AND FI.AWTYP IN ('VBRK','MKPF','IDOC','MLHD','MKPF','RMRP','BKPF','IDOC','CAJO','RMRP','TRAVL','AMBU','PRCHG','AUAK','AMDP','AIBU','AFRU','COBK','HRPAY')
            ...
            SELECT DISTINCT
             ...
             WHERE FI.RCLNT    = '300'
               AND FI.TIMESTAMP BETWEEN :LC_DATA_INIC AND  :LC_DATA_FIMC
               AND FI.RBUKRS IN ('0037','0035','0038','0042')
               AND FI.RLDNR    = '0L'
               AND FI.RYEAR    >= '2015'
               AND FI.AWTYP IN ('VBRK','MKPF','IDOC','MLHD','MKPF','RMRP','BKPF','IDOC','CAJO','RMRP','TRAVL','AMBU','PRCHG','AUAK','AMDP','AIBU','AFRU','COBK','HRPAY')
               AND FI.RCLNT    = CO.MANDT
            ...
            SELECT FI.BUDAT,
                   ...
                   FI.KSL,
                   FI.OSL,
                   CASE WHEN TRIM(FI.RUNIT) = '' AND SUBSTR(CO.AWREF,1,2) = 'ZG' AND SUBSTR(CO.AWREF,1,3) <> 'ZGF' THEN
                          (SELECT max( J1.MEINS )
                             FROM SAPHANADB.J_1BNFLIN J1
                            WHERE J1.REFTYP = 'ZW'
                              AND SUBSTR(J1.REFKEY,1,10) = SUBSTR(CO.AWREF||CO.AWORG,4,10)
                               )
                        WHEN TRIM(FI.RUNIT) = '' AND SUBSTR(CO.AWREF,1,2) = 'ZG' AND SUBSTR(CO.AWREF,1,3) = 'ZGF' THEN
                          (SELECT MAX ( J1.MEINS )
                             FROM SAPHANADB.J_1BNFLIN    J1,
                                  SAPHANADB.FAGL_SPLINFO CO2
                            WHERE CO2.MANDT = CO.MANDT
                              AND CO2.BELNR = SUBSTR(CO.AWREF||CO.AWORG,4,10)
                              AND CO2.GJAHR = SUBSTR(CO.AWORG,4,4)
                              AND CO2.BUKRS = CO.BUKRS
                              AND J1.REFTYP = 'ZW'
                              AND SUBSTR(J1.REFKEY,1,10) = SUBSTR(CO2.AWREF||CO2.AWORG,4,10)
                               )
                        WHEN FI.AWTYP = 'VBRK' THEN
                          coalesce((SELECT VR.GEWEI FROM SAPHANADB.VBRP VR WHERE VR.VBELN = CO.AWREF AND VR.POSNR = MG.KDPOS AND VR.SPART <> '08'),FI.RUNIT)
                        ELSE FI.RUNIT
                   END AS RUNIT,

                   CASE WHEN FI.MSL = 0                   AND SUBSTR(CO.AWREF,1,2) = 'ZG' AND SUBSTR(CO.AWREF,1,3) <> 'ZGF' THEN
                          (SELECT SUM(J1.MENGE)
                             FROM SAPHANADB.J_1BNFLIN J1
                            WHERE J1.REFTYP = 'ZW'
                              AND SUBSTR(J1.REFKEY,1,10) = SUBSTR(CO.AWREF||CO.AWORG,4,10) )
                   ...
                   CO.AUFNR,
                   CASE WHEN TRIM(MG.KMMAKL) = '' AND SUBSTR(CO.AWREF,1,2) = 'ZG' AND SUBSTR(CO.AWREF,1,3) <> 'ZGF' THEN
                          (SELECT max( TRIM(J1.MATKL) )
                             FROM SAPHANADB.J_1BNFLIN J1
                            WHERE J1.REFTYP = 'ZW'
                              AND SUBSTR(J1.REFKEY,1,10) = SUBSTR(CO.AWREF||CO.AWORG,4,10)
              ...
              FROM SAPHANADB.ACDOCA    FI LEFT JOIN SAPHANADB.FAGL_SPLINFO CO
                   ...
                   ( SELECT MANDT, AKTBO, PAOBJNR, PASUBNR, KNDNR, ARTNR, AUFNR, BUKRS, KOKRS, PRCTR, PPRCTR, MEINH, KMMAKL, AUART, KDPOS
                       FROM SAPHANADB.CE4MAGI_ACCT T1
                      UNION ALL
                     SELECT MANDT, AKTBO, PAOBJNR, PASUBNR, KNDNR, ARTNR, AUFNR, BUKRS, KOKRS, PRCTR, PPRCTR, MEINH, KMMAKL, AUART, KDPOS
                       FROM SAPHANADB.CE4MAGI T2 ) MG
             WHERE FI.RCLNT    = '300'
               AND FI.TIMESTAMP BETWEEN :LC_DATA_INIC AND  :LC_DATA_FIMC
               AND FI.RBUKRS   IN ('0004','0022','0002','0027','0001','0030','0034','0025','0028','0029','0024','0036','0018','0014','0015','0010','0026','0017','0023','0032','0005','0020','0040','0044', '0045', '0046',
                                   '0047', '0203', '0050','0051','0052','0053','0054','0055','0056','0048','0021','0013','0072' )
               AND FI.RLDNR    = '0L'
               AND FI.RYEAR    >= '2015'
               AND FI.AWTYP    IN ('VBRK','MKPF','IDOC','MLHD','MKPF','RMRP','BKPF','IDOC','CAJO','RMRP','TRAVL','AMBU','PRCHG','AUAK','AMDP','AIBU','AFRU','COBK','HRPAY')
            ...
            SELECT FI.BUDAT,
              ...
              FROM SAPHANADB.ACDOCA    FI LEFT JOIN SAPHANADB.FAGL_SPLINFO CO
                   ...
                   ( SELECT MANDT, AKTBO, PAOBJNR, PASUBNR, KNDNR, ARTNR, AUFNR, BUKRS, KOKRS, PRCTR, PPRCTR, MEINH, KMMAKL, AUART, KDPOS
                       FROM SAPHANADB.CE4MGLD_ACCT T1
                      UNION ALL
                     SELECT MANDT, AKTBO, PAOBJNR, PASUBNR, KNDNR, ARTNR, AUFNR, BUKRS, KOKRS, PRCTR, PPRCTR, MEINH, KMMAKL, AUART, KDPOS
                       FROM SAPHANADB.CE4MGLD T2 ) MG
             WHERE FI.RCLNT    = '300'
               AND FI.TIMESTAMP BETWEEN :LC_DATA_INIC AND  :LC_DATA_FIMC
               AND FI.RBUKRS   IN ('0037','0035','0038','0042')
               AND FI.RLDNR    = '0L'
         ...
         AND SUBSTR(CO.PAOBJNR,1,10) <> '0000000000'
            ...
            SELECT FI.BUDAT,
              ...
              FROM SAPHANADB.ACDOCA    FI LEFT JOIN SAPHANADB.FAGL_SPLINFO CO
                   ...
                   ( SELECT MANDT, AKTBO, PAOBJNR, PASUBNR, KNDNR, ARTNR, AUFNR, BUKRS, KOKRS, PRCTR, PPRCTR, MEINH, KMMAKL, AUART, KDPOS
                       FROM SAPHANADB.CE4MGAR_ACCT T1
                      UNION ALL
                     SELECT MANDT, AKTBO, PAOBJNR, PASUBNR, KNDNR, ARTNR, AUFNR, BUKRS, KOKRS, PRCTR, PPRCTR, MEINH, KMMAKL, AUART, KDPOS
                       FROM SAPHANADB.CE4MGAR T2 ) MG
               ...
               AND MG.AKTBO    = 'X'
                   ...
                   (SELECT CO.KOKRS, CO.KOSTL, CO.KOSAR
                     ...
                     WHERE current_date BETWEEN CO.DATAB AND CO.DATBI
                       AND CO.MANDT = '300' ) CO ON

             TRIM(TT.RCNTR) = CO.KOSTL
