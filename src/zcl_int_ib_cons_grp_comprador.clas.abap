CLASS zcl_int_ib_cons_grp_comprador DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    DATA:
      BEGIN OF zde_data_request ,
        ekgrp TYPE TABLE OF t024-ekgrp,
        eknam TYPE TABLE OF t024-eknam,
      END OF zde_data_request .
    DATA:
      BEGIN OF zde_data_response,
        grupos TYPE TABLE OF t024,
      END OF zde_data_response .
    CONSTANTS at_id_interface TYPE zde_id_interface VALUE '143' ##NO_TEXT.
...
CLASS ZCL_INT_IB_CONS_GRP_COMPRADOR IMPLEMENTATION.
...
*-----------------------------------------------------------------------------------------------------------------------*
    IF lwa_data_request-ekgrp IS INITIAL AND lwa_data_request-eknam IS INITIAL .
  ...
  METHOD zif_integracao_inject~set_integrar_inbound.
    ...
    IF lwa_data_request-ekgrp IS NOT INITIAL.

      LOOP AT lwa_data_request-ekgrp ASSIGNING FIELD-SYMBOL(<fs_ekgrp>).
        APPEND VALUE #( sign = 'I' option = 'EQ' low = <fs_ekgrp> ) TO lra_ekgrp.
      ENDLOOP.

    ENDIF.

    IF lwa_data_request-eknam IS NOT INITIAL.

      LOOP AT lwa_data_request-eknam ASSIGNING FIELD-SYMBOL(<fs_eknam>).
        APPEND VALUE #( sign = 'I' option = 'EQ' low = <fs_eknam> ) TO lra_eknam.
      ENDLOOP.

    ENDIF.


    IF lra_ekgrp  IS NOT INITIAL OR
       lra_eknam  IS NOT INITIAL.

      SELECT *
        FROM t024
        INTO TABLE lwa_data_response-grupos
        WHERE ekgrp IN lra_ekgrp
          AND eknam IN lra_eknam.
