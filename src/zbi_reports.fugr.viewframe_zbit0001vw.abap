*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZBIT0001VW
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZBIT0001VW          .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZBIT0001VW'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZBIT0001VW_EXTRACT
TOTAL = ZBIT0001VW_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZBIT0001VW'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZBIT0001VW-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZBIT0001VW_EXTRACT
TOTAL = ZBIT0001VW_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZBIT0001VW-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZBIT0001VW'
...
TOTAL = ZBIT0001VW_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZBIT0001VW-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZBIT0001VW-UPD_FLAG NE SPACE.
