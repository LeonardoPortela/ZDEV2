...
*                                                                      *
* Programa   : ZLESR0008                                               *
* Descrição  : Conhecimentos Emitidos por Contrato                     *
* Módulo     : LES                               Transação: ZLES0035   *
*                                                                      *
*----------------------------------------------------------------------*
* Autor      : Pathelle R C Morais                    Data: 26/08/2010 *
...
REPORT zlesr0008 NO STANDARD PAGE HEADING MESSAGE-ID sd.
...
TYPES: BEGIN OF type_alv,
         ...
         bezei_tv  TYPE ttdst-bezei,
         datbg     TYPE vttk-datbg,
         fknum     TYPE vfkp-fknum,
         netwr     TYPE vfsi-netwr,
         mwsbp     TYPE vfsi-mwsbp,
         kwert_lot TYPE konv-kwert,
         kwert_fre TYPE konv-kwert,
         kwert_seg TYPE konv-kwert,
         kwert_ped TYPE konv-kwert,
         kwert_iof TYPE konv-kwert,
         kawrt     TYPE konv-kawrt,
         kbetr     TYPE konv-kbetr,
         kwert_ipt TYPE konv-kwert,
         kwert_icm TYPE konv-kwert,
         kwert_iss TYPE konv-kwert,
         kwert_pis TYPE konv-kwert,
         kwert_cof TYPE konv-kwert,
         kwert_tra TYPE konv-kwert,
...
*   Leitura KONV
      ...
      sl_alv-bezei_tv        = sl_tvrot-bezei.
...
* Preenche FieldCat
  PERFORM z_preenche_fieldcat USING:
    ...
    'BEZEI_TV'        text-044 18,
    'DATBG'           text-033 10,
    'FKNUM'           text-034 10,
    'NETWR'           text-035 21,
    'MWSBP'           text-036 18,
    'KWERT_LOT'       text-045 14,
    'KWERT_FRE'       text-046 14,
    'KWERT_SEG'       text-047 14,
    'KWERT_PED'       text-048 14,
    'KWERT_IOF'       text-049 14,
    'KAWRT'           text-050 14,
    'KBETR'           text-051 14,
    'KWERT_IPT'       text-052 14,
    'KWERT_ICM'       text-053 14,
    'KWERT_ISS'       text-054 14,
    'KWERT_PIS'       text-055 14,
    'KWERT_COF'       text-056 14,
    'KWERT_TRA'       text-057 14,
    'EBELN'           text-037 10,
    'LBLNI'           text-038 10.
...
* Leitura Condições
  PERFORM z_ler_cond USING p_knumv
                           p_kposn
                           'ZLOT'
                  CHANGING p_alv-kwert_lot.

  PERFORM z_ler_cond USING p_knumv
                           p_kposn
                          'ZFRE'
                  CHANGING p_alv-kwert_fre.
  ...
  PERFORM z_ler_cond USING p_knumv
                           p_kposn
                          'ZIPT'
                  CHANGING p_alv-kwert_ipt.

  PERFORM z_ler_cond USING p_knumv
                           p_kposn
                          'ZISS'
                  CHANGING p_alv-kwert_iss.

  PERFORM z_ler_cond USING p_knumv
                           p_kposn
                          'ZPIS'
                  CHANGING p_alv-kwert_pis.

  PERFORM z_ler_cond USING p_knumv
                           p_kposn
                          'ZCOF'
                  CHANGING p_alv-kwert_cof.

  PERFORM z_ler_cond USING p_knumv
                           p_kposn
                          'ZTRA'
                  CHANGING p_alv-kwert_tra.
  ...
  IF sy-subrc IS INITIAL.
    p_alv-kwert_icm = sl_konv-kwert.
    p_alv-kawrt     = sl_konv-kawrt.
    p_alv-kbetr     = sl_konv-kbetr.
  ENDIF.
