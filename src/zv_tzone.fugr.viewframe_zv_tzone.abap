*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZV_TZONE
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZV_TZONE            .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZV_TZONE'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZV_TZONE_EXTRACT
TOTAL = ZV_TZONE_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZV_TZONE'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZV_TZONE-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZV_TZONE_EXTRACT
TOTAL = ZV_TZONE_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZV_TZONE-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZV_TZONE'
...
TOTAL = ZV_TZONE_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZV_TZONE-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZV_TZONE-UPD_FLAG NE SPACE.
