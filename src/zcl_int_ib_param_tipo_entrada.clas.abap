class ZCL_INT_IB_PARAM_TIPO_ENTRADA definition
  public
  final
  create public .
...
public section.
  ...
  constants AT_ID_INTERFACE type ZDE_ID_INTERFACE value '100' ##NO_TEXT.
...
CLASS ZCL_INT_IB_PARAM_TIPO_ENTRADA IMPLEMENTATION.
  ...
  method ZIF_INTEGRACAO_INBOUND~VALIDAR_DADOS_INBOUND.
    ...
    DATA: lit_data_inbound  TYPE zsdt0001te_t.
...
*-----------------------------------------------------------------------------------------------------------------------*
      ...
      CASE me->zif_integracao_inject~at_info_request_http-ds_metodo.
        WHEN C_POST OR C_DELETE.
          ...
          IF lwa_data-id_entrada IS INITIAL.
            r_msg_erro = 'Id. Entrada não foi informado!' && ' Index Objeto: ' && _tabix.
            RETURN.
          ENDIF.

          IF lwa_data-id_empresa IS INITIAL.
            r_msg_erro = 'Empresa não foi informada!' && ' Index Objeto: ' && _tabix.
            RETURN.
          ENDIF.
      ...
      CASE me->zif_integracao_inject~at_info_request_http-ds_metodo.
        WHEN C_POST.
          ...
          IF lwa_data-id_tp_operacao IS INITIAL.
            r_msg_erro = 'Id. Tp. Operação não foi informada!' && ' Index Objeto: ' && _tabix.
            RETURN.
          ENDIF.

          IF lwa_data-ds_entrada IS INITIAL.
            r_msg_erro = 'Desc. Tp. Entrada não foi informada!' && ' Index Objeto: ' && _tabix.
            RETURN.
          ENDIF.

          IF lwa_data-tp_nota IS INITIAL.
            r_msg_erro = 'Tp. Nota não foi informada!' && ' Index Objeto: ' && _tabix.
            RETURN.
          ENDIF.

          IF lwa_data-id_iva IS INITIAL.
            r_msg_erro = 'Id.Iva não foi informada!' && ' Index Objeto: ' && _tabix.
            RETURN.
          ENDIF.

          IF lwa_data-bn_empresa IS INITIAL.
            r_msg_erro = 'Banco Empresa não foi informada!' && ' Index Objeto: ' && _tabix.
            RETURN.
          ENDIF.

          IF lwa_data-tp_mov_mercadoria IS INITIAL.
            r_msg_erro = 'Tp.Mov. Merc. não foi informada!' && ' Index Objeto: ' && _tabix.
            RETURN.
          ENDIF.
    ...
    LOOP AT lit_data_inbound INTO DATA(lwa_data_inbound).
      ...
      CASE me->zif_integracao_inject~at_info_request_http-ds_metodo.
        WHEN 'POST'. "Inclusão/Modificação
          ...
          SELECT SINGLE *
            FROM zsdt0001TE INTO @DATA(lwa_zsdt0001TE)
           WHERE id_entrada  EQ @lwa_data_inbound-id_entrada
             AND id_empresa  EQ @lwa_data_inbound-id_empresa
             AND ck_nfe      EQ @lwa_data_inbound-ck_nfe.

          IF SY-SUBRC EQ 0.
            lwa_data_inbound-dt_registro = lwa_zsdt0001te-dt_registro.
            lwa_data_inbound-hr_registro = lwa_zsdt0001te-hr_registro.
          ...
          MODIFY ZSDT0001TE FROM lwa_data_inbound.
          ...
          "Textos
          SELECT SINGLE *
            FROM zsdt0001TETX INTO @DATA(lwa_zsdt0001TETX)
           WHERE id_entrada  EQ @lwa_data_inbound-id_entrada.

          lwa_zsdt0001TETX-id_entrada = lwa_data_inbound-id_entrada.
          lwa_zsdt0001TETX-ds_entrada = lwa_data_inbound-ds_entrada.

          MODIFY ZSDT0001TETX FROM lwa_zsdt0001TETX.
        ...
        WHEN 'DELETE'.
          ...
          SELECT SINGLE *
            FROM ZSDT0001TE INTO lwa_zsdt0001te
           WHERE id_entrada  EQ lwa_data_inbound-id_entrada
             AND id_empresa  EQ lwa_data_inbound-id_empresa
             AND ck_nfe      EQ lwa_data_inbound-ck_nfe.
          ...
          SELECT SINGLE *
            FROM ZSDT0001TETX INTO lwa_zsdt0001tetx
           WHERE id_entrada  EQ lwa_data_inbound-id_entrada.
