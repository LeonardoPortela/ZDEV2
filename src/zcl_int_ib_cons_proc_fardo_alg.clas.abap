class ZCL_INT_IB_CONS_PROC_FARDO_ALG definition
  public
  final
  create public .
...
types:
  ...
  DATA:
      ...
      tb_charg  TYPE TABLE OF CHARG_D,
      tb_ACHARG TYPE TABLE OF CHARG_D.
  ...
  data:
    BEGIN OF zde_data_request,
      WERKS  like tb_WERKS,
      charg  like tb_charg,
      ACHARG like tb_ACHARG,
      END OF zde_data_request .
  data:
    BEGIN OF zde_data_response,
        ZPPT0002 TYPE TABLE OF ZPPT0002,
        ZPPT0006 TYPE TABLE OF ZPPT0006,
      END OF zde_data_response .
  constants AT_ID_INTERFACE type ZDE_ID_INTERFACE value '137' ##NO_TEXT.
...
CLASS ZCL_INT_IB_CONS_PROC_FARDO_ALG IMPLEMENTATION.
...
*-----------------------------------------------------------------------------------------------------------------------*
    ...
    IF lwa_data_request IS NOT INITIAL.
      ...
      IF lwa_data_request-werks is INITIAL or lwa_data_request-charg is INITIAL.
  ...
  METHOD zif_integracao_inject~set_integrar_inbound.
    ...
    DATA:
      lra_filtro1 TYPE RANGE OF werks_d,
      lra_filtro2 TYPE RANGE OF CHARG_D,
      lra_filtro3 TYPE RANGE OF CHARG_D.
    ...
    LOOP AT lwa_data_request-werks INTO DATA(wa_filtro1).
      APPEND VALUE #( sign = 'I' option = 'EQ' low = wa_filtro1 ) TO lra_filtro1.
    ENDLOOP.

    LOOP AT lwa_data_request-charg INTO DATA(wa_filtro2).
      APPEND VALUE #( sign = 'I' option = 'EQ' low = wa_filtro2 ) TO lra_filtro2.
    ENDLOOP.

    LOOP AT lwa_data_request-acharg INTO DATA(wa_filtro3).
      APPEND VALUE #( sign = 'I' option = 'EQ' low = wa_filtro3 ) TO lra_filtro3.
    ENDLOOP.
    ...
    IF lwa_data_request IS NOT INITIAL.
      ...
      SELECT *
      FROM ZPPT0002
      INTO TABLE lwa_data_response-zppt0002
      WHERE ACHARG   IN lra_filtro3 AND
            WERKS   IN lra_filtro1.

      SELECT *
    FROM ZPPT0006
    INTO TABLE lwa_data_response-zppt0006
    WHERE werks   IN lra_filtro1 AND
          charg   IN lra_filtro2 and
          acharg  in lra_filtro3.
