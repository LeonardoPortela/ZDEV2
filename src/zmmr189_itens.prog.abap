...
DATA: it_dados_p22 TYPE STANDARD TABLE OF zfit0200 WITH HEADER LINE.
...
SELECTION-SCREEN BEGIN OF BLOCK a WITH FRAME TITLE t1.
  ...
  SELECT-OPTIONS p_cdind FOR zfit0200-cd_indic NO INTERVALS.
...
*... set the columns technical
  ...
  p22_lr_columns->set_column_position(  columnname = 'CD_ITEM' position = 2 ).
  p22_lr_columns->set_column_position(  columnname = 'NM_ITEM' position = 3 ).
...
FORM seleciona_dados_p22.
  SELECT * FROM zfit0200 INTO TABLE @it_dados_p22 WHERE cd_indic IN @p_cdind.
ENDFORM.
...
*&---------------------------------------------------------------------*
*&      Module  zmmr189_itens_pbo  OUTPUT
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
*MODULE zmmr189_itens_pbo OUTPUT.
*  PERFORM zmmr189_itens_pbo.
*ENDMODULE.                 " zmmr189_itens_pbo  OUTPUT

*&---------------------------------------------------------------------*
*&      Module  zmmr189_itens_pai  INPUT
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
*MODULE zmmr189_itens_pai INPUT.
*  PERFORM zmmr189_itens_pai.
*ENDMODULE.                 " zmmr189_itens_pai  INPUT

*&---------------------------------------------------------------------*
*&      Form  zmmr189_itens_pbo
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
*FORM zmmr189_itens_pbo .
...
*  PERFORM container_main_p22.
*
*ENDFORM.                                                    " zmmr189_itens_pbo

*&---------------------------------------------------------------------*
*&      Form  zmmr189_itens_pai
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
*FORM zmmr189_itens_pai .
...
FORM check_values.
  ...
  LOOP AT it_dados_p22 ASSIGNING FIELD-SYMBOL(<it_dados_p22>).
    ...
    DATA : l_dupl TYPE i.
    CLEAR: l_dupl.
    LOOP AT it_dados_p22 ASSIGNING FIELD-SYMBOL(<duplicados>) WHERE cd_indic = p_cdind-low AND  cd_item = <it_dados_p22>-cd_item.
      l_dupl = l_dupl + 1.
    ENDLOOP.

    IF l_dupl > 1.
      DELETE it_dados_p22 WHERE cd_indic = p_cdind-low AND cd_item = <it_dados_p22>-cd_item.
    ELSE.
      IF <it_dados_p22>-cd_indic IS INITIAL AND <it_dados_p22>-cd_indic = p_cdind-low.
      ...
      ENDIF.
      MODIFY zfit0200 FROM <it_dados_p22>.
    ENDIF.
  ENDLOOP.
ENDFORM.
*&---------------------------------------------------------------------*
