FUNCTION z_01_dre_consulta_regs_bukrs.
*"----------------------------------------------------------------------
*"*"Interface local:
*"  IMPORTING
        ...
        OPEN DOCUMENTOS FOR
          ...
          SELECT DISTINCT
                 ...
                 CASE WHEN FI.AWTYP = 'VBRK' THEN
                        CASE WHEN CC.KOKRS = 'MAGI' THEN
                             ...
                             WHEN CC.KOKRS = 'MTGG' THEN
                               coalesce((SELECT VR.GEWEI FROM SAPHANADB.VBRP VR WHERE VR.VBELN = FI.AWREF AND VR.POSNR = TGG.KDPOS AND VR.SPART <> '08'),FI.RUNIT)
                 ...
                 CASE WHEN FI.AWTYP = 'VBRK' THEN
                        CASE WHEN CC.KOKRS = 'MAGI' THEN
                             ...
                             WHEN CC.KOKRS = 'GAL5' THEN
                               coalesce((SELECT VR.NTGEW FROM SAPHANADB.VBRP VR WHERE VR.VBELN = FI.AWREF AND VR.POSNR = MGT.KDPOS AND VR.SPART <> '08') * CASE WHEN FI.DRCRK = 'S' THEN 1 ELSE -1 END ,FI.MSL)
                             WHEN CC.KOKRS = 'MTGG' THEN
                               coalesce((SELECT VR.NTGEW FROM SAPHANADB.VBRP VR WHERE VR.VBELN = FI.AWREF AND VR.POSNR = TGG.KDPOS AND VR.SPART <> '08') * CASE WHEN FI.DRCRK = 'S' THEN 1 ELSE -1 END ,FI.MSL)
            ...
            FROM (SELECT DISTINCT
                ...
                left join  (SELECT MANDT, AKTBO, PAOBJNR, PASUBNR, KNDNR, ARTNR, AUFNR, BUKRS, KOKRS, PRCTR, PPRCTR, MEINH, KMMAKL, AUART, KDPOS FROM SAPHANADB.CE4GAL5_ACCT T1 WHERE AKTBO   = 'X' AND PASUBNR = '0001'
                     UNION ALL
                    SELECT MANDT, AKTBO, PAOBJNR, PASUBNR, KNDNR, ARTNR, AUFNR, BUKRS, KOKRS, PRCTR, PPRCTR, MEINH, KMMAKL, AUART, KDPOS FROM SAPHANADB.CE4GAL5 T2 WHERE AKTBO   = 'X' AND PASUBNR = '0001' ) AL5

                    on CC.PAOBJNR  = AL5.PAOBJNR
                   AND CC.KOKRS    = AL5.KOKRS

                left join  (SELECT MANDT, AKTBO, PAOBJNR, PASUBNR, KNDNR, ARTNR, AUFNR, BUKRS, KOKRS, PRCTR, PPRCTR, MEINH, KMMAKL, AUART, KDPOS FROM SAPHANADB.CE4MTGG_ACCT T1 WHERE AKTBO   = 'X' AND PASUBNR = '0001'
                     UNION ALL
                    SELECT MANDT, AKTBO, PAOBJNR, PASUBNR, KNDNR, ARTNR, AUFNR, BUKRS, KOKRS, PRCTR, PPRCTR, MEINH, KMMAKL, AUART, KDPOS FROM SAPHANADB.CE4MTGG T2 WHERE AKTBO   = 'X' AND PASUBNR = '0001' ) TGG
                 ...
                 CASE WHEN TRIM(FI.RUNIT) = '' AND SUBSTR(CO.AWREF,1,2) = 'ZG' AND SUBSTR(CO.AWREF,1,3) <> 'ZGF' THEN
                      ...
                      WHEN TRIM(FI.RUNIT) = '' AND SUBSTR(CO.AWREF,1,2) = 'ZG' AND SUBSTR(CO.AWREF,1,3) = 'ZGF' THEN
                        (SELECT max( J1.MEINS )
                           FROM SAPHANADB.J_1BNFLIN    J1,
                                SAPHANADB.FAGL_SPLINFO CO2
                          WHERE CO2.GJAHR = SUBSTR(CO.AWORG,4,4)
