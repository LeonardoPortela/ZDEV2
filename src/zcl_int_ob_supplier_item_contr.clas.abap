class ZCL_INT_OB_SUPPLIER_ITEM_CONTR definition
  public
  final
  create public .
...
public section.
  ...
  constants AT_ID_INTERFACE type ZDE_ID_INTERFACE value '205' ##NO_TEXT.
  data AT_DADOS_ORDEM_ORC type ZPM_ORDEM_ORC .
  data AT_REQ type STRING .
  data AT_MATERIAL type STRING .
  data AT_CONTRATO type STRING .
  data AT_OFFSET type STRING .
...
CLASS ZCL_INT_OB_SUPPLIER_ITEM_CONTR IMPLEMENTATION.
...
*    ENDLOOP.
  ...
  METHOD zif_integracao_outbound~build_info_request.
    ...
    SPLIT i_info_request AT ';' INTO me->at_contrato me->at_material me->at_offset.
...
*         r_json = e_data.
  ...
  method ZIF_INTEGRACAO_OUTBOUND~GET_INSTANCE.
    ...
    IF zif_integracao_outbound~at_if_integracao_outbound IS NOT BOUND.
      CREATE OBJECT zif_integracao_outbound~at_if_integracao_outbound TYPE zcl_int_ob_supplier_item_contr.
    ENDIF.
...
*    APPEND VALUE #( name = 'accept'   value = 'application/json' ) TO LC_INTEGRAR~at_.
  ...
  method zif_integracao_outbound~set_url.
    ...
    select single *
     ...
     where service = 'CONS_SUPPL_ITEM_COUPA_CTR'.
      ...
      raise exception type zcx_integracao
        exporting
          textid = value #( msgid = zcx_integracao=>zcx_erro_geral-msgid
                            ...
                            attr2 = 'CONS_SUPPL_ITEM_COUPA' )
          msgid  = zcx_integracao=>zcx_erro_geral-msgid
          msgno  = zcx_integracao=>zcx_erro_geral-msgno
          msgty  = 'E'
          msgv1  = 'Serviço não configurado: '
          msgv2  = 'CONS_SUPPL_ITEM_COUPA'.
    endif.
    if at_contrato = 'DELETE'.
      lva_url = 'https://amaggi.coupahost.com/api/supplier_items/'. "PRD
      if sy-sysid = 'QAS'.
        lva_url = 'https://amaggi-test.coupahost.com/api/supplier_items/'. "QAS
      endif.
      call function 'CONVERSION_EXIT_ALPHA_OUTPUT'
        exporting
          input  = me->at_material
        importing
          output = me->at_material.

      lva_url = lva_url && me->at_material.

    else.
      lva_url = 'https://amaggi.coupahost.com/api/'. "PRD
      if sy-sysid = 'QAS'.
        lva_url = 'https://amaggi-test.coupahost.com/api/'. "QAS
      endif.

      lva_url = lva_url && lwa_webservice-url.

      call function 'CONVERSION_EXIT_ALPHA_OUTPUT'
        exporting
          input  = me->at_contrato
        importing
          output = me->at_contrato.
      ...
      replace first occurrence of 'CONTRATO' in lva_url with me->at_contrato.
      replace first occurrence of 'MATERIAL' in lva_url with me->at_material.
      lva_url = lva_url && '&offset=' && me->at_offset.
