class ZCL_INT_OB_CRIA_ORDEM_SUPERVIS definition
  public
  final
  create public .
...
public section.
  ...
  constants AT_ID_INTERFACE type ZDE_ID_INTERFACE value '208' ##NO_TEXT.
  data AT_DADOS_ORDEM type ZPPE_SUPERVISORIO .
...
CLASS ZCL_INT_OB_CRIA_ORDEM_SUPERVIS IMPLEMENTATION.
...
*    ENDLOOP.
  ...
  METHOD zif_integracao_inject~set_processa_retorno.
    ...
    UPDATE zppt0038 SET status = 'SINCRONIZADO' WHERE ordem = me->at_dados_ordem-ordem.
  ...
  METHOD zif_integracao_outbound~build_info_request.
    ...
    DATA: ls_ordem TYPE zppe_supervisorio.

    r_if_integracao_outbound = me.

    MOVE-CORRESPONDING i_info_request TO  ls_ordem.

    me->at_dados_ordem = ls_ordem.
  ...
  METHOD ZIF_INTEGRACAO_OUTBOUND~GET_INSTANCE.
    ...
    IF zif_integracao_outbound~at_if_integracao_outbound IS NOT BOUND.
      CREATE OBJECT zif_integracao_outbound~at_if_integracao_outbound TYPE zcl_int_ob_cria_ordem_supervis.
    ENDIF.
  ...
  METHOD ZIF_INTEGRACAO_OUTBOUND~SET_URL.
    ...
    SELECT SINGLE *
     ...
     WHERE service = 'ENVIA_ORDENS_SUPERVISORIO'.
