...
*--------------------------------------------------------------------------------------------------------------*
    ...
    SELECT SINGLE *
      FROM zib_nfe_dist_ter INTO gs_zib_nfe_dist_ter
...
*--------------------------------------------------------------------------------------------------------------*
    CASE abap_true.
      WHEN gwa_cte_read-cte_57.
        ...
        SELECT SINGLE *
          FROM zib_cte_dist_ter INTO gs_zib_cte_dist_ter
        ...
        IF ( sy-subrc IS NOT INITIAL ) OR ( i_force_upd_dist EQ abap_true ).
          ...
          IF ( i_force_upd_dist EQ abap_true ) AND ( gs_xml_sefaz_cte-cteproc-protcte-infprot-chcte IS NOT INITIAL ).
            DELETE FROM zib_cte_dist_n55 WHERE cd_chave_cte = gs_xml_sefaz_cte-cteproc-protcte-infprot-chcte.
            DELETE FROM zib_cte_dist_n01 WHERE cd_chave_cte = gs_xml_sefaz_cte-cteproc-protcte-infprot-chcte.
          ENDIF.
