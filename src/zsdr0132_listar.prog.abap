*&---------------------------------------------------------------------*
*& Report  ZSDR0132_LISTAR                                             *
*----------------------------------------------------------------------*
*                            AMAGGI                                    *
*----------------------------------------------------------------------*
* Descrição  : Lista de Baixas criadas                                 *
*----------------------------------------------------------------------*
...
*   legendas
  ...
  "Cancelar
  CLEAR w_toolbar.
  MOVE 'CANCELAR'           TO w_toolbar-function.
  MOVE icon_cancel TO w_toolbar-icon.
  MOVE '0 '               TO w_toolbar-butn_type.
  MOVE 'Cancelar'(010)    TO w_toolbar-quickinfo.
  MOVE 'Cancelar'(010)    TO w_toolbar-text.
  APPEND w_toolbar TO e_object->mt_toolbar.
  ...
  IF t_zsdt0276[] IS NOT INITIAL.
    ...
    SELECT * FROM zsdt0277
      ...
      WHERE id_baixa = @t_zsdt0276-id_baixa
        ...
        AND acao     = @t_zsdt0276-status.
*        AND acao     IN @s_status.
...
*----------------------------------------------------------------------*
  ...
  LOOP AT t_zsdt0276 INTO DATA(w_zsdt0276).
    ...
    CLEAR w_saida.
    w_saida-status = w_zsdt0276-status.

    CASE w_zsdt0276-status.
      WHEN 'A'.
        w_saida-ico_status = icon_okay.
      WHEN 'R'.
        w_saida-ico_status = icon_status.
        w_saida-just_icon  = icon_display_text.
      WHEN 'C'.
        w_saida-ico_status = icon_cancel.
        w_saida-just_icon  = icon_display_text.
      WHEN OTHERS.
...
FORM zf_handle_hotspot_click    USING    p_index  TYPE any
  ...
  CASE p_column.
    ...
    WHEN 'OBS_ICON'.
      PERFORM zf_popup_obs CHANGING <fs_saida>.
    WHEN 'JUST_ICON'.
      PERFORM zf_popup_texto USING <fs_saida>.
    WHEN 'ANEXOS_GERAL'.
      CLEAR w_bor.
      CONCATENATE <fs_saida>-id_baixa '0000000000' '000000'  INTO w_bor-objkey.
      w_bor-objtype = 'ZS_BAIXANF'.
      PERFORM zf_chamar_anexo USING <fs_saida>-status CHANGING <fs_saida> w_bor <fs_saida>-anexos_geral.
    WHEN 'ANEXOS_ESPEC'.
      CLEAR w_bor.
      CONCATENATE <fs_saida>-id_baixa <fs_saida>-docnum <fs_saida>-itmnum INTO w_bor-objkey.
      w_bor-objtype = 'ZS_BAIXANF'.
      PERFORM zf_chamar_anexo USING <fs_saida>-status CHANGING <fs_saida> w_bor <fs_saida>-anexos_espec.
  ENDCASE.
...
*  ************ - Titulo Legenda
  ...
  CALL METHOD l_document->add_icon
    EXPORTING
      sap_icon = 'ICON_STATUS'.
*
  CALL METHOD l_document->add_text
    EXPORTING
      text = 'Baixa(s) Reprovada(s)'(025).
  ...
  LOOP AT pt_fcat ASSIGNING FIELD-SYMBOL(<fs_fcat>).
    ...
    CASE <fs_fcat>-fieldname.
      ...
      WHEN 'ANEXOS_GERAL'.
        ...
        <fs_fcat>-hotspot = abap_true.
      WHEN 'ANEXOS_ESPEC'.
        <fs_fcat>-coltext = <fs_fcat>-reptext = 'Anexos(Específicos)'.
        <fs_fcat>-just = 'C'.
        <fs_fcat>-hotspot = abap_true.
      WHEN 'USUARIO'.
        <fs_fcat>-coltext = <fs_fcat>-reptext = 'Últ. Modificador'.
      WHEN 'DATA'.
        <fs_fcat>-coltext = <fs_fcat>-reptext = 'Data Modificação'.
      WHEN 'HORA'.
        <fs_fcat>-coltext = <fs_fcat>-reptext = 'Hora Modificação'.
      WHEN 'JUST_ICON'.
        <fs_fcat>-coltext = <fs_fcat>-reptext = 'Justificativa'.
        <fs_fcat>-hotspot = abap_true.
        <fs_fcat>-just = 'C'.
      WHEN 'JUSTIFICATIVA'.
        <fs_fcat>-no_out = abap_true.
...
*      <--P_<FS_SAIDA>  text
*----------------------------------------------------------------------*
FORM zf_popup_obs CHANGING p_saida TYPE ty_saida.
  ...
  DATA: t_tline        TYPE STANDARD TABLE OF tline,
        ...
        t_texto_espec  TYPE catsxt_longtext_itab,
        w_texto_espec  LIKE LINE OF t_texto,
  ...
  IF sy-subrc IS INITIAL.
    ...
    CASE p_saida-status.
      WHEN 'A'.
        ...
        DATA(l_edit) = abap_false.
        IF t_texto[] IS NOT INITIAL.
          APPEND INITIAL LINE TO t_texto ASSIGNING FIELD-SYMBOL(<fs_texto>).
          MOVE'[********As linhas acima não serão alteradas*********]' TO <fs_texto>.
          DESCRIBE TABLE t_texto LINES DATA(l_linhas).
        ENDIF.

        CONCATENATE p_saida-id_baixa p_saida-docnum p_saida-itmnum 'E' INTO w_header-tdname .
    ...
    IF t_texto[] IS NOT INITIAL.
      ...
      CLEAR  w_zsdt0277_upd.
      w_zsdt0277_upd-id_baixa = p_saida-id_baixa.
      w_zsdt0277_upd-docnum   = p_saida-docnum.
      w_zsdt0277_upd-itmnum   = p_saida-itmnum.
      w_zsdt0277_upd-data     = sy-datum.
      w_zsdt0277_upd-hora     = sy-uzeit.
      w_zsdt0277_upd-acao     = 'E'.
      w_zsdt0277_upd-usuario  = sy-uname.
      MODIFY zsdt0277 FROM w_zsdt0277_upd.

      DO l_linhas TIMES.
        DELETE t_texto INDEX 1.
      ENDDO.
...
*      <--P_<FS_SAIDA>  text
*----------------------------------------------------------------------*
FORM zf_chamar_anexo USING p_mode CHANGING p_saida TYPE ty_saida
                                           p_bor   TYPE borident
                                           p_anexos.

  DATA: w_srgbtbrel    TYPE srgbtbrel,
        w_zsdt0277_upd TYPE zsdt0277.
  ...
  l_ip_service  = COND #( WHEN p_anexos = space THEN 'PCATTA_CREA'
                                                ELSE 'VIEW_ATTA' ).
  ...
  "Atualizar tabela de saída
  CLEAR w_srgbtbrel.
  w_srgbtbrel-reltype  = 'ATTA'.
  w_srgbtbrel-typeid_a = 'ZS_BAIXANF'.
  w_srgbtbrel-instid_a = p_bor-objkey.

  "Buscar Anexos Especificos
  SELECT instid_a FROM srgbtbrel
            INTO TABLE @DATA(t_anexo)
     WHERE reltype   EQ @w_srgbtbrel-reltype
       AND instid_a  EQ @w_srgbtbrel-instid_a
       AND typeid_a  EQ @w_srgbtbrel-typeid_a.

  DESCRIBE TABLE t_anexo LINES DATA(l_nro_itens_old).


  lcl_anexo_obj->set_rw_mode( ip_mode = l_mode ).
  ...
  IF sy-subrc IS INITIAL.
    p_anexos = icon_attachment.
  ELSE.
    CLEAR p_anexos.
  ENDIF.

  DESCRIBE TABLE t_anexo LINES DATA(l_nro_itens_new).

  IF l_nro_itens_old <> l_nro_itens_new.
    w_zsdt0277_upd-id_baixa = p_saida-id_baixa.
    w_zsdt0277_upd-docnum   = p_saida-docnum.
    w_zsdt0277_upd-itmnum   = p_saida-itmnum.
    w_zsdt0277_upd-data     = sy-datum.
...
FORM zf_cancelar .
  ...
  IF  t_observ IS NOT INITIAL.
    ...
    CALL FUNCTION 'POPUP_TO_CONFIRM'
      EXPORTING
        ...
        text_question         = 'Após o cancelamento não será possível ativar novamente cancelar?'
        text_button_1         = 'Sim'
        text_button_2         = 'Não'
        default_button        = '1'
    ...
    IF l_answer = 1.
      ...
      LOOP AT t_row_no INTO w_row_no.
        ...
        IF sy-subrc IS INITIAL.
          ...
          <fs_saida>-status     = 'C'.
          <fs_saida>-ico_status = icon_cancel.
      ...
      IF t_zsdt0277_upd[] IS NOT INITIAL.
        ...
        IF t_zsdt0276_upd IS NOT INITIAL.
          ...
          COMMIT WORK AND WAIT.
          MESSAGE s000(zppr) WITH 'Documento(s) cancelado(s) com sucesso(s)!' DISPLAY LIKE 'S'.
          RETURN.
  ...
  ELSE.
    MESSAGE s000(zppr) WITH 'Justificativa do cancelamento não informada!' DISPLAY LIKE 'E'.
    RETURN.
  ENDIF.
...
*      <--P_<FS_SAIDA>  text
*----------------------------------------------------------------------*
FORM zf_popup_texto  USING p_saida TYPE ty_saida.
  ...
  CASE p_saida-status.
    WHEN 'C'.
      CONCATENATE p_saida-id_baixa p_saida-docnum p_saida-itmnum 'C' INTO w_header-tdname.
    WHEN 'R'.
      CONCATENATE p_saida-id_baixa p_saida-docnum p_saida-itmnum 'R' INTO w_header-tdname.
    WHEN OTHERS.
  ENDCASE.
