class ZCL_INT_IB_CONS_INVOICE definition
  public
  final
  create public .
...
public section.
  ...
  data:
    tb_ID_EMPRESA TYPE TABLE OF BUKRS.
  data:
    tb_INVOICE   TYPE TABLE OF  CHAR15.
  data:
    tb_STATUS  TYPE TABLE OF CHAR1.
  data:
  ...
  data:
    ...
    ID_EMPRESA  LIKE  tb_ID_EMPRESA,
    INVOICE     LIKE tb_INVOICE  ,
  ...
  data:
    begin of zde_data_response,
              INVOICES         TYPE z_ZFIT0036_t,
         end of zde_data_response .
  constants AT_ID_INTERFACE type ZDE_ID_INTERFACE value '126' ##NO_TEXT.
...
CLASS ZCL_INT_IB_CONS_INVOICE IMPLEMENTATION.
...
*
  ...
  METHOD ZIF_INTEGRACAO_INJECT~SET_INTEGRAR_INBOUND.
    ...
    TYPES:
      BEGIN OF ty_iD_EMPRESA,
       id_empresa TYPE BUKRS,
      END OF ty_iD_EMPRESA,
      ...
      BEGIN OF ty_belnr,
        ...
        END OF ty_belnr.
    DATA:
      lra_id_empresa TYPE RANGE OF BUKRS,
      lra_invoice    TYPE RANGE OF CHAR15,
    ...
    LOOP AT lwa_data_request-id_empresa INTO DATA(wa_id_empresa).
      APPEND VALUE #( sign = 'I' option = 'EQ' low = wa_id_empresa ) TO lra_id_empresa.
    ENDLOOP.



    LOOP AT lwa_data_request-invoice INTO DATA(wa_invoice).
      APPEND VALUE #( sign = 'I' option = 'EQ' low = wa_invoice ) TO lra_invoice.
    ENDLOOP.
    ...
    LOOP AT lwa_data_request-obj_key INTO DATA(wa_obj_key).
      APPEND VALUE #( sign = 'I' option = 'EQ' low = wa_obj_key ) TO lra_obj_key.
    ENDLOOP.

    LOOP AT lwa_data_request-belnr INTO DATA(wa_belnr).
      APPEND VALUE #( sign = 'I' option = 'EQ' low = wa_belnr ) TO lra_belnr.
    ENDLOOP.
    ...
    IF lwa_data_request IS NOT INITIAL.
      ...
      SELECT *
      FROM ZFIT0036
      INTO TABLE lwa_data_response-invoices
      WHERE bukrs   IN lra_id_empresa AND
            invoice IN lra_invoice AND
            status  IN lra_status AND
            obj_key IN lra_obj_key AND
            belnr   IN lra_belnr.
