*&-------------------------------------------------------------------------------------------------------*
*& Report         : ZCL_INT_OB_FINANFOR_MOV_CRIAR                                                        *
...
*&-------------------------------------------------------------------------------------------------------*
*&-------------------------------------------------------------------------------------------------------*
*& 11/07/2025  |DEVK9A2OOH  |NSEGATIN       |Integração Finanfor- Ciar Movimento. Desenvolvimento inicial*
*&                                          |Chamado: 184694.                                            *
*--------------------------------------------------------------------------------------------------------*
CLASS zcl_int_ob_finanfor_mov_criar DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    CONSTANTS: at_id_interface TYPE zde_id_interface VALUE '301' ##NO_TEXT,
               at_servico      TYPE c LENGTH 20 VALUE 'FINANFOR_MOVIMENTO' ##NO_TEXT.   "Integração MOV. Finanfor - Outbound

    DATA at_param_mov_criar TYPE zfie_finanfor_mov_criar .
...
CLASS ZCL_INT_OB_FINANFOR_MOV_CRIAR IMPLEMENTATION.
  ...
  METHOD zif_integracao_outbound~build_info_request.
    ...
    MOVE-CORRESPONDING i_info_request TO at_param_mov_criar.
  ...
  METHOD zif_integracao_outbound~get_instance.
    ...
    IF zif_integracao_outbound~at_if_integracao_outbound IS NOT BOUND.
      CREATE OBJECT zif_integracao_outbound~at_if_integracao_outbound TYPE zcl_int_ob_finanfor_mov_criar.
    ENDIF.
  ...
  METHOD zif_integracao_outbound~set_url.
    ...
    CLEAR: me->zif_integracao_outbound~at_auth_webservice-add01, me->zif_integracao_outbound~at_auth_webservice-add02.
...
* Corresponde ao parâmetro "category" da URL
    DATA(vl_add02) = |{ lwa_webservice-add02 CASE = LOWER }|.
    lwa_webservice-url = lwa_webservice-url && vl_add02.
