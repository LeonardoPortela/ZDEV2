CLASS zcl_int_ib_cons_moeda DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    DATA:
      tb_waers TYPE TABLE OF waers .
    DATA:
      tb_spras TYPE TABLE OF spras ,
      tb_moedas TYPE TABLE OF tcurt.
    ...
    DATA:
      BEGIN OF zde_data_response,
        moedas LIKE tb_moedas,
      END OF zde_data_response .
    CONSTANTS at_id_interface TYPE zde_id_interface VALUE '149' ##NO_TEXT.
...
CLASS ZCL_INT_IB_CONS_MOEDA IMPLEMENTATION.
...
*-----------------------------------------------------------------------------------------------------------------------*
  ...
  METHOD zif_integracao_inject~set_integrar_inbound.
    ...
    DATA:
      lra_filtro1 TYPE RANGE OF waers,
      lra_filtro2 TYPE RANGE OF spras.
    ...
    LOOP AT lwa_data_request-waers INTO DATA(wa_filtro1).
      APPEND VALUE #( sign = 'I' option = 'EQ' low = wa_filtro1 ) TO lra_filtro1.
    ENDLOOP.

    LOOP AT lwa_data_request-spras INTO DATA(wa_filtro2).
      APPEND VALUE #( sign = 'I' option = 'EQ' low = wa_filtro2 ) TO lra_filtro2.
    ENDLOOP.

    SELECT *
      FROM tcurt
      INTO TABLE lwa_data_response-moedas
      WHERE waers IN lra_filtro1
        AND spras IN lra_filtro2.
