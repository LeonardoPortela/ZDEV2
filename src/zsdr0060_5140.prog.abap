    ...
    IF wa_header_lote-inco1 NE 'FOB'.
      IF wa_header_lote-inco1 EQ 'CIF' OR wa_header_lote-inco1 EQ 'CPT'.
        IF screen-name NE 'F8' AND screen-name NE 'WA_HEADER_LOTE-DT_ENTREGA'
          AND screen-name NE 'WA_HEADER_LOTE-ARMAZEM_ORG'. " Rubenilson - 17.09.24 - #144012
          screen-input = 0.
        ENDIF.
      ELSE.
...
*-CS2019001891 - JT - 04.02.2021 - fim
  ...
  wa_zsdt0129-armazem_org   = wa_header_lote-armazem_org. " Rubenilson - 19.09.24 #144012
  wa_zsdt0129-armazem_org = |{ wa_zsdt0129-armazem_org ALPHA = IN }|. " Rubenilson - 30.09.24 - #144012
...
FORM check_parametro_5140 CHANGING vl_check.
  ...
  DATA: t_dias     TYPE STANDARD TABLE OF  rgsb4 WITH HEADER LINE,
        vg_dateref TYPE sy-datum.


*- Inicio User Story 144014 / aoenning

  IF wa_header_lote-dt_entrega IS INITIAL.
    MESSAGE e024(sd) WITH TEXT-204.
    vl_check = abap_true.
    EXIT.
  ENDIF.
  ...
  IF sy-subrc NE 0.
    ...
    CALL FUNCTION 'G_SET_GET_ALL_VALUES'
      EXPORTING
        class           = '0000'
        setnr           = 'ZSDT0112_QTD_DIAS_ENTRAD'
        no_descriptions = ' '
      TABLES
        set_values      = t_dias
      EXCEPTIONS
    ...
    READ TABLE t_dias INTO DATA(ws_dias) INDEX 1.
    vg_dateref = sy-datum + ws_dias-from.
    IF wa_header_lote-dt_entrega < vg_dateref.
      vg_dateref = vg_dateref - 1.
      MESSAGE e024(sd) WITH TEXT-203 vg_dateref.
      vl_check = abap_true.
    ENDIF.
  ENDIF.

*- Fim User Story 144014 / AOENNING
...
*-CS2019001891 - JT - 04.02.2021 - fim
  ...
  wa_header_lote-armazem_org = wa_zsdt0129-armazem_org. " Rubenilson - 18.09.24 - #144012
