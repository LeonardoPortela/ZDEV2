...
*"----------------------------------------------------------------------
  ...
  select * from zsdt0040
    into table @data(gt_zsdt0040)
    for all entries in @gt_zsdt0094
  where doc_simulacao eq @gt_zsdt0094-nro_sol_ov
    and vkorg         in @r_vkorg.

  select * from zsdt0041
    into table @data(gt_zsdt0041)
    for all entries in @gt_zsdt0094
  where doc_simulacao eq @gt_zsdt0094-nro_sol_ov
    and posnr         eq @gt_zsdt0094-fixacao.

  if not gt_zsdt0041 is initial.
    select * from mara
      into table gt_mara
...
*    INTO TABLE @DATA(GT_ZSDT0090)
*    FOR ALL ENTRIES IN @GT_ZSDT0094
*  WHERE DOC_SIMULACAO EQ @GT_ZSDT0094-NRO_SOL_OV
*    AND VBELN         EQ @GT_ZSDT0094-VBELN
*    AND ESTORNO       EQ @ABAP_FALSE.

*  IF NOT GT_ZSDT0090 IS INITIAL.
*    SELECT * FROM MARA
*      APPENDING TABLE GT_MARA
*      FOR ALL ENTRIES IN GT_ZSDT0090
*    WHERE MATNR EQ GT_ZSDT0090-MATNR.
*  ENDIF.
  ...
  loop at gt_zsdt0094 into gw_zsdt0094.
    ...
    read table gt_zsdt0040 into data(gw_zsdt0040) with key doc_simulacao = gw_zsdt0094-nro_sol_ov.
    if not ( r_vkorg is initial ) and ( gw_zsdt0040-vkorg not in r_vkorg  ).
      continue.
    endif.
    gw_saida-tpsim      = gw_zsdt0040-tpsim.
    gw_saida-waerk      = gw_zsdt0040-waerk.
    gw_saida-vkorg      = gw_zsdt0040-vkorg.

    case gw_zsdt0040-tpsim.
      when 'TS'. gw_saida-zterm = 'I001'.
      when 'AD'. gw_saida-zterm = 'I002'.
      when 'VV' or 'VF' or 'BN'. gw_saida-zterm = 'I003'.
      when 'TV'. gw_saida-zterm = 'I004'.
      when 'VP'. gw_saida-zterm = 'I005'.
      when 'TT'. gw_saida-zterm = 'I008'. "US169495
    endcase.
    ...
    gw_saida-vbeln                   = gw_zsdt0094-vbeln .
    ...
    loop at gt_zsdt0041 into data(gw_zsdt0041) where doc_simulacao eq gw_zsdt0040-doc_simulacao.
      read table gt_mara into gw_mara with key matnr = gw_zsdt0041-matnr.

      case gw_mara-matkl.
        when '658445'.
          gw_saida-dmbtr   = gw_saida-dmbtr + gw_zsdt0041-zmeng.
          gw_saida-dtde_logist   = gw_zsdt0040-dt_entrega_def.
        when '700150' or '658440'.
          gw_saida-dmbtr   = gw_saida-dmbtr + gw_zsdt0041-zmeng.
          gw_saida-dtde_logist   = gw_zsdt0040-dt_entrega_fet.
        when '700230' or '700130' or '700240'.
          gw_saida-dmbtr   = gw_saida-dmbtr + gw_zsdt0041-zmeng.
          gw_saida-dtde_logist   = gw_zsdt0040-dt_entrega_sem.
      endcase.
      gw_saida-charg   = gw_zsdt0041-charg.
    endloop.
    ...
    clear: gw_saida, gw_zsdt0094, gw_mara, gw_zsdt0041, gw_zsdt0040, gw_makt.", GW_ZSDT0090.
