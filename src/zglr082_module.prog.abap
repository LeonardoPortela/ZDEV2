...
MODULE user_command_0200 INPUT.
  ...
  DATA: it_conc_invest TYPE TABLE OF ty_list01,
        ...
        wa_zglt0114    TYPE zglt0114.

  CASE sy-ucomm.
    WHEN 'FB_BUSCAR'.
      CALL METHOD lcl_f4=>f4_custon_001.
    WHEN 'FB_SAIR'.
      CLEAR:wa_saida,wa_zglt0114,lt_return,ls_return,wa_saida.
      FREE: it_conc_invest,lt_return.
      SET SCREEN 0.
      LEAVE SCREEN.
    WHEN 'FB_SALVAR'.
      MOVE-CORRESPONDING wa_saida TO wa_zglt0114.
      MODIFY zglt0114 FROM wa_zglt0114.
      IF sy-subrc = 0 .
    ...
    WHEN 'FB_AGRUPAMENTO'.
      ...
      IF wa_saida-moeda_funcional IS NOT INITIAL AND wa_saida-pais IS NOT INITIAL.
        ...
        SELECT
          ...
          agrupamento AS low,
          agrupamento AS high
          FROM zglt0114 WHERE moeda_funcional = @wa_saida-moeda_funcional AND pais = @wa_saida-pais INTO TABLE @DATA(it_agrupamento_filter).
