class ZCL_FMCALL_COTTON_SAP definition
  public
  create public .
...
*"* public components of class ZCL_FMCALL_COTTON_SAP
...
CLASS ZCL_FMCALL_COTTON_SAP IMPLEMENTATION.
...
* Get function name from PATH_INFO
  ...
  i_info-ds_funcao_processa = CONV #( funcname ).
  id_referencia = server->request->get_header_field( 'IdReferencia' ).

  TRANSLATE funcname TO LOWER CASE.

  TRY.
      CASE funcname.
        WHEN zcl_fmcall_base=>at_servico_cotton_doucle_check.

          zcl_integracao_cotton_sap=>zif_integracao_cotton_sap~get_instance(
             )->set_ds_data( i_info = i_info
             )->get_double_check(
             )->set_send_msg( IMPORTING e_msg = o_cdata
          ).

        WHEN zcl_fmcall_base=>at_servico_processa_cotton or
             zcl_fmcall_base=>at_servico_estorno_cotton.
...
*-CS2021000713 - 14.10.2021 - JT - inicio
*        WHEN zcl_fmcall_base=>at_servico_estorno_cotton.
*
*          zcl_integracao_cotton_sap=>zif_integracao_cotton_sap~get_instance(
*             )->set_ds_data( i_info        = i_info
*                             id_referencia = id_referencia
...
*             )->set_send_msg( IMPORTING e_msg = o_cdata
*          ).
*-CS2021000713 - 14.10.2021 - JT - fim

        WHEN zcl_fmcall_base=>at_servico_status_job.

          zcl_integracao_cotton_sap=>zif_integracao_cotton_sap~get_instance(
             )->get_check_job( IMPORTING e_msg = o_cdata ).
