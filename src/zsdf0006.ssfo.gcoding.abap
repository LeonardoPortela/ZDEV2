...
IF LINE_EXISTS( IT_ITENS_IM[ SA = '04' ] ).
  CASE WA_IMPORT-CULTURA.
    WHEN 'SJ'.
      DESCRICAO = |{ DESCRICAO } Sementes de SOJA|.
    WHEN 'ML'.
      DESCRICAO = |{ DESCRICAO } Sementes de MILHO|.
  ENDCASE.
ENDIF.

ML_SJ = DESCRICAO.

IF LINE_EXISTS( IT_ITENS_IM[ SA = '02' ] ).
  IF DESCRICAO IS INITIAL.
    DESCRICAO = |Fertilizantes|.
  ELSE.
    DESCRICAO = |{ DESCRICAO }, Fertilizantes|.
  ENDIF.
ENDIF.

IF LINE_EXISTS( IT_ITENS_IM[ SA = '03' ] ).
  IF DESCRICAO IS INITIAL.
    DESCRICAO = |Defensivos Agrícolas|.
  ELSE.
    DESCRICAO = |{ DESCRICAO }  e Defensivos Agrícolas|.
  ENDIF.
ENDIF.

IF WA_IMPORT-WAERK EQ	'BRL'.
  MOEDA = 'R$'.
ELSE.
  MOEDA = 'U$'.
ENDIF.

LOOP AT IT_PARCEIROS INTO DATA(WA_PAR).
  IF SY-TABIX MOD 2 NE 0 .
    WA_ZSDS017-NAMEV_I = WA_PAR-NAMEV.
    WA_ZSDS017-NAME1_I = WA_PAR-NAME1.
    IF LINES( IT_PARCEIROS[] ) EQ SY-TABIX.
      APPEND WA_ZSDS017 TO IT_ZSDS017.
      APPEND WA_ZSDS017 TO IT_ZSDS017_ASS.
      CLEAR WA_ZSDS017.
    ENDIF.
  ELSE.
    WA_ZSDS017-NAMEV_P = WA_PAR-NAMEV.
    WA_ZSDS017-NAME1_P = WA_PAR-NAME1.
    APPEND WA_ZSDS017 TO IT_ZSDS017.
    APPEND WA_ZSDS017 TO IT_ZSDS017_ASS.
    CLEAR WA_ZSDS017.
  ENDIF.
