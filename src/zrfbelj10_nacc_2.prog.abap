...
PARAMETERS noheader LIKE rfpdo2-umsvhead DEFAULT ' '
  USER-COMMAND gc_ok_dummy.
* Posting Date in Title
PARAMETERS p_date   LIKE rfpdo2-beldates DEFAULT 'X'  MODIF ID hdr.
* VAT ID in header
PARAMETERS p_vat    AS CHECKBOX.                            "note990329
* Additional header
PARAMETERS title    LIKE rfpdo-allgline               MODIF ID hdr.
parameters p_pag    like sy-pagno default 1.
...
* Print totals only
PARAMETERS tabsum   LIKE rfpdo1-beljsumm
                    RADIOBUTTON GROUP r1.
* Print items only
PARAMETERS posten   LIKE rfpdo1-beljpost
                    RADIOBUTTON GROUP r1.
SELECTION-SCREEN SKIP.

* General ledger view
SELECTION-SCREEN BEGIN OF LINE.
PARAMETERS par_docc  LIKE b0sg-xdocc MODIF ID xgl.
SELECTION-SCREEN COMMENT 03(70) text-c01 FOR FIELD par_docc.
SELECTION-SCREEN END OF LINE.

* Print Line Item Text
PARAMETERS segmtext LIKE rfpdo1-beljsetx DEFAULT ' '.
* Display Alternative Account Number
SELECTION-SCREEN BEGIN OF LINE.
PARAMETERS p_altkt  LIKE rfpdo1-allgaltk.
...
SELECTION-SCREEN BEGIN OF LINE.
PARAMETERS p_accseq LIKE rfpdo2-beljhkto.
SELECTION-SCREEN COMMENT 03(70) text-c03 FOR FIELD p_accseq.
SELECTION-SCREEN END OF LINE.
* Do not print CPU date
PARAMETERS no_cpu   LIKE rfpdo2-no_cpu   DEFAULT ' '.

SELECTION-SCREEN SKIP.

* Number of Lines for Master Data Texts
PARAMETERS textline LIKE rfpdo1-beljtxli DEFAULT '2'.
* Continuous Document Number
PARAMETERS z_nummer LIKE rfpdo1-beljznum DEFAULT 'X'.
SELECTION-SCREEN END   OF BLOCK b03.

SELECTION-SCREEN BEGIN OF BLOCK b04 WITH FRAME TITLE text-f04.
* Totals Per Page
PARAMETERS seit_sum LIKE rfpdo1-beljsesu DEFAULT 'X'.
* Totals Per Posting Date
PARAMETERS budatsum LIKE rfpdo1-beljbusu DEFAULT ' '.
SELECTION-SCREEN SKIP.
* Sort Key
PARAMETERS sort_key LIKE rfpdo1-beljsort DEFAULT '2'.
SELECTION-SCREEN SKIP.
* Document Line Compression
PARAMETERS compress LIKE rfpdo1-beljcomp DEFAULT ' '.
* Business Area Compression
PARAMETERS gb_comp  LIKE rfpdo1-beljcogb DEFAULT ' '.
SELECTION-SCREEN END   OF BLOCK b04.

SELECTION-SCREEN BEGIN OF BLOCK b05 WITH FRAME TITLE text-f05.
SELECT-OPTIONS s_sblart  FOR t003t-blart.          "needed for Italy
SELECT-OPTIONS s_scpudt  FOR sy-datum."needed for Italy
SELECTION-SCREEN END   OF BLOCK b05.

begin_of_block 8.
PARAMETERS:
  listsep  LIKE rfpdo1-allglise,       "X - Listseparation
  p_miffl  LIKE rfpdo-allgmikf.        "X - Microfich-Information
end_of_block 8.
  ...
  IF fagl_active = 'X'.
    ...
    ELSE.
      ...
      IF br_budat-low > h_date.
        MESSAGE w439 WITH h_date.
      ENDIF.
      IF br_budat-low < h_date.
        MESSAGE w440 WITH h_date.
      ENDIF.
      IF italy = 'X'.
  ...
  ELSE.
    ...
    ELSE.                                "<---INSERT 66911
      ...
      IF br_budat-low > h_date.          "<---INSERT 66911
        MESSAGE w439 WITH h_date.        "<---INSERT 66911
      ENDIF.                             "<---INSERT 66911
      IF br_budat-low < h_date.          "<---INSERT 66911
        MESSAGE w440 WITH h_date.        "<---INSERT 66911
      ENDIF.                             "<---INSERT 66911
      IF italy = 'X'.
        pagno = trvor-pagno.
      ENDIF.
...
FORM output_newbukrs.
  ...
  IF sy-subrc = 0.
    ...
    IF sumso GT max_length OR sumha GT max_length.
      WRITE:
        110(23) sumha NO-ZERO CURRENCY t001-waers,
        /93(23) sumso NO-ZERO CURRENCY t001-waers.
    ELSE.
      WRITE:
         96(16) sumso NO-ZERO CURRENCY t001-waers,
        113(16) sumha NO-ZERO CURRENCY t001-waers.
    ENDIF.
    ULINE.
    IF seit_sum = 'X'.
      wasol = sumso.
...
*           (16) t-wrshb NO-ZERO CURRENCY t-waers,
                t-waers,
         96(16) t-dmsol NO-ZERO CURRENCY t001-waers,
        113(16) t-dmhab NO-ZERO CURRENCY t001-waers.
      ELSE.                                                 "note955881
        WRITE:                                              "note955881
*             (16) t-wrshb CURRENCY t-waers,                 "note955881
                  t-waers,                                  "note955881
           96(16) t-dmsol CURRENCY t001-waers,              "note955881
          113(16) t-dmhab CURRENCY t001-waers.              "note955881
      ENDIF.                                                "note955881
      wssol = wssol + whsol.
      wshab = wshab + whhab.
      wasol = wasol + whsol.
...
*       totals and delete them afterwards
          ...
          LOOP AT wrsum.
            IF wrsum-fwbtr NE 0.
              ...
              /(16) wrsum-fwbtr NO-ZERO CURRENCY wrsum-waers
                               UNDER t-wrshb,
                   wrsum-waers,
            96(16) wrsum-hwsol NO-ZERO CURRENCY t001-waers,
           113(16) wrsum-hwhab NO-ZERO CURRENCY t001-waers.
            ENDIF.
          ENDLOOP.
        ENDIF.
        CLEAR wrsum.
        ...
        WRITE: /10 text-003,
                 wbudat DD/MM/YY,
          96(16) sum(t-dmsol) NO-ZERO CURRENCY t001-waers,
         113(16) sum(t-dmhab) NO-ZERO CURRENCY t001-waers.
        ULINE.
      ENDIF.
    ENDIF.
  ENDIF.
  ...
  IF seit_sum = 'X'.
    ...
    IF wssol NE 0 OR wshab NE 0.
      ...
      IF wssol GT max_l_page OR wshab GT max_l_page.
        WRITE:
*           98(17) wssol CURRENCY t001-waers NO-ZERO NO-SIGN,
          111(17) wshab CURRENCY t001-waers NO-ZERO.
      ELSE.
        WRITE:
           96(16) wssol CURRENCY t001-waers NO-ZERO,
          113(16) wshab CURRENCY t001-waers NO-ZERO.
      ENDIF.
    ENDIF.
    IF wasol NE 0 OR wahab NE 0.
      ...
      IF wasol GT max_length OR wahab GT max_length.
        WRITE:
          110(23) wahab CURRENCY t001-waers NO-ZERO.
*          /93(23) wasol CURRENCY t001-waers NO-ZERO.
      ELSE.
        WRITE:
           96(16) wasol CURRENCY t001-waers NO-ZERO,
          113(16) wahab CURRENCY t001-waers NO-ZERO.
      ENDIF.
    ENDIF.
  ENDIF.
...
*---------------------------------------------------------------------*
  ...
  IF ld_no_standard_header IS INITIAL.                      "n1053772
    IF noheader IS INITIAL.              "INSERT HMY030297
      PERFORM batch-heading(zrsbtchh0) using p_pag.
  ...
  IF seit_sum = 'X'.
    IF wssol NE 0 OR wshab NE 0.
      ...
      IF wssol GT max_l_page OR wshab GT max_l_page.
        WRITE:
           98(17) wssol CURRENCY t001-waers NO-ZERO NO-SIGN,
          116(17) wshab CURRENCY t001-waers NO-ZERO.
      ELSE.
        WRITE:
           96(16) wssol CURRENCY t001-waers NO-ZERO,
          113(16) wshab CURRENCY t001-waers NO-ZERO.
...
* first posting period of the used fiscal year variant
  ...
  WRITE 122(11) pagno_year RIGHT-JUSTIFIED.
