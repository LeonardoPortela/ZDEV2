class ZCL_INT_OB_CONS_COMPROVANTES definition
  public
  final
  create public .
...
public section.
  ...
  constants AT_ID_INTERFACE type ZDE_ID_INTERFACE value '269' ##NO_TEXT.
  data AT_DADOS_ORDEM_ORC type ZPM_ORDEM_ORC .
  data AT_REQ type STRING .
  data AT_MATERIAL type STRING .
  data AT_AGENCIA type STRING .
  data AT_CONTA type STRING .
  data AT_CNPJ type STRING .
...
CLASS ZCL_INT_OB_CONS_COMPROVANTES IMPLEMENTATION.
...
*    ENDLOOP.
  ...
  METHOD ZIF_INTEGRACAO_OUTBOUND~BUILD_INFO_REQUEST.
    ...
    SPLIT i_info_request AT ';' INTO me->at_agencia me->at_conta me->at_cnpj me->at_offset.
  ...
  method ZIF_INTEGRACAO_OUTBOUND~GET_INSTANCE.
    ...
    IF zif_integracao_outbound~at_if_integracao_outbound IS NOT BOUND.
      CREATE OBJECT zif_integracao_outbound~at_if_integracao_outbound TYPE zcl_int_ob_cons_comprovantes.
    ENDIF.
...
*    APPEND VALUE #( name = 'accept'   value = 'application/json' ) TO LC_INTEGRAR~at_.
  ...
  METHOD zif_integracao_outbound~set_url.
    ...
    SELECT SINGLE *
     ...
     WHERE service = 'CONSULTA_COMPROVANTES_PIX'.
      ...
      RAISE EXCEPTION TYPE zcx_integracao
        EXPORTING
          textid = VALUE #( msgid = zcx_integracao=>zcx_erro_geral-msgid
                            ...
                            attr2 = 'CONSULTA_COMPROVANTES_PIX' )
          msgid  = zcx_integracao=>zcx_erro_geral-msgid
          msgno  = zcx_integracao=>zcx_erro_geral-msgno
          msgty  = 'E'
          msgv1  = 'Serviço não configurado: '
          msgv2  = 'CONSULTA_COMPROVANTES_PIX'.
    ENDIF.

    CONCATENATE sy-datum(4) '-' sy-datum+4(2) '-' sy-datum+6(2) INTO DATA(lv_data).

    CONCATENATE lwa_webservice-url 'agencia_operacao=' me->at_agencia '&conta_operacao=' me->at_conta '&cnpj_empresa=' me->at_cnpj
                 '&tipo_lista=Detalhada&data_inicial=' lv_data '&data_final=' lv_data INTO lva_url.

    IF me->at_offset > 0.
      lva_url = lva_url && '&page=' && me->at_offset.
    ENDIF.
