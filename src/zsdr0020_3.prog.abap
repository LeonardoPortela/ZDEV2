...
**            and NumDocs = 0.
      ...
      APPENDING TABLE @gt_ov_venci.
        ...
        SORT gt_ov_venci ASCENDING.
        DELETE ADJACENT DUPLICATES FROM gt_ov_venci.
...
*        <fs_saida>-moeda_forte = <fs_ov>-saldorefusd.
        ...
        IF sy-subrc = 0.
          ...
          LOOP AT gt_docs_finan ASSIGNING FIELD-SYMBOL(<fs_doc>) FROM sy-tabix.
            ...
            "04.02.2025 - RAMON - Chamada tela zfis26 -->
            <fs_saida_doc>-vbeln = <fs_ov>-ordem.
            "04.02.2025 - RAMON - Chamada tela zfis26 --<
...
FORM user_command1 USING ucomm TYPE sy-ucomm
  ...
  TYPES:
    BEGIN OF ty_log,
      ...
      valor_apr  TYPE zfit186-vl_sol_isen_jur,
...
*              <fs_fieldcat>-seltext_s = 'Data Aprovação'.
      ...
      SELECT SINGLE vl_sol_isen_jur, status_solicit
              FROM zfit186
...
"FIM BUG SOLTO 145061 - RU
    ...
    LOOP AT lt_0336 ASSIGNING <fs_0336>.
      ...
      <fs_zsdt0337>-valor_apr  = wa_cabec-saldo.
...
*** Rubenilson - 18.04.2024 - 131863 - Envio email
  ...
  MESSAGE 'Isenção cadastrada com sucesso' TYPE 'S'.
  ...
  IF p_venc IS NOT INITIAL.
    ...
    IF sy-subrc EQ 0.
      ...
      SELECT * FROM zi_in_agreg_02
        INTO TABLE @lt_simu
        FOR ALL ENTRIES IN @lt_filtro
          WHERE ( vbeln_p = @lt_filtro-vbeln
          OR vbeln = @lt_filtro-vbeln )
         AND doc_simulacao IN @p_docsi[]. " 19.02.2025
    ...
    SELECT * FROM zi_in_agreg_02
      INTO TABLE @lt_simu
        WHERE bukrs IN @p_bukrs
          ...
          AND vkbur IN @p_vkbur
          AND doc_simulacao IN @p_docsi[]
          AND ( vbeln_p IN @p_vbeln
          OR vbeln IN @p_vbeln )  " 19.02.2025
          AND erdat IN @p_erdat.

  ENDIF.


  PERFORM f_visao_insumos_filtro CHANGING lt_simu.
...
FORM f_preenche_insumos_cab USING us_dados TYPE zi_in_agreg_02
  ...
  <fs_saida>-vbeln = us_dados-vbeln_p.
...
FORM f_preenche_insumos_item USING t_simu TYPE ty_agreg_02
  ...
  LOOP AT t_simu ASSIGNING FIELD-SYMBOL(<fs_lcto>) FROM uv_indice WHERE vbeln = us_dados-vbeln.
    ...
    <fs_saida_doc>-vbeln = us_dados-vbeln_p.
...
*&---------------------------------------------------------------------*
FORM f_visao_insumos_filtro CHANGING ct_select TYPE ty_agreg_02.

  IF rb3 IS INITIAL.

    IF ct_select IS NOT INITIAL.
