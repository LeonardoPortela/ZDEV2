class ZCL_INT_IB_CONS_TREINAMENTO definition
  public
  final
  create public .
...
public section.
  ...
  data:
    tb_treina TYPE TABLE OF zhcmv0004-treinamento .
  data:
  ...
  data:
    BEGIN OF zde_data_request,
        treinamento LIKE tb_treina,
        nome_like   TYPE zhcmv0004-nome,
      END OF zde_data_request .
  data:
    BEGIN OF zde_data_response,
        treinamentos TYPE TABLE OF zhcmv0004,
      END OF zde_data_response .
  constants AT_ID_INTERFACE type ZDE_ID_INTERFACE value '157' ##NO_TEXT.
...
CLASS ZCL_INT_IB_CONS_TREINAMENTO IMPLEMENTATION.
...
*-----------------------------------------------------------------------------------------------------------------------*
  ...
  METHOD zif_integracao_inject~set_integrar_inbound.
    ...
    DATA:
      lra_treina TYPE RANGE OF zhcmv0004-treinamento,
      lra_nome   TYPE RANGE OF zhcmv0004-nome.
    ...
    LOOP AT lwa_data_request-treinamento INTO DATA(wa_filtro1).
      APPEND VALUE #( sign = 'I' option = 'EQ' low = wa_filtro1 ) TO lra_treina.
    ENDLOOP.

    "CONCATENATE '*' lwa_data_request-nome_like '*' INTO lwa_data_request-nome_like. "Cliente da API que dicidirá como será o like

    APPEND VALUE #( sign = 'I' option = 'CP' low = lwa_data_request-nome_like ) TO lra_nome.

    IF lwa_data_request IS NOT INITIAL.

      SELECT DISTINCT *
        FROM zhcmv0004
        INTO TABLE lwa_data_response-treinamentos
        WHERE treinamento IN lra_treina
          AND nome        IN lra_nome.
