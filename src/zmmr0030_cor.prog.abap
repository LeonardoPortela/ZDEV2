...
**|                                                                           |*
**|  Funcional:                                                               |*
**|    + Cleudo Silva ( cleudo.silva@amaggi.com.br )                         |*
**|  Changelog:                                                               |*
**|                                                                           |*
**/===========================================================================\*
...
**/===========================================================================\*
**| Descrição:                                                                |*
**| JOB de processamento do Trace para Correções                              |*
**|                                                                           |*
**/===========================================================================\*
...
DATA: id_referencia TYPE zintegracao-id_referencia,
      qtd           TYPE sy-tabix,
      dat_ini       TYPE sy-uzeit,
      it_integracao TYPE TABLE OF zintegracao,
      wa_integracao TYPE zintegracao.
...
*&---------------------------------------------------------------------*
START-OF-SELECTION.
  PERFORM reprocessajob.
  PERFORM reprocessa_02.
...
FORM f_seleciona_dados.
  ...
  DATA: hr TYPE n LENGTH 2.

  hr = sy-uzeit(2) - '01'.
  dat_ini = |{ hr }{ sy-uzeit+2(4) }|.
...
*  AND   status           EQ 'R'
*  AND   laeda            EQ sy-datum
*  AND   laehr            LE dat_ini.

  SELECT * FROM zppt0002
    APPENDING TABLE it_zppt0002_far
  WHERE status_registro  EQ '01'
  AND   status           EQ 'W'.
...
*  AND   laehr              LE dat_ini.
  ...
  SELECT * FROM zppt0002
    APPENDING TABLE it_zppt0002_tra
  WHERE ( status_registro  EQ '07'   "TRANSFERE DEPOSITO
  OR    status_registro    EQ '09' ) "TRANSFERE DE MATERIAL
  AND   status             EQ 'W'.
...
*       AND   id_cotton  = WA_ZPPT0002_TOT-id_cotton..
  ...
  UPDATE zppt0002 SET status = 'P'
    ...
    AND ( status     EQ 'R' OR status EQ 'W' ).
...
** MONTAR MSG DE SUCESSO E ERRO fardinho
  ...
  UPDATE zppt0002 SET status = 'P'
                      status_registro = wa_return-message_v1+0(3)
   WHERE charg  EQ wa_zppt0002-charg
   AND   acharg EQ wa_zppt0002-acharg
   AND ( status EQ 'R' OR status EQ 'W' ).
...
FORM f_envia_trace.
  ...
  SELECT *
    ...
    WHERE a~flag_envio EQ 'R'
     AND a~data LE @sy-datum
     AND a~hora LE @dat_ini
  ...
  SELECT *
    FROM zppt0006 AS a
    APPENDING TABLE t_zppt0006
    WHERE a~flag_envio EQ 'W'
     AND a~id   EQ ( SELECT MAX( id )
                    FROM zppt0006
                   WHERE flag_envio EQ 'W'
                     AND charg  EQ a~charg
                     AND acharg EQ a~acharg
                 ).
    ...
    IF _ret_code EQ 200.
      ...
      LOOP AT it_zppt0006_tot INTO wa_zppt0006_tot.
        UPDATE zppt0006 SET flag_envio = 'P'
           ...
           AND  ( flag_envio EQ 'R' OR flag_envio EQ 'W' ).
      ENDLOOP.
      COMMIT WORK.
...
FORM reenvia_dados .
  ...
  SELECT SINGLE *
    FROM zppt0026
    INTO @DATA(wa_26)
    WHERE id_referencia = @wa_integracao-id_referencia
      AND dt_fim = '00000000'.
  ...
  SELECT COUNT(*)
    FROM tbtco
    WHERE jobname EQ @wa_26-jobname
      AND status IN ('R','F').  "// R Ativo, F concluido

  CHECK sy-subrc IS NOT INITIAL.

  cl_http_server=>if_http_server~get_location( IMPORTING host = host  port = port ).
  DATA(url) = |http://{ host }:{ port }/cotton/processafardo|.
  ...
  CALL METHOD http_client->request->set_header_field
    EXPORTING
      name  = 'IdReferencia'
      value = CONV #( wa_integracao-id_referencia ).
  ...
  CALL METHOD http_client->request->set_cdata
    EXPORTING
      data = CONV #( wa_integracao-ds_body ).
  ...
  http_client->response->get_status( IMPORTING code = DATA(ret_code) ).

  WRITE / 'Status Code:' && ret_code.


ENDFORM.
*&---------------------------------------------------------------------*
*&      Form  REENVIA_DADOS_ZPPT0006
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
*  -->  p1        text
...
FORM reenvia_dados_zppt0006.
  ...
  DATA: e_fardo           TYPE zpme0059,
        ...
        it_zppt0006_tot   TYPE TABLE OF zppt0006,
        it_zppt0006_tot_c TYPE TABLE OF zppt0006,
        it_zppt0002_far   TYPE TABLE OF zppt0002,
        i_inbound         TYPE zppt016,
        in_ctt            TYPE RANGE OF zppt0002-id_cotton.

  DATA: obj_trace TYPE REF TO zcl_webservice_trace.
  FREE: obj_trace.
  CREATE OBJECT: obj_trace.

  FREE: i_inbound, in_ctt.
  /ui2/cl_json=>deserialize( EXPORTING json = wa_integracao-ds_body CHANGING data = i_inbound ).

  LOOP AT i_inbound INTO DATA(w_inbound).
    APPEND VALUE #( sign = 'I' option = 'EQ' low = w_inbound-nr_fardo_origem ) TO in_ctt.
  ENDLOOP.

  CHECK in_ctt IS NOT INITIAL.
...
*       e_fardo-nr_maquina            = w_zppt0002-verid.
*       e_fardo-id_material_sap       = w_zppt0002-matnr.
      e_fardo-nr_maquina = CONV #( w_zppt0002-verid ).
      e_fardo-id_material_sap = CONV #( w_zppt0002-matnr ).
*<--- 14/06/2023 - Migração S4 - JS
      e_fardo-quantidade            = w_zppt0002-menge.
...
*<--- 14/06/2023 - Migração S4 - JS
  ...
  IF t_fardo[] IS NOT INITIAL.
    ...
    IF _ret_code EQ 200.
      ...
      LOOP AT it_zppt0006_tot INTO wa_zppt0006_tot.
        UPDATE zppt0006 SET flag_envio = 'P'
                            id_referencia = wa_integracao-id_referencia
           WHERE werks  = wa_zppt0006_tot-werks
           AND   charg  = wa_zppt0006_tot-charg
           AND   acharg = wa_zppt0006_tot-acharg
           AND   id_cotton = wa_zppt0006_tot-id_cotton
...
* Reprocessa JOB Cancelados da Area de PP
  WRITE / 'Busca de JOBs Cancelados'.

  SELECT *
    FROM tbtco
    INTO TABLE @DATA(it_job_erro)
   WHERE jobname LIKE 'JOB_COTTON_0%'
   AND status EQ 'A'
   AND sdlstrtdt EQ @sy-datum.

  IF it_job_erro IS NOT INITIAL.

    SORT it_job_erro BY jobname.
    DELETE ADJACENT DUPLICATES FROM it_job_erro COMPARING jobname.

    SELECT *
      FROM tbtco
      INTO TABLE @DATA(it_job_final)
      FOR ALL ENTRIES IN @it_job_erro
     WHERE jobname EQ @it_job_erro-jobname
      AND status EQ 'F'.

    SELECT *
      FROM tbtco
      INTO TABLE @DATA(it_job_tot)
      FOR ALL ENTRIES IN @it_job_erro
     WHERE jobname EQ @it_job_erro-jobname.

    LOOP AT it_job_erro INTO DATA(wa_job_erro).

      CHECK NOT line_exists( it_job_final[ jobname = wa_job_erro-jobname ] ).

      qtd = REDUCE #( INIT x = 0 FOR ls IN it_job_tot WHERE ( jobname = wa_job_erro-jobname ) NEXT x = x + 1 ).
      CHECK qtd < 2.

      id_referencia = wa_job_erro-jobname+11(11).

      SELECT * FROM zintegracao
        INTO TABLE it_integracao
       WHERE id_referencia EQ id_referencia
        AND id_interface EQ '021'.

      SORT it_integracao BY id_interface id_referencia.
      DELETE ADJACENT DUPLICATES FROM it_integracao COMPARING id_interface id_referencia.

      LOOP AT it_integracao INTO wa_integracao.

        PERFORM reenvia_dados_zppt0006.
        PERFORM reenvia_dados.
        PERFORM reenvia_dados_zppt0006.

      ENDLOOP.

      WRITE / 'Reenviando a Referencia: ' && id_referencia && ' Data/Hora: ' && sy-datum && sy-uzeit.
      WRITE / ' JOB ' && id_referencia && 'Processado!'.

    ENDLOOP.
  ELSE.
    WRITE / 'Sem JOBs Cancelados'.
  ENDIF.


* Reprocessa JOB Cancelados da Area de SD
  ...
  SELECT *
    FROM zsdt0261
    INTO TABLE @DATA(ir_zsdt0261).

  IF ir_zsdt0261 IS NOT INITIAL.
*    CHECK ir_zsdt0261 IS NOT INITIAL.

    SORT ir_zsdt0261 BY strtdate DESCENDING.

    DATA(data) = ir_zsdt0261[ 1 ]-strtdate.

    IF sy-batch EQ abap_true.
      TRY .
          zcl_job=>get_ck_program_execucao( EXPORTING i_nome_program = 'ZSDR0041_INI' IMPORTING e_qtd = DATA(e_qtd) ).
        CATCH zcx_job.
          e_qtd = 0.
      ENDTRY.
    ...
    IF it_job_erro IS NOT INITIAL.
      ...
      WRITE / 'Busca de JOBs Cancelados Mercado Interno'.

      name = 'MAGGI_EMAIL_NOTAS_EXPORT_PRAZO'.
      ...
      IF sy-subrc IS INITIAL.
        ...
        SUBMIT zsdr0041_ini VIA JOB name NUMBER number
                            AND RETURN.
...
FORM f_corrige_26.
  ...
  DATA: s_cotton  TYPE zrsdsselopts,
        i_inbound TYPE zppt016,
        qtd_itens TYPE int4.
  ...
  SELECT *
    ...
    WHERE qtd_item EQ 0
       OR qtd_item EQ 999999.

  LOOP AT it_0026 INTO DATA(wa_0026).

    SELECT SINGLE *
      FROM zintegracao
      INTO @DATA(wa_requisicao)
      WHERE id_referencia EQ @wa_0026-id_referencia
      AND id_interface = '021'.

    IF sy-subrc IS INITIAL.
      /ui2/cl_json=>deserialize( EXPORTING json = wa_requisicao-ds_body CHANGING data = i_inbound ).
    ...
    UPDATE zppt0026 SET qtd_item = qtd_itens
        WHERE seq EQ wa_0026-seq.
  ...
  FROM zppt0026
  INTO TABLE it_0026
  WHERE dt_done = '00000000'
    AND dt_fim >= data_corte.

  LOOP AT it_0026 INTO wa_0026.

    SELECT SINGLE *
      INTO @DATA(wa_tbtco)
      FROM tbtco
     WHERE status EQ 'F'
      AND jobname = @wa_0026-jobname.

    IF sy-subrc IS INITIAL.
      wa_0026-dt_done = wa_tbtco-enddate.
      wa_0026-hr_done = wa_tbtco-endtime.
    ENDIF.

    UPDATE zppt0026 SET dt_done = wa_0026-dt_done
                        hr_done = wa_0026-hr_done
        WHERE seq EQ wa_0026-seq.
  ...
  data_corte = |{ sy-datum(4) - 2 }0101|.
  data_corte = data_corte - 1.

  DELETE FROM zppt0006 WHERE id_cotton EQ ' '.
  DELETE FROM zppt0006 WHERE data <= data_corte.
