...
*   append own functions
  ...
  METHOD handle_link_click.
    ...
    DATA: w_alv_aux     TYPE zfis0010,
          w_node_layout TYPE lvc_s_layn,
          t_item_layout TYPE lvc_t_layi,
          l_fname       TYPE lvc_fname,
          l_node_text   TYPE lvc_value,
          l_nkey        TYPE lvc_nkey,
          l_vlr_estorno TYPE j_1btaxval.

    l_fname = fieldname.
    l_nkey  = node_key.

    CALL METHOD tree1->get_outtab_line(   "tree1
      EXPORTING
        i_node_key     = node_key "sender"ls_checked_items-nodekey
      IMPORTING
        e_outtab_line  = w_alv_aux
        e_node_text    = l_node_text
        et_item_layout = t_item_layout
        es_node_layout = w_node_layout
      EXCEPTIONS
        node_not_found = 1 ).
    ...
    CASE l_fname.
      ...
      WHEN 'VBLNR'.
        CHECK w_alv_aux-vblnr IS NOT INITIAL.
        SET PARAMETER ID 'BLN' FIELD w_alv_aux-vblnr.
        SET PARAMETER ID 'GJR' FIELD w_alv_aux-zaldt(4).
        SET PARAMETER ID 'BUK' FIELD w_alv_aux-zbukr.
        CALL TRANSACTION 'FB03'  AND SKIP FIRST SCREEN.

      WHEN 'STBLG'.
        CHECK w_alv_aux-vblnr IS NOT INITIAL.
        SET PARAMETER ID 'BLN' FIELD w_alv_aux-stblg.
        SET PARAMETER ID 'GJR' FIELD w_alv_aux-zaldt(4).
        SET PARAMETER ID 'BUK' FIELD w_alv_aux-zbukr.
        CALL TRANSACTION 'FB03'  AND SKIP FIRST SCREEN.
