...
*----armo el cuit
      ...
      CONCATENATE detail-stcd1(2)
                  detail-stcd1+2(8)
                  detail-stcd1+10(1)
                  INTO stl_bajada_per-cuit SEPARATED BY '-'.

      CASE st_bkpf-blart.
        WHEN 'DR' OR 'DA'.
          MOVE 'F' TO stl_bajada_per-t_comp.
        WHEN 'DG' OR 'D2'.
          MOVE 'C' TO stl_bajada_per-t_comp.
        WHEN 'DY'.
          MOVE 'D' TO stl_bajada_per-t_comp.
      ENDCASE.

      MOVE: st_bkpf-xblnr+4(1) TO stl_bajada_per-letra   ,
            st_bkpf-xblnr(4)   TO stl_bajada_per-nrosuc  ,
            st_bkpf-xblnr+5(8) TO stl_bajada_per-emision .
      ...
      IF sy-subrc EQ 0.
        CLEAR vl_dmbtr.
        WRITE stl_item-x_text1     TO  vl_dmbtr RIGHT-JUSTIFIED.
        stl_bajada_per-monto = vl_dmbtr+4(12).
        CLEAR vl_dmbtr.
        WRITE stl_item-x_text2     TO vl_dmbtr RIGHT-JUSTIFIED.
        stl_bajada_per-perc = vl_dmbtr+5(11).
      ENDIF.

*---Paso La fecha por dias mes año
      WRITE detail-budat TO stl_bajada_per-fecha DD/MM/YYYY.

*---Saco los espacio y completo con ceros
      SHIFT stl_bajada_per-perc   RIGHT DELETING TRAILING ' '.
      IF st_bkpf-blart = 'DG' OR st_bkpf-blart = 'D2'.
        OVERLAY stl_bajada_per-perc WITH  '-0000000000'.
      ELSE.
        OVERLAY stl_bajada_per-perc WITH  '00000000000'.
      ENDIF.

*---Saco los espacio y completo con ceros
      SHIFT stl_bajada_per-monto   RIGHT DELETING TRAILING ' '.
      IF st_bkpf-blart = 'DG' OR st_bkpf-blart = 'D2'.
        OVERLAY stl_bajada_per-monto WITH '-00000000000'.
      ELSE.
        OVERLAY stl_bajada_per-monto WITH '000000000000'.
      ENDIF.

*---si viene el decimal con coma deberia ser con punto
      DO 5 TIMES.
        REPLACE '.'   WITH ',' INTO stl_bajada_per-perc.
        REPLACE '.'   WITH ',' INTO stl_bajada_per-monto.
        REPLACE '.'   WITH '/' INTO stl_bajada_per-fecha.
      ENDDO.

      stl_bajada_per-tipo = 'A'.

      APPEND stl_bajada_per TO tl_bajada_per.
    ...
    ELSE.
      ...
      FROM t059o
      INTO stl_t059o
      WHERE wt_qscod EQ detail-qscod
      AND   land1    EQ 'AR'.

      IF sy-subrc EQ 0.
        MOVE stl_t059o-j_1anatxcd TO stl_bajada_ret-cod .
      ENDIF.
      ...
      DATA: lv_cuit TYPE lfa1-stcd1.

      IF detail-stcd1 IS NOT INITIAL.
        lv_cuit = detail-stcd1.
      ELSEIF detail-stcd2 IS NOT INITIAL.
        lv_cuit = detail-stcd2.
      ENDIF.

*----armo el cuit
      DO 5 TIMES.
        REPLACE '-'   WITH ' ' INTO lv_cuit.
        REPLACE '/'   WITH ' ' INTO lv_cuit.
      ENDDO.

      CONDENSE lv_cuit NO-GAPS.
      CONCATENATE lv_cuit(2)
                  lv_cuit+2(8)
                  lv_cuit+10(1)
...
*      OVERLAY stl_bajada_ret-ret WITH  '000000000000'.
      ...
      IF sy-subrc EQ 0.
        ...
        MOVE stl_item-date      TO stl_bajada_ret-fecha .
        CLEAR vl_dmbtr.
        WRITE stl_item-x_text2  TO vl_dmbtr RIGHT-JUSTIFIED.
        stl_bajada_ret-ret = vl_dmbtr+5(11).

      ENDIF.

      MOVE: detail-ctnum TO stl_bajada_ret-ctnum.

*---Saco los espacio y completo con ceros
      SHIFT stl_bajada_ret-ret   RIGHT DELETING TRAILING '-'.
      OVERLAY stl_bajada_ret-ret WITH  '00000000000'.

      stl_bajada_ret-tipo = 'A'.
...
FORM f_archivo_per USING sl_bajada TYPE ty_bajada_per.
  ...
  CONCATENATE
        ...
        sl_bajada-perc
        'A'
  INTO  st_bajada1-linea.
  APPEND st_bajada1 TO t_bajada1.
  CLEAR st_bajada1.
...
FORM f_archivo_ret USING sl_bajada TYPE ty_bajada_ret.
  ...
  DATA: l_linea(47) TYPE c.
  CONCATENATE sl_bajada-cuit
              sl_bajada-fecha
              '0000'
              sl_bajada-ctnum+2(8)
              sl_bajada-ret
