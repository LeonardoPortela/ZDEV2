CLASS zcl_int_ib_cons_taxa_cambio DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      ...
      END OF ty_tcurr.
    DATA:
      tb_kurst TYPE TABLE OF kurst_curr .
    DATA:
      tb_fcurr   TYPE TABLE OF  fcurr_curr .
    DATA:
      tb_tcurr  TYPE TABLE OF tcurr_curr .
    DATA:
      tb_gdatu   TYPE TABLE OF char10 .
    DATA:
    ...
    DATA:
      BEGIN OF zde_data_response,
        tcurr TYPE TABLE OF ty_tcurr,
      END OF zde_data_response .
    CONSTANTS at_id_interface TYPE zde_id_interface VALUE '129' ##NO_TEXT.
...
CLASS ZCL_INT_IB_CONS_TAXA_CAMBIO IMPLEMENTATION.
...
*TCURR_CURR
    ...
    LOOP AT lwa_data_request-kurst INTO DATA(wa_kurst).
      APPEND VALUE #( sign = 'I' option = 'EQ' low = wa_kurst ) TO lra_kurst.
    ENDLOOP.



    LOOP AT lwa_data_request-fcurr INTO DATA(wa_fcurr).
      APPEND VALUE #( sign = 'I' option = 'EQ' low = wa_fcurr ) TO lra_fcurr.
    ENDLOOP.



    LOOP AT lwa_data_request-tcurr INTO DATA(wa_tcurr).
      APPEND VALUE #( sign = 'I' option = 'EQ' low = wa_tcurr ) TO lra_tcurr.
    ENDLOOP.



    LOOP AT lwa_data_request-gdatu INTO DATA(wa_gdatu).
      CONCATENATE wa_gdatu+8(2) '.' wa_gdatu+5(2) '.' wa_gdatu(4) INTO DATA(v_data).
      ...
      APPEND VALUE #( sign = 'I' option = 'EQ' low = v_gdatu_o ) TO lra_gdatu.
    ENDLOOP.
    ...
    IF lwa_data_request IS NOT INITIAL.
      ...
      SELECT *
      FROM tcurr
      INTO TABLE @DATA(tl_tcurr)
      WHERE kurst  IN @lra_kurst AND
            fcurr  IN @lra_fcurr AND
            tcurr  IN @lra_tcurr AND
            gdatu  IN @lra_gdatu .


      LOOP AT tl_tcurr INTO DATA(w_tcurr).

        APPEND INITIAL LINE TO lwa_data_response-tcurr ASSIGNING FIELD-SYMBOL(<fs_response>).

        IF w_tcurr-gdatu IS NOT INITIAL.
          ...
          CONCATENATE  v_gdatu_u+6(4) '-' v_gdatu_u+3(2) '-' v_gdatu_u(2) INTO v_data_saida.

        ENDIF.

        <fs_response>-kurst = w_tcurr-kurst.
        <fs_response>-fcurr = w_tcurr-fcurr.
        <fs_response>-tcurr = w_tcurr-tcurr.
        <fs_response>-gdatu = v_data_saida.
        <fs_response>-ukurs = w_tcurr-ukurs.
        <fs_response>-ffact = w_tcurr-ffact.
        <fs_response>-tfact = w_tcurr-tfact.

      ENDLOOP.

      SORT lwa_data_response-tcurr BY gdatu  kurst fcurr.
