class ZCL_INT_IB_PARAM_LOC_ESTOQUE definition
  public
  final
  create public .
...
public section.
  ...
  constants AT_ID_INTERFACE type ZDE_ID_INTERFACE value '105' ##NO_TEXT.
...
CLASS ZCL_INT_IB_PARAM_LOC_ESTOQUE IMPLEMENTATION.
  ...
  method ZIF_INTEGRACAO_INBOUND~VALIDAR_DADOS_INBOUND.
    ...
    DATA: lit_data_inbound  TYPE zsdt0001le_t.
...
*-----------------------------------------------------------------------------------------------------------------------*
      ...
      CASE me->zif_integracao_inject~at_info_request_http-ds_metodo.
        WHEN C_POST.
          ...
          IF lwa_data-ds_local_entrega IS INITIAL.
            r_msg_erro = 'Ds. Local Entrega não foi informado!' && ' Index Objeto: ' && _tabix.
            RETURN.
          ENDIF.
    ...
    LOOP AT lit_data_inbound INTO DATA(lwa_data_inbound).
      ...
      CASE me->zif_integracao_inject~at_info_request_http-ds_metodo.
        WHEN 'POST'. "Inclusão/Modificação
          ...
          SELECT SINGLE *
            FROM zsdt0001le INTO @DATA(lwa_zsdt0001le)
           WHERE id_local_entrega EQ @lwa_data_inbound-id_local_entrega.

          IF SY-SUBRC EQ 0.
            lwa_data_inbound-dt_registro = lwa_zsdt0001le-dt_registro.
            lwa_data_inbound-hr_registro = lwa_zsdt0001le-hr_registro.
          ELSE.
          ...
          MODIFY zsdt0001le FROM lwa_data_inbound.
        ...
        WHEN 'DELETE'.
          ...
          DELETE FROM zsdt0001le
           WHERE id_local_entrega EQ lwa_data_inbound-id_local_entrega.

          SELECT SINGLE *
            FROM zsdt0001le INTO lwa_zsdt0001le
           WHERE id_local_entrega EQ lwa_data_inbound-id_local_entrega.
