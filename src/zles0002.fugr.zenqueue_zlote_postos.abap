FUNCTION zenqueue_zlote_postos.
*"----------------------------------------------------------------------
*"*"Interface local:
*"  IMPORTING
...
* Inicialização argumento bloqueio:
  ...
  IF NOT mandt IS INITIAL.
    MOVE mandt TO: %a_zlest0015-mandt.
  ENDIF.
...
* Fixar bloqueio:
  PERFORM send_enqueue(saplsena) TABLES __seqta_tab USING '1' __scope __wait ' ' 'EZLEST0015' _collect.
