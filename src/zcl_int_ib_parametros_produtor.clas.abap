class ZCL_INT_IB_PARAMETROS_PRODUTOR definition
  public
  final
  create public .
...
public section.
  ...
  constants AT_ID_INTERFACE type ZDE_ID_INTERFACE value '099' ##NO_TEXT.
...
CLASS ZCL_INT_IB_PARAMETROS_PRODUTOR IMPLEMENTATION.
  ...
  method ZIF_INTEGRACAO_INBOUND~VALIDAR_DADOS_INBOUND.
    ...
    DATA: lit_data_inbound TYPE zsdt0001pd_t.
...
*-----------------------------------------------------------------------------------------------------------------------*
      ...
      CASE me->zif_integracao_inject~at_info_request_http-ds_metodo.
        WHEN C_POST OR C_DELETE.
          ...
          IF lwa_data-id_produtor IS INITIAL.
            r_msg_erro = 'Id. Produtor não foi informado!' && ' Index Objeto: ' && _tabix.
            RETURN.
          ENDIF.
    ...
    LOOP AT lit_data_inbound INTO DATA(lwa_data_inbound).
      ...
      CASE me->zif_integracao_inject~at_info_request_http-ds_metodo.
        WHEN 'POST'. "Inclusão/Modificação
          ...
          SELECT SINGLE *
            FROM zsdt0001PD INTO @DATA(lwa_zsdt0001pd)
           WHERE id_produtor EQ @lwa_data_inbound-id_produtor
          ...
          IF SY-SUBRC EQ 0.
            lwa_data_inbound-dt_registro = lwa_zsdt0001pd-dt_registro.
            lwa_data_inbound-hr_registro = lwa_zsdt0001pd-hr_registro.
          ...
          MODIFY zsdt0001pd FROM lwa_data_inbound.
        ...
        WHEN 'DELETE'.
          ...
          SELECT SINGLE *
            FROM zsdt0001PD INTO lwa_zsdt0001PD
           WHERE id_produtor EQ lwa_data_inbound-id_produtor
             and nr_safra    EQ lwa_data_inbound-nr_safra
             and id_bukrs    EQ lwa_data_inbound-id_bukrs
             and id_branch   EQ lwa_data_inbound-id_branch.
