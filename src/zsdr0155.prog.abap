...
* ==================================================================== *
* Program.....: ZSDR0155                                               *
* Title.......: Envio de XML NF E-Commerce                             *
...
SELECT-OPTIONS so_doc FOR zsds090-docnum NO-EXTENSION NO INTERVALS.
PARAMETERS p_act TYPE j_1bnfe_active NO-DISPLAY.
...
FORM f_seleciona .
  ...
  DATA lv_chave TYPE j_1b_nfe_access_key_dtel44.
  DATA lv_key TYPE j_1b_nfe_access_key.

  DATA(lv_docnum) = so_doc[ 1 ]-low.

  IF p_act-docnum9 IS NOT INITIAL.
    MOVE-CORRESPONDING p_act TO lv_key.
  ENDIF.

  lv_chave = lv_key.

  IF lv_docnum IS INITIAL.
    lv_docnum = p_act-docnum.
  ENDIF.
  ...
  ELSE.
    ...
    DATA(lv_task) = 'ZENVIO_NF_ECOMMERCE_' && lv_docnum.
    ...
    PERFORM f_mensagem_insere
     TABLES gt_bapiret2
      USING 'S'
            'DS'
            '016'
            'Processo iniciado em background com ID'
            lv_task
            space space.
