class ZCL_TIP_FRETE_AQ_COBRANCA definition
  public
  final
  create public .
...
public section.
  ...
  methods VISUALIZAR_TEXTO_ARQUIVO
    importing
      !I_ARQUIVO type CHAR01 default 'C' .
  methods VISUALIZAR_LOG_ARQUIVO
    raising
      CX_SALV_MSG .
  methods SET_DS_FILENAME_PED
    importing
      !I_DS_FILENAME type /BOFU/FILE_PATH .
  PROTECTED SECTION.
private section.

  data ARQUIVO type ZLEST0139 .
  data CORPO_ARQUIVO type ZDE_LINHA_TXT_1000_T .
  data CORPO_ARQUIVO_PED type ZDE_LINHA_TXT_1000_T .
ENDCLASS.



CLASS ZCL_TIP_FRETE_AQ_COBRANCA IMPLEMENTATION.
  ...
  METHOD SET_CORPO_ARQUIVO_PED.
    ME->CORPO_ARQUIVO_PED[] = I_CORPO_ARQUIVO[].
  ENDMETHOD.
  ...
  METHOD SET_DS_FILENAME_PED.
    IF ME->ARQUIVO-DS_FILENAME_PED NE I_DS_FILENAME.
      ME->ZIF_CADASTRO~CK_ALTEROU = ABAP_TRUE.
    ENDIF.
    ME->ARQUIVO-DS_FILENAME_PED = I_DS_FILENAME.
  ENDMETHOD.
...
*  CATCH CX_SALV_MSG.    "
  ...
  METHOD VISUALIZAR_TEXTO_ARQUIVO.
    ...
    CASE I_ARQUIVO.
      WHEN 'C'.
        ...
        ENDLOOP.
      WHEN 'P'.
        LOOP AT ME->CORPO_ARQUIVO_PED INTO WA_CORPO.
          WA_LINES-TDLINE = WA_CORPO.
          APPEND WA_LINES TO IT_LINES.
        ENDLOOP.
    ENDCASE.
  ...
  METHOD ZIF_CADASTRO~EXCLUIR_REGISTRO.
    ...
    IF ME->VALIDAR_EXCLUSAO( ) EQ ABAP_TRUE.
      ...
      DELETE FROM ZLEST0139 WHERE CD_IMPORTACAO EQ ME->ARQUIVO-CD_IMPORTACAO.
      COMMIT WORK.
  ...
  METHOD ZIF_CADASTRO~GRAVAR_REGISTRO.
    ...
    IF ME->CK_ALTEROU EQ ABAP_TRUE.
      ...
      IF ME->VALIDAR_REGISTRO( ) EQ ABAP_TRUE.
        ...
        MODIFY ZLEST0139 FROM ME->ARQUIVO.

        "Grava Arquivo de Cobrança
        CLEAR: IT_TLINES[].
        ...
        CLEAR: IT_TLINES, IT_TLINES[].
        LOOP AT ME->CORPO_ARQUIVO_PED INTO WA_TEXTO.
          CLEAR: WA_TLINES.
          MOVE WA_TEXTO TO WA_TLINES-TDLINE.
          APPEND WA_TLINES TO IT_TLINES.
        ENDLOOP.
  ...
  METHOD ZIF_CADASTRO~SET_REGISTRO.
    ...
    SELECT SINGLE * INTO ME->ARQUIVO FROM ZLEST0139 WHERE CD_IMPORTACAO EQ I_ID_REGISTRO.
    ...
    IF SY-SUBRC IS INITIAL.
      ...
      IF SY-SUBRC IS INITIAL.
        LOOP AT IT_TLINES INTO WA_LINE.
          APPEND WA_LINE-TDLINE TO ME->CORPO_ARQUIVO_PED.
        ENDLOOP.
      ENDIF.
  ...
  METHOD ZIF_PESQUISA~PESQUISAR.
    ...
    SELECT * INTO TABLE @DATA(IT_ZLEST0139)
      FROM ZLEST0139
     WHERE BUKRS       IN @LC_FILTRO-BUKRS
       AND BRANCH      IN @LC_FILTRO-BRANCH
       AND NR_LOTE_ADM IN @LC_FILTRO-NR_LOTE_ADM.

    CHECK SY-SUBRC IS INITIAL.

    MOVE IT_ZLEST0139[] TO LC_RETORNO[].
