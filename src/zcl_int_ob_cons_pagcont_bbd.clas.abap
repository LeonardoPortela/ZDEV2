class ZCL_INT_OB_CONS_PAGCONT_BBD definition
  public
  final
  create public .
...
public section.
  ...
  constants AT_ID_INTERFACE type ZDE_ID_INTERFACE value '309' ##NO_TEXT.
  data AT_IDTRANSACAO type STRING .
  data AT_AGENCIA type STRING .
  data AT_CONTA type STRING .
  data AT_TP_CONTA type STRING .
...
CLASS ZCL_INT_OB_CONS_PAGCONT_BBD IMPLEMENTATION.
...
*    ENDLOOP.
  ...
  METHOD ZIF_INTEGRACAO_OUTBOUND~BUILD_INFO_REQUEST.
    ...
    SPLIT i_info_request AT ';' INTO  AT_IDTRANSACAO
                                      AT_AGENCIA
                                      AT_CONTA
                                      AT_TP_CONTA.
  ENDMETHOD.
...
*    TRANSLATE e_data to LOWER CASE.
  ...
  METHOD zif_integracao_outbound~get_instance.
    ...
    IF zif_integracao_outbound~at_if_integracao_outbound IS NOT BOUND.
      CREATE OBJECT zif_integracao_outbound~at_if_integracao_outbound TYPE zcl_int_ob_cons_pagcont_bbd.
    ENDIF.
...
*    APPEND VALUE #( name = 'accept'   value = 'application/json' ) TO LC_INTEGRAR~at_.
  ...
  METHOD zif_integracao_outbound~set_url.
    ...
    SELECT SINGLE *
     ...
     WHERE service = 'BBD_PAGCONTAS_CONSULTA_CONS_TRIB'.
      ...
      RAISE EXCEPTION TYPE zcx_integracao
        EXPORTING
          textid = VALUE #( msgid = zcx_integracao=>zcx_erro_geral-msgid
                            ...
                            attr2 = 'CONSULTA_COMPROVANTES_PAGCONTAS' )
          msgid  = zcx_integracao=>zcx_erro_geral-msgid
          msgno  = zcx_integracao=>zcx_erro_geral-msgno
          msgty  = 'E'
          msgv1  = 'Serviço não configurado: '
          msgv2  = 'CONSULTA_COMPROVANTES_PAGCONTAS'.
    ENDIF.


    "https://openapisandbox.prebanco.com.br/pagamento/arrecadacao-via-codbarras/v1/"

    CONCATENATE sy-datum(4) '-' sy-datum+4(2) '-' sy-datum+6(2) INTO DATA(lv_data).

    CONCATENATE lwa_webservice-url  me->at_agencia  me->at_conta me->at_tp_conta '?' '&segmentoConsulta=99'
                 '&dataInicial=' lv_data '&dataFinal=' lv_data '&idTransacao=' me->at_idtransacao INTO lva_url.
