...
**/===========================================================================\*
"""""""""""""" PROGRAMA COPIA PARA ANALISARMOS O NOVO DEV COM O ANTIGO ****************
" FALAR COM: SAMUEL CABANA OU RAMON LIMA - ABAP.
...
REPORT zsdr0119_copy2 MESSAGE-ID zjob.
...
"PARAMETERS p_teste AS CHECKBOX." #debug ramon
...
FORM montar_htm USING tipo.
  ...
  LOOP AT it_saida INTO DATA(_saida).
      ...
      WHEN: 'S'.
        LOOP AT _0055 INTO DATA(w_0055).
          ...
          ELSE.
            ...
            CASE w_0055-zieme.
              WHEN 'KG'. DIVIDE   mon BY 1000.
              WHEN 'TO'. MULTIPLY mon BY 1000.
                " 19.01.2024 - 131275 - RBL -->
              WHEN 'L'. DIVIDE   mon BY 1000.
              WHEN 'M3'.MULTIPLY mon BY 1000.
                " 19.01.2024 - 131275 - RBL --<
            ENDCASE.
...
*     "// Regra para ñ calcular a Troca caso esteja na mesma OV pq isso ñ dispara o Hedge
      ...
      IF wa_0090-categoria EQ 'C'.
        ...
        " 24.05.2024 - RAMON ----->
        " VERSÃO NOVA DE PREENCHIMENTO DOS VALORES, FOI ALTERADO PQ FOI AGRUPADO PELO SPART NA 59, ENTÃO PRECISA DO MESMO AGRUPAMENTO PARA A VERIFICAÇÃO DA TRAVA DE CAMBIO ---->>

        "BREAK rblima.
        PERFORM f_soma_trava_cambio
          USING wa_0090-doc_simulacao
                wa_0090-vbelv
                i_spart
       CHANGING vl_brgew
                vl_netwr
        ...
        IF wa_0090-prev_pgto_usd IS NOT INITIAL.
          vl_netwr = wa_0090-prev_pgto_usd.
        ENDIF.

***        " 24.05.2024 - RAMON - 141604 -->
***        vl_brgew = REDUCE #( INIT x = vl_brgew FOR ls1 IN it_vbap WHERE ( vbeln =  wa_vbak-vbeln AND lifsp <> '12' ) NEXT x = x + ls1-brgew ).
***        "vl_brgew = REDUCE #( INIT x = vl_brgew FOR ls1 IN it_vbap WHERE ( vbeln =  wa_vbak-vbeln ) NEXT x = x + ls1-brgew ).
***        " 24.05.2024 - RAMON - 141604 -->
***
***        " 13.05.2024 - RAMON - Se nova trava de cambio, o total do hedge tem que ser o que foi disparado na zsdt0090
***        " então, para saber se é a trava de cambio nova, é verificado o campo PREV_PGTO_USD, que só existe para a nova forma
***        IF wa_0090-prev_pgto_usd IS NOT INITIAL.
***          vl_netwr = wa_0090-prev_pgto_usd.
***        ELSE.
***
***          " 24.05.2024 - RAMON - 141604 -->
***          vl_netwr = REDUCE #( INIT y = vl_netwr FOR ls2 IN it_vbap WHERE ( vbeln =  wa_vbak-vbeln AND lifsp <> '12' ) NEXT y = y + ls2-netwr ).
...
***
***        " 24.05.2024 - RAMON - 141604 -->
***        vl_mwsbp = REDUCE #( INIT z = vl_mwsbp FOR ls3 IN it_vbap WHERE ( vbeln =  wa_vbak-vbeln AND lifsp <> '12' ) NEXT z = z + ls3-mwsbp ).
