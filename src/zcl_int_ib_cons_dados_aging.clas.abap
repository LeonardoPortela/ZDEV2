CLASS zcl_int_ib_cons_dados_aging DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    DATA:
      tb_t_doc_venda TYPE TABLE OF vbeln_va.
    DATA:
      tb_t_doc_simulacao TYPE TABLE OF zsded003 .
    DATA:
      tb_lifnr TYPE TABLE OF lifnr .
    DATA:
      tb_kunnr TYPE TABLE OF kunnr.
    DATA:
      tb_bukrs TYPE TABLE OF bukrs .

    DATA:
      BEGIN OF zde_data_request,
        i_tp_proc       TYPE char01,
        i_zfbdt_ini     TYPE datum,
        i_zfbdt_fim     TYPE datum,
        t_bukrs         LIKE tb_bukrs,
        t_kunnr         LIKE tb_kunnr,
        t_lifnr         LIKE tb_lifnr,
        t_doc_simulacao LIKE tb_t_doc_simulacao,
        t_doc_venda     LIKE tb_t_doc_venda,
      END OF zde_data_request .
    DATA:
      BEGIN OF zde_data_response,
        t_saida TYPE TABLE OF zaging_siacorp,
      END OF zde_data_response .
    CONSTANTS at_id_interface TYPE zde_id_interface VALUE '198' ##NO_TEXT.
...
CLASS ZCL_INT_IB_CONS_DADOS_AGING IMPLEMENTATION.
...
*-----------------------------------------------------------------------------------------------------------------------*
  ...
  METHOD zif_integracao_inject~set_integrar_inbound.
    ...
    DATA:
      ...
      lt_bukrs         TYPE zfist0001,
      lt_kunnr         TYPE  zfist0001_1,
      lt_lifnr         TYPE  zfist0001_2,
      lt_doc_simulacao TYPE  zfist0001_3,
      lt_doc_venda     TYPE  zfist0001_4,
      lt_saida         TYPE zfist0002.
...
*    ENDLOOP.
    ...
    IF lwa_data_request IS NOT INITIAL.
      ...
      lt_bukrs         = lwa_data_request-t_bukrs        .
      lt_kunnr         = lwa_data_request-t_kunnr        .
      lt_lifnr         = lwa_data_request-t_lifnr        .
      lt_doc_simulacao = lwa_data_request-t_doc_simulacao.
      lt_doc_venda     = lwa_data_request-t_doc_venda    .

      CALL FUNCTION 'ZFI_GET_DATA_AGING'
        EXPORTING
          i_tp_proc       = lwa_data_request-i_tp_proc
          i_zfbdt_ini     = lwa_data_request-i_zfbdt_ini
          i_zfbdt_fim     = lwa_data_request-i_zfbdt_fim
        TABLES
          t_bukrs         = lt_bukrs
          t_kunnr         = lt_kunnr
          t_lifnr         = lt_lifnr
          t_doc_simulacao = lt_doc_simulacao
          t_doc_venda     = lt_doc_venda
          t_saida         = lt_saida.

      lwa_data_response-t_saida = lt_saida.
