CLASS ZESZCL_APP_APROV_LANC_MANUAL DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES: BEGIN OF ty_filter,
             ...
             status  TYPE RANGE OF zfi_lotes_imp-status,
           END OF ty_filter.
  ...
  PRIVATE SECTION.
    ...
    METHODS get_paging
      ...
      CHANGING  ct_report   TYPE zfitt_lotes_imp.
ENDCLASS.



CLASS ZESZCL_APP_APROV_LANC_MANUAL IMPLEMENTATION.
  METHOD if_rap_query_provider~select.

    DATA lt_itens TYPE TABLE OF ZESZSFI_APROV_LANCMANUAL.
...
*        lv_exp_msg = lo_ex_filter->get_longtext( ).
    ...
    TRY.
        CALL FUNCTION 'ZSVPL_FCN_GETWORKLIST07_APP'
          EXPORTING
            in_usuario = sy-uname
          IMPORTING
            e_t_lotes  = lt_itens.
...
* ---------------------------------------------------------------------------
    ...
    IF it_filters IS NOT INITIAL.
      ...
      ENDTRY.
      TRY.
          gs_filter-status[] = CORRESPONDING #( it_filters[ name = 'STATUS' ]-range ). "#EC CI_STDSEQ
        CATCH cx_root INTO lo_root.
          lv_exp_msg = lo_root->get_longtext( ).
      ENDTRY.
...
*      gs_filter-param_puanos[]        = CORRESPONDING #( it_puanos[] ).
  ...
  METHOD get_paging.
    ...
    DATA lt_itens TYPE SORTED TABLE OF ZESZSFI_APROV_LANCMANUAL WITH NON-UNIQUE KEY lote.
