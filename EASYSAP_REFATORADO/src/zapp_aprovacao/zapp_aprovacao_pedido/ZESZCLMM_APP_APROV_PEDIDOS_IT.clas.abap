CLASS ZESZCLMM_APP_APROV_PEDIDOS_IT DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES: BEGIN OF ty_filter,
             filters TYPE string,
             empresa TYPE RANGE OF ZESZSMM_APROV_PEDIDOS_ITEM-bukrs,
             ebeln   TYPE RANGE OF ZESZSMM_APROV_PEDIDOS_ITEM-ebeln,
             ebelp   TYPE RANGE OF ZESZSMM_APROV_PEDIDOS_ITEM-ebelp,
           END OF ty_filter.
  ...
  PRIVATE SECTION.
    ...
    METHODS get_paging
      ...
      CHANGING  ct_report   TYPE zmmtt_aprov_pedidos_it.
ENDCLASS.



CLASS ZESZCLMM_APP_APROV_PEDIDOS_IT IMPLEMENTATION.
  METHOD if_rap_query_provider~select.

    DATA lt_itens TYPE TABLE OF ZESZSMM_APROV_PEDIDOS_ITEM.
    ...
    TRY.
        ...
        IF gs_filter IS NOT INITIAL.
          ...
          IF gs_filter-empresa IS NOT INITIAL.
            DELETE lt_itens WHERE bukrs NOT IN gs_filter-empresa.
          ENDIF.
...
** ---------------------------------------------------------------------------
  ...
  METHOD get_paging.
    ...
    DATA lt_itens TYPE SORTED TABLE OF ZESZSMM_APROV_PEDIDOS_ITEM WITH NON-UNIQUE KEY sww_wiid ebeln ebelp.
...
* ---------------------------------------------------------------------------
    ...
    IF it_filters IS NOT INITIAL.
      ...
      TRY.
          gs_filter-ebelp[] = CORRESPONDING #( it_filters[ name = 'EBELP' ]-range ). "#EC CI_STDSEQ
        CATCH cx_root INTO lo_root.
          lv_exp_msg = lo_root->get_longtext( ).
      ENDTRY.
