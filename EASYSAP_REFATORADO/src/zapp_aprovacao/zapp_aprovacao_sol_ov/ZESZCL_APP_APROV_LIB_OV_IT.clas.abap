CLASS ZESZCL_APP_APROV_LIB_OV_IT DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      BEGIN OF ty_filter,
        filters  TYPE string,
        nrosolov TYPE RANGE OF ZESZSSD_APROV_PAGTO_OV_DOC-nrosolov,
        item     TYPE RANGE OF ZESZSSD_APROV_PAGTO_OV_DOC-item,
      END OF ty_filter .
  ...
  PRIVATE SECTION.
    ...
    METHODS get_paging
      ...
      CHANGING  ct_report   TYPE zsdtt_aprov_pagto_doc.

ENDCLASS.

CLASS ZESZCL_APP_APROV_LIB_OV_IT IMPLEMENTATION.

  METHOD if_rap_query_provider~select.

    DATA lt_itens TYPE zsdtt_aprov_pagto_doc.
...
*        lv_exp_msg = lo_ex_filter->get_longtext( ).
    ...
    TRY.
        CALL FUNCTION 'ZSVPL_FCN_GETWORKLIST12_APP'
          EXPORTING
            in_usuario = sy-uname
          IMPORTING
            et_docs    = lt_itens.
        ...
        IF gs_filter IS NOT INITIAL.
          ...
          IF gs_filter-nrosolov IS NOT INITIAL.
            DELETE lt_itens WHERE nrosolov NOT IN gs_filter-nrosolov.
          ENDIF.
...
* ---------------------------------------------------------------------------
    ...
    IF it_filters IS NOT INITIAL.
      ...
      TRY.
          gs_filter-nrosolov   = CORRESPONDING #( it_filters[ name = 'NROSOLOV' ]-range ). "#EC CI_STDSEQ
        CATCH cx_root INTO DATA(lo_root).
          DATA(lv_exp_msg) = lo_root->get_longtext( ).
      ENDTRY.

      TRY.
          gs_filter-item   = CORRESPONDING #( it_filters[ name = 'ITEM' ]-range ). "#EC CI_STDSEQ
  ...
  METHOD get_paging.
    ...
    DATA lt_itens TYPE SORTED TABLE OF ZESZSSD_APROV_PAGTO_OV_DOC WITH NON-UNIQUE KEY nrosolov item .
