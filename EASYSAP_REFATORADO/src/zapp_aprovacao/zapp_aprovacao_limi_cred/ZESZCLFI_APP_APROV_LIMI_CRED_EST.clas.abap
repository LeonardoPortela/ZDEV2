CLASS ZESZCLFI_APP_APROV_LIMI_CRED_EST DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES: BEGIN OF ty_filter,
             filters   TYPE string,
             lote      TYPE RANGE OF ZESZSFI_APROV_LIM_CRED_DOC_EST-lote,
             nivel     TYPE RANGE OF ZESZSFI_APROV_LIM_CRED_DOC_EST-nivel,
             aprovador TYPE RANGE OF ZESZSFI_APROV_LIM_CRED_DOC_EST-aprovador,
           END OF ty_filter.
  PROTECTED SECTION.
  PRIVATE SECTION.
    ...
    METHODS get_paging
      ...
      CHANGING  ct_report   TYPE zfitt_aprov_limcred_est.

ENDCLASS.



CLASS ZESZCLFI_APP_APROV_LIMI_CRED_EST IMPLEMENTATION.

  METHOD get_paging.

    DATA lt_itens TYPE SORTED TABLE OF ZESZSFI_APROV_LIM_CRED_DOC_EST WITH NON-UNIQUE KEY lote nivel.
...
* ----------------------------------------------------------------------
  ...
  METHOD if_rap_query_provider~select.
    ...
    DATA lt_itens TYPE  zfitt_aprov_limcred_est.
...
*        lv_exp_msg = lo_ex_filter->get_longtext( ).
    ...
    TRY.
        CALL FUNCTION 'ZSVPL_FCN_GETWORKLIST10_APP'
          EXPORTING
            in_usuario = sy-uname
          IMPORTING
            et_estra   = lt_itens.
        ...
        IF gs_filter IS NOT INITIAL.
          ...
          IF gs_filter-aprovador IS NOT INITIAL.
            DELETE lt_itens WHERE aprovador NOT IN gs_filter-aprovador.
          ENDIF.
...
* ---------------------------------------------------------------------------
    ...
    IF it_filters IS NOT INITIAL.
      ...
      TRY.
          gs_filter-aprovador[] = CORRESPONDING #( it_filters[ name = 'APROVADOR' ]-range ). "#EC CI_STDSEQ
        CATCH cx_root INTO lo_root.
          lv_exp_msg = lo_root->get_longtext( ).
      ENDTRY.
