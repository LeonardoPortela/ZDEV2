CLASS ZESZCL_APP_APROV_LIMI_CRED DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      BEGIN OF ty_filter,
        filters TYPE string,
        lote    TYPE RANGE OF zfi_lotes_imp-lote,
        kunnr   TYPE RANGE OF zsd_roma_imp-kunnr,
      END OF ty_filter .
  ...
  PRIVATE SECTION.
    ...
    METHODS get_paging
      ...
      CHANGING  ct_report   TYPE zfitt_aprov_limcred_lote.

ENDCLASS.

CLASS ZESZCL_APP_APROV_LIMI_CRED IMPLEMENTATION.

  METHOD if_rap_query_provider~select.

    DATA lt_itens TYPE zfitt_aprov_limcred_lote.
...
*        lv_exp_msg = lo_ex_filter->get_longtext( ).
    ...
    TRY.
        ...
        IF gs_filter IS NOT INITIAL.
          ...
          IF gs_filter-kunnr IS NOT INITIAL.
            DELETE lt_itens WHERE cliente NOT IN gs_filter-kunnr.
          ENDIF.
...
* ---------------------------------------------------------------------------
    ...
    IF it_filters IS NOT INITIAL.
      ...
      ENDTRY.
      TRY.
          gs_filter-kunnr[] = CORRESPONDING #( it_filters[ name = 'CLIENTE' ]-range ). "#EC CI_STDSEQ
...
*      gs_filter-param_puanos[]        = CORRESPONDING #( it_puanos[] ).
  ...
  METHOD get_paging.
    ...
    DATA lt_itens TYPE SORTED TABLE OF ZESZSFI_APROV_LIM_CRED_LOTE WITH NON-UNIQUE KEY lote.
