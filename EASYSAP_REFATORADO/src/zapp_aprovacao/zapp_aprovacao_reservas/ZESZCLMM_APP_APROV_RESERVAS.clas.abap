CLASS ZESZCLMM_APP_APROV_RESERVAS DEFINITION
...
PUBLIC
  ...
  PUBLIC SECTION.
    ...
    TYPES: BEGIN OF ty_filter,
             filters TYPE string,
             rsnum   TYPE RANGE OF ZESZSMM_APROV_REMESSAS-rsnum,
             rspos   TYPE RANGE OF ZESZSMM_APROV_REMESSAS-rspos,
             matnr   TYPE RANGE OF ZESZSMM_APROV_REMESSAS-matnr,
             werks   TYPE RANGE OF ZESZSMM_APROV_REMESSAS-werks,
           END OF ty_filter.
  ...
  PRIVATE SECTION.
    ...
    METHODS get_paging
      ...
      CHANGING  ct_report   TYPE zmmtt_aprov_reserva.
ENDCLASS.



CLASS ZESZCLMM_APP_APROV_RESERVAS IMPLEMENTATION.

  METHOD if_rap_query_provider~select.

    DATA lt_itens TYPE TABLE OF ZESZSMM_APROV_REMESSAS.
...
*        lv_exp_msg = lo_ex_filter->get_longtext( ).
    ...
    TRY.
        CALL FUNCTION 'ZSVPL_FCN_GETWORKLIST16_APP'
          IMPORTING
            et_item = lt_itens.

        IF gs_filter IS NOT INITIAL.

          IF gs_filter-rsnum IS NOT INITIAL.
            DELETE lt_itens WHERE rsnum NOT IN gs_filter-rsnum.
          ENDIF.

          IF gs_filter-matnr IS NOT INITIAL.
            DELETE lt_itens WHERE matnr NOT IN gs_filter-matnr.
          ENDIF.

          IF gs_filter-rspos IS NOT INITIAL.
            DELETE lt_itens WHERE rspos NOT IN gs_filter-rspos.
          ENDIF.
...
* ---------------------------------------------------------------------------
    ...
    IF it_filters IS NOT INITIAL.
      ...
      TRY.
          gs_filter-rsnum   = CORRESPONDING #( it_filters[ name = 'RSNUM' ]-range ). "#EC CI_STDSEQ
        CATCH cx_root INTO DATA(lo_root).
          DATA(lv_exp_msg) = lo_root->get_longtext( ).
      ENDTRY.
      TRY.
          gs_filter-rspos[] = CORRESPONDING #( it_filters[ name = 'RSPOS' ]-range ). "#EC CI_STDSEQ
        CATCH cx_root INTO lo_root.
          lv_exp_msg = lo_root->get_longtext( ).
      ENDTRY.
      TRY.
          gs_filter-matnr[] = CORRESPONDING #( it_filters[ name = 'MATNR' ]-range ). "#EC CI_STDSEQ
        CATCH cx_root INTO lo_root.
          lv_exp_msg = lo_root->get_longtext( ).
      ENDTRY.
  ...
  METHOD get_paging.
    ...
    DATA lt_itens TYPE SORTED TABLE OF ZESZSMM_APROV_REMESSAS WITH NON-UNIQUE KEY rsnum rspos.
