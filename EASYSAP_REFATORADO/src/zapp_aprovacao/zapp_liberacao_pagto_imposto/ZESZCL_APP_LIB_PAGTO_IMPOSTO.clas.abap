CLASS ZESZCL_APP_LIB_PAGTO_IMPOSTO DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES: BEGIN OF ty_filter,
             filters TYPE string,
             lote    TYPE RANGE OF ZESZSFI_LIB_PAGTO_IMPOSTO_LOTE-lote,
             bukrs   TYPE RANGE OF ZESZSFI_LIB_PAGTO_IMPOSTO_LOTE-bukrs,
           END OF ty_filter.
  ...
  PRIVATE SECTION.
    ...
    METHODS get_paging
      ...
      CHANGING  ct_report   TYPE zfitt_lib_pagto_imposto_lote.

ENDCLASS.



CLASS ZESZCL_APP_LIB_PAGTO_IMPOSTO IMPLEMENTATION.

  METHOD if_rap_query_provider~select.


    DATA lt_itens TYPE zfitt_lib_pagto_imposto_lote.
...
*        lv_exp_msg = lo_ex_filter->get_longtext( ).
    ...
    TRY.
        ...
        DATA: lv_lote  TYPE ZESZSFI_LIB_PAGTO_IMPOSTO_LOTE-lote,
              lv_bukrs TYPE ZESZSFI_LIB_PAGTO_IMPOSTO_LOTE-bukrs.

        lv_lote = gs_filter-lote[ 1 ]-low.
        lv_bukrs = gs_filter-bukrs[ 1 ]-low.

        CALL FUNCTION 'ZSVPL_FCN_GETWORKLIST13_APP'
          EXPORTING
            in_empresa = lv_bukrs
            in_lote    = lv_lote
          IMPORTING
        ...
        IF gs_filter IS NOT INITIAL.
          ...
          IF gs_filter-bukrs IS NOT INITIAL.
            DELETE lt_itens WHERE bukrs NOT IN gs_filter-bukrs.
          ENDIF.
...
* ---------------------------------------------------------------------------
    ...
    IF it_filters IS NOT INITIAL.
      ...
      ENDTRY.
      TRY.
          gs_filter-bukrs[] = CORRESPONDING #( it_filters[ name = 'BUKRS' ]-range ). "#EC CI_STDSEQ
...
*      gs_filter-param_puanos[]        = CORRESPONDING #( it_puanos[] ).
  ...
  METHOD get_paging.
    ...
    DATA lt_itens TYPE SORTED TABLE OF ZESZSFI_LIB_PAGTO_IMPOSTO_LOTE WITH NON-UNIQUE KEY bukrs lote.
