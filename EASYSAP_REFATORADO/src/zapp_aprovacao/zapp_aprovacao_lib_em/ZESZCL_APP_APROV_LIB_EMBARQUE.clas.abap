CLASS ZESZCL_APP_APROV_LIB_EMBARQUE DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      BEGIN OF ty_filter,
        filters   TYPE string,
        org_venda TYPE RANGE OF ZESZSSD_APROV_LIB_EMB_OV-orgvenda,
        vbeln     TYPE RANGE OF ZESZSSD_APROV_LIB_EMB_OV-vbeln,
        seq       TYPE RANGE OF ZESZSSD_APROV_LIB_EMB_OV-seq,
        esc_venda TYPE RANGE OF ZESZSSD_APROV_LIB_EMB_OV-esc_venda,
      END OF ty_filter .
  ...
  PRIVATE SECTION.
    ...
    METHODS get_paging
      ...
      CHANGING  ct_report   TYPE zsdtt_aprov_lib_emb_ov.

ENDCLASS.

CLASS ZESZCL_APP_APROV_LIB_EMBARQUE IMPLEMENTATION.

  METHOD if_rap_query_provider~select.

    DATA lt_itens TYPE zsdtt_aprov_lib_emb_ov.
...
*        lv_exp_msg = lo_ex_filter->get_longtext( ).
    ...
    TRY.
        ...
        IF gs_filter IS NOT INITIAL.
          ...
          IF gs_filter-org_venda IS NOT INITIAL.
            DELETE lt_itens WHERE orgvenda NOT IN gs_filter-org_venda.
          ENDIF.
...
* ---------------------------------------------------------------------------
    ...
    IF it_filters IS NOT INITIAL.
      ...
      TRY.
          gs_filter-org_venda   = CORRESPONDING #( it_filters[ name = 'ORGVENDA' ]-range ). "#EC CI_STDSEQ
...
*      gs_filter-param_puanos[]        = CORRESPONDING #( it_puanos[] ).
  ...
  METHOD get_paging.
    ...
    DATA lt_itens TYPE SORTED TABLE OF ZESZSSD_APROV_LIB_EMB_OV
            WITH NON-UNIQUE KEY orgvenda vbeln esc_venda seq.
...
* ----------------------------------------------------------------------
    ...
    SELECT * FROM @lt_itens AS dados
         ORDER BY orgvenda, vbeln, esc_venda, seq
