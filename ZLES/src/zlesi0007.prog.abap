...
*              Etapa 3 Ferro - Terminal - Confirmação de chegada       *
* Prefixo    : L3                                                      *
...
REPORT  zlesi0007 MESSAGE-ID zles.
*----------------------------------------------------------------------*
* Tabelas                                                              *
*----------------------------------------------------------------------*
TABLES:
...
DATA: st_files_doc     TYPE sdokpath,
      ...
      st_zlest0088_sel TYPE zlest0088,
...
CONSTANTS: c_x            TYPE c VALUE 'X',
           ...
           c_l3(2)        TYPE c VALUE 'L3',      " Confirmação de saída Ferroviário
...
* Tipo de registro 20
    ELSEIF t_file-linha(2) = '20'.
      ...
      st_reg20-idvagao      = t_file-linha+2(10).
      st_reg20-dcl          = t_file-linha+12(10).
...
* Filial
    ...
    IF t_branch IS NOT INITIAL OR st_zlest0041 IS NOT INITIAL.
...
*              MODIFY T_REG30 FROM ST_REG30 TRANSPORTING ERRO WHERE TABIX = ST_REG30-TABIX.
            ...
            ELSE.
              ...
              SELECT SINGLE bukrs
                INTO vl_bukrs
                FROM j_1bbranch
               WHERE  branch = st_zlest0041-centro_comprador.


              lt_j_1bnfdoc-bukrs  = vl_bukrs.
              lt_j_1bnfdoc-branch = st_zlest0041-centro_comprador.
...
* Dados de nota fiscal
  ...
  LOOP AT t_reg30a INTO st_reg30_2.
    ...
    DATA(lc_cpf_cnpj) = st_reg30_2-cnpj.
    ...
    IF NOT sy-subrc IS INITIAL.
      ...
      IF NOT sy-subrc IS INITIAL.
        ...
        ELSE.
          ...
          MODIFY t_reg30 FROM st_reg30 TRANSPORTING nf docnum bukrs branch nr_nf_terceiro cod_fornecedor pesonota
           WHERE nf   = vl_nfe_terc
             AND tipo = st_reg30-tipo
             AND cnpj = st_reg30-cnpj.
        ENDIF.
...
FORM consiste_detalhe_arquivo  USING p1_file
  ...
  DATA: vl_tabix     TYPE sy-tabix,
        ...
        vl_sem_l2    TYPE c LENGTH 1,
        vl_ocorr19   TYPE sy-tabix,
        vl_ocorr88   TYPE sy-tabix,
        fg_88(1).


  REFRESH: t_zlest0019, t_idxerro,t_zlest0088.
  CLEAR:   t_zlest0019, vl_tabix, vl_name1, vl_bukrs,vl_branch,vl_nfnum,vl_nfenum, vl_ctrl,vl_id_refkey, vl_cnpjferro,vl_ocorr19,vl_ocorr88,fg_88.
  ...
  LOOP AT t_reg30 INTO st_reg30.
    ...
    IF st_reg30-erro NE 'T'."Terceiros com erro
      ...
      IF vl_ocorr19 = 1.
        ...
        CONCATENATE st_reg10-empresa(20) vl_id_refkey INTO st_zlest0019-chave SEPARATED BY space.
      ...
      IF sy-subrc IS INITIAL.
        ...
        IF st_zlest0019-cnpjferro IS INITIAL.
          CONCATENATE 'CNPJ Ferro não informado para o dcl : ' st_reg20-dcl INTO v_mensagem.

          "MESSAGE 'CNPJ Ferro não informado para o dcl : ' INTO v_mensagem.
          PERFORM envia_mensagem_procto  USING p1_file
                                               c_e
                                               '999'
                                               v_mensagem.
    ...
    CLEAR: vl_bukrs, vl_branch, vl_nfenum, vl_nfnum, vl_nfenuma, vl_nfnuma, fg_88.
...
*    WHERE stcd1 = st_reg30-cnpj.
    ...
    SELECT bukrs branch stcd1
      FROM j_1bbranch INTO TABLE t_branch_aux
     WHERE stcd1 = st_reg30-cnpj.

    IF t_branch_aux[] IS NOT INITIAL.
      "Conforme chamado quando encontrar 2 branch ignorar o 0001
      IF lines( t_branch_aux[] ) > 1.
        DELETE t_branch_aux WHERE branch EQ '0001'.
      ENDIF.

      READ TABLE t_branch_aux INTO st_branch_aux WITH KEY stcd1 = st_reg30-cnpj.
      IF sy-subrc = 0.
        vl_bukrs =  st_branch_aux-bukrs.
        vl_branch = st_branch_aux-branch.
      ENDIF.
    ENDIF.
...
* Documento de nota fiscal eletrônica
    ...
    IF NOT sy-subrc IS INITIAL.
      ...
      IF sy-subrc IS INITIAL.
        ...
        IF NOT sy-subrc IS INITIAL.
          MESSAGE i000(z01) WITH 'Etapa L2 não encontrada para a NFe AMAGGI : ' vl_nfenum
      ...
      ELSE.
        ...
        ELSEIF vl_nfenum IS INITIAL.
          MESSAGE i000(z01) WITH 'Etapa L2 não encontrada para a NFe : ' vl_nfenuma.
          vl_sem_l2 = 'X'.
        ELSE.
          MESSAGE i000(z01) WITH 'Etapa L2 / DE - PARA não encontrada para a NFe de Terceiro : ' vl_nfenuma.
          vl_sem_l2 = 'X'.
        ENDIF.
      ENDIF.
    ...
    IF fg_88 = 'X'.
      ...
      IF vl_ocorr88 = 1.
        "REG10
        CLEAR st_zlest0088.
        st_zlest0088-idinter          = c_l3.
        st_zlest0088-tp_movi          = c_e.
        ...
        CONCATENATE st_reg10-empresa(20) vl_id_refkey INTO st_zlest0088-chave SEPARATED BY space.
      ...
      st_zlest0088-dcl             = st_reg20-dcl.
      st_zlest0088-seriedcl        = st_reg20-seriedcl.
      st_zlest0088-bukrs           = st_zlest0088_sel-bukrs.
      st_zlest0088-werks           = st_zlest0088_sel-werks.
      st_zlest0088-nr_nf           = vl_nfenuma.
      st_zlest0088-serie           = st_zlest0088_sel-serie.
      ...
      SELECT SINGLE *
       ...
       AND   bukrs   = st_zlest0088_sel-bukrs.
      IF sy-subrc = 0.
        st_zlest0088-produto          = st_zlest0089-matnr.
      ENDIF.

      CLEAR vl_nfnum.
      CONCATENATE st_zlest0088-bukrs '-' st_zlest0088-werks '-' vl_nfenuma vl_nfnum INTO st_zlest0088-chave SEPARATED BY space.
      CONDENSE st_zlest0088-chave NO-GAPS.
...
*      IF sy-subrc = 0. "161420 IR207175 - avaliar job ATUALIZA_COMP_J2 PSA
      ...
      UPDATE zlest0039 SET datachegada    = ''
                           pesochegada    = ''
                           unidadechegada = ''
                           status         = 'L2'
...
*                           'Pois existem algumas notas que não possuem o registro L2'.
  ENDIF.
